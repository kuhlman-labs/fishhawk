...
**Readiness gate verifies listener identity (`scripts/dev`, #965).** A healthy `/healthz` plus a live spawned pid is not proof the spawn succeeded — a stale daemon squatting on the port answers the gate while the fresh fishhawkd dies on `bind: address already in use` (the reload false-success). `up`/`reload` now (a) preflight the fishhawkd port (from `FISHHAWKD_ADDR`, post-.env) before spawning and fail loud naming any squatting pid + command, and (b) after the gate require the pid listening on the port to BE the spawned pid (`_verify_listener_identity`), failing per the #628 contract (expected-vs-actual pids, log tail, exit 1) otherwise. `down` no longer returns success on a missing/stale pid file alone: every path falls through to a port fallback (`_down_port_fallback`) that kills-and-reports a stale fishhawkd listener, but a foreign process on the port is only reported — never killed — and `down` exits 1 rather than claiming a clean shutdown. Both commands sweep numbered stray pid files (`dev 2.pid` …), and `down` sources `.env` for port parity with `up`. Hosts without `lsof` degrade to the old behavior with a warning. The contract is locked by `scripts/test-dev`'s real-listener (nc) squatter tests plus call-site body-greps.
