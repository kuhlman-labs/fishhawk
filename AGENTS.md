...
| `fishhawk-mcp` | **auto-swapped** by the registered [`fishhawk-mcp-shim`](backend/cmd/fishhawk-mcp-shim/README.md) (ADR-060 / #1922) — the shim hot-swaps the rebuilt child and `up`/`reload` print a one-line auto-swap note. A manual `/mcp` reconnect is only the fallback when the shim is **not** registered (the Claude Code harness owns the MCP server process, so `scripts/dev` cannot restart it) |
