...
**Full-suite runs bound test-binary parallelism (#972):** `scripts/test` (default and `coverage`) passes `go test -p "${FISHHAWK_TEST_P:-2}"` so a developer-Mac Docker daemon isn't asked to start ~GOMAXPROCS testcontainers-Postgres instances at once (the "context deadline exceeded after 9 retries" start-timeout flake). The bound was lowered 4 → 2 after -p 4 still single-flaked a container start; raise via `FISHHAWK_TEST_P` on high-core machines if wall-clock matters; `scripts/test single` is unbounded. **`scripts/test` also exports `TESTCONTAINERS_RYUK_DISABLED=true`:** the ryuk reaper is a shared, single-point-of-failure container — under daemon load the reaper itself times out starting, and every package that then registers with the dead reaper fails (`No such container: <reaper-id>`), cascading one flake into a whole-suite red (and a category-A run failure). Tests Terminate their own containers via `t.Cleanup`, so the reaper is a redundant backstop; disabling it caps a slow start at ONE flaked package instead of the cascade. The runner's committed-tree verify gates additionally absorb ONE such (now single-package) flake per stage (re-run the verify in place, `verify_infra_flake_retry` trace event) before failing.
...
5. **File issues for deferred work** before the PR opens. Any TODO, "follow-up PR", "deferred to E…", or obvious operability gap gets a tracking issue. **Prefer `fishhawk_file_issue`** (the work-management tool, #1005) over the hand-rolled `gh issue create` + four labels + `gh project item-add` + GraphQL-status dance: it applies this repo's `work_management` conventions automatically — the `[E<parent>.<n>]` / `[ADR-NNN]` title format, the `type:*` default label, the per-type body skeleton, `Status=Backlog`, ADR numbering, and the parent-epic link — so you pass `type` + `summary` + the `area:*/autonomy:*/phase:*` labels + `relations.parent_epic` and it renders a conventions-complete item. For a numbered type (`adr`) the tool does NOT yet discover numbers itself: you must pass `existing_numbers` (the numbers already in use, scanned from the tracker) so it can allocate the next one — omission now fails `work_item_invalid` rather than silently allocating `ADR-001` (#1265), and a genuinely-first numbered item is filed by seeding `existing_numbers:[0]` (which yields 1). Full tracker-side discovery of the existing numbers is a deferred follow-up. Reference the filed issue from the PR body's `## Notes` so the deferral is reviewable.
