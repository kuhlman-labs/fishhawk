...
longer have to correlate against the log. It also carries `provider` and
`login`.
...
**Which page you land on depends on your topology, and only the backend page
names the login.** In the split-origin layout (the SPA served on its own
origin — Vite dev, or a self-host that does not proxy the SPA), the deny
redirect is relative and resolves against **fishhawkd**, so fishhawkd's own
`GET /access-denied` renders it and states "You signed in to GitHub as
`<login>`". Behind a same-origin reverse proxy that routes non-`/v0` paths to
the SPA, the React page renders instead: it shows the same branch-specific
explanation but **not** the login, because on that path the value is an
unverifiable query parameter and claiming an identity from one would let a
crafted URL show a misleading login on your own denial page.

If the page shows the generic body instead of a branch, the deployment recorded
the reason but did not carry it — check the fishhawkd log for
`oauth sign-in denied`.
