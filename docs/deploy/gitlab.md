...
- **The deploy gate reads checks from the evaluated run's REVIEW stage** (`server.findCISignalStage`, E68.67 / #3489) — the stage GitHub's `ingestCheckRun` records rows against. That is independent of the GitLab gap: a GitLab run has no ingester writing to that stage at all (#3490).
- **RESIDUAL — the GitLab pipeline→`stage_checks` ingester lands in the sibling slices of #3490.** The snapshot half is captured at run creation (above); until the writer merges, a snapshot-bearing GitLab run reports `contexts_pending` on the deploy gate (honest, not vacuous) and `ci_green` remains a GitHub-only signal; the refusals and the `deferred_unresolvable` label are what make that visible rather than silent. Tracked in **E45.55 / #3490**.
...
4. **Pass `workflow_spec` inline** (the MCP server auto-discovers it from `working_dir`; the CLI reads `--workflow-spec`). There is no GitLab spec-fetch fallback on `POST /v0/runs`: an empty spec on a gitlab run is `422 workflow_spec_required`.
