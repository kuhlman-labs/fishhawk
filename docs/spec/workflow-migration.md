...
Fishhawk's own `.fishhawk/workflows.yaml` now declares `version: "2"` — the walk below is the RECORD of that completed operator migration, retained because the five steps are the reusable procedure for the next major and for any customer migrating their own v1 spec.

**No agent stage can migrate it**: the `feature_change` implement stage's `forbidden_paths` entry contains `".fishhawk/**"`, so an implement commit touching that path fails the constraint — for any run on this workflow, not just this one. The migration was therefore an **operator base commit**, walked in this order:

1. **Dry-run the codemod.** `fishhawk migrate-spec .fishhawk/workflows.yaml --report-only` writes nothing (verify with `git status --porcelain`) and prints the eligibility report. Against the then-live v1.3 spec it reported that the document *migrates cleanly to workflow-v2*.
2. **Read the report's four WIDENED gates.** All four approval gates — `feature_change` plan, `feature_change` review, `human_led_change` review, and `release` deploy — are flagged **WIDENED**, because the legacy `approvers: {any_of: [founder]}` form carries no source for a `not:` exclusion (see [What is never fabricated](#what-is-never-fabricated)). The migrated `approvals: {count: 1, members: [kuhlman-labs]}` would let the run's resolved change author satisfy the gate and count toward its quorum — an author is resolved wherever the run carries a human-authorship signal (today an operator-vouched commit), so the widening is real on those runs and inert on the rest. The `agent` half widens nothing: that floor is unconditional (#2358). **Restore `not: [author, agent]` by hand on all four gates**, matching the shipped presets, before committing — which is why the report, not the migrated bytes, is the product. All four gates in the live spec carry that exclusion today; the restoration was done as part of the base commit.
