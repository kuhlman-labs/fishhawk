...
**Absent `reviewers` block:** When the `reviewers` field is omitted, **no reviewers are configured**: `Stage.Reviewers` stays `nil`, the effective agent count is `0`, and the stage resolves **gateless**. There is **no `{human: 1}` default** — nothing in the backend, runner or CLI materializes one, and callers reading `Stage.Reviewers == nil` interpret that nil directly rather than substituting a default. A human approval requirement is declared by a stage `gates:` entry of type `approval` (read by `hasApprovalGate`), **not** by `reviewers.human`, so an absent block never silently confers a human approval gate (E52.12 / #2322).
...
| `may_route_fixup` | `convergent_concerns` | all reviewer verdicts are in, at least one concern is open, and no **gating-authority** reviewer rejected — under advisory authority (agent + human reviewers, ADR-027) an agent reject is arbitrable and does NOT disqualify; under gating authority (agent-only) a reject fires `reviewer_reject` and pages. When every verdict is approve-class (no reject), the auto-route additionally requires an open concern at or above `route_fixup_min_severity` (default `medium`) — otherwise the gate parks for the operator (#1964); see below |
...
**`route_fixup_min_severity` (default `medium`).** Tunes when `may_route_fixup` / `convergent_concerns` auto-routes on an all-approve round (#1964). When every implement-review verdict is approve-class (`approve` / `approve_with_concerns`) — no reject to arbitrate — the fix-up auto-routes only if at least one open concern ranks at or above this threshold (`low` < `medium` < `high`). A dual-approve round whose sole open concern is `low`-severity therefore **parks** for the operator (waive, defer, or route deliberately) rather than spending a full fix-up pass. Set `route_fixup_min_severity: low` to restore the legacy route-on-any-concern behavior. A **reject** verdict bypasses the threshold entirely: advisory arbitration and the gating-reject page are unchanged. Inherited under the same wholesale-override semantics as the `may_*` knobs (a gate- or campaign-level block replaces the workflow block, never merged). An unrecognized concern severity ranks below `low` and parks (fail-closed). Additive optional field — accepted at every advertised version, no version bump.
...
| `reviewers.agent`           | integer `>= 0` (default `0`)                                                 | absent block → no reviewers configured; `Reviewers` nil; agent count `0`; resolves `gateless` (no `{human:1}` default); superseded by a non-empty `reviewers.agents`. **v0/v1 only — REMOVED in workflow-v2 (E52.3 / #2215)**, where `reviewers.agents[]` is the sole declaration and the effective count is `len(agents)` |
...
| `operator_agent.must_page_human` | `reviewer_reject`, `advisory_reviewer_reject`, `gating_reviewer_reject`, `plan_rejection`, `scope_amendment`, `budget_override`, `policy_override`, `exception_request`, `requirement_arbitration`, `clarification_request` | closed set; always pages the human regardless of `may_*` knobs. The explicit reject classes (v0.7+) make the taxonomy self-documenting; legacy `reviewer_reject` resolves to the gating sense. The bare `reviewer_reject` token is **v0/v1 only — REMOVED in workflow-v2 (E52.3 / #2215)**, where the taxonomy is exactly `advisory_reviewer_reject` + `gating_reviewer_reject` |
