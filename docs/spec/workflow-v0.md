...
| `reviewers.agent`           | integer `>= 0` (default `0`)                                                 | absent block → nil → backend defaults to `{human:1}`; superseded by a non-empty `reviewers.agents`. **v0/v1 only — REMOVED in workflow-v2 (E52.3 / #2215)**, where `reviewers.agents[]` is the sole declaration and the effective count is `len(agents)` |
