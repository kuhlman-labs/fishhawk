...
The anchor is delivered on BOTH forges (E45.52 / #3481): the notifier derives the run's comment family from `installation_ref` — the github family posts through the GitHub App client unchanged, and every other family (`gitlab:<project_id>`) posts through `forge.IssueOperations`. Edit-in-place is capability-negotiated: a forge implementing the optional `forge.IssueCommentEditor` (the GitLab adapter does — `PUT /projects/:id/issues/:iid/notes/:note_id`) keeps the single anchor edited in place; a forge that implements `IssueOperations` WITHOUT the editor degrades to append-only (each update is a new comment), and the `status_comment_posted` audit row names which happened via `forge` + `comment_mode` (`edit_in_place` | `append_only`). Contract: [`docs/issue-comment-surfaces.md`](../issue-comment-surfaces.md) § "Forge family routing". Residual: the notifier is constructed only when a GitHub App client is configured, so a GitLab-only fishhawkd with no GitHub App still delivers no echo.
...
- **`limit_usd`** is the **primary** cost lever and is **not required**. A budget declaring only `max_tokens`, only `limit_usd`, only `max_runtime`, any combination, or nothing at all is valid: ADR-067 ratified unifying the units, not mandating a cost model. It is **enforced per stage at spec major 2 and above** (#2328): on each raw trace upload the backend sums this stage's recorded cost (`cost_recorded` ledger entries, deduped by bundle content hash) and compares it against the ceiling. A breach with `enforcement: blocking` **halts (cancels) the run** and records a `stage_budget_exceeded` audit entry; with `enforcement: advisory` it records the same entry and the run proceeds. A stage with no `limit_usd` (or a `limit_usd` of 0, which the schema forbids) is never enforced, and v0/v1 documents cannot declare `limit_usd` at all, so legacy specs never enforce a stage ceiling.
- **`max_runtime`** is the same duration form as every other v2 duration — the identical pattern (`^([0-9]+(ns|us|ms|s|m|h))+$`) as `policy.max_stage_runtime`, `executor.timeout` and `executor.verify.timeout`, parsed by the same code path. The pattern is a strict subset of what Go's duration parser accepts (which also takes fractional and signed forms), matching the convention of those three fields rather than the parser's full grammar. Sub-minute caps such as `90s` are expressible. **Declared but not enforced** (#2328): under the cache-aware four-bucket token sum (#1349) the shipped preset numbers are off by roughly two orders of magnitude, so the runtime cap is calibration-pending, not enforcement-pending.
- **`max_tokens`** is an optional secondary lever. **Declared but not enforced** (#2328), for the same calibration reason as `max_runtime`; `limit_usd` is the enforced ceiling.
- **`enforcement`** — `advisory` (the default) records a `stage_budget_exceeded` audit entry on an overrun and lets the run proceed; `blocking` additionally cancels the run. Enforcement is evaluated only for the `limit_usd` ceiling on a spec major ≥ 2, and only on the raw trace-upload variant (the redacted variant of the same bundle is not re-evaluated). This repo's own workflows declare `enforcement: advisory` on every agent stage.
...
**When a `report` entry fires:** when the gate is **live**, and — only when the entry declares a `when` — additionally when that condition is met. A bare `report` entry with no `when` therefore surfaces whenever the gate is reachable; requiring a condition that need not exist would make the mode inert. A `when` declared on a `report` entry is subject to the **same per-class binding** as `mode: auto`: it must name that class's own condition. The row is emitted **at most once per gate occurrence per class**. A gate occurrence spans one opening of the gate: a gate a human takes an hour to reach produces one row, not one per poll cycle, while a gate that closes and re-opens is a **new occurrence** and re-surfaces the proposal. What counts as a re-opening is **per class**: a **fresh review round** for `approve` / `route_fixup` / `waive` (a fix-up round trip), a **subsequent distinct failure of the same stage** for `retry` (a stage that fails, is retried and fails again), and a **re-becoming-ready merge gate** for `merge` (a gate that became ready, un-became ready and became ready again).

**Known limitation (residual under-emission).** The re-occurrence discriminators are read from the audit chain, and each degrades to the base occurrence key on an unreadable read — so an unreadable discriminator **under-emits** (it suppresses a re-surface) rather than flooding, the safe direction. The `merge` proxy is the run's approval count, which advances only across an un-ready/re-ready cycle driven by an approval; a merge gate re-readied **without an intervening approval** (a gate re-closed by a retry or a fix-up re-dispatch) keeps its key and under-emits. Closing that gap needs a new audit category recording a stage entering/leaving `awaiting_approval` — summing today's available signals would instead break the at-most-once guarantee, so it is deliberately deferred.
