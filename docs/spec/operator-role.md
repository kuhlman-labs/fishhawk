...
## Campaigns under the operator_agent contract

A **campaign** (ADR-047 / #1437) drives a whole epic's child issues as a dependency-ordered batch of runs (the lifecycle is narrated in [`ARCHITECTURE.md` §4.3](../ARCHITECTURE.md)). The operator role's relationship to a campaign is the SAME delegation it has to a solo run — authority is still entirely in each run's workflow-spec `operator_agent` knobs, never re-derived per campaign:

- **Auto-drive is the delegated knobs applied at scale.** The campaign driver's per-run gate action (E25.6) acts on a run gate only when the delegated knob's condition is met AND the real gate state independently matches (double-gated, fail-closed to observe-only) — the same `may_approve` / `may_route_fixup` / `may_retry` / `may_merge` semantics a solo operator agent applies, taken under the campaign actor identity (subject `operator-agent/campaign`, stamping `actor_kind=agent` exactly like every other delegated action — see [Identity and token issuance](#identity-and-token-issuance-d4)).
- **A `must_page_human` condition PAGES, it never acts (E25.7).** When a run hits a judgment point the contract flags `must_page_human` (`reviewer_reject`, `requirement_arbitration` — the same `escalation.always_page` posture as a solo run), the driver REFUSES the gate, pauses the item (and, under `pause_campaign`, the whole campaign), and pages the human/operator-agent with a distilled hand-off. The paged judgment is handled by the human or the operator-agent under delegation, exactly as a solo run's page would be.
- **Resume is the operator's explicit hand-back, never derived.** After the paged cause is resolved, the operator-agent resumes the campaign via `POST /v0/campaigns/{id}/resume` (or `fishhawk_resume_campaign`); a paused campaign is sticky (a sibling settling can never auto-unpause it). Resuming to mask an unresolved `must_page_human` cause is forbidden by the campaign procedure.

The campaign-scoped operator procedure (poll `fishhawk_get_campaign_status`; handle the distilled page; then `fishhawk_resume_campaign`) lives as the `conventions.campaign` entry in the shipped default spec, **not** as a new `gate_procedures` key — per the thinness rule, `gate_procedures` stays the closed five keys and named procedures that don't expand the closed set go under the open `conventions` map. Cross-references: the [`operator_agent` delegation knobs](./workflow-v0.md#operator-agent-delegation-v05) the auto-driver reads, and [`ARCHITECTURE.md` §4.3](../ARCHITECTURE.md) for the driver/pause/page/resume realization.
...
- **Issuance**: `fishhawkd token issue --subject operator-agent/operator-role-v0`. The default operator scope set (#526: `read:runs`, `read:audit`, `write:runs`, `write:approvals`, `write:stages`, `write:deploy`, `write:campaigns`) applies unchanged — the subject carries no `mcp:` prefix and the convention introduces no new scopes. Authority remains entirely in the workflow spec's `operator_agent` delegation knobs.
