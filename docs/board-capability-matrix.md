...
`TestBoardCapabilityMatrixDocumentsEveryDeclaredUnavailableReason` (`backend/internal/workmgmt/conventions_test.go`) AST-walks `backend/internal/workmgmt`'s non-test source, collects every `UnavailableReason` value declared as a string literal — whether under a spec typed `UnavailableReason` (`const R UnavailableReason = "x"`, the style `reader.go` uses) or as an explicit conversion (`const R = UnavailableReason("x")`), in a `const` or a `var` block — and asserts each declared value appears in this document. It is a **source-derived sweep, not a frozen list**: a seventh reason added to `reader.go` reddens the test on the day it is added, with nobody having to remember this file exists. `TestCollectReasonsFromFileAcceptsBothDeclarationStyles` pins those accepted styles against synthetic source, so the sweep cannot quietly narrow to whatever `reader.go` happens to look like. Two residuals, stated rather than implied: a value built from an expression that is neither a string literal nor a conversion of one (a concatenation, a reference to another constant) is still skipped, and the per-reason document check is substring containment, which a value appearing in unrelated prose would satisfy — acceptable for these machine-generated snake_case values.
