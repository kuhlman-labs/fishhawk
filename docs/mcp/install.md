...
**Blocking on stage completion — use `fishhawk run watch`, never log-grepping.** When a stage is driven detached (e.g. `fishhawk_dispatch_stage`) rather than through the blocking `fishhawk_run_stage`, block on the stage settling with `fishhawk run watch <run-id> --stage <type>` (E32.3 / #1550) instead of tailing the per-run runner log for a guessed event name — the fragile contract that silently stalled runs when the guessed name never appeared. `run watch` polls the durable `(run_id, stage_id)` stage-wait and (with `--until amendment|any`) the run's pending scope amendments over the existing `?wait` long-poll endpoints, exits `0` terminal-ok / `1` failed-or-error / `3` amendment-pending / `4` timeout, and prints one JSON summary line to stdout so the outcome is machine-readable regardless of exit class. Run it with the operator/operator-agent token. See [cli/README.md](../../cli/README.md) for the full flag and exit-code reference.
