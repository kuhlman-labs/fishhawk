...
## Relation to `fishhawk init`

`fishhawk init` (E29.3) seeds a repo's `.fishhawk/workflows.yaml` from a
workflow preset and writes the managed agent-docs bridge. It runs this same
preflight as a closing step, so `init` finishes by telling the operator exactly
which first-run prerequisites still need attention.
