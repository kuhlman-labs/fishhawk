...
- **Runner-side.** A runner refuses pre-spawn (`runner_failed`, reason `forge_writes_denied`, category C — before any agent invocation, branch push, PR open or corpus push) when EITHER its env carries `FISHHAWK_FORGE_WRITES=deny` OR the fetched prompt carries `forge_writes: "deny"`. The acceptance invocation env ALWAYS carries `FISHHAWK_FORGE_WRITES=deny` (a fixed `acceptenv` injection; a `FISHHAWK_ACCEPTANCE_ENV_FISHHAWK_FORGE_WRITES` passthrough is refused), so every descendant runner the agent could spawn inherits the deny; `scripts/dev preview` sets the same variable on the preview daemon's serve line so runners its embedded `/mcp` verbs spawn inherit it too. Contract: `runner/README.md` § "Forge-writes gate".
