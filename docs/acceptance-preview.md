...
- **The spend baseline ages with wall-clock time.** `trace-upload-target`'s four rows are aged 1h5m / 2h / 3h / 4h from apply time so they land in four distinct prior hour buckets; `spendalert` averages the populated prior buckets in its 24h window, so a ~$5 upload in the seed hour or the hour after trips at ratio ≈ 5000. Seed and upload **contiguously** — a long gap moves the baseline out of the comparison. `unpriced_model_alert` dedups once per window across ALL runs, so an exactly-one assertion holds on a fresh preview target only.
