...
| Per-stage prompt construction | `backend/internal/prompt/Build` (pure, by stage type); served at `GET /v0/stages/{id}/prompt` from `backend/internal/server/prompt.go`. Signed canonical message: `sha256("prompt:" + stage_id)`. Runner-side `--fetch-prompt-from` flag tracked in #128. |
