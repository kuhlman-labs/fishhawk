...
POST   /v0/stages/{stage_id}/fixup           — route advisory implement-review concerns back to the
                                               ...
                                               body: {concerns:[int], reason?} — concerns indexes the
                                                 stage's implement_reviewed approve_with_concerns set;
                                                 re-opens awaiting_approval → pending, commits onto the
                                                 SAME PR branch (vs /retry's fresh diff). Bounded to one
                                                 pass (counts prior stage_fixup_triggered audit entries).
...
| `fixup_not_applicable` | 422 | `POST /v0/stages/{id}/fixup` on a stage that can't take a fix-up pass: not an implement stage, not parked at `awaiting_approval`, or with no recorded `approve_with_concerns` implement-review concerns to route back (#762). A fresh run or an operator hand-edit is the next step. |
