...
| `retry_not_applicable` | 422 | `POST /v0/stages/{id}/retry` on a stage whose state or failure category isn't retriable (E8.3 #146). Distinct from `retry_not_implemented`: this category will *never* retry; a fresh run is the right next step. |
| `retry_not_implemented` | 501 | `POST /v0/stages/{id}/retry` on category-A or -C: orchestrator re-dispatch isn't shipped yet (E8.3 follow-up). |
