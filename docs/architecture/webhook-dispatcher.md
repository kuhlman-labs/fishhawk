...
- the required-checks snapshot comes from a different source with different semantics (E45.55 / #3490): GitLab's protected-branches API contributes no required-status contexts, so instead of the GitHub `ResolveRequiredChecks` union the create path reads the project's `only_allow_merge_if_pipeline_succeeds` setting once through `forge.CIRequirementReader` (`Dispatcher.GitLabCIRequirements`, wired from `registeredCIRequirementReader("gitlab")`) — ON → `{contexts:["gitlab/pipeline"], sources:["gitlab:only_allow_merge_if_pipeline_succeeds"], gitlab_allow_skipped_pipeline}`, OFF → a present-but-empty snapshot (authoritatively nothing required, not the ADR-017 empty-union refusal), reader unwired or failing → nil + WARN with the run still minted. The `stage_checks` writer for `gitlab/pipeline` lands in the sibling slices of #3490, so `ci_green` on GitLab is not fully live until those merge;
...
A deferral is not a silent nothing. Every non-correlation writes a `ci_retry_skipped` audit row naming WHICH reason applied — `first_stage_pipeline_on_non_run_branch`, `merge_request_pipeline_ref_not_a_run_branch`, `no_candidate_run_owns_pipeline_ref`, `pipeline_sha_does_not_match_run_head_sha`, `run_head_sha_lookup_failed`, `run_lineage_cancelled`, `retry_policy_unresolvable_from_cached_spec`, `gitlab_pipeline_trigger_unconfigured`, `run_has_no_credential_scope` — attributed per the anchor contract owned by `backend/internal/webhook/README.md` § "Audit attribution", so a failure that was correctly ignored is distinguishable from one that should have retried and did not (#2860). The one case with no audit row is "no gitlab_ci runs for this project at all": there is no run to chain against, and the delivery is not a Fishhawk failure.

**A MERGE-REQUEST pipeline gets its OWN deferral reason (E45.30 / #2881).** An MR pipeline's ref is the merge request's TARGET branch, so it too fails the run-branch prefix test and fell into the first-stage arm — drawing a reason that is affirmatively FALSE about it: it is neither the run's first pipeline nor a default-ref one. Under the #2860 principle the reason set exists to serve, a wrong reason is worse than a generic one, because an operator debugging a missing retry reads "first stage on non-run branch", concludes the deferral was correct, and stops looking. Inside the SAME early-return arm the handler now selects `merge_request_pipeline_ref_not_a_run_branch` when the pipeline is identifiable as a merge-request one by EITHER discriminator: `object_attributes.source == "merge_request_event"` (the documented Pipeline Hook signal, and the PRIMARY one — carried onto `PipelineRef.Source` by `matchGitLabCIFailure`) OR an anchored `refs/merge-requests/<iid>/{head,merge}` ref. The ref arm is defensive breadth INFERRED from the `CI_MERGE_REQUEST_REF_PATH` predefined-variable and merged-results documentation, not transcribed from a Pipeline Hook example; it may never fire on a real Pipeline Hook, and its cost is one regexp branch on a delivery that is already being skipped. A non-zero `merge_request.iid` is deliberately NOT a third signal — GitLab attaches that block to a BRANCH pipeline with an associated open MR, so keying on it would relabel ordinary first-stage pipelines and invert the defect. Correlation is untouched: a pipeline whose ref IS a run branch takes the same path it always did, MR-sourced or not, and no retry is newly dispatched or newly suppressed.
