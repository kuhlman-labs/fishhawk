...
Every publish surface above is one-shot best-effort: a transient GitHub failure (the #971 401 during the 2026-06-10 incident) drops the Check Run, the publisher dedup cache stays unrecorded (a failed publish never poisons it — pinned by `TestPublish_GitHubError_Returned`), and nothing retries until the next SPA visit or webhook (#973). The merge reconciler (`backend/internal/mergereconciler/`, `--enable-merge-reconciler`) closes this **for a run still PARKED at the review gate** (see the next paragraph for the merged-run hole it structurally cannot reach): each tick calls `server.RepublishAuditCheck` (→ `recomputeAndPublishAuditComplete`) for every review stage parked in `awaiting_approval`, BEFORE the merge poll so a GitHub poll failure cannot also skip the heal. A dropped publish heals within one tick of GitHub recovering; an already-published state dedups to a no-op. Re-creating a Check Run that already exists is safe — GitHub evaluates the latest check run per `(name, head_sha)`.

**A merged run is OUTSIDE that sweep (E64.42 / #3159).** The sweep enumerates review stages parked at `awaiting_approval`, so the moment a merge resolves that stage the run leaves the only retry path — a check a fix-up push left at `in_progress` stays `in_progress` on the merged head forever, and the reconciler can never reach it. Nine live runs stranded that way. Two recompute-and-republish points close it, and neither is reachable from any of the six pre-existing publish call sites:

Both delegate to `server.recomputeAndForcePublishAuditComplete` (E64.59 / #3190), which FORCES past the publisher's `(forge, repo, head_sha, state)` dedup consult and returns the computed `Result`. The dedup skip is what makes them able to clear a strand at all: a run whose acceptance stage a fix-up push re-opened (#1682) recomputes to the SAME `pending` the synchronize already published, so the unforced republish posted nothing. The reconciler heal path above and the review-loop republish stay UNFORCED on `recomputeAndPublishAuditComplete` — a forced publish on a 60s timer would post a check run per minute per parked run.

- **Before the merge is dispatched** (`server.republishAuditCheckBeforeMerge`), at BOTH merge sites: `merge_run.go::handleMergeRun` (the operator merge endpoint) and `autodrive.go::dispatchAcceptanceGatedMerge` (the delegated `may_merge` arm). The operator endpoint does not route through the delegated tail — it calls `GateMerger.MergePullRequest` directly — so both need the call. This half is REQUIRED-CHECK-SPECIFIC and is the load-bearing one: since #3160 marked the context Required, GitHub reports the PR in UNSTABLE status and refuses to queue a merge while the check is `in_progress`, which `handleMergeRun` classifies as `409 merge_checks_pending`. A republish placed only on run termination is therefore unreachable for exactly the blocking case — the merge that would trigger it can never happen.
- **After the run reaches its terminal state on a merge** (`server.republishAuditCheckOnRunTerminal`), on BOTH merged arms of `pullrequest_review_events.go::resolveReviewStageOnMerge` — the review-stage arm and the implement-only (`reviewStage == nil`) arm. This keeps the promise the `in_progress` summary makes ("Fishhawk will update this check when the run terminates").

Both are best-effort and never unwind the merge, its durable verdict row, or the run completion. Ordering on the terminal half is load-bearing and was settled empirically, not argued: the call must sit AFTER `advanceRunAfterReviewResolve`, because `ComputeResult` reports `pending` while any non-review stage is non-terminal and the Advance is what walks the remaining stages to terminal. Hoisting it above the Advance turns `TestResolveReviewStageOnMerge_RepublishesAuditCheckOnTermination` RED. The OBSERVED failure changed with #3190 and the earlier explanation no longer holds: it was recorded as "the dedup cache suppresses it and NOTHING is posted — the stale `in_progress` simply rides", which was true while the republish was unforced. Now that it forces past the dedup, the hoist was re-run and the observation is that the republish DOES post, carrying `pending`, so the terminal publish is `in_progress` with no conclusion instead of `completed`/`success` — the stale check is replaced by a fresh `in_progress` and the green never lands. Same constraint, same RED, different mechanism; re-run the mutation rather than trusting either sentence.

Deliberately NOT added: the `checkImplementReviewSettled` early return (that run is still parked at `awaiting_approval` and so still inside the sweep) and the closed-without-merge arm (an abandoned head strands no gate).

**Named consequence.** A pre-merge republish can flip a stale `success` to `failure` and block a merge the stale green would have allowed. That is the control working — the audit gate exists to refuse an incomplete chain — but it IS a behavior change. It is bounded: the call sits AFTER the acceptance-gate guard, so a genuinely mid-flight run is refused there and never reaches the publish; the only way to reach a `fail` is a genuinely broken chain (a real missing plan, trace, or PR artifact). A residual remains: GitHub re-evaluates branch protection asynchronously, so the first dispatch after the republish can still return UNSTABLE — absorbed as today's `409 merge_checks_pending` with a durable verdict row, which the merge verb re-POSTs across a bounded wait. One extra retry cycle, never a wedge.
