...
Both are best-effort and never unwind the merge, its durable verdict row, or the run completion. Ordering on the terminal half is load-bearing and was settled empirically, not argued: the call must sit AFTER `advanceRunAfterReviewResolve`, because `ComputeResult` reports `pending` while any non-review stage is non-terminal and the Advance is what walks the remaining stages to terminal. Hoisting it above the Advance turns `TestResolveReviewStageOnMerge_RepublishesAuditCheckOnTermination` RED, and the observed failure is sharper than "it publishes pending": the recompute yields the state already published at that head, the publisher's dedup cache suppresses it, and NOTHING is posted — the stale `in_progress` simply rides.
