...
| `persistAcceptanceScenarios` | Never errors; returns `{outcome, reason, headSHA, baseSHA, scenarioIDs, retirementsLedgered}`. Order: no tree → `persist_skipped no_acceptance_tree`; no run branch → `no_run_branch`; passed verdict + issue number → `scenario.Compose`/`Write` from the VALIDATED, REDACTED verdict bytes (issue 0 → `acceptance_scenario_record_skipped`, never the PR number); served retirements → `MergeRetired` into `retired.yaml` (written only when the ledger grew; otherwise `retirementsLedgered`); dirty set outside `acceptance/scenarios/**` → `persist_refused dirty_outside_corpus`; nothing dirty → `unchanged`; `git add -A -- acceptance/scenarios` + a second confinement check on the staged set; commit with `-c user.name/email` (the run's author, `gitops.DefaultAuthor*` fallback) under `gitops.HardeningArgs()` with `--signoff`; no remote → `no_remote`; `PushCommittedBranch` pinned to the new SHA (plain push: a moved tip is non-fast-forward → `persist_failed push:`). | `TestPersistAcceptanceScenarios_PushesScenarioOnlyCommit`, `TestPersist_{RefusesDirtyOutsideCorpus,WritesFullRetirementOnFailedVerdict,UnknownPRRecordsZeroNotIssue,BestEffort,AlreadyLedgeredDisarms}` |
