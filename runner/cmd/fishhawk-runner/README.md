...
**Deadline during the park.** The blocking wait runs after the agent has finished, so it consumes stage wall clock without producing work. The runner's stage deadline (`cfg.timeout`) bounds the AGENT INVOCATION only; the post-agent path has no deadline of its own, so the only runner-observable form of "the deadline expired while parked" is the process context being cancelled — which the driver's ctx branch turns into `(false, nil)`, exactly as the budget branch does. The stage then reaches the same failure it would have reached un-parked. `TestRun_MigrationRenumber_ParkExpiry_ClassificationMatchesUnparked` asserts that equivalence directly.
