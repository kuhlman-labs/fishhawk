...
- `verifyFixPrompt` — fix-iteration prompt embedding the captured output, BOUNDED through `boundVerifyFixOutput` (#3408, see below); it returns the prompt plus the elided byte count so no caller can build an unbounded fix prompt.
