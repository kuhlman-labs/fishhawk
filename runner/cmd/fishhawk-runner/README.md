...
|---|---|---|
| `retirementDropReporter` | Armed in `run()` IMMEDIATELY after `fetchPromptToFile` returns served retirements — before the version-skew/agent-version checks, the lineage block, the target gate and `provisionAcceptanceTree` — via one `defer`, so every later return is covered. `note(reason)` records the exit path (last note wins; the guard, the verdict branches and the persist outcomes never follow one another on one exit). Disarmed ONLY by a successful `acceptance_scenarios_pushed` report (`pushed_and_reported`) or an already-ledgered served set (`already_ledgered`). `report` ships `acceptance_scenario_retirement_dropped` on a fresh 30s context detached from the run context; a nil client/key logs `_drop_unreported`. | `TestRetirementDropReporter`, `TestReportRetirementDrop_{TargetGateFailsPreSpawn,ProvisionFails,GuardCategoryB,VerdictInvalid,NoRunBranch,PersistRefused,PushFailure,PushReportFails,NotShippedAfterPush}` |
