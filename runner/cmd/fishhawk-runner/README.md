...
- `scope_amendment_grant_unused` — `{event, run_id, stage_id, grants:[{amendment_id, path, operation}]}`
- `scope_amendment_grant_check_failed` — `{event, run_id, stage_id, detail}` (the `git status` failure path)

**Trace policy_event** — `{"check":"scope_amendment_grant_unused", "grants":[{amendment_id, path, operation}]}`, mirroring the `scope_drift` / `scope_amendments_folded` / `scope_amendment_settle_wait` shape so it rides into BOTH bundle variants (`PackBytes` / `redactEvents`). The backend relay renders an unknown policy_event by name; no wire-contract change.
...
Tests: `scopeamendgrant_test.go` — `TestVerifyFixPrompt_RendersEffectiveScope`, `TestVerifyFixPrompt_RendersGrantedMidStage`, the `TestUnusedScopeAmendmentGrants` table, `TestAnnotateUnusedAmendmentFailure_ExactLine`, `TestDetectUnusedScopeAmendmentGrants_GitErrorFailsOpen`, `TestDetectUnusedScopeAmendmentGrants_OtherStageOnly_NoOp`, `TestEmitScopeAmendmentGrantUnused_SeamContract`, and the end-to-end pair over the #2601 harness: `TestRun_ApprovedScopeAmendment_FixPromptCarriesGrant_UnusedGrantFailsLoud` (the done-means test) and its discriminating twin `TestRun_ApprovedScopeAmendment_GrantUsed_NoUnusedSignal`, plus the no-grant control `TestRun_NoAmendment_FixPromptScopeOnly`.
