...
Log lines: `verify_fix_reinvoke`, `verify_fix_reinvoke_error`, `verify_fix_skipped`, `verify_infra_flake_retry`, `verify_autoformatted`, and the LOG-ONLY `verify_autoformat_skipped` (one line per non-firing auto-format branch, carrying a `reason` — `already_absorbed` / `no_eligible_files` / `formatter_failed` / `no_change`).
