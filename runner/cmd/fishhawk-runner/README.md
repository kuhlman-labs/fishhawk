...
`isReverifyInfraFailure` is `isVerifyInfraFailure` PLUS a `signal: <name>` death, and the signal rule is deliberately confined to this one site: it is the only one carrying prior evidence that the SAME tree already verified (the gate recorded a passing outcome for this diff). At a committed-tree gate, with no such prior pass, a signal death does not support the infrastructure claim — a diff can hang until a supervisor kills it, OOM, or crash a subprocess — so those gates keep today's classification. Residual even here: `signal: killed` / `signal: terminated` can be rendered when a supervisor kills a verify the DIFF made genuinely slow or hung, so a stage going category-C REPEATEDLY on this signature warrants inspecting the change for a hang rather than blaming the host.
