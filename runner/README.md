...
## Conflict-resolution pass ([E64.62 / #3202](https://github.com/kuhlman-labs/fishhawk/issues/3202))

When a base merge CONFLICTS, the backend can authorize a bounded agent pass that resolves the conflict **on the run branch**, so the operator never pushes to a branch [ADR-035](../docs/ARCHITECTURE.md) declares runner-owned. The backend signals it on the prompt response with four `conflict_resolution*` fields (`conflict_resolution`, `_branch`, `_base_ref`, `_expected_head_sha`); the runner routes on them in `run()` and hands the whole stage to `conflictresolve.go`. This path opens **no PR** and shares none of the implement path's branch routing, worktree provisioning, verify gate or coverage gate.

**Sequence.** Verify the checked-out tip is the head the trigger authorized → `git merge --no-commit --no-ff <remote>/<base>` → capture the BASELINE → invoke the agent under a working-tree-edits-only contract → read the state back → run the confinement gate → `git add` **exactly** the conflicted paths → one `git commit --no-edit`.

**The baseline is every input that feeds the commit the gate authorizes**, captured in ONE read immediately after the merge stops and BEFORE the agent runs: `HEAD`, `MERGE_HEAD`, the merge message source, every non-conflicted stage-0 index entry, and each conflicted path's bytes exactly as git wrote them plus the FILE MODE git left on it (the resolution contract is bytes-only, but the scoped `git add` stages mode alongside content, so an unstaged `chmod +x` or a symlink swap would otherwise ride into the commit unexamined). `MERGE_HEAD` and the merge message are in there for a specific reason: a merge commit's **second parent** comes from `MERGE_HEAD` and its message from `MERGE_MSG`, and both are on-disk repository metadata an agent can write while leaving HEAD, the index and the working tree byte-identical. A gate that checked only the three obvious inputs would authorize a commit with an ancestry or a message it never inspected. The happy-path parent-set assertion does not discharge this — it proves the parents are right when nothing tampered, not that tampering is refused.

Clean base changes git already auto-staged in other files ARE part of the baseline and are therefore **authorized**: they ride into the merge commit.

**Named refusals.** The decision logic lives in [`internal/conflictresolve`](internal/conflictresolve/README.md), which decides over captured structs and returns one named reason per rule: `head_moved`, `merge_head_changed`, `merge_message_changed`, `index_entry_changed`, `index_entry_removed`, `newly_staged_path`, `unstaged_change_outside_set`, `untracked_path_outside_set`, `unmerged_entry_outside_set`, `conflicted_path_not_unmerged`, `conflicted_path_missing`, `conflicted_mode_changed`, plus the partition verdicts `outside_hunk`, `residual_marker`, `binary_conflict` and `delete_modify_content` (all prefixed `conflict_resolution_`). The git wiring adds its own: `unexpected_head`, `no_conflict`, `merge_failed`, `baseline_capture_failed`, `agent_failed`, `observe_failed`, `commit_failed`.

**Recovery is the contract.** A merge that starts and cannot finish is worse than one that never starts, so EVERY refusal restores the pre-merge state: `git merge --abort`, then a VERIFIED postcondition (HEAD back at the pre-merge tip, `git status --porcelain` empty) with a `git reset --hard <pre-merge tip>` + `git clean -fd` fallback when the abort did not fully restore it. The two layers are both part of the control — deleting only the abort leaves the fallback restoring the same state, which is why the counterfactual for this path deletes the WHOLE recovery. `conflictResolveResult.Recovered` REPORTS whether restoration succeeded rather than assuming it did; a `false` there is the one outcome an operator must look at by hand.

**Two deliberate refusals that look like successes.** A merge that completes with NO conflict means the base advanced past the conflict between the trigger and the pass: the merge is aborted and reported (`no_conflict`) rather than pushed, because nobody authorized *that* merge — the operator re-invokes the verb, which then takes the clean-advance path. And a BINARY conflict (or one carrying `-merge`) is refused outright: git leaves OURS on disk with no markers, so there is no hunk boundary to confine an edit to.

**What the gate does NOT prove.** It bounds the BLAST RADIUS mechanically — no edit outside a conflicted hunk can be committed — but it does not make the agent immune to being misled about which side to keep by hostile content inside a conflict hunk. Structural containment, not behavioural resistance, the same posture `AGENTS.md` states for the #2291 injection corpora.

**Path handling is NUL-delimited end to end.** Every enumeration uses `-z` and splits on NUL: git C-quotes a path carrying a quote, a backslash, a control character or a non-ASCII byte in its non-`-z` output, and a newline inside a name splits one path into two. A gate a filename can evade is not a gate.
