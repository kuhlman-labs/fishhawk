...
**Where it is set.** Two places set it, neither by hand: `acceptenv.Env` injects it as a FIXED entry into the acceptance agent's env (so every descendant `fishhawk-runner` the sandboxed agent could spawn inherits it; a `FISHHAWK_ACCEPTANCE_ENV_FISHHAWK_FORGE_WRITES` passthrough is refused, never honoured), and `scripts/dev preview` puts it on the preview daemon's `serve` exec line (never on `migrate up`), so a runner the preview's embedded `/mcp` spawn verbs launch inherits it through `os.Environ()`.
