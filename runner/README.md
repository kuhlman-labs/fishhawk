...
### Fix-up landing controls (E68.1 / #2884, E68.14 / #3022)
...
**Control 1a — the no-changes branch (`strandedFixupWork`).** Before reporting `fixup_no_changes`, four probes run, each fail-closed:
...
- a DANGLING commit created during the pass, reachable from neither the base tip nor the branch (`reflogStrandedCommits`, a reflog walk bounded by the pre-pass snapshot). This is the residue a `fishhawk verify wip` commit leaves after `gitResetSoftHEAD1` returns HEAD to the base tip;
- **probe 4 (`verifiedFixupWorkStranded`, #3022) — verified work that did not reach the branch.** The instrument is NOT the reflog and NOT commit creation: it is the verify gate's own certified tree. The committed-tree verify gate (`runVerifyFixLoop` / `runVerifyGateCommitted`) records `verifiedTreeSHA` — the tree object hash of the scope-only throwaway commit it certified — and that value is threaded into `openPRAndShipArtifact` as a parameter. When it differs from the tree of the fix-up base tip yet `CommitAndPush` reports NoChanges, the pass verifiably HAD in-scope work at verify time that is now on neither the working tree nor the branch. Probe 4, NOT probe 3, is the instrument for the commit-reset-no-stash-no-changes shape (#3022): probe 3's pre-pass reflog snapshot is captured in `openPRAndShipArtifact` AFTER `runVerifyFixLoop` already committed and unwound its throwaway commit, so that `fishhawk verify wip` commit is already in probe 3's baseline and is skipped — no snapshot ordering can blind probe 4's certified-tree witness. Probe 4 cannot false-fire on the verify gate's normal residue: a healthy pass never reaches the no-changes branch (its edits stay staged in the working tree after `gitResetSoftHEAD1`, so `CommitAndPush` commits and pushes them), and a genuinely-no-work pass carries an EMPTY `verifiedTreeSHA` (`commitVerifyWIP` makes no commit when nothing is staged), which probe 4 treats as no witness and never flags. It ABSTAINS on three honest residuals — no verify witness (empty `verifiedTreeSHA`), no base tip, or a certified tree byte-identical to the base tree — and FAILS CLOSED (a non-nil error → category C) when the base tip's tree cannot be resolved, never a silent clean verdict.

Any reason → `ErrFixupWorkStranded` (category B), the failure path reports `{failed, B}`, and #788 recovery restores the pre-fix-up review gate. A probe error or an unavailable snapshot → `ErrVerifyInfraFailure` (category C). A clean check falls through to the byte-identical `fixup_no_changes` report. The `fixup_work_stranded` log line carries `verified_tree_sha` so the operator can see the certified tree probe 4 compared against.
