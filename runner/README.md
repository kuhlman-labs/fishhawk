...
**Two deny sources, either sufficient.** The gate is `denied = env == "deny" || prompt.forge_writes == "deny"` and reports `source` as `env`, `backend` or `env+backend`. The backend signal is the prompt response's `forge_writes` field (`upload.FetchedPrompt.ForgeWrites`, a CROSS-MODULE WIRE field mirrored from `promptResponse`), which a dev-mode `fishhawkd` (`FISHHAWKD_DEV_FIXTURES` / `FISHHAWKD_DEV_STUB_FORGE`) stamps `"deny"` and a production daemon omits.

**Pre-spawn, category C.** On denial the runner logs `{"event":"runner_failed","reason":"forge_writes_denied","category":"C","source":"…"}` and exits `exitFailure` BEFORE any agent is invoked and before any forge write is reachable — no branch push, no PR open, no scenario-corpus push, no status comment. Two call sites of `refuseIfForgeWritesDenied` in `run()`: the first directly after the version-skew / agent-version checks inside the `--fetch-prompt` block (ahead of the conflict-resolution pass, which pushes its own merge commit, the lineage-worktree provisioning and the held-commit PR-open), the second at the convergence point the `--prompt-file` path reaches (env is the only signal there — nothing was fetched). Category C because re-dispatching under the same posture refuses identically. Pinned by `TestRun_ForgeWritesDenied_{EnvSource,BackendSource,PromptFilePath_EnvSource}_FailsPreSpawn`, `TestRun_ForgeWritesDenied_RealFetchPath_BackendDeny` (production `upload.Client` against an httptest backend serving `forge_writes:"deny"`) and the positive control `TestRun_ForgeWritesAbsent_Proceeds`.

**Residual (stated, not closed here).** A runner invoked with `--prompt-file` from an environment that SCRUBBED the variable, against a dev-mode backend, is not refused: the prompt-file path fetches nothing, so the backend's `forge_writes` never reaches it, and the env carried no deny. The `/healthz` `dev_mode` field the backend advertises is the hook for a follow-up pre-spawn probe; tracked as a follow-up issue named in the slice-1 PR. In the loop as shipped this path is unreachable from the sandbox: the runner spawn marker on a dev-mode daemon refuses every caller (`403 host_dispatch_refused_dev_mode`), so no runner is spawned to take it.
