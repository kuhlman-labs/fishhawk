...
The audit ledger ends up with three `installation_token_issued` events per implement stage: the OIDC one at workflow start (used by `actions/checkout`), the Ed25519 one minted at the top of the push path (used by the pre-verify base fetch), and a second Ed25519 one re-minted AFTER the committed-tree verify and immediately before the lease `ls-remote` + `git push` + PR creation (#3443 — the verify can hold the first token for 25-30 minutes, longer than the ~5 minutes of life the backend cache guarantees at mint). Contract: [`cmd/fishhawk-runner/README.md`](cmd/fishhawk-runner/README.md) "Post-verify push-token refresh".
...
`internal/gitops`'s tests push into `t.TempDir()` bare origins, so this package carries the same git auto-maintenance pin as `cmd/fishhawk-runner` (#3503, extended here by #3507): `internal/gitops/gitmaint_test.go` adds a `TestMain` that writes a one-key `[maintenance]\n\tauto = false\n` gitconfig into its own temp dir and exports `GIT_CONFIG_GLOBAL` at it before `m.Run()`, plus a `TestHarnessDisablesAutoMaintenanceOnLocalPush` that pushes a real commit into a real bare origin under `GIT_TRACE2_EVENT` and fails if the trace shows a spawned `maintenance` child. Deleting the `TestMain` `os.Setenv` line turns it RED under `GIT_CONFIG_GLOBAL=/dev/null go test -run TestHarnessDisablesAutoMaintenanceOnLocalPush ./internal/gitops/...` (the `scripts/test` posture). This package has an extra wrinkle the sibling packages don't: `commit_test.go`'s `gitNoPromptEnv` sets `GIT_CONFIG_GLOBAL` per-test for the #3443 wire-fixture tests, and one of those pushes into a `t.TempDir()` bare origin too — so `gitNoPromptEnv` is repointed at the `TestMain` pin file (falling back to `/dev/null` only if that file was never written) rather than hardcoding `/dev/null`, and `TestHarnessDisablesAutoMaintenanceOnLocalPush`'s `after_gitNoPromptEnv` subtest pins that coupling.
...
- **Default-deny egress proxy.** The invocation's `HTTP(S)_PROXY` points at a runner-embedded filtering proxy whose allow-list is exactly the workflow spec's `egress.target_hosts` (the only customer-controlled entries), the model API endpoint, and the Fishhawk backend. Anything else is refused `403`. Hostname resolutions are DNS-pinned for the proxy's lifetime and a public hostname resolving into loopback/private space is refused (anti-rebinding). The proxy env binds cooperating HTTP clients only; the raw-socket / cleared-env bypass is CLOSED on macOS by the OS-level net sandbox below (#3393) and remains a residual on Linux.
- **`FISHHAWK_ACCEPTANCE_NET_SANDBOX=auto|require|off` (operator input, E72 / [#3393](https://github.com/kuhlman-labs/fishhawk/issues/3393)).** OS-level confinement of the acceptance agent's WHOLE process tree: on macOS the spawn is wrapped in `sandbox-exec -p <profile>` (`internal/netsandbox`), a Seatbelt profile that denies every outbound IP connection except the proxy's own loopback port and the allow-list's loopback entries (the preview target, a loopback backend). Descendants inherit it regardless of their env, so `env -i fishhawk-runner …`, `git push` (HTTPS or SSH), a Go binary dialing the forge, or a curl with no proxy vars all fail `EPERM` at `connect()` — the #3393 incident shape, where env-carried controls (`HTTP(S)_PROXY`, `FISHHAWK_FORGE_WRITES`) were shed by a nested process that then minted its own credential via `gh auth token`. Default `auto`: apply when available, otherwise proceed env-only with the LOUD `acceptance_net_sandbox_unavailable {reason, enforcement:"env-only"}` event (never silent); `require` fails the stage category-C pre-spawn (`acceptance_net_sandbox_required`) when unavailable; `off` logs `acceptance_net_sandbox_disabled` and is the kill switch. Applied logs `acceptance_net_sandbox_applied {mechanism:"seatbelt", admitted_ports}`; an invalid value (`acceptance_net_sandbox_config`) or a profile render error (`acceptance_net_sandbox_profile`) is category-C pre-spawn. Linux is honestly `unavailable` (`unshare -n` cannot see the host loopback the proxy binds). Residuals — nested `sandbox_apply` refused (an operator-enabled Claude Code built-in Bash sandbox breaks every Bash call: set `off`), unix sockets open, the DNS pre-flight risk — are in `internal/netsandbox/README.md`. **Validating this live needs the #3086 self-host rebuild recipe** (below): the runner is respawned from `bin/fishhawk-runner` built from `main`, so a run whose diff adds this control executes its own acceptance under the unfixed binary.
- **`FISHHAWK_ACCEPTANCE_ENV_<NAME>` (operator input).** The explicit channel for customer-supplied target-instance test credentials: set `FISHHAWK_ACCEPTANCE_ENV_APP_PASSWORD=…` on the runner env and the acceptance invocation sees `APP_PASSWORD=…`. Everything else is default-denied; the model API keys are the one secret class that survives. The acceptance invocation NEVER carries `FISHHAWK_API_TOKEN` (its evidence ships signature-authed, no MCP token — ADR-050) or any repo/deploy token, and a passthrough whose stripped name collides with a denied key, a proxy variable or `FISHHAWK_FORGE_WRITES` is refused and logged, never honored. The invocation ALWAYS carries `FISHHAWK_FORGE_WRITES=deny` (E72.13 / #3500, next section) so a runner spawned from inside the sandbox refuses before any forge write.

### Forge-writes gate — `FISHHAWK_FORGE_WRITES` (E72.13 / [#3500](https://github.com/kuhlman-labs/fishhawk/issues/3500))

**Operator-facing env var.** `FISHHAWK_FORGE_WRITES=deny` is the only recognized value; an absent variable, an empty value, `allow`, or any other spelling (mixed case included) is "proceed". Read by `forgeWritesDenied` (`forgewrites.go`) from the runner's own `os.Environ()`.

**Where it is set.** Two places set it, neither by hand: `acceptenv.Env` injects it as a FIXED entry into the acceptance agent's env (so a descendant `fishhawk-runner` that inherits the agent env refuses; a descendant spawned with a CLEARED env does not — #3393 — which is why the OS-level net sandbox above, not this inheritance, is the confinement; a `FISHHAWK_ACCEPTANCE_ENV_FISHHAWK_FORGE_WRITES` passthrough is refused, never honoured), and `scripts/dev preview` puts it on the preview daemon's `serve` exec line (never on `migrate up`), so a runner the preview's embedded `/mcp` spawn verbs launch inherits it through `os.Environ()`.

**Three deny sources, any sufficient.** The gate is `denied = env == "deny" || prompt.forge_writes == "deny" || healthz.dev_mode == true` and reports `source` as the fired sources joined with `+` in the FIXED order env, backend, healthz — `env`, `backend`, `healthz`, `env+backend`, `env+healthz`, `backend+healthz`, `env+backend+healthz` (the three pre-#3510 strings are byte-identical). The backend signal is the prompt response's `forge_writes` field (`upload.FetchedPrompt.ForgeWrites`, a CROSS-MODULE WIRE field mirrored from `promptResponse`), which a dev-mode `fishhawkd` (`FISHHAWKD_DEV_FIXTURES` / `FISHHAWKD_DEV_STUB_FORGE`) stamps `"deny"` and a production daemon omits. The healthz signal is the `/healthz` `dev_mode: true` field the same dev-mode daemon advertises (E72.16 / [#3510](https://github.com/kuhlman-labs/fishhawk/issues/3510)), probed by the runner itself — see the next paragraph.

**`/healthz` probe (E72.16 / #3510).** `run()` GETs `<backend-url>/healthz` exactly ONCE, at the top — before the `--fetch-prompt` block, the first backend contact of the process — DIRECT (`forgeWritesProbeClient`: `Proxy` nil, 5s timeout, never through an agent egress proxy; a dedicated client, not `previewprobe.go`'s `probeHTTPClient`) and classifies the answer into one of four outcomes, always logged as `{"event":"forge_writes_healthz_probe","backend_url":…,"outcome":…,"detail":…}`: `dev_mode` (200 whose COMPLETE body is a single JSON object with a boolean-true `dev_mode` member — the ONLY outcome that denies), `production` (200 + JSON object with `dev_mode` absent, `false`, or not a boolean — a string `"true"` is production), `unverifiable` (non-200; or a body that is not a single JSON object: `null`, an array, a scalar, non-JSON, trailing garbage, or over the 64 KiB read cap) and `unreachable` (request build or transport error). Both `refuseIfForgeWritesDenied` sites consume the one result, so the fetch path pays no second probe and the `--prompt-file` path — which fetches nothing — is covered too. **`unreachable` and `unverifiable` are FAIL-OPEN BY DESIGN**: a production daemon omits the key (`omitempty`), so absence must mean proceed, and a hosted deployment's `/healthz` may be firewalled from a runner. Stated residual: a dev-mode backend whose `/healthz` the runner cannot reach is NOT detected by this probe — the event line names the outcome so the fail-open is visible, never silent. The probe classifies the BACKEND, not the credential: a runner pointed at a non-dev-mode daemon but holding the operator's forge credentials is unchanged by it.

**Pre-spawn, category C.** On denial the runner logs `{"event":"runner_failed","reason":"forge_writes_denied","category":"C","source":"…"}` and exits `exitFailure` BEFORE any agent is invoked and before any forge write is reachable — no branch push, no PR open, no scenario-corpus push, no status comment. Two call sites of `refuseIfForgeWritesDenied` in `run()`: the first directly after the version-skew / agent-version checks inside the `--fetch-prompt` block (ahead of the conflict-resolution pass, which pushes its own merge commit, the lineage-worktree provisioning and the held-commit PR-open), the second at the convergence point the `--prompt-file` path reaches (env and the `/healthz` probe are the signals there — nothing was fetched). Category C because re-dispatching under the same posture refuses identically. Pinned by `TestForgeWritesDenied_Table` (all seven source strings), `TestProbeBackendDevModeHTTP_Table` (every classifier outcome against real httptest servers, including `null` / `[]` / `"ok"` / trailing-garbage / string-`"true"` / over-cap bodies and a closed loopback port), `TestRun_ForgeWritesDenied_{EnvSource,BackendSource,PromptFilePath_EnvSource}_FailsPreSpawn`, `TestRun_ForgeWritesDenied_RealFetchPath_BackendDeny` (production `upload.Client` against an httptest backend serving `forge_writes:"deny"`), `TestRun_ForgeWritesDenied_PromptFilePath_HealthzDevMode_FailsPreSpawn` (#3510 done-means 3: scrubbed env + `--prompt-file` + real HTTP probe against a `dev_mode:true` backend → refused, source `healthz`, `/healthz` hit once), `TestRun_ForgeWritesDenied_RealFetchPath_HealthzDevMode` (the fetch path honours the probe), `TestRun_ForgeWritesDenied_PromptFilePath_EnvAndHealthz` (source `env+healthz`), the positive controls `TestRun_ForgeWritesAbsent_Proceeds` and `TestRun_ForgeWritesAbsent_PromptFilePath_HealthzProduction_Proceeds`, and the fail-open pin `TestRun_ForgeWritesAbsent_PromptFilePath_HealthzUnreachable_Proceeds`. Every other `run()`-driving test in the package sees a no-dial stub installed by `runTestMain` (`probeBackendDevMode` seam); `withRealDevModeProbe` is the only way a test reaches the real probe.

**Launch paths.** A MARKER-MEDIATED launch (`fishhawk_run_stage` / `fishhawk_dispatch_stage` / `fishhawk_run_children` / `fishhawk_drive_run` → the host-dispatch marker) never reaches this gate on a dev-mode daemon: layer 1 refuses the marker for every caller (`403 host_dispatch_refused_dev_mode`), so no runner is spawned. A DIRECT launch — `bin/fishhawk-runner --prompt-file …` from a shell, or a descendant spawned with a cleared env (#3393) — is one layer 1 never sees; it fetches no prompt (no `forge_writes` signal) and may carry no env deny, so before #3510 it passed all three #3500 layers and could push a branch or open a PR on the real forge. The `/healthz` probe above is what covers the direct launch.
...
- **unverifiable** — reachable but no comparable identity (non-200, non-JSON, missing/`unknown` `git_sha`). Logged `acceptance_target_unverified` and the agent **proceeds** — never a hard fail on a target that simply exposes no identifier.
- **no expectation** — the backend sent no `acceptance_expected_head_sha` at all (an older backend, or backend-side ledger resolution failure). Stage fails pre-spawn, category C, reason `acceptance_expected_head_unresolved`, BEFORE any provision command runs (#3091). This is deliberately NOT the unverifiable warn-and-proceed case: with no expectation there is nothing to compare against, so the gate could report success while unable to fail for the reason it named, and the stage would validate whatever build answered at the shared slot and record a verdict bound to no tree. The verb-side gate (`backend/internal/mcpserver/acceptance_target.go`) refuses with the same named reason before spawning.

No declared target hosts skips the gate entirely — including on the missing-expectation path, which is checked after the no-hosts early return. The probe dials direct from the runner process — the egress proxy contains the agent, not the runner.

#### Backend wire and gate internals

`backend/internal/server/prompt.go::resolveAcceptanceExpectedHeadSHA` resolves the run's merge-candidate identity in two ordered steps. First the newest reported PR-head SHA from the reported-head audit ledger (the `resolveFixupExpectedHeadSHA` pattern). When that resolves nothing it falls back to the run's OWN `integration_commit_recorded` entries and takes the highest-**sequence** `merge_sha` (#3091): a DECOMPOSED PARENT never writes a reported-head entry on its own chain — each child's push lands on the CHILD's chain and the consolidated PR is opened with a `pull_request_url`-only payload — so without the fallback such a run resolves `""`. The fallback is strictly SUBORDINATE (a reported head always wins, so a non-decomposed run's value is byte-identical to before) and does NOT widen `lineageLedgerCategories` / `auditcomplete.HeadReportCategoriesByPrecedence`, which are shared with the branch-lineage guard and the audit-check publisher. Both prompt handlers set the result as `acceptance_expected_head_sha` on their acceptance branches, next to `egress_target_hosts`.

Resolution still degrades WARN-and-omit on an unconfigured audit repo, a read error, or an empty ledger — but an omitted expectation is no longer warn-and-proceed at the gates: both the runner gate and the verb gate fail closed with `acceptance_expected_head_unresolved`, and the ship handler clamps a `passed` verdict whose validated head is unresolvable to `undecidable` (`backend/internal/server/README.md`).
...
- **`FISHHAWK_ACCEPTANCE_PREVIEW_TEARDOWN_CMD`** — optional teardown hook (same `sh -c` + env contract), deferred so it runs on **every** post-provision exit: after the verdict ships on the happy path, and before the stage failure returns on readiness-timeout/stale/any pre-spawn gate failure. Best-effort — a teardown failure logs `acceptance_preview_teardown_failed` and never changes the stage outcome. It runs **detached from the runner's cancellation** (`context.WithoutCancel`) but still **bounded by `FISHHAWK_ACCEPTANCE_PREVIEW_TIMEOUT_SECS`** (#3394): `exec.CommandContext` refuses to start on an already-done context, so a teardown bound to the runner context was refused before it forked on a cancelled stage (the SIGTERM chain from the MCP verb) and the preview leaked on exactly that path; the residual is a `SIGKILL` of the runner itself, which no defer survives — `scripts/dev preview` reclaims the port on the next provision (#1675). `fishhawk_dispatch_stage(auto_preview:true)` injects this variable (`scripts/dev preview-down`) alongside the provision default, so an auto_preview dispatch that leaves `FISHHAWK_ACCEPTANCE_PREVIEW_CMD` unset carries both hooks. **The default teardown keys on the provision SOURCE, not on the flag**: an operator-set `FISHHAWK_ACCEPTANCE_PREVIEW_CMD` receives **no** default teardown even under `auto_preview:true` (`scripts/dev preview-down` only knows how to take down what `scripts/dev preview` stood up), so such an operator must configure this variable explicitly. Configuring a provision command with **no** teardown command leaks the provisioned instance; the runner emits an advisory `acceptance_preview_teardown_missing` warning on that path (it does not block provisioning — a self-tearing-down provision command is legitimate).
...
- `chat <model>` — child model-call span; GenAI-semconv attrs `gen_ai.system=anthropic`, `gen_ai.operation.name=chat`, `gen_ai.request.model`, `gen_ai.usage.input_tokens` / `output_tokens`, optional `gen_ai.request.temperature`; plus `fishhawk.*` cost/repro attrs `fishhawk.cost.usd`, `fishhawk.cost.estimated`, `fishhawk.cost.priced`, `fishhawk.pricing.as_of`, `fishhawk.latency_ms`, `fishhawk.repro.temperature_available` (spelled fully qualified because that is what `EmitStage` emits and `otelemit_test.go` asserts).

To view traces locally, start the opt-in Jaeger all-in-one (`docker compose --profile otel up -d`), set `OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318`, and open the Jaeger UI at http://localhost:16686. **Caveat**: the collector must be reachable from where the runner actually executes — under the standard dogfood loop the runner runs on a GitHub-hosted CI runner where `localhost:4318` is the CI host's loopback, so end-to-end local viewing requires invoking `fishhawk-runner` locally (see "Local invocation" above). The customer-facing description of the export — the single `OTEL_EXPORTER_OTLP_ENDPOINT` switch, the vendor-neutral OTLP posture, and the reachability matrix over all three runner kinds — is the public [Operating > Tracing](https://kuhlman-labs.github.io/fishhawk/operating/tracing/) page. Full span-attribute reference, the k8s Jaeger story (#895), and the GHA-export deferral are in `internal/otelemit/README.md`.

### Fix-up landing controls (E68.1 / #2884, E68.14 / #3022, E68.15 / #3023)

A fix-up pass must prove its work reached the PR branch before the stage reports success. Run 8ae65577 (PR #2883) reported `fixup_no_changes` with its work stranded in dangling `fishhawk verify wip` commits and a stash, and the re-review then certified fixes against a sha that is not on the PR. Two controls in `fixupland.go` close it; both run on the isFixup path only. **Ordering, stated correctly** (it was documented backwards until #3023): the pre-push snapshot is captured BEFORE `CommitAndPush` but AFTER the committed-tree verify gate has already created and unwound its throwaway commit — `runVerifyFixLoop` runs in `run()` and `openPRAndShipArtifact`, which owns the snapshot, is called after it. No control detects a dangling commit; the one that claimed to was removed in #3023 (see "The dangling-commit gap" below).

**Snapshot (captured before the push).** `fixupStashList` (`git stash list`) records the pre-pass stash stack. A snapshot READ failure sets `fixupSnapshotErr`, which makes the stranded-work check below fail CLOSED (category C, retryable) rather than silently skip — with no baseline the runner cannot prove the work landed, the exact condition #2884 exploited. **Since #3023 the STASH snapshot is the SOLE input to that fail-closed signal, and the reflog is no longer read at all.** This is a real NARROWING of a fail-closed surface, recorded here so it is operator-visible: a `git reflog` read failure used to fold into `fixupSnapshotErr` and fail the pass category-C, and no longer does. That is correct — the runner no longer consults the reflog, so a reflog fault carries no information about whether the work landed — but it is a behavior change, not merely a deletion of dead code. The stash read remains the fail-closed baseline (pinned by M4).
...
- **probe 3 (`verifiedFixupWorkStranded`, #3022) — verified work that did not reach the branch.** The instrument is NOT the reflog and NOT commit creation: it is the verify gate's own certified tree. The committed-tree verify gate (`runVerifyFixLoop` / `runVerifyGateCommitted`) records `verifiedTreeSHA` — the tree object hash of the scope-only throwaway commit it certified — and that value is threaded into `openPRAndShipArtifact` as a parameter. When it differs from the tree of the fix-up base tip yet `CommitAndPush` reports NoChanges, the pass verifiably HAD in-scope work at verify time that is now on neither the working tree nor the branch. Probe 3 is the instrument for the commit-reset-no-stash-no-changes shape (#3022), and no snapshot ordering can blind its certified-tree witness. Probe 3 cannot false-fire on the verify gate's normal residue: a healthy pass never reaches the no-changes branch (its edits stay staged in the working tree after `gitResetSoftHEAD1`, so `CommitAndPush` commits and pushes them), and a genuinely-no-work pass carries an EMPTY `verifiedTreeSHA` (`commitVerifyWIP` makes no commit when nothing is staged), which probe 3 treats as no witness and never flags. It ABSTAINS on three honest residuals — no verify witness (empty `verifiedTreeSHA`), no base tip, or a certified tree byte-identical to the base tree — and FAILS CLOSED (a non-nil error → category C) when the base tip's tree cannot be resolved, never a silent clean verdict.
...
**Deliberately narrow (condition, risks).** Control 1 fires only on the no-changes branch, where #2884 landed. A fix-up that DID commit and push while also leaving a stash behind is not failed by control 1 — control 1b proves that pass's own commit reached the branch, but a second stashed edit would still be silently lost. Making a leftover stash fatal on the push path too would fail passes where the pre-agent tree was already dirty; the narrower rule is the deliberate choice. Probe 3 (#3022) closes the commit-reset-no-stash-no-changes shape but does NOT close two blind spots, stated as honest residuals: (a) a pass whose work was ENTIRELY out of declared scope — `StageScoped` stages nothing, so `commitVerifyWIP` makes no commit and `verifiedTreeSHA` is empty, leaving no witness; and (b) a stage running with NO verify command configured, where no gate runs and `verifiedTreeSHA` is always empty. Both are the direct consequence of choosing a runner-OBSERVED witness (the gate's certified tree) over an agent-DECLARED one; this repo's own `.fishhawk` workflow configures a verify command, so the dogfood loop is covered.

**The dangling-commit gap, stated explicitly (#3023).** NO control here detects a dangling commit as such, and none claims to. #2884 shipped a reflog provenance probe (`reflogStrandedCommits`) that did claim it; #3023 REMOVED it rather than repositioning it, because it was provably inert and read as coverage it never provided. Two facts, both checked against the tree: its pre-pass snapshot is captured inside `openPRAndShipArtifact`, AFTER `runVerifyFixLoop` has already created and unwound the gate's throwaway commit, so every `fishhawk verify wip` commit it could have flagged was already in its own baseline and skipped; and `strandedFixupWork` runs ONLY on the `CommitAndPush` NoChanges branch, where `CommitAndPush` short-circuits "with no other side effects" — so on the one branch that probe executed, its snapshot-to-check window contained no commit-creating operation at all. Repositioning the snapshot cannot fix this: an EARLIER snapshot fires on every legitimate pass where verify ran, and a `fishhawk verify wip` SUBJECT FILTER excludes exactly the commits of interest. No reflog instrument can separate stranded fix-up work from ordinary verify-gate residue at ANY snapshot placement, because the two are formally identical. Probe 3's certified-tree witness is what covers that shape, subject to its own two residuals above.
...
**The `record` text NEVER crosses the upload boundary.** It is REQUIRED (it forces the agent to have actually run the mutation) and then DISCARDED on the runner, verbatim to #2737's rationale: the fix-up agent runs arbitrary repository commands, so it controls every byte of that text, and carrying it over the trace bundle into a reviewer prompt would be an egress path for repository content that never appears in the committed diff. What crosses is the `{control_path, kind, observed, restored}` quad — a path the operator already declared, a runner-DERIVED classification, a closed enum, and a bool. `control_path` is constrained to the declared scope set for the same reason, not merely checked non-empty. The drop log echoes only sanitized values (`<not-in-scope>` / `<invalid>`).

**`kind` is RUNNER-DERIVED, never agent-authored (E64.13 / #3107).** For each RETAINED entry the runner stamps `kind` = `production` | `test` by `classifyCounterfactualControlKind(control_path)`, computed AFTER the scope-membership check so it is only ever derived from a path the operator already declared. The decode struct `fixupCounterfactualReport` carries NO `kind` member, so an agent that writes `"kind":"test"` into the sidecar is ignored by `encoding/json` exactly as any other unknown key is — no new agent-controlled string crosses the upload boundary. `TestValidateFixupCounterfactuals_AgentSuppliedKindIsIgnored` pins that derived-not-declared property. The classifier is a closed set: a base name (`filepath.Base`, so a directory named `foo_test.go/` cannot flip a file below it) matching `*_test.go`, `*.test.ts(x)` or `*.spec.ts(x)` classifies `test`; EVERYTHING else — including a test helper living in a non-test `.go` file — classifies `production`. That default-to-production is the fail-safe direction: a production control mislabelled `test` would tell the reviewer a real green is EXPECTED and SUPPRESS a genuine defect signal, whereas a test-side control mislabelled `production` only draws the concern we already get today. The lens lets the reviewer distinguish an EXPECTED green on a test-side guard (deleting an assertion cannot redden the test that holds it) from an ANOMALOUS green on an unpinned production control. `kind` is stamped at the ONE shared append site in `validateFixupCounterfactuals`, so BOTH channels (initial + fix-up) carry it with no fork.

The surviving quads ride the `fixup_counterfactuals` trace event into `gate_evidence` (`gateevidence.go`), and the backend renders them to the implement re-review under explicitly AGENT-CLAIM authority — the runner never witnessed the mutation, so `observed: red` does NOT close the no-op-mutation class. EVIDENCE ONLY: the block never touches `res.OK`, `res.FailureCategory` or budget. `kind` is additive on the wire (`omitempty`): an older bundle carrying no `kind` decodes to the empty string and its row renders exactly as it did before the field existed.
...
|---|---|---|---|
| absent (nothing at the path, confirmed by `os.Lstat`) | `nil` | none (the common no-op) | still absent |
| present but UNREADABLE (a directory at the path, a permission failure, a DANGLING SYMLINK) | `nil` | `counterfactual_report_unreadable`, plus `counterfactual_report_unremovable` if cleanup ALSO failed | REMOVED (recursively), or SURVIVES and is named as such |
...
The absent-vs-unreadable split is a deliberate improvement over `loadFixupSelfReport`, which returns on the read error BEFORE installing its deferred removal and so leaves an unreadable sidecar on disk contrary to its stated every-return-path cleanup invariant.

**Absence is decided by `os.Lstat`, NOT by the read error.** `os.ReadFile` follows symlinks, so a DANGLING SYMLINK at the keyed path — present agent-authored state — reports the missing TARGET's `ENOENT`, byte-indistinguishable from genuine absence. A loader trusting the read error alone would take the silent no-op branch and leave that symlink on disk, bypassing the cleanup row above entirely. So the not-exist branch re-checks with `pathExists` (`os.Lstat`, which does not follow the link): actual absence returns silently, a present-but-broken link falls through and is removed fail-closed under `counterfactual_report_unreadable`. `TestLoadCounterfactualReport_DanglingSymlinkIsPresentState` pins it, asserting the log and the on-disk state via `os.Lstat` — the return value is `nil` either way, and an `os.Stat` absence check would report not-exist for a SURVIVING dangling symlink and green the bug.

**Cleanup failure is DETECTED and surfaced, never swallowed.** An agent controls what sits at the sidecar path, so it can construct one for which both the read and the recursive removal fail (a non-empty directory whose parent denies write). Reporting only `counterfactual_report_unreadable` would let a PERSISTENT path read as successfully cleaned, so a failed removal emits its own `counterfactual_report_unremovable` line naming the error — BESIDE the unreadable line, not instead of it, since the read failure and the cleanup failure are two distinct facts. The same holds on the SUCCESSFULLY READ path: the deferred consumption reports its failure too, because the return value is identical either way and only the log and the surviving path can tell an operator what happened. The stale log echoes the AGENT-AUTHORED embedded ids through `safeSidecarID`, which bounds each to 64 bytes with a `…` marker: `%q` escapes control characters but does not bound length, so without it an agent could push arbitrary-length text into the runner log.
...
**The two channels are EXACT COMPLEMENTS.** The initial-pass branch is guarded by `stageType == "implement" && !cfg.fixup`, the precise negation of the fix-up block's `cfg.fixup`, so exactly ONE channel reads per pass and a claim can never be double-counted into `gate_evidence`. `TestCounterfactualChannels_AreExactComplements` pins both branch CONDITIONS structurally, by parsing `main.go` — run() takes `(args []string, logSink io.Writer)`, spawns the agent CLI and does real git work, so the conditions have no other vehicle — along with the evidence-only invariant.

**What each branch DOES is pinned behaviorally, not structurally.** The two branch bodies live in `emitInitialCounterfactuals` and `sweepUnreadCounterfactualReport` precisely so a unit test can drive them against a redirected `counterfactualReportDir`: `TestEmitInitialCounterfactuals_AppendsTheEventAndConsumesTheSidecar` asserts the `fixup_counterfactuals` event actually lands on `res.Events` with the validated triples and the sidecar is gone from disk, and `TestSweepUnreadCounterfactualReport_RemovesTheFileAndDoesNotValidate` asserts the file's absence POSITIVELY via `os.Stat` (rather than inferring it from a missing event, which is equally consistent with the file being left unread) plus the absence of a `fixup_counterfactual_dropped` line, which is the observable signature of the sweep having READ the sidecar it must only delete — its fixture entry is deliberately INVALID so that half can go red at all. It makes NO claim about `res.Events`: `sweepUnreadCounterfactualReport` never receives `res`, so such an assertion is a tautology no implementation can redden; the mutual-exclusion property lives in the two tests named around it instead. A text match cannot reach either: a call placed under dead code, or a sweep that runs but fails to remove the file, passes the structural assertion and reddens the behavioral one — both proven by deletion. `TestFixupBranch_SweepsTheInitialPassSidecar` is correspondingly narrowed to the WIRING claim: the fix-up branch reaches the sweep helper and does NOT call `loadCounterfactualReport`.

**`sweepStaleCounterfactualReport` is called from TWO places, and each names its OWN reason** — the two situations are different facts about the run and an operator must be able to tell them apart from the log alone, without inferring from where in the stage the line appeared:

| Call site | `reason` | What a removed file means |
|---|---|---|
| pre-invoke, beside `sweepStaleFixupSelfReport` | `pre_invoke_stale_leftover` | a leftover from a prior retry of this same run/stage; no agent on THIS attempt authored it |
| end of the fix-up branch (`sweepUnreadCounterfactualReport`) | `fixup_pass_authored_unread` | one THIS pass's agent wrote, which the fix-up channel deliberately does not read (a fix-up carries its counterfactuals in the #3042 self-report sidecar instead) — swept rather than left in `/tmp`, where a human would find a file that looks like live evidence |

Removal is RECURSIVE (`os.RemoveAll`), because an agent can put a non-empty DIRECTORY at the sidecar path that a non-recursive `os.Remove` could never clear; presence is decided by an `os.Lstat` first, since `os.RemoveAll` reports success for an absent path and the common no-op must stay silent. A sweep that FAILS emits `counterfactual_report_sweep_failed` with the error instead of `counterfactual_report_swept` — a path that survives is never reported as removed.

That recursion is pinned POSITIVELY by `TestSweepStaleCounterfactualReport_NonEmptyDirectoryIsRemovedRecursively`: a non-empty directory whose parent is WRITABLE, asserting `counterfactual_report_swept` and `os.Lstat` not-exist afterwards. The writable parent is what makes it discriminating — `TestSweepStaleCounterfactualReport_RemovalFailureIsSurfaced` denies the parent write, a fixture on which BOTH `os.Remove` and `os.RemoveAll` fail, so it would pass identically under either implementation and the claim above would rest on nothing.
...
### Approval-condition responses — commit-message sidecar PEEK (#3400)

Beside the fix-up self-report above, EVERY implement pass kind (initial, decomposed child, fix-up) PEEKS its commit-message sidecar before the trace bundle is packed (`peekApprovalConditionResponses`, `approvalresponses.go`; wired under `stageType == "implement"` in `run()` ahead of `composeGateEvidence`). It selects `fixupCommitMessagePath` when `cfg.fixup` else `implementCommitMessagePath`, reads through `readSidecarBounded` WITHOUT deleting (the consuming loaders `loadImplementCommitMessage` / `loadFixupCommitMessage` still consume + delete at commit time), and parses with the SAME `parseCommitMessageSidecar` the loaders use — so not-exist, oversize (`errSidecarTooLarge`; the loader owns the `*_commitmsg_oversize` diagnostic, the peek logs nothing new), empty and no-`Approval conditions:`-section are all ABSENT to the peek exactly as they are to the loader (`TestPeekAndConsumeAgree`). `extractApprovalConditionResponses` returns the lines after the FIRST line whose TrimSpace is exactly `Approval conditions:` (a bare heading with a whitespace-only remainder is not found); the text is pre-redacted with `redactString` (load-bearing: the review dispatches on the RAW bundle, #793) and THEN bounded to 4 KiB on a rune boundary (`truncated` flag) — redaction runs first because a credential straddling the cut would otherwise be truncated to a prefix the patterns no longer match (`ghp_` needs exactly 36 trailing chars) and leak its leading bytes; redacting the full text first means the cut can only land inside a marker, and the bound still holds because it is applied last (pinned by the straddling-credential case in `TestPeekApprovalConditionResponses`, red with the order reversed) — then emitted as an `approval_condition_responses` event `{run_id, stage_id, source: implement_commitmsg|fixup_commitmsg, text, truncated}` with an `approval_condition_responses_captured` log line; `composeGateEvidence` folds it into `gate_evidence.approval_condition_responses` (`approvalConditionResponsesEvidence`, pinned against the backend's `bundle.ApprovalConditionResponsesEvidence` by the shared `approvalConditionResponsesWireFixture` literal in both modules). EVIDENCE ONLY: never touches `res.OK` / `res.FailureCategory` / budget.

**Peeked ≠ committed — the two are logged distinctly.** The peek runs at pack time, BEFORE the commit exists (the commit is made after the #742 forward-gated upload), so what it captures is the agent's PROPOSED commit message, and the backend's reviewer text says exactly that. The persistence half is `logApprovalConditionResponsesCommitted`, called in `openPRAndShipArtifact` immediately after `CommitAndPush` returns a head: it logs `approval_condition_responses_committed {head_sha, source}` when the message actually committed carries the section (no head, i.e. `NoChanges`, or no section logs nothing). So a `captured` line with NO later `committed` line is the distinguishable unpersisted state — the commit failed, parked, the run halted, or the message fell back — rather than reading identically to a recorded response. Residual, stated honestly: the commit ACTUALLY made can differ from the peeked sidecar only via the loaders' fallback path (a sidecar absent/oversize/empty at consume time → synthetic message), in which case the peek also reported absent unless something rewrote the file between pack and commit; the agent invocation has returned by then, so nothing sanctioned writes it. Pinned by `TestPeekApprovalConditionResponses` (read-not-delete, fixup path selection, not-exist / oversize / empty / no-section → nil, redaction, truncation), `TestLogApprovalConditionResponsesCommitted`, `TestImplementPrePackRegion_PeeksApprovalConditionResponses` (peek → real composer → packed member present/absent), and the two wiring pins `TestImplementBranch_PeeksApprovalConditionResponsesEvidenceOnly` / `TestOpenPRAndShipArtifact_LogsApprovalConditionResponsesCommittedAfterCommit`.
