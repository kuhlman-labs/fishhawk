...
It proves the same NARROW SYNTACTIC PROPERTY (a `.go`-only `A`/`M` change set whose emitted changed lines are blank or ordinary `//` comments, no directive, exactly one hunk-bearing section per file, and neither a backtick nor `import "C"` in the section) over `Diff.Patch` — which `computeAndEmitDiff` now carries alongside `Diff.PatchTruncated`, the SAME text the `git_diff` event ships, so the runner and the backend read one patch. Every other input is fail-closed against the same closed `Reason` set. It is NOT behavioral emptiness, and it knowingly admits TWO residuals, both documented in `backend/internal/policy/README.md`: a changed line inside a raw string whose backticks lie outside the emitted context, and a changed cgo-preamble `//` line (compiled C) in a file whose `import "C"` lies outside the emitted context.
