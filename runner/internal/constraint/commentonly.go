...
// MIRROR (#2660): this file is a logic-identical port of
// backend/internal/policy/commentonly.go — the same duplication
// IsGeneratedPath and isTestPath already live under, because this module
// deliberately carries no dependency on the backend's policy package.
// CHANGE THEM TOGETHER; a divergence makes the runner fail a stage
// category-B that the (authoritative) backend would admit, or vice versa.
//
// ASYMMETRY: the backend derives this verdict ONCE at trace-upload time
// and carries it on policy.Constraints so its post-CI re-evaluation — which
// reconstructs the diff from the audit payload and therefore has no patch —
// re-emits rather than recomputes it. The runner has no signal channel and
// no audit round trip, so it computes the verdict IN-LINE from the patch it
// just captured, the same way it already diverges on verification_reported.
...
// change this detector refuses. See runner/internal/constraint/README.md.
