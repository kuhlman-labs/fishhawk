...
The prompt travels as the trailing `codex exec … <prompt>` positional argv string, so a prompt past the OS argument-size limit is refused by `cmd.Start` with `E2BIG`. `Invoke` classifies it — after the binary-missing arm — via `agent.IsArgListTooLong` into `agent.ErrPromptTooLarge` (`err_class=prompt_too_large`), wrapping the prompt byte count and `agent.ArgvBytes(binary + args)`. No `Result` fields are set and no event beyond `invocation_start` is emitted. `TestInvoke_PromptTooLarge_NamedSentinel` performs a REAL spawn through `forwardingHelperCommand` (which forwards the adapter's argv, unlike the other helpers) with a 4 MiB prompt and asserts the named sentinel on the live `fork/exec` error.
