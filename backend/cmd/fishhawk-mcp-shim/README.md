...
`--status` spawns no child and reads no stdin — it only reads the state dir, so running it against a live session is safe. It also prunes the leftover snapshots of shims that are no longer running — and only those (see below). It hashes each snapshot's `child_path` to compare against the launch baseline, but only when that path is a **regular file**: `child_path` is untrusted input, and opening a FIFO or a device node would block a reader that has no timeout. Anything non-regular, unstattable or unhashable classifies `pending`, never `stale`.
...
- **schema-major bump, shim not registered (or the shim binary was itself rebuilt)** — a `.fishhawk/workflows.yaml` `version:` MAJOR change; unconditional and first in precedence (#1422). The stern `ACTION REQUIRED` wording: the live `fishhawk-mcp` validates the spec locally on its stale embedded schema, so the loop stays broken until you run `/mcp`. The rebuilt-shim conjunct is fail-safe — a running shim keeps its open inode, so a shim-binary rebuild needs a manual `/mcp` regardless and the stern wording stays correct.
- **schema-major bump, shim registered and not itself rebuilt** — the same bump and the same precedence slot, but a different body (#2349): the registered shim **should have hot-swapped** the rebuilt child, so the new major is expected to be live already and the banner asks you to VERIFY (a `fishhawk_doctor` reporting `spec.valid: true`, or a version-returning tool call reflecting the new GitSHA) rather than reconnect. Like the auto-swap note it states the swap as an expectation, never as fact — `scripts/dev` cannot observe the harness-owned shim — and names `bin/fishhawk-mcp-shim --status` then `/mcp` as the refutation path (#2831).
...
`scripts/dev up` (inherited by `reload`/`post-merge`) also runs a **non-fatal stale-shim advisory**, mirroring the stale-worktree one: it probes `bin/fishhawk-mcp-shim --status --stale-only` and, on non-empty output, prints a warning naming each stale shim and pointing at `/mcp`. Every degrade is silence — no shim binary, a non-executable path, a non-zero probe exit, empty output, probe stderr (which is discarded outright, so a future diagnostic writing to stderr cannot turn the advisory into noise), and a probe that **does not return in time**: the call is wall-clock bounded (`_run_with_timeout`, 5s) because `cmd_up` runs it before anything else, so an unbounded probe would hang `scripts/dev up`/`reload` rather than degrade.
