...
// replacement (rather than addition) of the scope set when write:deploy was
// added: the pre-existing operator scopes must still be present.
func TestOperatorDefaultScopes_RetainsBaseline(t *testing.T) {
	for _, want := range []string{"read:runs", "read:audit", "write:runs", "write:approvals", "write:stages"} {
