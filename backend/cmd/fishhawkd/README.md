...
|---|---|---|---|
| `FISHHAWKD_DEV_FIXTURES` | `--dev-fixtures` | With `--db`: registers `GET/POST /v0/dev/fixtures` and `POST /v0/dev/sign` (a `WARN` names the surface at boot). Every route refuses a non-loopback peer `403 dev_surface_loopback_only` and is credential-less (no CSRF exemption is needed — the csrf middleware passes a session-less identity). Without `--db` the surface stays OFF and the log says so (`dev fixtures requested but no database configured; surface stays off`). **Trace-store consequence:** with `FISHHAWKD_S3_BUCKET` unset the daemon selects an in-memory trace store (`trace store: in-memory (dev fixtures)`) so the preview's `POST /v0/runs/{id}/trace` → `cost_recorded` → `spend_alert` / `unpriced_model_alert` path is drivable (#1874); a configured bucket always wins. If the trace store is the ONLY thing you want, use `--dev-trace-store` instead — it selects the same store without mounting a dev surface, so forge writes stay live (E45.76 / #3601). | `false` |
| `FISHHAWKD_DEV_STUB_FORGE` | `--dev-stub-forge` | **DEV ONLY (E72.3 / #3327).** Serves GitHub AND GitLab from ONE in-process stub forge (`backend/internal/forge/stub`): `cfg.GitHub` and the registered `gitlab` forge are pointed at `http://stub-github.invalid` / `http://stub-gitlab.invalid` through an in-process round-tripper (no socket, no second port), the gate merger is bound as the App block binds it, and the six loopback-only, credential-less `/v0/dev/forge*` control routes are registered (snapshot, reset, seed/read issues, seed pulls, signed in-process webhook delivery). **Refuses to coexist** with `FISHHAWKD_GITHUB_APP_ID` or `FISHHAWKD_GITLAB_TOKEN` — boot fails with `dev stub forge misconfigured … --dev-stub-forge cannot coexist with a configured GitHub App or GitLab token`. **Defaults both webhook secrets** (`FISHHAWKD_GITHUB_WEBHOOK_SECRET` / `FISHHAWKD_GITLAB_WEBHOOK_SECRET`) to the stub's fixed dev constants when unset, so both receivers and the delivery store come up; an operator-supplied secret wins per family. A `WARN` (`DEV STUB FORGE ENABLED`) names the surface at boot. Does not need `--db`. Never enable in production. |  `false` |
| `FISHHAWKD_DEV_TRACE_STORE` | `--dev-trace-store` | **DEV ONLY (E45.76 / #3601).** With `FISHHAWKD_S3_BUCKET` unset, selects the same in-memory trace store `--dev-fixtures` does — boot line `trace store: in-memory (dev trace store)` — but mounts **NO dev surface**, so the daemon is NOT in dev mode: forge writes stay live, host-dispatch is not refused, and `/healthz` omits `dev_mode`. The store is **EPHEMERAL**: every trace bundle is lost on restart, which the boot log says in its own `WARN` (`trace store is EPHEMERAL … selected_by=dev trace store`). A configured bucket always wins; `--dev-fixtures` wins the `selected_by` attribution when both are set. Prefer `FISHHAWKD_S3_BUCKET` + `make s3-init` for a durable local loop. Does not need `--db`. | `false` |

**A dev-mode boot emits a one-line consequence `WARN`** (`DEV MODE ACTIVE (… mounted): forge writes are DENIED …`, E45.76 / #3601) naming every consequence above plus both trace-store alternatives, emitted once after both surfaces resolve. It is keyed on the MOUNTED surfaces, not on the flags: `--dev-fixtures` without `--db` mounts nothing, so that boot draws no denial warning — a daemon never warns about a denial it is not applying.
