...
func implementStageNextActions(run *Run, impl, acceptance *Stage, implementReviewStatus *ReviewStatus, hint *ReviewActionHint, acceptanceSkippedOutOfScope bool, acceptanceVerdict, acceptanceTriageDisposition string) *NextActions {
	...
	case "dispatched":
		...
		return &NextActions{
			...
			Actions: []SuggestedAction{pollAction(run,
				suggestedStageWaitPollIntervalSeconds,
				"the implement stage is dispatched — a spawn attempt exists (#1912) and a runner is in flight; re-poll until implement_stage_wait_status goes terminal (fishhawk_drive_run auto-re-dispatches a probed-dead runner and only hands back a manual re-dispatch when its liveness probe is ambiguous — a live-but-unregistered process or pgrep unavailable)")},
		}
