...
type GetPlanOutput struct {
	...
	// SurfaceSweep surfaces the plan-gate multi-surface pattern sweep
	// (#763): scope.files matched against a static registry of surfaces that
	// must move in lockstep with siblings.
	SurfaceSweep *SurfaceSweep `json:"surface_sweep,omitempty" jsonschema:"plan-gate surface sweep (#763): when scope.files touches one instance of a known multi-surface pattern (an @-mention render site, or an audit-kind-emitting surface) but omits its sibling surfaces, each missing sibling is flagged here. Present (possibly with empty findings) when the plan stage ran the sweep; absent on older runs predating it. Advisory only — a non-empty findings[] means the plan likely needs to add the named sibling paths to scope.files so the change stays consistent across all surfaces it touches"`
}

// SurfaceSweepFinding is one matched multi-surface pattern decoded from a
// plan_surface_sweep audit entry (#763). Pattern names the registry entry;
// TriggerPath is the scope.files path that matched; MissingSiblings lists
// the sibling surface paths absent from scope.files. Mirrors the
...
// plan's scope.files touch no multi-surface pattern incompletely;
// ScannedFiles is the number of scope.files the sweep evaluated.
type SurfaceSweep struct {
	Findings     []SurfaceSweepFinding `json:"findings,omitempty" jsonschema:"sibling-surface gaps; empty when the plan's scope.files touch each matched multi-surface pattern completely (or touch none)"`
...
// for the run and decodes its payload into a SurfaceSweep. Mirrors
// loadScopePrecheck exactly: the per-run audit endpoint returns entries
// sequence-ascending so the authoritative entry is the last one (a
// schema-retry run re-opens the plan stage and writes a second sweep on the
// re-upload). Returns nil when no entry exists (an older run predating the
