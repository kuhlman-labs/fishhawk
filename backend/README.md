...
- **Issue-set resolution budget** (E54.59 / #3113) — `--issue-set-resolution-budget` (or `FISHHAWKD_ISSUE_SET_RESOLUTION_BUDGET`, default `120s`, permitted maximum `10m`) bounds the no-epic campaign source's issue-set dependency resolution on `POST /v0/campaigns`. That path costs one forge round-trip per named item, so a full ratified grooming order can run for minutes; before #3113 it had **no server-side deadline at all** and the caller's own client timeout was what gave up, producing a transport error carrying no counts. On expiry the endpoint now answers `504 issue_set_resolution_timeout` naming how far it got (`resolved` of `items_total`) and, when one can be proven to fit, a `suggested_grooming_order_limit` to retry with. A **negative** value, or one **above the permitted maximum**, REFUSES startup naming both the configured value and the maximum: the maximum is what the MCP client's issue-set request timeout is set above, so a larger budget would put the client back in the position of being what gives up first. `server.issueSetResolutionBudget()` clamps to the maximum at the read site regardless of how the `server.Config` was built, so the relationship holds for embedded uses too. The **epic** path (`epic_ref` present) is not bounded by this budget — it reads the sibling set in one call.

- **Review grounding** (#2486) — `--review-grounding` (or `FISHHAWKD_REVIEW_GROUNDING`, default `false`) grounds the plan-/implement-review agents against an exported read-only source tree at the reviewed commit, so a reviewer can read and search the repository's tracked files to confirm diff-invisible facts. Set `FISHHAWKD_REVIEW_GROUNDING=true` to opt in on a trusted single-tenant host; the environment scrub is independent of this flag and always applied. **Grounding ships DORMANT and the default STAYS `false`** (#2522): a grounded reviewer's reads are now bounded per adapter — codex gets OS-enforced deny-by-default confinement via a synthesized `confined` permission profile, claude gets a bounded credential-root deny-rule BLOCKLIST that is defence-in-depth and explicitly NOT confinement — but flipping the default is a SEPARATE operator-filed follow-up gated on a recorded operator run of the opt-in live harness passing on BOTH adapters. `--reviewer-env-passthrough` (or `FISHHAWKD_REVIEWER_ENV_PASSTHROUGH`, comma-separated EXACT variable names) appends to each reviewer adapter's scrub allow-list — the escape hatch for a Bedrock/Vertex/proxy deployment whose auth vars the minimal built-in list omits. Since E45.90 / #3625 the posture is operator-visible: `fishhawk doctor` (and the `fishhawk_doctor` MCP tool) renders a deployment-scoped `review_grounding` rung from `GET /v0/onboarding/readiness` reporting whether grounding is on and restating the per-adapter bounds honestly — it is a readout only, the default is unchanged, and `off` renders at status `ok` with a hint rather than a warning. Details: `backend/internal/reviewsandbox/README.md`; rung contract: `backend/internal/server/README.md` § "Review-grounding rung".
...
- **Per-stage budget enforcement** (`stage_budget_exceeded`, #2328 / E48.55) — the stage-scoped sibling of the per-run tripwire: on each raw trace upload the backend (`trace.go::checkStageBudget`) sums a single stage's recorded `cost_usd` (deduped by bundle `content_hash`) and compares it against the stage's workflow-declared `budget.limit_usd`, but **only for a spec major ≥ 2 with a positive `limit_usd`** — `workflow-v0`/`workflow-v1` cannot express `limit_usd` (their `$defs/budget` is `additionalProperties:false`), so legacy specs never enforce a stage ceiling. `enforcement: blocking` cancels the run (terminal `cancelled`) and short-circuits further advancement; `enforcement: advisory` (the default this repo declares on every agent stage) records the audit entry and lets the run proceed. Operator-facing: enable a ceiling by declaring `budget.limit_usd` on a v2 stage; `max_tokens`/`max_runtime` are decoded but **not** enforced (calibration-pending, #2328). No config knob and no Notifier surface — the ceiling is a spec field, evaluated per stage. The single-owner predicate + fail-open ladder live in `backend/internal/budget/README.md`.
