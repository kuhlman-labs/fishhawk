...
- `--workmgmt-conventions` (or `FISHHAWKD_WORKMGMT_CONVENTIONS`) — path to a YAML work-management conventions file served for **every** repo, replacing the shipped `Default()` stub (ADR-058 Phase 2, #1856). It is **parsed fail-fast at startup**: an unreadable or invalid file aborts serve with a precise error naming the path + cause, rather than serving a half-configured provider. This deployment-level override is the honest minimum that unblocks a non-`github_projects` provider (e.g. `provider: gitlab`) end-to-end for a single-tenant deployment; the true in-repo per-repo loader (which fetches `.fishhawk/work-management.yaml` from each repo) is deferred to #2022 because the server cannot know which forge to fetch that file from before the conventions themselves declare the provider. Unset leaves the `Default()` behavior unchanged.
