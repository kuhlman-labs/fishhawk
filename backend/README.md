...
- **Review grounding** (#2486) — `--review-grounding` (or `FISHHAWKD_REVIEW_GROUNDING`, default `true`) grounds the plan-/implement-review agents against an exported read-only source tree at the reviewed commit, so a reviewer can read and search the repository's tracked files to confirm diff-invisible facts. Set `FISHHAWKD_REVIEW_GROUNDING=false` to disable the behaviour change (both adapters revert to a diff-only posture) without a rollback; the environment scrub is independent of this flag and always applied. `--reviewer-env-passthrough` (or `FISHHAWKD_REVIEWER_ENV_PASSTHROUGH`, comma-separated EXACT variable names) appends to each reviewer adapter's scrub allow-list — the escape hatch for a Bedrock/Vertex/proxy deployment whose auth vars the minimal built-in list omits. Details: `backend/internal/reviewsandbox/README.md`.
