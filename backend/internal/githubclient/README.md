...
## Repository issue enumeration (#2230)

`ListRepoIssues` (`projects.go`) enumerates a repository's ISSUES via the GraphQL `repository.issues` connection — the primitive the work-management read capability (`workmgmt.WorkItemReader`, see `../workmgmt/README.md`) lists through.

- **Never a ProjectV2 board item list.** A board item list caps at 100 and truncates silently (the AGENTS.md Project #7 pagination trap); the search REST API caps a result set at 1000 (`searchByTitleMaxPages`). This connection paginates to completion instead: `first:100` pages ordered ascending by number, following `pageInfo.endCursor` while `hasNextPage` is true. Modelled on `ListSubIssues` (#2102).
- **Fail-closed page cap.** `listRepoIssuesMaxPages = 100` (10 000 issues). Reaching it with more pages remaining returns an error naming the repo and the accumulated count, and NO partial slice. A nil `repository` node is likewise a hard error, not an empty set.
- **Filters.** `ListRepoIssuesOptions.Labels` and `.States` reach the `labels:` / `states:` connection arguments; an empty filter encodes as GraphQL `null` ("no filter"), never an empty list.
- **Optional board status.** With `WantBoardStatus` and a `ProjectID`, each node also selects `projectItems` and the item whose `project.id` matches supplies `BoardStatus` — the same unambiguous match `ProjectItemStatus` makes. The nested page is bounded (`listRepoIssuesProjectItemsFirst = 20`) and **fails closed on truncation**: a FULL page that does not carry the target project makes board membership undecidable, so the call returns a typed `*BoardMembershipUndecidableError` (naming the repo, issue, cap and target project — matchable with `errors.As`, not just greppable) rather than reporting `OnBoard=false` for an issue that may well be on the board. Without the opt-in the `projectItems` selection is omitted from the document entirely, so a caller pays neither the selection nor its Projects (v2) permission surface.
- Routes through `doGraphQL`, so `ErrForbidden`/`ErrValidation` classification and the `WithProjectsToken` opt-in (#1114) both apply.

## Sub-issue PARENT query — `IssueParent` (#2179)

`IssueParent` (`projects.go`) is the parent-direction peer of `ListSubIssues`: given an issue number it resolves that issue's sub-issue PARENT (number + title) in ONE GraphQL round trip (`repository(owner,name){ issue(number){ parent { number title } } }`), going number → parent directly so it needs no `IssueNodeID` pre-step. It is the query the live-validation walk hook (`server/live_validation_filing.go`, #2179) needs to resolve the TRUE epic of a triggering CHILD issue.

- **A nil parent is NOT an error.** A nil `repository`, `issue`, or `parent` all return `(nil, nil)` — an issue with no sub-issue parent is the normal answer — as does a parent that decodes with `Number <= 0` (a bogus `#0` ref is never returned).
- **Guards reject before any request** (blank owner/name, `number <= 0`), mirroring `ListSubIssues`' guard style. Sub-issues are repo-scoped, so it uses the installation-token path (no `WithProjectsToken` opt-in) and propagates `doGraphQL`'s `ErrForbidden`/`ErrValidation`/transport errors verbatim.
