...
// #2215), RESHAPES (E52.6 / #2218) or RENAMES (E52.5 / #2217). Raw JSON Schema
// would reject all five — the bare token falls out of the must_page_human
// enum, `agent`, `drive` and `budget.max_runtime_minutes` become undeclared
// properties of an additionalProperties:false block, and a list-form
// `constraints` fails the object type — but none of those generic messages
// names the replacement surface, which is the whole point of the sweep. The
// CLI carries a byte-identical set in cli/internal/spec/validate.go; the two
// modules are deliberately separate (see that package's doc comment), so the
// strings are kept in lockstep by the message-content assertions on both sides
...
// key, any `drive` key, any `constraints` value that is a LIST, and any
// `budget` map carrying a `max_runtime_minutes` key. It is
...
// `must_page_human` that is not an array, a `reviewers` or `budget` that is
// not a map, or a `constraints` that is neither a list nor an object — those
// shapes are not a legacy form, and schema validation (which runs next)
// reports them.
...
// map node. The five forms are checked in a FIXED order — page event,
