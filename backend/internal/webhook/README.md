...
- **Auth is a verbatim token, NOT an HMAC.** GitLab sends the configured secret token verbatim in `X-Gitlab-Token`; `VerifyGitLabToken` compares it constant-time against the configured secret. This is the load-bearing difference from the GitHub `X-Hub-Signature-256` HMAC path.
