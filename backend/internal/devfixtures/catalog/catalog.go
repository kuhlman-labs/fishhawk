...
var descriptions = map[string]string{
	"acceptance-dispatched": "feature_change run: plan, implement and review succeeded, acceptance stage (agent) dispatched with a pull_request_opened head and an acceptance_dispatched anchor; carries an approved standard_v1 plan whose verification.acceptance_criteria declares two drivable criteria plus a pull_request artifact, so the acceptance prompt serves real criteria ids — ready to receive an acceptance transcript and verdict.",
	...
	"plan-gate-criteria":    "feature_change run: plan stage parked at awaiting_approval carrying a valid standard_v1 plan whose verification.acceptance_criteria declares two drivable criteria, plus implement/review/acceptance stages at pending — drives the approve-time amend_acceptance_criteria channel (retire / restate / add) and renders the resulting acceptance prompt via the pending acceptance stage.",
