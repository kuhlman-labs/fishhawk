...
| `plan-gate-criteria` | `feature_change` run, running (#3181). `plan` (plan, agent, requires_approval) at `awaiting_approval` carrying a valid `standard_v1` plan whose `verification.acceptance_criteria` declares two DRIVABLE criteria (`run-readable` / `stages-listed`); `implement` / `review` / `acceptance` (sequence 2-4) at `pending`; no approvals. The one scenario that can drive the approve-time `amend_acceptance_criteria` channel — `plan-gate-parked`'s plan carries no criteria, so the channel refuses it `422 acceptance_criteria_unavailable` — and, after the approve, `GET /v0/stages/{acceptance_stage_id}/prompt-render` renders the effective acceptance prompt (the operator-authored block for an `add` included). Pinned by `TestEmbeddedScenarios_DeclaredShapes/plan-gate-criteria`. |
| `split-parent-linked` | TWO `feature_change` runs on repo `stub/parent-close`, running, one per forge family (`github-run` / `gitlab-run`), each with a single `plan` stage at `succeeded` and ONE `split_children_filed` audit row in the shape `server.writeSplitChildrenFiledAudit` produces: `parent_repo` `stub/parent-close`, `parent_issue` `100`, `contract_child_number` `103`, `contract_classification` `contract`, empty `children`, and `parent_forge` `github` / `gitlab` respectively. This is the E50.6 parent-close LINKAGE (E72.3 / #3327): the watcher enumerates the category with `AuditRepo.ListAll` (spans both chains; untenanted rows are visible to every caller), so a signed `issues.closed` / `Issue Hook` close delivery for `#103` through the preview's in-process stub forge closes parent `#100` with one comment. Pinned by `TestApply_SplitParentLinked_LinkageRowsListableByCategory`. |
| `acceptance-dispatched` | `feature_change` run, running. `plan` / `implement` (agent) and `review` (human, requires_approval) at `succeeded`; `acceptance` (acceptance, agent, sequence 4) at `dispatched` — ready for a signed `POST /v0/runs/{id}/acceptance/transcript` followed by `POST /v0/runs/{id}/acceptance` (E72.5 / #3329). Carries the full post-approval artifact set (#3397): a `standard_v1` plan artifact on `plan` whose `verification.acceptance_criteria` declares two DRIVABLE criteria (`run-readable` / `stages-listed`), an `approve` approval on `plan`, and a `pull_request` artifact on `implement` whose `head_sha` equals the `pull_request_opened` head — so the acceptance prompt serves real `acceptance_criteria_ids` and a shipped verdict can be driven against seeded criteria. Two audit rows in ORDER: a `pull_request_opened` row on `implement` carrying a 40-hex `head_sha`, then an `acceptance_dispatched` anchor on `acceptance` (shaped after `emitHostDispatchAcceptanceAnchor`), so `acceptanceValidatedHeadSHA` resolves and a shipped verdict is not clamped to `undecidable(basis=head_unresolved)`. The first catalog scenario past the plan stage (the #3397 problem-1 gap). Pinned by `TestEmbeddedScenarios_DeclaredShapes/acceptance-dispatched`. |
...
| `apply_pg_test.go` | `pgtest.NewPool` | committed-state read-backs of every scenario through the real repositories (chain links + recomputed hashes for the baseline rows; the split-parent-linked linkage rows listable by category) and fresh rows per call. |
