...
- `infra_flake` — latest failed stage is category-A with an infrastructure signature in its `FailureReason`, which embeds the verify output verbatim: the literal `verify_infra_flake_retry` marker, the #972 testcontainers start-flake signature, or (widened alongside the runner's matcher by #2718) testcontainers-go's `port "9000/tcp" not found` rendering and the Docker daemon-unreachable markers. The set MIRRORS `isVerifyInfraFailure` in `runner/cmd/fishhawk-runner/main.go`; the two live in different Go modules and cannot share a fixture, so each is pinned against the same corpus of verbatim observed outputs.
