...
Each `ChangedFiles` row also carries `OldPath`, the rename/copy SOURCE path from the runner's per-file `old_path` field (#2398): populated only for `R`/`C` rows, empty for ordinary rows, and empty for older bundles that predate the field (no error, no fabricated default). It is informational — no policy check reads it — so it never affects constraint evaluation; the backend's scope-provenance evidence uses it to map a declared-deleted path to its rename destination rather than reporting it untouched. The `old_path` json tag is part of the lockstep runner↔backend wire contract (mirror of the runner's `gitDiffFile`), pinned by the shared `testdata/wire/git_diff_rename_event.json` fixture both modules' tests read.
...
`ExtractGateEvidence` returns the digested gate results from the runner's synthesized `gate_evidence` event (#963) — verify run/summary outcomes with bounded pre-redacted tails, flake retries, scope facts (including the per-path A/B drift categories, #991; nil on older bundles), constraint violations, and the agent's peeked commit-body `approval_condition_responses` section (`ApprovalConditionResponsesEvidence{source, text, truncated}`, #3400; nil on older bundles or when no sidecar carried an `Approval conditions:` section; agent-authored free text the reviewer prompt renders only inside an untrusted envelope) — or `ErrNoGateEvidence` when absent (older bundles / no-gate stages; fail-open like `ErrNoHeadSHA`), surfaced to the implement-review prompt's "Gate evidence" section.

`ExtractVerifyTreeSHA` (#3655) returns the tree_sha of the bundle's AUTHORITATIVE verify_run — the LAST non-empty one — or `ErrNoVerifyTreeSHA` (fail-open: an absent tree never suppresses the review, it only disables the ship-side `review_head_mismatch` check). It is the implement-review round's reviewed-tree identity, stamped on `implement_review_started`. The rule is deliberately the OPPOSITE of `ExtractHeadSHA`'s first-non-empty: `ExtractHeadSHA` needs any deterministic #797 dedup key, while this needs the tree the gate CERTIFIED — a multi-iteration `runVerifyFixLoop` emits one verify_run per iteration and only the terminal one is authoritative (`VerifyRunEvidence.Superseded` marks the absorbed ones), so first-non-empty would return a failing iteration's tree and manufacture a false mismatch. Same authoritative-is-last rule as `ExtractGateEvidence`. `TestExtractHeadSHA_FirstNonEmptyUnchangedOnMultiIterationBundle` pins that the two rules stay different.
