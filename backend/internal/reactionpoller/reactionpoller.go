...
type ReactionLister interface {
	ListIssueCommentReactions(ctx context.Context, scope forge.CredentialScope, repo githubclient.RepoRef, commentID int64) ([]githubclient.IssueCommentReaction, error)
}
...
func (t *Ticker) pollStage(ctx context.Context, logger *slog.Logger, now time.Time, s *run.Stage) bool {
	...
	reactions, err := t.Reactions.ListIssueCommentReactions(ctx, forge.FromGitHubInstallationID(*runRow.InstallationID), repo, commentID)
	if err != nil {
