...
- Reads the deploy stage's `executor.delegate` config and, by target, fires a github_actions `workflow_dispatch` of `workflow_ref` (carrying the `fishhawk_run_id`/`fishhawk_stage_id` correlation as inputs, then best-effort `ResolveDispatchedRun` for the run id) or POSTs to a webhook `url` — carrying the delegate's secret channel when declared (E45.57 / #3497: `secret_env` resolved from fishhawkd's own environment at dispatch, placed in the `secret_header` — default `PRIVATE-TOKEN` — or the `secret_field` body key; an unset/empty variable fails the stage naming the variable, never a value). The webhook body is `{fishhawk_run_id, fishhawk_stage_id, repo, workflow_id, variables: {FISHHAWK_*}}`: the additive `variables` object and the redirect refusal below are INTENTIONAL tightenings that apply to EVERY webhook target, not only GitLab — a relay validating with `additionalProperties: false` must admit `variables`, and a target that relied on fishhawkd following a 3xx now fails the stage with that status. Neither POST ever follows a redirect (`CheckRedirect` → `ErrUseLastResponse`; a 3xx is a non-2xx failure carrying only the status), transport errors are recorded by class not string, and the resolved secret is redacted from every sink. Contract: `backend/internal/server/README.md` § "Webhook delegate secret channel".
...
- It re-dispatches the SAME delegate down its rollback path (github_actions: `workflow_dispatch` of `workflow_ref` + the `fishhawk_rollback=true` input — re-using the same ref avoids a `workflow-v*` bump; a distinct `rollback_workflow_ref` is a deferred additive spec field; webhook: POST `{fishhawk_rollback:true, variables.FISHHAWK_ROLLBACK:"true", …}` through the SAME shared dispatcher as the forward trigger, so the rollback carries the same secret header/field, redirect refusal, error classification and redaction) and writes a `deployment_rollback_initiated` audit carrying the rollback run handle **distinct** from the initial `deployment_dispatched` handle.
