...
**Constant-size by construction.** A `Hint` carries only registry-owned strings — no failure-reason passthrough, no caller text of any kind — so a matched block cannot grow with run data. `TestMatchBlockIsConstantSize` marshals every catalog entry against a named cap and `TestMatchBlockNeverEchoesEvidence` proves a hint built from a 200 KB failure reason is byte-for-byte the size of one built from a short reason. Those two tests are what LICENSE classifying `next_actions.signature` as `tierNever` in the MCP response byte ladder (`backend/internal/mcpserver/bound.go`): the block rides the budget untiered precisely because it cannot grow. A future field echoing caller input turns them RED.
...
2. A table case in `failuresig_test.go` driving REALISTIC evidence (the literal shape the emitting site renders, not a bare anchor) to the new id.
