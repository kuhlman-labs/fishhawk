...
func TestMigrateDown_RemovesTables(t *testing.T) {
	...
	// 0049 (#1551) is now the latest migration: it adds the additive
	// campaign_items.autonomy column + fail-closed CHECK, touching no other
	// table. So its one-step rollback drops that column (asserted below) and
	// leaves every prior migration's effect intact — including 0045's (#1531)
	// 'acceptance' artifact-kind widening, which SURVIVES the one-step down (it
	// is no longer the migration rolled back).
	if !strings.Contains(artifactsKindCheckDef, "acceptance") {
	...
	// 0049 (#1551) IS the migration just rolled back, so its additive
	// campaign_items.autonomy column must be GONE after the one-step down.
	var campaignItemsAutonomyCol int
	if err := pool.QueryRow(context.Background(),
		`SELECT count(*) FROM information_schema.columns
		 WHERE table_name = 'campaign_items' AND column_name = 'autonomy'`,
	).Scan(&campaignItemsAutonomyCol); err != nil {
		t.Fatalf("query campaign_items.autonomy column: %v", err)
	}
	if campaignItemsAutonomyCol != 0 {
		t.Errorf("campaign_items.autonomy count after MigrateDown = %d, want 0 (0049 should have rolled it back)", campaignItemsAutonomyCol)
	}
