...
- **before that poll, the non-github arm cross-checks the URL's project against the run's own repository** (#3493, the reconciler mirror of `resolveObservationTarget`'s GitLab arm): `parseMergeRequestURL` also returns the URL's FULL project path, and `resolvePoll` requires `strings.EqualFold(projectPath, strings.Trim(runRow.Repo, "/"))` — a stored `pull_request_url` naming a DIFFERENT project than `runRow.Repo` is refused with a WARN log (`run_id`, `pr_url`, `repo`, `url_project`) and no poll, no resolve. Full-path (not last-two-segments) comparison is deliberate, so a nested group path (`group/sub/project`) still matches `runRow.Repo` verbatim rather than only its tail. **Operator remediation when the WARN fires:** the mismatch means the run's `pull_request_url` or its `repo` row is inconsistent — correct the run row (or cancel the run and re-trigger it) rather than assuming the reconciler itself is wrong. All three in-repo writers of a GitLab `pull_request_url` (the webhook receiver's `project.path_with_namespace`, the `forge/gitlab` adapter's `mr.WebURL`, and the dev stub's `<host>/<path_with_namespace>/-/merge_requests/<n>`) produce the project's FULL `path_with_namespace`, so no pre-existing divergent rows are expected from normal operation — a WARN here signals a genuine data inconsistency, not a false positive from the new check.
