...
- `MaxRevisionConstraintBytes = 12000` (#2871) — the cap for the **plan-gate `revise` operator constraint** (`buildPlan`'s and `buildGroomingPropose`'s `### Revision constraint` blocks, fed by `server.loadRevisionConstraint`). That text is BINDING in exactly the sense the approve-with-conditions text is (#558: it AMENDS the plan and wins on conflict), so it takes the **refuse-on-binding** posture: an over-cap constraint is REFUSED at the revise gate (`server/revise.go validateReviseConstraint` → `400 validation_failed` naming `bytes` / `max_bytes` / `overflow_bytes`), and the refusal precedes every stateful step, so no revise pass is consumed and no `plan_revised` audit entry is written. Raised from the historical 4000, which the handler AND both renderers each sliced independently and silently — and because the `plan_revised` audit payload carried the ALREADY-TRUNCATED text, the hash-chained record corroborated the shortened version, so unlike #2583 the loss was undetectable after the fact. Two observed live losses: run `bff9a242` dropped conditions C5-C7 of seven, and the #2062 drive lost a BINDING item for four rounds and the loss was misattributed to agent non-compliance. 12000 matches `MaxApprovalConditionBytes` because both channels carry binding operator instructions.
- `MaxClarificationAnswerBytes = 12000` (#3063) — the cap for the **clarification-answers** channel (`writeClarificationAnswers`, shared by `buildPlan` and `buildGroomingPropose`, fed by `server.loadClarificationAnswers`). That text is BINDING in exactly the #558 sense — the planner declared the issue NOT plannable without it, so a half-answer converts a question the operator RESOLVED back into a guess that then arrives dressed as an authoritative non-derivable fact — so it takes the **refuse-on-binding** posture: an over-cap submission is REFUSED at the answer gate (`server/clarification_answer.go validateClarificationAnswers` → `400 validation_failed` naming `bytes` / `max_bytes` / `overflow_bytes`), and the refusal precedes every stateful step, so the plan stage stays parked at `awaiting_input`, is re-answerable, and no `clarification_answered` audit entry is written. Raised from the historical 4000, which was applied TWICE in series and silently at both layers — `loadClarificationAnswers`' `CapText` (the binding one in production) and each renderer's own local `const maxAnswerBytes = 4000` blind, non-rune-safe byte prefix — destroying the LAST answer first while the earlier ones still looked complete. The observed live loss is run `7984a5a8`: 8256 bytes across three answers plus the trailing comment, over half of it dropped with no operator-facing signal and no audit trace. The cap is measured on the RENDERED blob (the `Q<id> (<question>): ` framing counts), which is the value actually persisted and rendered. 12000 matches `MaxApprovalConditionBytes` and `MaxRevisionConstraintBytes` because all three carry binding operator instructions.
- `MaxConditionBytes = 4000` — the unchanged cap for the **recovery resume reason** (`loadRecoveryResumeReason`), now the ONLY channel it owns. That channel is advisory/bounded and is NOT gate-refused, so its 4000-byte behavior is preserved. Two channels have LEFT this constant: the revision constraint (#2871) and the clarification answers (#3063) — both binding, both gate-refused, both on their own 12000-byte constant above.
...
- `MaxIssueCommentBytes = 2000` / `MaxTotalIssueCommentBytes = 12000` (#2946) — the caps for the **untrusted issue-comment refinement channel** (`writeIssueComments`): each comment body is cut at 2000 bytes (strict `>`, so an exactly-2000-byte comment renders verbatim), the total rendered comment block at 12000 with the OLDEST comments dropped first. Posture is **visibility, not refusal** — the OPPOSITE of the three binding channels above, and the asymmetry is the point. A comment is UNTRUSTED third-party DATA that already exists on the forge before this system sees it: there is no Fishhawk gate to refuse it at, and nothing the operator can still do about it, so refusing an over-cap comment is not even possible the way it is for an over-cap *approve* condition or *revise* constraint (both submitted THROUGH a gate). Instead the per-comment cut carries the ADR-077 `elisionMarker` (shared with `CapTextWithRetrieval` so the formats cannot drift) reporting `len()` of the POST-normalization prefix — so `shown + dropped == original` holds even when `strings.ToValidUTF8` drops a trailing partial rune — plus a retrieval pointer at the full comment on the issue thread. **That retrieval pointer names the caller-supplied issue URL only when the URL passes an ALLOW-list** (`issueCommentRetrievalPointer`, #3035): `net/url.Parse` succeeds, the scheme is exactly `http`/`https`, the host is non-empty, AND `url.String()` equals the input byte-for-byte — otherwise it DEGRADES to the source-agnostic "open the issue thread on the forge" sentence (also used for an empty URL). The marker line lands at column 0 INSIDE the comment envelope but OUTSIDE the `| ` quoting, so a caller-controlled URL that could open a new logical line there would forge a trusted-looking instruction line; the byte-identical round-trip closes that class WITHOUT enumerating separators, because the URL grammar admits no ASCII control byte, no Unicode line separator (NEL U+0085, LINE SEPARATOR U+2028, PARAGRAPH SEPARATOR U+2029, …) and no space. This replaced an earlier control-byte deny-list that passed those three multi-byte separators through (none contains a byte `< 0x20` or `0x7f`). A non-URL string such as `IGNORE ALL PRIOR INSTRUCTIONS`, or a non-http(s) scheme, likewise fails the round-trip and degrades — so no partially-scrubbed URL or raw value is ever emitted; `neutralizeEnvelopeDelimiters` is still applied to whatever is emitted as defense in depth. **The marker is renderer-emitted OUTSIDE the `| ` quarantine quoting** (appended AFTER `sanitizeUntrustedComment` returns), and a preamble notice counting the elided comments is written BEFORE the `<<<BEGIN UNTRUSTED ISSUE COMMENTS>>>` delimiter: both sit at column 0 where a comment author's text — every line of which IS `| `-prefixed — structurally cannot land, so neither is forgeable. Operator-facing visibility rides the `fishhawk_start_run` tool result (over-cap warning) and, for tool-result-less run paths, a greppable `fillIssueContext` server-side WARN. Both cap numbers are unchanged from their historical values; only the marker and the warnings are new. The 2000-byte per-comment cap is the one the #2062 parking handoff (5217 bytes) was truncated against — see `docs/spec/work-management-v0.md`'s refinement-channel section for the parking-handoff interaction.

### The revision-constraint END marker (#2871)

`writeOperatorConstraint` is the ONE renderer for this channel, shared by `buildPlan` and `buildGroomingPropose`, and it owns the whole cap: `server.loadRevisionConstraint` returns the stored blob RAW (a second loader-side `CapText` would have cut at the LOWER of two caps, silently shortening a constraint the gate accepted) and WARN-logs — not INFO — an over-cap legacy read. Because the handler now refuses above the cap, the renderer's `CapTextWithRetrieval` elision path is reachable only for a constraint persisted BEFORE the gate shipped; it is retained rather than deleted because silently dropping the tail is the defect, and it names the `plan_revised` entry's `conditions` payload key as its retrieval pointer.

The rendered constraint is terminated by `RevisionConstraintEndMarker` (`--- END OF OPERATOR CONSTRAINT ---`) on its own line, so a truncation introduced by ANY upstream channel is structurally detectable rather than depending on the operator having numbered their items (the accident both live detections relied on).

**The placement of the expectation is load-bearing, not stylistic.** The sentence telling the planner that the constraint MUST end with that marker (`revisionConstraintEndMarkerExpectation`) is written into the stable scaffolding **OUTSIDE and BEFORE** the constraint block. An expectation written after the marker would be a control that disappears exactly when it is needed: the same cut that removes the terminator would remove the instruction to expect it. This is the same reason `buildPlan` emits the `CapTextWithRetrieval` `risks_and_assumptions` instruction from the RENDERER rather than inside the truncatable text. `TestBuild_Plan_RevisionConstraint_AbsentMarkerIsDetectable` pins it by asserting the ordering AND by simulating the cut — truncating the rendered prompt at the constraint body and asserting the surviving prefix still carries both the expectation and the what-to-do-on-absence instruction. A test that merely proves an intact renderer emits a marker proves nothing about the failure mode.

`CapText(s, max) (string, bool)` is the one rune-safe truncation helper for the bare-marker channels: it cuts at `max` **bytes** then applies `strings.ToValidUTF8(…, "")` (the `sanitizeScopePath` idiom) so a cut through a multi-byte rune cannot emit invalid UTF-8, appends the byte-identical `...[truncated]` marker, and returns whether it truncated. The boundary is strictly `>`: a string of exactly `max` bytes renders verbatim. `CapText` remains the marker for the schema-validation error and the recovery resume reason, and for the LOADER-side residual cut on the clarification channel (`loadClarificationAnswers`); it is **no longer the marker the clarification RENDERER emits** ([#3063](https://github.com/kuhlman-labs/fishhawk/issues/3063)) — see the clarification-answers section below. **It is no longer the marker for the revision BASE plan** ([#3087](https://github.com/kuhlman-labs/fishhawk/issues/3087)) — see the revision-base section below. The old rationale for the bare cut there (a server-derived convenience superseded by the enumerated carry-forward scope list) did not survive scrutiny: the base is the document the planner is instructed to REVISE, and a bare mid-sentence cut told it nothing about which steps it had lost.

`CapTextWithRetrieval(s, max, retrieval) (string, bool)` (#2680) is the ADR-077-shaped elision variant used by the prior-rejection-feedback channel, by `writeOperatorConstraint`'s legacy path (#2871), by `writeClarificationAnswers`' legacy path (#3063), and by the revision-base fallback (#3087). It performs the identical rune-safe cut and the same strict `>` boundary, but instead of the bare `...[truncated]` marker it appends a self-describing block naming the bytes-shown / original-byte / dropped-byte accounting, the cap, an explicit **INCOMPLETE** statement (the visible prefix must NOT be read as the whole instruction), and the caller-supplied `retrieval` pointer to where the full value lives. For the rejection channel that pointer (`priorRejectionRetrievalPointer`) names the `approval_submitted` audit entry's `rejection_comment` payload key on the rejecting run (id threaded via `Trigger.PriorRejectionFeedbackRunID`, reachable via `fishhawk_list_audit`), degrading to a source-agnostic phrasing when the id is unknown. When and only when it truncates, `buildPlan` ALSO emits an instruction line telling the planner its steering is incomplete and that it MUST record the truncation (and what it could not see) in the plan's `risks_and_assumptions` — codifying the mitigation the live agent improvised. Following the pointer is best-effort (whether `fishhawk_list_audit` is wired into a given plan agent's tool set is not guaranteed by this change, ADR-021), which is why the marker does not DEPEND on retrieval succeeding — the `risks_and_assumptions` instruction surfaces the gap to the operator regardless.
...
### The clarification-answers channel: ONE renderer, refuse at the gate (#3063)

`writeClarificationAnswers` is the ONE renderer for this channel, shared by `buildPlan` and `buildGroomingPropose` (the heading is identical; only the body wording differs, plan vs `grooming_report_v1`), in the same spirit as `writeOperatorConstraint` owning the revision constraint. Both call sites previously carried their own `const maxAnswerBytes = 4000` and a blind `answers[:maxAnswerBytes]` byte prefix — two copies to keep in sync, a cut that could split a multi-byte rune and emit invalid UTF-8 into the prompt, and a bare `...[truncated]` marker that told the planner nothing about how much it had lost or where to recover it.

**Now exactly one cap value governs the channel end to end.** `server.loadClarificationAnswers` applies `prompt.MaxClarificationAnswerBytes` — the SAME constant the renderer applies — so the #2871 two-caps-lower-one-wins-silently shape cannot recur. Before #3063 the loader cut at 4000 and each renderer cut at 4000 again; the loader was the binding one in production, which is why a test that raised only the renderer's cap would have passed while the live path stayed truncated.

**The residual path is loud and audited.** Because the answer gate now refuses above the cap, the renderer's `CapTextWithRetrieval` elision is reachable only for a blob persisted BEFORE the gate shipped. It is retained rather than deleted because silently dropping the tail is the defect: a legacy entry still renders, but with the ADR-077 marker (bytes shown / original / dropped / cap, plus `clarificationAnswerRetrievalPointer` naming the `clarification_answered` entry's `conditions` payload key), and — when and ONLY when it actually elided (the #3087 false-truncation-claim rule) — `clarificationAnswersElidedNotice`, which instructs the planner to record in `risks_and_assumptions` that its answers were truncated and which question's answer it could not fully see. That notice is written in the stable scaffolding **ABOVE** the blob (the #2871 placement rule), so the very cut that removes the tail cannot also remove the instruction to notice it.

`loadClarificationAnswers` additionally appends a `clarification_answers_truncated` audit entry on that residual path (`server.CategoryClarificationAnswersTruncated`, payload `{original_bytes, cap_bytes, dropped_bytes, source, source_entry_id}`), best-effort, exactly mirroring `approval_conditions_truncated` (#2583/#2622). The `source_entry_id` key is the id of the `clarification_answered` entry whose blob was cut and is the key of migration 0086's partial unique index, so the entry is **at most one per (run, source answer set)** — repeated prompt builds do not multiply it, while a re-park-and-re-answer cycle's genuinely second over-cap blob still records its own truncation. A store-layer collision is the benign already-recorded outcome (INFO, not WARN) and never blocks prompt construction.

**Honest residual:** the refusal cannot reach a blob already persisted over the cap. The truncation path is narrowed, made loud and made auditable — not eliminated.

### The revision BASE plan: whole under 60000, step-complete digest above (#3087)

`writeRevisionBase` (in `revisionbase.go`) is the ONE renderer for the revision-base block, shared by `buildPlan` ("Prior plan (the revision base):") and `buildGroomingPropose` ("Prior report (the revision base):"), in the same spirit as `writeOperatorConstraint` owning the constraint channel. Both call sites previously carried their own inline `const maxBaseBytes = 4000` and sliced independently.

**The cap.** `MaxRevisionBasePlanBytes = 60000`, boundary strictly `>` like every sibling channel. It sits far ABOVE realistic plan size rather than at a size a real plan can reach: a 45-file, 15-step `standard_v1` plan with acceptance criteria marshals to roughly 15-25 KB under `json.MarshalIndent`. So on every realistic revise the agent now sees the WHOLE prior plan. The observed live loss it closes is run `6059c011` ([#3048](https://github.com/kuhlman-labs/fishhawk/issues/3048)): an eleven-step plan cut inside step 2, so the agent told to "REVISE the prior plan below" and to "not discard the parts of the plan the constraint does not touch" saw two of eleven steps, with nothing in the prompt naming the nine it had lost. `TestRevisionBase_RealisticPlanFitsWholeWithHeadroom` pins the headroom assumption so a future plan-schema growth reddens rather than silently re-entering the digest path.

**Above the cap: a step-complete digest, not a cut.** `revisionBaseDigest` decodes the blob with a plain `json.Unmarshal` into `plan.Plan` — deliberately NOT `plan.Parse`, which strict-decodes with `DisallowUnknownFields` and would push a perfectly structured plan carrying an unknown field onto the byte-cut path — and renders every approach step's identity, the plan-level counts, the verification bodies, every acceptance criterion's id (the downstream join key, never dropped), the decomposition/split titles and the risks. Each individually-variable body is capped at `maxRevisionBaseFieldBytes = 2000` with a NAMED per-item elision, so a shortened body always says which body it is and reports `shown + dropped == original`.

**The identity contract is HONEST, not absolute.** The digest guarantees that no step is SILENTLY absent — not that every step identity is always rendered. Identities render in order up to `maxRevisionBaseStepIdentities = 200` (and `maxRevisionBaseListItems = 100` for every other list), after which ONE structured line states exactly how many further steps exist and their position range, so `listed + counted == total` is arithmetically checkable from the rendered text. A second SHRINK pass fires only when the first pass still overflows the cap: it drops step and criterion bodies entirely, keeps every identity line, and replaces the auxiliary lists with counted lines. That is what bounds the digest by construction — worst case a ~1500-byte header plus ~12000 bytes of identity lines plus ~30000 bytes of criterion ids, comfortably under the cap.

**Whole-document accounting + elision manifest.** The blob is decoded a SECOND time into a generic `map[string]json.RawMessage`, so every top-level key the digest does not render — including one this binary's `plan.Plan` does not know at all — is NAMED in an elision manifest rather than vanishing with the typed decode. Key names go through `sanitizeScopePath`: they are document-supplied text landing in a trusted prompt section. Alongside the per-body figures the digest carries document-level accounting — `original bytes, rendered bytes, elided bytes` — where `elided` is DERIVED as `original - rendered` (the `elisionMarker` discipline, #2946), so `rendered + elided == original` holds by construction and `rendered` counts only source-derived bytes actually carried, never this renderer's own prose. The accounting is therefore conservative in the safe direction: JSON punctuation and key names count as elided. **That claim is load-bearing in the SHRINK pass specifically**, where step descriptions and criterion statements are withheld entirely: `capField` is what accumulates the carried-byte counter, so it is called ONLY when the body is actually rendered — calling it and then discarding its result would count withheld content as carried and make `elided` UNDER-report, which the derived `rendered + elided == original` identity cannot detect. `TestRevisionBaseDigest_ShrinkPassRenderedBytesAreCarriedOnly` pins the shrink-pass figure against an expected count established INDEPENDENTLY of the renderer (the sum of the fixture's own still-rendered fields), which is the assertion the derived-identity test structurally cannot make.

**Every rendered body is line-neutralized, not just the manifest's key names.** The digest DECODES the prior plan and renders its string bodies as LITERAL text, which the whole-delivery path never does: under the cap the base rides as raw JSON, where a newline inside a step description is the two-character escape `\n` and cannot start a prompt line. So the digest is a new injection surface — a decoded `\n` becomes a REAL newline and can land planner-authored text at column 0 inside a trusted, binding section, forging a heading. The plan is planner-authored, but the planner consumes untrusted issue text, so plan bodies are untrusted-influenced under Fishhawk's threat model. Every body `capField` renders — summary, step descriptions, criterion ids and statements, the verification bodies, `risks_and_assumptions` entries, decomposition/split titles — therefore goes through `neutralizeLineStructure`, the transform factored out of `sanitizeScopePath` so the VALUE side and the manifest's KEY side cannot grow two subtly different escapers. It escapes line terminators and control characters to a visible two-character form and breaks triple-backtick/tilde fences; it neutralizes STRUCTURE, not content, so a body with no control character passes through byte-unchanged. The cut and the byte accounting are taken on the ORIGINAL bytes, before escaping: `sourceBytes` counts bytes of the prior DOCUMENT carried forward, and escaping only expands their rendered form. `TestRevisionBaseDigest_InjectedBodiesSanitized` is the value-side counterfactual (`TestRevisionBaseManifest_KeyNamesSanitized` covers keys only).

**The undecodable base falls to a LOUD elision, never a bare cut.** A blob that does not decode as a plan, or decodes with ZERO approach steps (nothing step-complete to say — an empty digest would be a worse lie), falls to `CapTextWithRetrieval` with `revisionBaseRetrievalPointer`: the ADR-077 marker with byte accounting, the INCOMPLETE statement, and `fishhawk_get_plan` as the retrieval path. This is the path the `buildGroomingPropose` branch actually takes, since a grooming run's artifact is a grooming report, not a `standard_v1` plan — asserted explicitly in `TestBuild_GroomingPropose_OptionalChannels` so a wrong assumption in either direction says which path was taken. `revisionBaseElidedNotice` is written by the RENDERER OUTSIDE and BEFORE the elidable text (the #2871 placement rule) and instructs the planner to declare the incompleteness in `risks_and_assumptions`; retrieval is best-effort (ADR-021), so the declaration is what surfaces the gap to the operator regardless.

### The elision is an operator-visible fact (#3442)

`AssessRevisionBase(base) RevisionBaseAssessment` is the exported measurement behind the revise-time surfaces (the REST `revision_base` field, the `plan_revised` payload key, the MCP warning, the CLI notice). It is computed by the SAME core that renders the prompt text: `assessRevisionBase(base)` returns `(rendered, assessment)`, `renderRevisionBase` is a thin wrapper returning `(rendered, assessment.Elided)` so every #3087 rendered byte is unchanged, and `AssessRevisionBase` renders and discards the text — cheap relative to a prompt build, and the guarantee that the operator surface and the planner's prompt cannot disagree. Fields: `original_bytes`, `cap_bytes`, `elided`, `mode`, `rendered_bytes`, `elided_bytes` (the digest header's own figures — `rendered` counts only carried source bytes, `elided` is derived as `original - rendered`; in cut mode they are the marker's shown/dropped figures), `elisions` (the NAMED per-body cuts the digest marks inline — `capField` appends one per cut, `droppedBody` one per shrink-pass withhold with `bytes_shown: 0` — bounded at `maxRevisionBaseElisionEntries = 50`, past which only `elisions_omitted` increments, so `len(elisions) + elisions_omitted` equals the number of named markers in the text while the inline markers stay complete), and `unrendered_keys` (the sorted list `writeRevisionBaseManifest` renders — it now returns it — so the assessment's list IS the manifest's).

**The four modes and the wording rule.** `whole` (under cap, verbatim), `digest` (first pass), `digest_shrunk` (the shrink pass was delivered), `cut` (over cap AND undecodable as a plan carrying approach steps: `CapTextWithRetrieval`). `RevisionBaseElisionSummary(&a)` is the ONE operator-facing sentence, selected by mode: it claims a "step-complete digest" only in the two digest modes and says "byte-cut prefix … NOT a digest" for `cut`, so no surface can describe a digest the planner did not receive; the server WARN log and the MCP warning render it directly, and the CLI (a separate module) mirrors the same split in `httpclient.RevisionBaseNotice`. Pinned by `TestRevisionBaseElisionSummary_WordingAgreesWithMode`; the accounting/inline-marker agreement by `TestAssessRevisionBase_OverCapPlan_DigestMatchesInlineMarkers` (every reported elision matches its inline marker byte-for-byte and the header line equals the reported figures), the bound by `TestAssessRevisionBase_ShrinkPass_ModeAndBound`, the cut mode by `TestAssessRevisionBase_UndecodableBase_Cut`. The cap is NOT changed and no refusal is added — the operator has no lever to shrink a prior plan, so the assessment makes the elision visible pre-review and durable in the audit chain instead.

**The carry-forward lead sentence is now conditional.** The #2516 enumerated block used to assert unconditionally that "The prior-plan blob above is TRUNCATED at 4000 bytes" — false on every realistic revise after this change, and already false when no blob was rendered at all. Three leads now select on `(baseRendered, baseElided)`: `scopeCarryForwardElidedLead`, `scopeCarryForwardWholeLead`, `scopeCarryForwardNoBaseLead`. Only the truncation CLAIM varies: `scopeCarryForwardObligation` — the BINDING carry-forward instruction and the `scope_removals` / scope-regression language — is shared verbatim, so no branch can drift into a weaker obligation. Both non-elided leads keep the list authoritative on its own INDEPENDENT ground: it is resolved from the newest `plan_scope_retry` entry, which on a corrective re-dispatch is NOT the newest plan artifact.

### Every fixed cap on the plan-prompt path (#3087, proposal 4)

The sweep this change owed. One row per fixed cap reachable while rendering a plan prompt, with the content class it meets and an explicit verdict on whether that pairing is safe or is gambling.

| Cap | Site | Value | Content class | Posture | Verdict |
|---|---|---|---|---|---|
| `MaxApprovalConditionBytes` | `writeApprovalConditions` / reinforcement | 12000 | unbounded operator text | refuse-at-gate, then bare cut for legacy | **Safe.** The approval gate refuses above the cap, so the renderer's cut is reachable only for pre-gate data (#2583). |
| `MaxRevisionConstraintBytes` | `writeOperatorConstraint` | 12000 | unbounded operator text | refuse-at-gate, then LOUD elision + END marker | **Safe.** Refused at the revise gate before any stateful step; the legacy path elides loudly (#2871). |
| `MaxClarificationAnswerBytes` | `writeClarificationAnswers` | 12000 | unbounded operator text | refuse-at-gate, then LOUD elision + AUDITED residual | **Safe.** Refused at the answer gate before any stateful step, so the stage stays parked and re-answerable; the legacy path elides loudly AND records a `clarification_answers_truncated` audit entry (#3063). Residual, stated: a blob persisted before the gate existed is still cut — narrowed and made visible, not eliminated. |
| `MaxConditionBytes` | recovery resume reason | 4000 | unbounded operator text | bare 4000-byte cut | **Acceptable.** The resume reason is a short recovery note, not a binding instruction, and a cut loses context rather than an operator DECISION. The clarification channel left this row in #3063; if the resume reason ever grows into a steering channel this row needs the same treatment. |
| `MaxRejectionFeedbackBytes` | `### Prior plan-stage rejection feedback` | 12000 | unbounded operator text | LOUD elision + `risks_and_assumptions` notice | **Acceptable.** Not refusable (the rejection already happened), but the loss is named and retrievable (#2680). |
| `MaxIssueCommentBytes` / `MaxTotalIssueCommentBytes` | `writeIssueComments` | 2000 / 12000 | unbounded UNTRUSTED third-party text | LOUD per-comment elision, oldest-first drop | **Acceptable by construction.** There is no Fishhawk gate to refuse a forge comment at; visibility is the only available posture (#2946). |
| `MaxRevisionBasePlanBytes` | `writeRevisionBase` | 60000 | STRUCTURED document, planner-authored bodies | whole under cap, structured digest (line-neutralized bodies) above, loud elision when undecodable | **Safe, with a stated judgement.** 60000 is not a bound that exists — it is chosen ~2.5x above realistic plan size, and exceeding it is no longer silent (#3087). The document is server-STORED but its bodies are planner-authored and the planner reads untrusted issue text, so the digest — which renders those bodies as literal prompt text — escapes them through `neutralizeLineStructure`. |
| inline `4000` on `PriorSchemaValidationError` | `buildPlan` + `buildGroomingPropose` | 4000 | validator-generated error | bare cut | **Bounded by construction.** The text is produced by our own schema validator, not by an operator or a third party. |
| `maxScopePathBytes` | `sanitizeScopePath` | 512 | one repo path | bare cut + control-character escaping | **Bounded by construction.** A repo path near 512 bytes is already pathological. |
| `maxScopeCarryForwardPaths` | carry-forward + restoration lists | 200 | path COUNT | counted truncation marker instructing carry-forward anyway | **Bounded by construction.** A plan past 200 paths is far past the implement-stage `max_files_changed` cap already, and the marker never reads as permission to drop. |

Read the table by its middle column: the caps that are **bounded by construction** (a repo path, a validator-generated error, a path list bounded by `max_files_changed`) are safe because no input can realistically reach them. The caps meeting **unbounded** text are safe only where a GATE refuses above them or the elision is LOUD. `MaxConditionBytes` is now the only row that is neither — but its scope shrank to the recovery resume reason alone (#3063 moved the clarification answers to their own gate-refused constant), and a short recovery note losing its tail costs context rather than an operator DECISION, which is why that row reads *acceptable* rather than *gambling*. If the resume reason ever becomes a steering channel, it needs the refuse-at-gate treatment the three binding channels have.
...
| Verify **output** (gate evidence) | `writeUntrustedVerifyOutput` | `<<<BEGIN/END UNTRUSTED VERIFY OUTPUT>>>` | no | no |
...
**Verify output (gate evidence, #3192).** The implement-review prompt's `### Gate evidence` section digests the runner's committed-tree verify gate. Its free text — a verify run's `output tail` and a `verify_summary` `detail`, at BOTH the parent site (`writeGateEvidence`) and the per-slice site (`writeSliceVerify`, #3132) — is repository-controlled: a test name, an assertion message, a captured log line, any of which can be attacker-influenced through the code under test. Before #3192 those tails were merely INDENTED under a machine-verified heading, and indentation is not an instruction/data boundary (pre-redaction protects secrets, not instructions). `writeUntrustedVerifyOutput` now envelopes every tail and detail at both sites through one shared writer, with `neutralizeEnvelopeDelimiters` applied so no tail can forge a `<<<`/`>>>` run and close its own envelope, and a single `verifyOutputEnvelopeFraming` ignore-and-report paragraph precedes every envelope in the section (emitted once, only when the section carries an enveloped tail or detail, so a no-tail no-detail section stays byte-identical — prompt-hash replay stability). The binding instruction for how to read the tails stays OUTSIDE the envelope (it is the BINDING rules above it).

**Verbatim-versus-indented residual (NOT closed, recorded per #3192 operator condition 5).** The enveloped verify text renders VERBATIM (delimiter-neutralized) rather than per-line indented, which deliberately DROPS the pre-#3192 property that an indented tail's line content could not collide with the diff section's code fences. This is a considered trade: a multiline adversarial payload must survive as a literal substring for the offline containment gate to assert on it, and an envelope is a strictly stronger boundary than an indent. The residual is the SAME one the body envelope accepts — an attacker-influenced tail can still emit a convincing heading or fence INSIDE its envelope, bounded by the envelope plus its framing. The containment claim is offline-STRUCTURAL (the probe lands inside a span), not behavioural, until #3187's live arm runs. This residual is recorded here, not claimed closed.

**The #2291 eval corpora now exist, and they are EVAL-GATING this envelope (E60.2).** `backend/internal/agenteval` carries two committed corpora: an injection corpus (six attack classes — the sixth, `verify-output-instruction-injection`, plants its payload in verify-gate OUTPUT rather than an issue body, #3192) whose offline gate asserts every adversarial probe lands at an offset STRICTLY INSIDE its channel's envelope span in all three reviewed renders (a `verify_output` probe only in `implement_review`, the sole reviewed render that ingests gate evidence, and wholly absent from `plan`/`plan_review`) and nowhere in the implement render, and an envelope-quality corpus that A/Bs plan quality with and without the envelope. **A change to this file's untrusted-intake rendering must re-run both** — the offline halves run in `scripts/test verify` already, so a regression fails in-loop. Read the residual honestly: those offline halves prove STRUCTURAL CONTAINMENT only. The BEHAVIOURAL arms (is an injected instruction actually not followed? does the envelope dilute plan quality?) are opt-in, double env-gated on `FISHHAWK_AGENTEVAL_*_LIVE` plus `FISHHAWKD_ANTHROPIC_API_KEY`, and have NOT run — #3187 owns them and the treatment decision they license, so the residual above is unchanged, not retired.

**No production off-switch exists for the body envelope, by design.** The eval's no-envelope arm is produced by a HARNESS-side transform (`agenteval.StripBodyEnvelope`), never by a flag in this package: a shipped way to disable the envelope would be a worse defect than the dilution it measures. That transform carries a byte-exact copy of the framing paragraph and ERRORS on drift, so editing the framing here reddens `TestStripBodyEnvelope_AcceptsRealPromptOutput` rather than silently contaminating the measurement. Contract: `backend/internal/agenteval/README.md`; evidence: `docs/compliance/prompt-injection-evidence.md`.
...
- `TestPrompt_UntrustedIssueFieldsReadOnlyByEnvelopingWriters` — an allow-list over `go/parser` selector reads of all **three** watched fields: `Trigger.IssueBody`, `Trigger.IssueComments` and `Trigger.IssueTitle` (#2939), in two halves. **Who** may read: a PER-FIELD set (`allowedReadersFor`), because the readers genuinely differ — body and comments may be read only by `writeIssueContext` and `writeReviewIssueContext`, while the title may ALSO be read by `writeIssueLink`, which renders it on the implement path and must never touch the other two (the never-re-ingest invariant). A single shared set would silently authorize a body read from `writeIssueLink`. **How** they may read: every read inside those writers must be a direct argument to that field's mapped function (`writeUntrustedIssueBody` for the body, `writeIssueComments` for the comments, `sanitizeIssueTitle` for the title — the title's control is NORMALIZATION, not quarantine), or a non-rendering presence test against `""`. The second half is what makes the guard a control rather than a name check — a function allow-list alone cannot distinguish a sanctioned call from a raw render performed INSIDE an allowed function, so it stays GREEN under the very counterfactual the guard claims to detect (reverting `writeIssueContext` to `b.WriteString(t.IssueBody)`, or `writeIssueLink` to `b.WriteString(t.IssueTitle)`). Its completeness half additionally fails when an allowed writer silently DROPS its sanitizing call. Covering the body alone would leave the criterion partly enforced — a conditional raw COMMENT or TITLE render the fixture matrix never activates would evade both halves.

### Closed residual: the issue TITLE is normalized (E60.9 / #2939)

`Trigger.IssueTitle` is still Fishhawk-rendered metadata that renders OUTSIDE both envelopes by design — the inline `#N · <title>` shape does not admit an envelope. But it is forge-supplied, and nothing upstream enforces that it is single-line, so **every** render now routes it through the pure chokepoint `sanitizeIssueTitle`, which (1) maps every Unicode line-break character (LF, CR, CRLF as ONE space, VT, FF, U+0085, U+2028, U+2029 — the UAX #14 mandatory classes) to a single space, making the title single-line BY CONSTRUCTION, and (2) applies `neutralizeEnvelopeDelimiters` so no title text can emit a `<<<`/`>>>` token. A title carrying an embedded line break whose continuation text is an envelope delimiter can therefore no longer open a second column-0 delimiter line.

It covers **five write sites across three writers**: `writeIssueLink` (the inline `Triggering issue: #N · <title>` form and the number-less `Title: ` form — implement and implement fix-up), `writeIssueContext` (the `Title: ` line — plan, grooming-propose, acceptance) and `writeReviewIssueContext` (the inline `Issue: #N · <title>` form and the number-less `Title: ` form — plan-review, implement-review, supplemental-reinvoke review). The `writeIssueLink` pair is beyond the six enveloping renders and is deliberately included: the implement path renders NO envelope, so the envelope-shaped assertions structurally cannot cover it, which is exactly the shape that produced this issue.

The sanitizer is the **IDENTITY** for any title with no line-break character and no run of three or more `<`/`>` — including one carrying leading, trailing or interior spaces and tabs, which are **not** trimmed. That identity is load-bearing: it is what keeps every existing prompt hash and the frozen `testdata/plan-prompt-pre-change.golden` byte-identical, so the sanitizer is exactly as aggressive as the delimiter-injection threat requires and no more. `TestSanitizeIssueTitle` pins it both ways (one case per line-break character, plus explicit identity cases including boundary whitespace), and `TestBuild_IssueTitle_SingleLineAndNonDelimiting` pins the shipped behavior across all eight title-rendering stage renders.

**Residual that REMAINS**: the title is NORMALIZED, not QUARANTINED. An attacker can still put instruction-shaped prose in a title, and that text renders outside the envelope with no untrusted framing, bounded only by the role/scope constraints. Closing that would require an envelope the inline render shape does not admit.
...
**Which file set the labels came from (#3029).** Both this block and the `operator_scope_path_undelivered` block NAME the set they were evaluated against, because an unqualified "absent from the committed file set" is what let a fix-up DELTA masquerade as the implement stage's whole committed history. Two wording branches, selected by the backend-set `GateEvidence.OperatorScopeUndeliveredStageCumulative` / `GateScopeProvenance.CommittedSetStageCumulative`:

- **stage-cumulative** — the block states the paths are absent from the STAGE-CUMULATIVE committed file set, printing the span as `base <sha> .. head <sha>` (`OperatorScopeUndeliveredCumulativeBaseSHA` / `...CumulativeHeadSHA`) and saying explicitly "NOT merely from this pass's delta". This branch alone keeps the machine-verified HIGH-priority framing and its BINDING preamble bullet. When either SHA is EMPTY the branch names the set WITHOUT a span rather than printing an empty one — `base  .. head )` reads as reproducible and is not (pinned by `TestWriteGateEvidence_OperatorScopeUndelivered/stage_cumulative_without_span_names_the_set_span_less`); likewise a hedged block carrying no machine reason omits the parenthetical entirely rather than rendering `(reason: )` (`.../this_pass_hedged_without_reason_omits_the_clause`).
- **this-pass (hedged)** — the backend could not establish the cumulative state. The block says so, names the backend machine reason (`OperatorScopeUndeliveredIncompleteReason` / `CommittedSetIncompleteReason`, e.g. `cumulative_compare_truncated`), states that an earlier pass may already have delivered a listed path, and instructs verification against the PR's cumulative `base..head` diff. It deliberately **drops** the "deterministic, machine-verified signal" framing and the high-priority bullet — a hedged block is never given established-fact authority.

The pre-existing `#2398` rename-indeterminate hedge is orthogonal and takes precedence in the block header; it now names the evaluated set too. An empty/nil `OperatorScopeUndelivered` and a nil `ScopeProvenance` still render NOTHING, so the all-delivered path stays byte-identical (pinned by `TestWriteGateEvidence_OperatorScopeUndelivered/empty_is_byte_identical_and_renders_nothing`).
...
## Operator-authorized in-scope review sections (#829, #2820, #2874)

`buildImplementReview` renders up to three informational sections naming paths that are in-scope despite being absent from the plan's raw `scope.files`, so the reviewer does not flag an operator-authorized edit as scope drift under criterion 4. Each is conditional on a non-empty field, so an empty field renders NO section (section absence is guaranteed; see the prompt-hash note below):
...
- **`### Scope amended mid-stage` (#2874)** — rendered when `Trigger.MidStageAmendedScopeFiles` is non-empty: each path the operator APPROVED mid-stage via `fishhawk_decide_scope_amendment` **for the stage under review**, naming the authorizing amendment id and the operator's decision reason verbatim (an empty reason — the decision endpoint does not require one — degrades to path + id, never an empty parenthetical). Backend-derived by `stageApprovedAmendmentScopePaths` (`server/stage_scope_amendment.go`), which is APPROVED-only (a denied or pending request confers nothing) and STAGE-scoped by the same `a.StageID == stageID` filter the enforced fold `resolveApprovedScopeAmendments` applies — so the section is provably a SUBSET of what the runner permitted and a sibling stage's amendment never leaks in. The decision reason is rendered on purpose: a reviewer that disagrees with an amendment on its merits can raise that as a NON-scope concern naming the amendment id; a reviewer that cannot see the amendment can only mislabel it as drift (run bff9a242, where both reviewers flagged an approved amended path independently). A path already surfaced by `AmendedScopeFiles` is deliberately NOT suppressed here — the two lists have different provenance and each section is the authority for its own channel.
...
**Criterion 4's wording CHANGED in #2874, and implement-review prompt hashes therefore DIFFER from prior runs.** The standing scope-adherence criterion now names all three sections and defines drift as "in NONE of `scope.files`, the approval-amended list, the mid-stage-amended list, or the child-slice-amended list" — replacing the two-list "in NEITHER `scope.files` NOR the amended-scope list" text that both reviewers on run bff9a242 quoted back verbatim while flagging an operator-approved path. That text is rendered **unconditionally**, on every implement review, amendment or not. So whole-prompt hash stability against a PRE-change build is deliberately broken once, for that reason, and is not claimed anywhere. What the three sections DO guarantee, and what the tests pin, is SECTION ABSENCE: an empty field renders no heading, and a populated render differs from the empty one by exactly the new section — pinned as post-change-with-field vs post-change-without-field, never against a pre-change snapshot. (The only frozen full-prompt golden in this package, `testdata/plan-prompt-pre-change.golden`, is a PLAN prompt and is unaffected.) **#2119 breaks implement-review prompt hashes AGAIN, for the same deliberate once-per-change reason** — standing criteria 9/10, the adversarial carve-out, the reworded standing-criteria lead-in and the `### Severity calibration` block all render unconditionally (criteria 9/10 render in one of two variants, never absent). The cost is a prefix-cache miss on the first render after deploy and nothing else. The in-package frozen whole-prompt pin `nilSliceVerifyPromptGolden` was HAND-EDITED line by line for it, never regenerated from post-change code.
...
- **`addressed` / `superseded`** — resolved-in-a-prior-round context. A re-raise REMAINS insertable: a genuine regression of an addressed fix is RECORDED, not discarded, and reaches the operator. The reviewer tags such a re-raise with `settled_ref` + `new_evidence` for lineage only. A `superseded` row that carries a server `StateReason` (a plan revise or stage retry, #3593) renders that reason UNPREFIXED as a `superseded reason:` line — distinct from the operator-prefixed `waived`/`deferred` lines — and the tier's guidance adds one sentence: such a concern was DISCARDED TOGETHER WITH THE TREE IT REVIEWED, NOT fixed, so if the same finding still holds on the tree under review the reviewer raises it again in `concerns[]` with `settled_ref` for lineage.
...
- **Plan prompt** (`buildPlan`, "Counterfactual attainability rule") — design-time prevention. `verification.test_strategy` must name, for every control the change adds or tightens, a test that goes RED when the control is deleted. It lives at the plan stage because implement faithfully writes whatever test the plan names, and a condition attached at approval arrives after the test strategy is already authored (#2436 produced five vacuous tests under exactly such a condition). The seven compressed rules: (1) name the deletion-reddened test and make implement DELETE → RUN → observe RED → restore rather than reason, seeding bad state BY CONSTRUCTION (a freshly generated unrelated key is definitionally non-matching) so the RED lands on the assertion, not on fixture setup; (2) if a test cannot serve as a counterfactual vehicle, prove that EMPIRICALLY by running it under the deletion (#2433); (3) error IDENTITY is insufficient when the control's effect is committed state — a fire-then-rollback returns a byte-identical error, so read the state after the call returns; (4) self-pair a malformed input where a byte-exact comparison would otherwise reject it, and point hop/target URLs at reachable in-test servers. E68.28 (#3057) added three more, all SETUP obligations rather than assertion rules, because in all four reported instances the defect was in the fixture's setup: (5) STATE THE MECHANISM — name the fixture state that makes the deletion observable, i.e. what the test would produce if the control were absent, and treat an arm whose fixture would produce the SAME observable outcome either way as not a control at all; (6) a mutation that stops COMPILATION is not a counterfactual — mutate the BODY or delete only the CALL SITES and run the behavioural test; (7) watch for a MASKING downstream guard and either construct an ISOLATING case or declare the control unverifiable in `test_strategy`. The clause also names the deterministic `counterfactual_mechanism_missing` plan-gate advisory (`plan.Warnings`) as the detection half, stating it is advisory and never refuses the plan.
- **Implement + fix-up prompts** (`writeCounterfactualDiscipline`, called from BOTH `buildImplement` and `buildImplementFixup`) — execute-and-record. DELETE each control, RUN its guarding test, observe RED, restore byte-identically, and record the observed RED output in PR `## Notes`, plus the six concrete vacuity traps — (a) error identity vs committed state, (b) a malformed input paired with a different clean value, (c) an unreachable hop/target address, and, since E68.28 (#3057), (d) a deletion the FIXTURE masks (state what the test would produce with the control absent before deleting; if it is the same, change the FIXTURE, never the assertion), (e) a mutation that stops COMPILATION (mutate the body or delete only the call sites), and (f) a MASKING downstream guard (construct an isolating case, or report the control unverifiable having run it). The fix-up call site is deliberate (#2453): a fix-up reason is itself a place where a control gets designed, downstream of every plan-gate condition — #2453's certificate/key correspondence check landed correct and completely unpinned for exactly that reason. One renderer, two call sites, maintained once. Unlike `writeFailureModeTestChecklist` (#1199), which is deliberately fix-up-exempt.

The wording is deliberately NOT gated on the change looking security-relevant or being Go — #2453 was a zsh tooling change with no production surface and produced the same shape. The framing is economic, not protective: the heterogeneous reviewers already catch these post-hoc; what the rule saves is fix-up passes (#2436 spent 3 of 3 and four review rounds on this class). Cost is paid honestly on every render. RE-MEASURED at E68.28 / #3057 (the prior figures, 1578 and 1466 bytes, predate clauses (5)-(7) and traps (d)-(f)): the plan-stage rule paragraph is **3159 bytes** on every plan prompt and the implement/fix-up block **2434 bytes** once per implement pass and once per fix-up pass; the sibling Deciding-test paragraph #3057 added to the acceptance-criteria section is a further **1036 bytes** on every plan prompt. These numbers are UNPINNED — no test asserts them — so re-measure before quoting rather than trusting the figure above.

Pinned by six tests in `prompt_test.go`: `TestBuild_Plan_CounterfactualAttainabilityRule`, `TestBuild_Implement_CounterfactualDiscipline_Rendered`, `TestBuild_Implement_CounterfactualDiscipline_RenderedOnFixup` (the #2453 fix-up-path pin), `TestBuild_Implement_CounterfactualDiscipline_DistinctFromFailureModeHeading` (a presence+absence pair over the same fix-up string), and, since E68.28 (#3057), `TestBuild_Plan_DecidingTestRule` (the acceptance-criteria clause and the `blocking_criterion_undecided` rule name) and `TestBuild_Plan_CounterfactualMechanismClause_AbsentFromImplementReview` (a self-paired presence on plan + implement and ABSENCE on the implement-review path, over one shared clause constant, so the absence half cannot pass vacuously). Because `_Rendered` and `_RenderedOnFixup` drive two different builders through two different call sites, deleting either single call site reddens exactly one of them.
...
### Standing criteria 9 and 10 + severity calibration — cutting operator arbitration cost (#2119, E50.10)

Four additions to `buildImplementReview`, shipped as one unit because they share the review-criteria / verdict-rule section. They target the three operator-arbitration cost centres the E44 campaign measured: severity inflation from an unchecked baseline, mechanical predictions asserted without tracing, and stale reopens on re-review.

- **Standing criterion 9 — baseline check before severity.** Before assigning a severity to a PATTERN-based finding (an unbounded read or decode, a missing cap or limit, an absent guard or check), the reviewer must first establish whether sibling or surrounding code already exhibits the same pattern; if it does, say so and calibrate DOWN — pre-existing convention this diff MATCHES, not a regression it INTRODUCED.
- **Standing criterion 10 — trace mechanical predictions.** Any claim about what a specific code path, test or handler WILL DO (a status code returned, an error surfaced, a branch taken) must be traced to the definitions that actually govern it — the fake, the override, the wiring, the fixture — never inferred from a type or function name, because a test fake routinely overrides the base behavior its name implies.
- **The adversarial-reasoning carve-out.** Both rules are bounded, in one sentence rendered in BOTH postures, to PATTERN-based and MECHANICAL-PREDICTION findings only. They are explicitly NOT a requirement to cite a line for every claim: adversarial reasoning about implications — a threat model, a privilege-escalation path, a fail-open, a cross-tenant leak — is a claim about what COULD happen, is not citable to a line, and must be neither withheld for want of a citation nor downgraded on that ground. #2119's "Explicitly NOT the ask" section names that class as the one a broadly written grounding rule would wrongly suppress; it is the class the campaign's highest-value findings came from (the read-mirror-gates-mutations regression, the key/granularity privilege escalation, the prefixless-subject fail-open, the cross-forge identical-name leak), so the sentence carries its own test pin rather than relying on the surrounding criteria tests.
- **`### Severity calibration`.** The rubric that was previously absent entirely: `high` = the defect is REACHABLE in a supported configuration; `medium` = reaching it requires a misconfiguration or an unusual wiring; `low` = defense-in-depth hardening, documentation accuracy, or test hardening with no reachable defect behind it. Plus the binding requirement that each note state WHICH tier was applied and why (so the operator reconciling two reviewers' verdicts reads the basis of a disagreement rather than re-deriving it), and a pointer that a criterion-9 baseline finding is a `low`, not a `high`.
- **The prior-concerns re-read bullet.** A fourth binding bullet in `### Prior concerns (delta verification)`: before emitting a `reopened` resolution, READ the current diff state for that concern's subject and either CONFIRM the claimed fix against it or state SPECIFICALLY what remains missing and where. Reopening on prior-round reasoning is a defect in the review.
- **The reopen-substantiation bullet (E68.41 / [#3319](https://github.com/kuhlman-labs/fishhawk/issues/3319)).** A fifth binding bullet immediately after the re-read bullet, extending it from ADVICE to a statement of the SERVER-SIDE REFUSAL that now backs it: a `reopened` resolution whose OWN `note` is blank is REFUSED when the verdict is `reject` and raises no concern of its own — the concern keeps its state and the refusal is recorded against the review. It names both exclusions explicitly, because both are the failure shapes the issue reports: a SIBLING resolution's note substantiates that sibling and never this one, and `free_form` prose substantiates nothing (it is unmatched to any concern id). Rendered inside the same `len(t.PriorConcerns) > 0` guard, so the no-prior-concerns prompt stays byte-identical; pinned present-and-absent by `TestImplementReview_PriorConcerns_ReopenSubstantiation{,_EmptyByteIdentical}`. The absence half is a BYTE comparison, not a fragment sweep: `testdata/implement-review-no-prior-concerns-pre-change.golden` holds the no-prior-concerns render captured at the PRE-change base commit `29df5ba2` (second instance of the `plan-prompt-pre-change.golden` convention), so any edit that reaches the empty-PriorConcerns prompt — a hoist out of the guard included — fails the test. A failure is a finding about the prompt; the golden is never re-captured from the current tree, which would make the pin a tautology. This touches the REVIEW prompt's prior-concerns section, NOT the untrusted-intake envelope framing, so the byte-exact framing literals in `backend/internal/agenteval` are untouched.

**Criteria 9 and 10 are grounding-CONDITIONAL, and the ungrounded variant is the DEFAULT path, not the exception.** Both branch on `Trigger.ReviewTreeCommit` — the same signal `writeReviewToolClause` and `writeReviewRepoAccess` key on — inside one helper, `writeGroundedCalibrationCriteria`. GROUNDED (non-empty): resolve the baseline / the prediction against the exported tree and CITE it; an unresolved question about a sibling's behavior is a search to run, not a hedge. UNGROUNDED (empty): say plainly that the baseline is UNESTABLISHED / the prediction UNTRACED and calibrate the severity DOWN, rather than asserting a regression the reviewer structurally cannot check. Since E45.90 / #3625 the UNGROUNDED criterion-10 branch — the one that mints UNTRACED — additionally instructs the reviewer to name `FISHHAWKD_REVIEW_GROUNDING` in the concern note alongside the operator-runs-the-checker resolution, so the note the operator actually reads carries the switch that would have made the prediction traceable. Read that honestly: it is satisfied at the PROMPT layer. The render is pinned by a golden; nothing proves a reviewer complies, and a deterministic annotation would need the grounding config threaded into the concern renderers. The branch exists because ADR-078's operator correction ships grounding **dormant** (`FISHHAWKD_REVIEW_GROUNDING` defaults false, after a live check found both adapters could read files outside the exported tree), so an unconditional "go read the sibling file" instruction would be unsatisfiable on the path that actually executes — the same failure mode ADR-078 records against the pre-#2486 prompt.

**Placement.** Criteria 9/10 are appended AFTER standing rule 8 and BEFORE `### Verdict decision rule`, leaving criteria 1-8 and the numbered lenses byte-identical so the verdict rule's `standing rule 7` cross-reference and every existing ordering pin are unmoved. Only the standing-criteria LEAD-IN changed, from the count-bearing "Three standing criteria..." (stale since rules 7 and 8 landed — a reviewer reading "Three" has a live reason to stop at criterion 6) to the count-neutral "The standing criteria below, orthogonal to the lenses above, also apply:". The `### Severity calibration` block renders unconditionally on BOTH gate-evidence branches, immediately after the verdict decision rule and ahead of the approved-plan section — inside the cache-stable prefix, well before `ImplementReviewSplitMarker`, so it adds nothing to the per-round variable payload. The prior-concerns bullet renders only inside the `len(t.PriorConcerns) > 0` guard, so the empty case stays byte-identical to the pre-#984 output.

**What this change ships, and what it does NOT.** It ships the prompt TEXT plus its structural pins. It does **not** measure calibration: the live re-run of the changed prompt against epic #1824's operator dispositions was DEFERRED with an explicit operator acceptance at the #2119 approval gate, because those dispositions live in the operator audit database rather than the tree and building the labelled corpus is the #1290 distill-corpus path (tracked under E50). Whether the bounded wording actually preserves the adversarial class in a live reviewer, and whether severities actually move toward operator dispositions, are behavioural questions this change does not answer — do not read the green tests as calibration evidence.

**#3309 UPDATE — the measurement now has an APPARATUS, and it costs a LOCKSTEP EDIT here.** The paragraph above is unchanged as a description of #2119 itself: that change measured nothing, and its green tests are still not calibration evidence. What E50.22 / #3309 added is the harness that CAN take the measurement — `backend/internal/agenteval`'s severity-calibration and adversarial-retention corpora, driven as a pre-/post-#2119 two-arm A/B. The arms still have not RUN (they are double-gated on `FISHHAWK_AGENTEVAL_CALIBRATION_LIVE` + `FISHHAWKD_ANTHROPIC_API_KEY` and the runner denies that credential to gate subprocesses by design), so the behavioural questions this section names remain UNANSWERED — see [`backend/internal/agenteval/README.md`](../agenteval/README.md) and [`docs/compliance/severity-calibration-evidence.md`](../../../docs/compliance/severity-calibration-evidence.md).

**THE COUPLING: the #2119 treatment set is EXACTLY FIVE literals, and `agenteval` carries a BYTE-EXACT copy of each.** `agenteval.StripCalibrationCriteria` produces the pre-#2119 arm by removing them from a genuinely built implement-review prompt, and it ERRORS rather than best-efforts when a literal it is asked to remove does not byte-match — the same drift-detector discipline `agenteval.bodyEnvelopeFraming` already applies to `prompt.go`'s issue-body framing. The five, with their render sites in this file:

1. `standing-criterion-9-lead` — criterion 9's lead sentence, `writeGroundedCalibrationCriteria`.
2. `standing-criterion-10-lead` — criterion 10's lead sentence, `writeGroundedCalibrationCriteria`.
3. `adversarial-carve-out` — the `These two standing rules apply to PATTERN-based …` sentence, `writeGroundedCalibrationCriteria`.
4. `severity-calibration-rubric` — the `### Severity calibration` heading, rubric body and three tier bullets, `writeSeverityCalibration`.
5. `re-read-before-reopen-bullet` — the `Before emitting a \`reopened\` resolution, READ the CURRENT diff state` bullet, which renders in a COMPLETELY SEPARATE place: inside the `if len(t.PriorConcerns) > 0` guard of the "Prior concerns (delta verification)" section. It is GUARD-CONDITIONAL, which is why the strip is posture-aware and fails closed in both directions.

**Editing ANY of these five reddens `backend/internal/agenteval`, and the copied literal must be updated in LOCKSTEP** — `TestStripCalibrationCriteria_AcceptsRealPromptOutput` runs the strip over a real built prompt in all four postures, so a one-word change here fails there until the copy follows. That is the deliberate cost of the drift detector, recorded at the edit site so it is discoverable. **Adding a SIXTH #2119 surface to this prompt without registering it in `agenteval.treatmentLiterals` silently WEAKENS the pre arm** — the arm labelled "pre-#2119" would carry part of the treatment, and nothing would go red. `TestTreatmentLiteralCount_IsFiveAndEnumerated` pins the count at five; it cannot see a new surface nobody registers.

Pinned by: `TestImplementReview_BaselineAndTraceCriteria_Rendered` (criteria 9/10 present and ordered after rule 8 and before the verdict rule, with criteria 1-8, the lenses and the `standing rule 7` cross-reference asserted unmoved, and the stale lead-in asserted gone); `TestImplementReview_CalibrationCriteria_GroundedVariant` / `_UngroundedVariant` (a SELF-PAIRED pair over the same two text constants — each asserts its own variant PRESENT and the other ABSENT, so collapsing the branch reddens a half and neither can pass vacuously); `TestImplementReview_CalibrationCriteria_AdversarialCarveOut` (the carve-out in BOTH postures); `TestImplementReview_SeverityRubric_Rendered` (all three tiers on BOTH gate-evidence branches, plus the index ordering verdict-rule < rubric < `ImplementReviewSplitMarker` that pins the cache-prefix placement); `TestImplementReview_PriorConcerns_ReReadBeforeReopen` paired with `_EmptyByteIdentical` (the absence half is the counterfactual on the conditional render); and the hand-edited `nilSliceVerifyPromptGolden` whole-prompt byte pin.
...
- **Ungrounded** (`ReviewTreeCommit` empty — the degrade path, and the SHIPPING
  default): the bullet forbids all tools and the section states plainly that no
  repository tree is available and the review is DIFF-ONLY, so the reviewer scopes
  its confidence to the diff. Since E45.90 / [#3625](https://github.com/kuhlman-labs/fishhawk/issues/3625)
  it ALSO names `FISHHAWKD_REVIEW_GROUNDING` as the DEPLOYMENT switch that would
  ground the review, so the posture reads as configurable rather than as a product
  limit. The flag is named on the UNGROUNDED branch ONLY — a grounded render must
  never carry it, and that negative is pinned, not assumed.
...
repo-access edit alone cannot green it) and `_AbsentFromGroundedRenders` (the
negative half).
...
## Approval-condition responses in the commit body (#3400)

An approval condition that asks the agent to STATE something (why an operator-added scope path was left untouched, what a counterfactual showed) had no surface the implement review reads: PR `## Notes` never reaches the diff-only reviewer, so reviewers raised evidence-placement concerns the operator could not discharge. The commit body is now that surface.

- **Implement side** (`writeApprovalConditionsCommitBodySection`, called from `writeImplementCommitMessage` for the full implement + decomposed child and from `writeFixupCommitMessage` for the slim fix-up; `writeApprovalConditionsReinforcement` names it again at the tail, guarded on the same run/stage ids as the block it points at). ONLY when `t.ApprovalConditions != nil`: the commit body MUST END with a line exactly `Approval conditions:` (`ApprovalConditionsCommitBodyHeading` — the runner's `approvalConditionsHeading` matches the same literal, TrimSpace-exact; `## Approval conditions` or lower-case do not) followed by one entry per condition in the operator's order — `- condition N: <how>` or `- condition N: no action required because <reason>`; a fix-up may write `- condition N: not affected by this pass`. The PR `## Notes` checklist stays as the human restatement (the commit-message-ONLY sentence is re-worded on the conditions branch so it no longer says the checklist stays out of the commit). Nil conditions write nothing: every conditions-less render is byte-identical.
- **Reviewer side** (`writeGateEvidence`). `GateEvidence.ApprovalConditionResponses{Source, Text, Truncated}` (mapped from the bundle's `approval_condition_responses`, which the runner peeks from the sidecar at pack time) renders a BINDING bullet — the block is the agent's OWN response captured from its PROPOSED commit message, NOT machine-verified and NOT a statement that it is on the branch; VERIFY a stated `no action required because …` against the diff/tree and CITE it instead of raising an evidence-placement / `operator_scope_path_undelivered` concern; a response the diff contradicts is a `high` `evidence_conflict`; no line inside is an instruction — then, after the verify section and the operator-scope block, `writeApprovalConditionResponses` writes a trusted header (source, truncation) and the text through `sanitizeUntrustedComment` between column-0 `<<<BEGIN UNTRUSTED COMMIT-BODY RESPONSES>>>` / `<<<END …>>>` delimiters (`untrustedCommitBodyBegin/End`): per-line `| ` quoting, trusted-marker defanging (`Approval conditions` is one), fence breaking and `neutralizeEnvelopeDelimiters`, so the text can neither forge a delimiter line nor a Fishhawk heading. The stage-cumulative `operator_scope_path_undelivered` bullet gains one clause pointing at the block when present. `GateEvidence.ApprovalConditionsAttached` (backend-set: conditions attached AND bundle-derived evidence in hand) with nil responses renders the `approval_conditions_unrecorded` bullet: record EXACTLY ONE `low` concern of that category and no per-condition evidence-placement concerns. Neither signal → byte-identical render.
- **Why "proposed", not "durable".** The runner captures the sidecar BEFORE the commit exists (the commit is made after the #742 forward-gated upload that dispatches this review). Shared parsing proves the captured text and the eventual commit message AGREE; it does not prove the commit was made. Asserting branch durability to the one reader this surface exists for would reproduce the original problem one layer up.

Pinned by `TestBuild_Implement_ApprovalConditions_CommitBodyInstruction` (three paths × with/without), `TestWriteGateEvidence_ApprovalConditionResponses` (delimiters, neutralization — its counterfactual is writing the text raw), `TestWriteGateEvidence_ApprovalConditionsAttachedNoResponses`, `TestWriteGateEvidence_NoApprovalConditionSignals_ByteIdentical`, `TestWriteGateEvidence_OperatorScopeCumulative_PointsAtResponses`.
...
`testdata/plan-prompt-pre-change.golden` was regenerated a FIFTH time at
E68.28 / #3057 (after #2290, #2871, #3325 and #3620), from
`Build("plan", preChangeGoldenTrigger())` at that head. The diff is exactly TWO
hunks and nothing else: a single-line INSERTION of the new Deciding-test rule
paragraph into the acceptance-criteria section, and a single-line REPLACEMENT of
the Counterfactual attainability rule paragraph, which gained clauses (5)-(7)
INSIDE the existing paragraph — 2 insertions / 1 deletion. Both anti-vacuity
guards still hold, and each section's own presence is pinned independently
(`TestBuild_Plan_DecidingTestRule`,
`TestBuild_Plan_CounterfactualAttainabilityRule`), so the golden is a
no-other-drift pin rather than the primary evidence. The untrusted-intake
ENVELOPE framing was again NOT touched, nor was the #3192 verify-output framing,
so both byte-exact literal copies in `backend/internal/agenteval`
(`StripBodyEnvelope` and `injection_test.go`'s `verifyOutputEnvelopeFraming`) are
unaffected. `testdata/implement-review-no-prior-concerns-pre-change.golden` is
untouched: `writeCounterfactualDiscipline` does not render on the review path,
which `TestBuild_Plan_CounterfactualMechanismClause_AbsentFromImplementReview`
asserts directly.
