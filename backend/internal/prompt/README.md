...
**Criteria 9 and 10 are grounding-CONDITIONAL, and the ungrounded variant is the DEFAULT path, not the exception.** Both branch on `Trigger.ReviewTreeCommit` — the same signal `writeReviewToolClause` and `writeReviewRepoAccess` key on — inside one helper, `writeGroundedCalibrationCriteria`. GROUNDED (non-empty): resolve the baseline / the prediction against the exported tree and CITE it; an unresolved question about a sibling's behavior is a search to run, not a hedge. UNGROUNDED (empty): say plainly that the baseline is UNESTABLISHED / the prediction UNTRACED and calibrate the severity DOWN, rather than asserting a regression the reviewer structurally cannot check. The branch exists because ADR-078's operator correction ships grounding **dormant** (`FISHHAWKD_REVIEW_GROUNDING` defaults false, after a live check found both adapters could read files outside the exported tree), so an unconditional "go read the sibling file" instruction would be unsatisfiable on the path that actually executes — the same failure mode ADR-078 records against the pre-#2486 prompt.
