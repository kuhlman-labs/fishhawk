...
- `MaxConditionBytes = 4000` — the unchanged cap for the sibling operator-text channels that share this rendering path: clarification answers (`loadClarificationAnswers`) and the recovery resume reason (`loadRecoveryResumeReason`). These are advisory/bounded and are NOT gate-refused, so their 4000-byte behavior (and its existing `clarification_answer_test.go` pin) is preserved. The revision constraint is NO LONGER one of them (#2871) — see `MaxRevisionConstraintBytes` above.
...
| `MaxConditionBytes` | clarification answers + recovery resume reason | 4000 | unbounded operator text | **bare 4000-byte cut** | **Gambling.** No gate refuses an over-cap clarification answer, and the LAST answer dies first. Not fixed here — [#3063](https://github.com/kuhlman-labs/fishhawk/issues/3063) owns it. |
