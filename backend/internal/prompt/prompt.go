...
// GeneratedSurfaceRelation is one generated_surface rule-table row (#3437)
// threaded into the plan-stage prompt's Generated-surface derivative map so the
// planner scopes (or exempts) a canonical source's generated derivatives at
// first emission instead of the plan gate refusing on a deterministic,
// machine-derivable miss. Trigger is the canonical source path; Derivatives are
// the generated paths that must also be scoped; Generator names the command that
// regenerates them. The prompt package cannot import server (server imports
// prompt), so the data is threaded IN via the Trigger from the server's
// generatedSurfaceRelationsForPrompt accessor over the single-source rule table
// — prompt stays a pure renderer of injected data.
type GeneratedSurfaceRelation struct {
	Trigger     string
	Derivatives []string
	Generator   string
}

// GeneratedSurfaceRefusal is one recorded generated-surface miss (#3437): a
// canonical source (TriggerPath) scoped without its generated derivative(s)
// (Missing), regenerated by Generator, attributed to SubPlanTitle for a
// decomposition sub-plan scope (empty for the flat parent scope). buildPlan
// renders each as a line in the binding "Generated-surface scope restoration"
// section on the corrective re-dispatch.
type GeneratedSurfaceRefusal struct {
	TriggerPath  string
	Missing      []string
	Generator    string
	SubPlanTitle string
...
// GeneratedSurfaceRestoration is the plan gate's refusal notice for a plan that
// scoped a canonical source without its generated derivative(s) (#3437). When
// non-nil with a non-empty Findings, buildPlan renders a dedicated
// "### Generated-surface scope restoration (binding — this plan was REFUSED)"
// section naming each finding and the two admissible remedies, plus the refused
// plan itself (RefusedPlan) when available so the planner re-emits it with the
// derivatives added. RefusedPlan is nil-safe: the section still names the
// missing derivatives without it.
type GeneratedSurfaceRestoration struct {
...
type Trigger struct {
	...
	// GeneratedSurfaceRelations is the generated_surface rule-table projection
	// (#3437) rendered into the plan-stage prompt's Generated-surface derivative
	// map so the planner scopes (or exempts) a canonical source's generated
	// derivatives at first emission instead of the plan gate refusing on a
	// deterministic, machine-derivable miss. Threaded IN from the server
	// (generatedSurfaceRelationsForPrompt over the single-source rule table);
	// populated ONLY on the plan-stage build, so buildPlan guards on len>0 and
	// every non-plan build renders byte-unchanged.
	GeneratedSurfaceRelations []GeneratedSurfaceRelation
	// GeneratedSurfaceRestoration is set when the plan gate REFUSED the previous
	// plan for scoping a canonical source without its generated derivative(s)
	// (#3437). When non-nil with non-empty Findings, buildPlan renders a
	// dedicated "### Generated-surface scope restoration (binding — this plan was
	// REFUSED)" section. Nil on a normal plan dispatch and on a plan that was not
	// refused, and always nil on a non-plan build, so those prompts stay
	// byte-unchanged.
...
//
// APPROVAL REASON IS BINDING (#3322). Step 3's approve branch formerly said
// only "the paths are folded into the effective scope — edit them as normal",
// which framed the operator's decision_reason as a deny-only artefact even
// though the wire already delivers it on an approved row (amendmentToResponse
// copies DecisionReason unconditionally, and the runner-side upload.ScopeAmendment
// decodes it). Nothing TOLD the agent to read it, so run ddb31a40's approval
// instruction and 6d3ba9fe's fixture-shape instruction were both recorded on
// the audit entry and then never acted on. Step 3 now tells the agent an
// approval reason is a BINDING instruction on the amended paths — the same
// standing as an approval condition on the plan — while tolerating an EMPTY
// reason (the decision endpoint does not require one). Step 2's branch-summary
// sentence no longer implies the reason exists only on deny.
//
...
func writeScopeAmendments(b *strings.Builder) {
	...
	b.WriteString("2. Await the decision with the bounded long-poll: GET `$FISHHAWK_BACKEND_URL/v0/runs/<run_id>/scope-amendments?wait=30` (same bearer). The `?wait=30` makes the server hold the request up to 30 seconds and return as soon as your request's `status` leaves `pending`; re-issue the wait-poll each time it returns still-`pending`. Keep working on in-scope files while you wait. Loop the wait-poll until your request leaves `pending` OR ~15 minutes total have elapsed. Then take the branch that matches what you observed, and note the difference between the two ways the wait can end: a decided status — `approved` (step 3) or `denied` (step 4) — is a DECISION, and EITHER may carry a `decision_reason` you MUST read and act on. A request still `pending` at the ~15-minute cap is an EXPIRY — the operator has not answered, your request stays live, and you MUST NOT treat it as a refusal or state that the operator denied it (step 5).\n")
	b.WriteString("3. On `approved`: the paths are folded into the effective scope — edit them as normal. READ the `decision_reason`: an approval reason is a BINDING instruction on the amended paths, exactly like an approval condition on the plan — follow it rather than re-deriving the answer yourself, and if you cannot follow it, say so in your final response instead of silently departing from it. The field may be EMPTY (the operator is not required to leave a reason); an absent reason adds no instruction and the approval stands on its own.\n")
...
func buildPlan(t Trigger) string {
	...
	// Generated-surface scope restoration (#3437): the plan gate REFUSED the
	// previous plan for scoping a canonical source without its generated
	// derivative(s). Rendered on the corrective re-dispatch, independent of the
	// blocks above. Nil on a normal plan dispatch and on a plan that was not
	// refused, so those prompts stay byte-unchanged.
	if t.GeneratedSurfaceRestoration != nil && len(t.GeneratedSurfaceRestoration.Findings) > 0 {
		findings := t.GeneratedSurfaceRestoration.Findings
		truncated := false
		if len(findings) > maxScopeCarryForwardPaths {
			findings = findings[:maxScopeCarryForwardPaths]
			truncated = true
		}
		b.WriteString("### Generated-surface scope restoration (binding — this plan was REFUSED)\n\n")
		b.WriteString("Your previous plan was NOT admitted to review. It scoped a canonical source below without " +
			"its generated derivative(s) in the SAME scope. A generated derivative missing from scope.files is " +
			"excluded from the implement commit, so the canonical source and its generated output ship out of " +
			"sync — which is why the gate refuses rather than leaving a review comment.\n\n")
		for _, f := range findings {
			prefix := ""
			if f.SubPlanTitle != "" {
				prefix = "sub-plan \"" + sanitizeScopePath(f.SubPlanTitle) + "\": "
			}
			missing := make([]string, 0, len(f.Missing))
			for _, m := range f.Missing {
				missing = append(missing, sanitizeScopePath(m))
			}
			fmt.Fprintf(&b, "- %s%s generates %s via `%s`\n",
				prefix, sanitizeScopePath(f.TriggerPath), strings.Join(missing, ", "), sanitizeScopePath(f.Generator))
		}
		if truncated {
			fmt.Fprintf(&b, "- ...[%d more findings truncated]\n",
				len(t.GeneratedSurfaceRestoration.Findings)-maxScopeCarryForwardPaths)
		}
		b.WriteString("\nExactly two remedies are admissible, per derivative:\n\n" +
			"1. SCOPE the derivative(s) into the SAME scope.files (top-level, or the owning decomposition " +
			"sub-plan) with operation modify, and have the implement step run the named generator so the " +
			"derivative is regenerated from the canonical source. This is the default.\n" +
			"2. DECLARE a top-level surface_sweep_exemptions entry {\"pattern\": \"<generator>\", " +
			"\"sibling\": \"<derivative>\", \"reason\": \"...\"} ONLY when the edit provably leaves the " +
			"derivative byte-identical. Reviewers read the reason and can challenge it, so do NOT use it to " +
			"shed a derivative the change actually regenerates.\n\n")
		if t.GeneratedSurfaceRestoration.RefusedPlan != nil && *t.GeneratedSurfaceRestoration.RefusedPlan != "" {
			writeRevisionBase(&b, *t.GeneratedSurfaceRestoration.RefusedPlan,
				"Refused plan (re-emit it with the derivative paths added — do NOT replan blank-slate):")
		}
	}
	...
	if len(t.GeneratedSurfaceRelations) > 0 {
		b.WriteString("\n")
		b.WriteString("Generated-surface derivative map (#3437): the plan gate REFUSES — before any reviewer runs, with one bounded in-run re-dispatch — a plan that scopes a canonical source below without EVERY listed derivative in the SAME scope.files (the same sub-plan on a decomposed plan). Scope each derivative with operation modify and have the implement step run the named generator; only when the edit provably leaves the derivative byte-identical, declare a surface_sweep_exemptions entry {\"pattern\": \"<generator>\", \"sibling\": \"<derivative>\", \"reason\": \"...\"} — reviewers see and can challenge the reason:\n")
		for _, rel := range t.GeneratedSurfaceRelations {
			fmt.Fprintf(&b, "- %s: %s generates [%s]\n",
				rel.Generator, rel.Trigger, strings.Join(rel.Derivatives, ", "))
		}
	}
	b.WriteString("\n")
