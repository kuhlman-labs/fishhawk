...
// AmendmentPollWindowSeconds (900), the SERVER-side undecidability threshold that
// decides the undecidable_before_deadline flag step 1 above reads (#2540). No
...
func writeScopeAmendments(b *strings.Builder) {
	...
	b.WriteString("\n")
	b.WriteString("1. POST `$FISHHAWK_BACKEND_URL/v0/runs/<run_id>/scope-amendments` with header `Authorization: Bearer $FISHHAWK_API_TOKEN` and body `{\"paths\": [{\"path\": \"dir/file.ext\", \"operation\": \"modify\"|\"create\"}], \"reason\": \"why each path must change\"}`. Paths are repo-relative; use `create` for net-new files. The POST response may carry `undecidable_before_deadline: true`: this means this stage's remaining wall clock (`stage_deadline_seconds_remaining`) is already shorter than the amendment poll window (`amendment_poll_window_seconds`, ~15 minutes), so an operator decision cannot land before this stage is killed. When you see it, do NOT enter the step-2 wait-poll and do NOT re-file — go straight to the step-5 branch, naming the amendment id, the requested paths, and the remaining seconds. This is NOT a denial: your request stays `pending`, so an operator decision plus a fishhawk_retry_stage still folds the paths later.\n")
