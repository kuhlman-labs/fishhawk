...
Plan-stage approvals are additionally gated on a budget check: when the plan's runtime estimate exceeds the resolved (p95-aware) implement-stage budget, approval is blocked (422 `plan_violates_budget`) unless the plan includes `decomposition.sub_plans` or the comment contains `--override-budget`.
...
- The estimate the gate reads is `plan.GateRuntimeMinutes()` — `max(predicted_runtime_minutes, raw_predicted_runtime_minutes)` — **not** the calibrated `predicted_runtime_minutes` alone (#2862). The plan prompt's calibration hint tells the planner to multiply its raw estimate by the fleet factor and write the product to `predicted_runtime_minutes`, so with a sub-1.0 factor a raw 90 lands at ~50 and a required decomposition dissolves silently. Reading the maximum means calibration may only ADD structure: a factor above 1.0 can still push a plan over the budget, but one below 1.0 can no longer pull it under. A plan that omits `raw_predicted_runtime_minutes` (absent or zero) is gated byte-for-byte as before, on the calibrated value alone.
- Outcomes emit `plan_violates_budget` or `plan_budget_override_acknowledged` audit entries carrying the resolved `budget_minutes`, `budget_source` (`spec`|`p95`|`ceiling`), the raw `spec_budget_minutes`, and (#2862) `raw_predicted_minutes` + `gate_predicted_minutes` so the entry says which number the gate actually read.
- `plan_budget_calibration_crossing` (#2862) is written when the two estimates STRADDLE the budget — i.e. calibration moved the estimate across the threshold in either direction. It is emitted on EVERY outcome branch, including the two that let the approval proceed, with `gate_outcome` one of `refused` / `decomposition_satisfied` / `override_acknowledged`. A crossing is always over budget by construction (the gate takes the maximum), so `within_budget` never appears. The payload also carries `implied_factor` (`predicted / raw`) and, best-effort, `fleet_calibration_ratio` — omitted rather than zeroed when the workflow has too little history for a hint. An append failure logs and never blocks the approval.
- The gate can only evaluate a raw estimate the planner REPORTS. A plan that ignores the prompt instruction is gated exactly as it was before #2862; the mitigation is visibility, not prevention — `runPlanWarnings`' unreported-raw advisory (`server/plan_warnings.go`) fires at the plan gate when the workflow HAS a resolvable calibration hint and the plan omits the field, so the absence is itself a trace the approver sees.
