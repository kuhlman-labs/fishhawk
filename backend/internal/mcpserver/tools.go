...
func registerTools(srv *mcp.Server, resolver *runResolver) {
	...
	registerAwaitChildren(srv, resolver)
	...
	registerPreviewCampaign(srv, resolver)
...
type GetPlanOutput struct {
	...
	// the poll.
	PlanReviewStatus *ReviewStatus `json:"plan_review_status,omitempty" jsonschema:"review lifecycle for the plan stage: status is one of none, pending, complete, skipped, failed. Re-polling fishhawk_get_run_status is the authoritative way to reach a terminal status. 'complete' means ALL configured agent reviewers have landed a terminal verdict (reviews[] carries one row per configured reviewer; reviews[] is ALWAYS present and is an EMPTY array for none/pending, never absent); 'pending' means a review was dispatched but the configured reviewers have not all landed yet — re-poll on the advertised poll_interval_seconds (this now also covers the partial-landing window where some but not all heterogeneous reviewers have returned; fishhawk_await_review is an optional convenience block); 'failed' means the reviewer errored or timed out (terminal)"`
...
type GetRunStatusOutput struct {
	...
	PlanReviewStatus      *ReviewStatus `json:"plan_review_status,omitempty" jsonschema:"review lifecycle for the plan stage: status is one of none, pending, complete, skipped, failed. Re-polling fishhawk_get_run_status is the AUTHORITATIVE way to reach a terminal review status. 'complete' means ALL configured agent reviewers have landed a terminal verdict (reviews[] carries one row per configured reviewer; reviews[] is ALWAYS present and is an EMPTY array for none/pending, never absent); 'pending' means a review was dispatched but the configured reviewers have not all landed yet — re-poll on the advertised poll_interval_seconds (this now also covers the partial-landing window where some but not all heterogeneous reviewers have returned; fishhawk_await_review is an optional convenience block over the same poll); 'failed' means the reviewer errored or timed out (terminal)"`
...
blocks that poll for you; it is not the primary mechanism. Its reviews[]
field has ONE shape on every status: always present, and an EMPTY array on
none/pending rather than absent.
...
type ListAuditInput struct {
	...
	Cursor        string `json:"cursor,omitempty" jsonschema:"OPAQUE pagination token: the only accepted value is the next_cursor field of a prior list response, copied VERBATIM. It is not an offset or an index and cannot be constructed by hand — a hand-built value is rejected with cursor_invalid, never silently reset to the first page"`
}
...
Inputs:
  ...
  - cursor    — OPAQUE pagination token. The only accepted value is a
                prior response's next_cursor copied VERBATIM; it is not
                an offset or an index and cannot be constructed by hand.
                A hand-built value is rejected with cursor_invalid,
                never silently reset to the first page.
...
type ApprovePlanInput struct {
	...
	AmendAcceptanceCriteria []AcceptanceCriteriaAmendment `json:"amend_acceptance_criteria,omitempty" jsonschema:"optional list of amendments to the approved plan's acceptance criteria, recorded on the SAME approval row as 'reason' (#2581). Each entry has id (a criterion id from the approved plan), action ('retire' — stop validating it, so an acceptance failure naming ONLY retired criteria is neutralized; or 'restate' — replace its statement, which keeps it LIVE and still failing if it genuinely fails), a REQUIRED reason (the reconstructable why), and statement (required for restate). Refused 400 for an unknown id, a blank reason, a restate with no statement, a duplicate id, an id a prior approval already retired, or use on a reject/non-plan stage; refused 422 acceptance_criteria_all_retired when the amendment would retire EVERY criterion (re-plan instead of emptying the contract) and 422 acceptance_criteria_unavailable when the plan carries no acceptance_criteria. Use it when your approval conditions changed the design a criterion assumed; a criterion merely made contested by remove_scope_files needs no amendment — the acceptance prompt already carries the dropped paths as contested context. A third action 'retire_scenario' (E72.4 / #3328) retires a persisted replay SCENARIO instead of a plan criterion: id is the scenario id (scenario:issue-<N>/<criterion-id>), reason is REQUIRED, no statement; it never counts toward the all-retired gate and is permitted with zero plan criteria. The FULL entry {id, reason, run_id, pr, retired_at} is recorded on the approval row as retired_scenarios and merged into acceptance/scenarios/retired.yaml by the acceptance runner. Refused 400 for an id without the scenario: prefix, a blank reason, a statement, a duplicate, or an already-retired scenario; refused 400 acceptance_scenario_retirement_unpersistable when the plan's shape (acceptance_surface none, out_of_scope with no criteria, empty criteria, or every criterion skip_expected with a basis) settles the acceptance stage with no runner spawn — retire it on a run whose acceptance stage executes. A fourth action 'add' (#3181) APPENDS an operator-authored criterion instead of burning a revise_plan pass: id is a FRESH lowercase slug (^[a-z0-9][a-z0-9-]*$) not already in the plan and not added by a prior approval, statement and reason are both REQUIRED. The added criterion is always drivable and NEVER blocking — it did not pass plan review, so it is advisory: it is served to the acceptance validator under an operator-authored block, and a failed acceptance naming ONLY operator-added criteria is neutralized at ingest (basis added_criteria_only). Refused 400 criterion_id_exists (id in the plan, already added, or carrying the scenario: prefix), invalid_criterion_id, statement_required, reason_required, duplicate_id; refused 422 acceptance_criteria_unavailable on a plan with no acceptance_criteria and 422 acceptance_criteria_all_operator_authored when the amendment would leave NO reviewed criterion live while any add is present (in one call or cumulatively) — an added criterion may never become the whole contract"`
...
type AcceptanceCriteriaAmendment struct {
	ID        string `json:"id" jsonschema:"the acceptance criterion id from the approved plan (verification.acceptance_criteria[].id) this amendment targets, or for 'add' a FRESH lowercase slug not already in the plan"`
	Action    string `json:"action" jsonschema:"'retire' (the criterion is no longer validated; an acceptance failure that names ONLY retired criteria is recorded as passed), 'restate' (replace the criterion statement — the criterion stays LIVE and still fails if it genuinely fails), 'add' (#3181: append an operator-authored criterion with a fresh id — drivable, advisory, never blocking; a failure naming ONLY added criteria is neutralized), or 'retire_scenario' (E72.4: id is a scenario:issue-<N>/<criterion-id> replay-corpus id, not a plan criterion; the acceptance runner merges the retirement into acceptance/scenarios/retired.yaml)"`
	Reason    string `json:"reason" jsonschema:"REQUIRED per criterion: why this criterion is retired, restated or added. Recorded on the approval audit entry and rendered into the acceptance prompt, so it is the reconstructable why"`
	Statement string `json:"statement,omitempty" jsonschema:"the criterion statement; REQUIRED when action is 'restate' (the replacement) or 'add' (the new criterion) and ignored for 'retire'"`
}
