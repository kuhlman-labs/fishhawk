...
type StartCampaignInput struct {
	...
	GroomingAllowSuperseded bool `json:"grooming_allow_superseded,omitempty" jsonschema:"OPTIONAL with grooming_run_id: build from this order even though a NEWER approved grooming run has superseded it (or the supersession check could not prove it is current). Default false REFUSES both cases — prefer grooming the newer run. Set true only when you deliberately want the older ratified order; the choice is recorded in the campaign's durable provenance"`
