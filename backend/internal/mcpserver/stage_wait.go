...
type FixupRecovery struct {
	...
	// #3395 round closure. DeliveredNothing is a pointer so the three states
	// stay distinct on the wire: true, false, and ABSENT (a legacy recovery
	// entry, or one whose fixup_pushed read failed).
	DeliveredNothing   *bool    `json:"delivered_nothing,omitempty" jsonschema:"true when the backend verified no fixup_pushed entry landed after the trigger and therefore restored the PRIOR review round — implement_review_status reads that round's verdicts again (not pending) — and re-opened the routed concerns; false when the pass pushed a commit before it died, so the pushed head's re-review round governs; absent on a legacy entry or when the backend could not read the push ledger"`
	ReopenedConcernIDs []string `json:"reopened_concern_ids,omitempty" jsonschema:"the routed concern ids the backend transitioned addressed_pending → reopened when the pass delivered nothing (backend-authored UUIDs, passed through unchanged). Absent when none was transitioned — read concern_reopen_error to tell 'none needed re-opening' from 'the re-open did not happen'"`
	ConcernReopenError string   `json:"concern_reopen_error,omitempty" jsonschema:"present when re-opening the routed concerns could NOT be attempted or partially failed, naming what went wrong — the concern ledger may still show them addressed_pending, so do not read an empty reopened_concern_ids as a clean ledger while this is set. STRUCTURE-NEUTRALIZED and bounded like source_failure_reason (it carries store error text); read the stage_fixup_recovered audit entry with fishhawk_list_audit for the exact bytes"`
