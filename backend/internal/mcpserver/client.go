...
type onboardingWorkItemProvider struct {
	...
	// CampaignSources (#3658): always served as an array.
	CampaignSources []string `json:"campaign_sources" jsonschema:"the campaign sources the resolved provider can serve, in the fixed order epic_ref, items (computed from the provider's compile-time capabilities); always served as an array - EMPTY on status unregistered and unknown (no capability claim), and an empty array on status registered means fishhawk_start_campaign refuses 501 in either mode"`
