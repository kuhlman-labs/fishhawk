...
- `recovery: "none"` — no verb applies; `reason` names the state and says what the stage needs instead (`running`, `dispatched`, `awaiting_children`, the deploy park states, `awaiting_scope_decision`, and an admissible park on a run whose PR has NOT merged).
