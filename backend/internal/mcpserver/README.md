...
**Credentials on the MCP host:** the runner reads `FISHHAWK_GITLAB_TOKEN` from its own environment (`runner/cmd/fishhawk-runner/main.go`, `mintImplementToken`), and the spawn passes `os.Environ()` through, so the operator must export it on the host running `fishhawk-mcp`. A missing token fails the IMPLEMENT stage's push / MR-open at the runner with the runner's existing message, not at spawn. Operator walkthrough: `docs/deploy/gitlab.md` § "Driving a GitLab project from MCP / CLI with a local runner".
