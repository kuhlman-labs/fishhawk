...
- **Compact-default review prose is elided with a VISIBLE marker (#3043).** On the compact-by-default `fishhawk_get_run_status` path, `compact.go::stripReviewProse` replaces a NON-EMPTY review `free_form` and each non-empty concern `note` with a short stand-in (`elidedReviewProseMarker`) naming the full-note surfaces (`fishhawk_get_gate_view` / `include_review_prose=true`), instead of blanking it to `""`. A genuinely-empty note is left empty — so an operator can tell an elided note apart from one the reviewer never wrote (the two states this issue's half (b) is about).
