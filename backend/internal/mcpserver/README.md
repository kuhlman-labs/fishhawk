...
**Why the one-minute margin is enough.** The client's timeout measures the ENTIRE request; a budget starting at the resolver call would leave the handler's pre-resolution work outside the measured span and unbounded. `handleCreateCampaign` therefore anchors its deadline at **handler entry** (`requestStart`), so the two measure the same span. The residual the margin covers is network transit, the middleware chain that runs *ahead* of the handler (request-id, bearer authentication — `requireWriteScope` reads an Identity the auth middleware already resolved, so the token lookup is outside `requestStart`), and the response write. That is a database lookup plus transit, not per-item forge round-trips.
