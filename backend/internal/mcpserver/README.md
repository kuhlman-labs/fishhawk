...
- **The dynamic detail is UNTRUSTED and handled as such.** `source_failure_reason` and `source_failure_category` are lifted out of an audit payload whose text was produced while an agent ran commands against an untrusted repository, and this marker promotes them from an explicitly-requested audit view into ROUTINE `await_stage` / `get_run_status` results — so provenance cannot be established for them and they are handled as untrusted instead. `latestFixupRecovery` is the single choke point: both fields go through `neutralizeUntrustedFailureText` (control characters — including newlines — collapse to spaces; runs of 3+ `<` or `>` are RUN-SPLIT so no live `<<<`/`>>>` delimiter can be emitted; ``` and `~~~` fences are broken) before they are capped, so the structured JSON fields and the prose built from them are both defanged. Every WORD survives — the text exists to be diagnosed from. The message then reproduces the detail inside a labelled `<<<BEGIN UNTRUSTED FIX-UP FAILURE TEXT>>>` / `<<<END …>>>` quarantine envelope preceded by a preamble naming its provenance and the handling rule, mirroring the untrusted-issue-body / issue-comment / acceptance-failure envelopes `backend/internal/prompt` already uses (ADR-029 / [#650](https://github.com/kuhlman-labs/fishhawk/issues/650)). The run-splitting is deliberately not a pairwise `ReplaceAll("<<<", "<< <")`, which leaves `>>>>` as a live `> >>>`. `restored_state` and `restored_review_stage_id` are backend-authored enum/UUID values and are passed through unchanged.
