...
- **On a conflict: an agent pass, then fail closed (E64.62 / [#3202](https://github.com/kuhlman-labs/fishhawk/issues/3202)).** The merge is never forced and the branch is never written to. With the conflict-resolution budget intact the verb **triggers** a bounded, operator-authorized agent pass that resolves the conflict ON the run branch and returns `conflict_resolution_triggered: true` (HTTP 202) with the re-opened `conflict_resolution_stage_id`. At that point **no merge commit exists** and the branch is unchanged (`new_head_sha` equals `prior_head_sha`): await the named stage, then re-invoke the verb, which takes the already-contains-base short-circuit and re-parks + republishes at the resolved head. The pass is counted against its **own ceiling-1 counter** (`stage_conflict_resolution_triggered`) and consumes **no fix-up budget**; a pass that refuses writes `stage_conflict_resolution_failed` and restores the pre-pass review gate rather than failing the run. Once that single pass is **spent** — or when the run has no re-openable implement stage — the verb is refused (`rebase_conflict`, 422) having written **nothing**, naming the spent budget, the failed pass's confinement-gate reason and the fallback (resolve on the run branch, `fishhawk_vouch_commit` the resulting commit, re-merge). `next_actions` surfaces that fallback on both the status and post-stage snapshots the moment the pass fails, so the operator learns the budget is spent without re-invoking the verb. Every uncertain anchor is likewise refused (`rebase_not_determinable`, 422) rather than merged on a guess.
