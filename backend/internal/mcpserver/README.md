...
- **`fishhawk://onboarding-skill` resource** (E29.6 / #1516) — a second listable/readable `text/markdown` resource: a Claude Code SKILL.md-shaped document (YAML frontmatter `name`/`description`, then a body) walking `fishhawk_doctor` → read the readiness rungs → `fishhawk_init` → write `workflow_yaml` to `.fishhawk/workflows.yaml` → `fishhawk_validate` until `valid: true` (E45.65 / #3579 — the pre-commit check; the skill's Step 3 used to say "re-run `fishhawk_doctor` until `spec.valid`", which is a dead loop because the doctor's spec rung reads the DEFAULT BRANCH) → the operator commits and opens the PR under their own identity (ADR-040) → post-merge `fishhawk_doctor` confirms `spec.valid` and `reviewers[].model_status` → the first `fishhawk_start_run`. `TestOnboardingSkill_Step3ValidatesBeforeCommit` pins the Step 3 SECTION body (not the frontmatter) to name `fishhawk_validate` and not the doctor loop. Repo-agnostic like the grooming section — no owner, org or forge URL — because it ships with the binary to every connecting repository, including one with no fishhawk files yet. Carries a one-line install hint: copy the resource to `.claude/skills/fishhawk-onboarding/SKILL.md` in the target repo to make it a standing project skill there.
...
## Onboarding tools (`fishhawk_doctor` / `fishhawk_init` / `fishhawk_validate`, [#1506](https://github.com/kuhlman-labs/fishhawk/issues/1506), [#3579](https://github.com/kuhlman-labs/fishhawk/issues/3579))

Three thin tools wrap the E29 onboarding engine so a connecting Claude Code agent can drive a conversational "help me onboard a repo" flow — **one engine, another frontend** (the CLI `fishhawk doctor` / `fishhawk init` / `fishhawk validate` and the App-PR path are the other frontends). `fishhawk_doctor` and `fishhawk_init` live in `onboard.go` (E29.6); `fishhawk_validate` in `validate_spec.go` (E45.65).

- **`fishhawk_doctor`** (read-only) wraps `GET /v0/onboarding/readiness` (E29.4 / [#1511](https://github.com/kuhlman-labs/fishhawk/issues/1511)) and returns the `report` — the server-side-only readiness checks a repo's first `feature_change` run needs (five on GitHub, six on GitLab — E45.43 / [#3348](https://github.com/kuhlman-labs/fishhawk/issues/3348), E45.66 / [#3580](https://github.com/kuhlman-labs/fishhawk/issues/3580) and E45.68 / [#3582](https://github.com/kuhlman-labs/fishhawk/issues/3582)), plus, on both families, the deployment-scoped `trace_store` rung (E45.75 / [#3600](https://github.com/kuhlman-labs/fishhawk/issues/3600)) and `review_grounding` rung (E45.90 / [#3625](https://github.com/kuhlman-labs/fishhawk/issues/3625)) — which the agent cannot introspect locally:
  - `forge` — `github` | `gitlab`: the family that answered (absent against a pre-#3348 fishhawkd).
  - `app` — `{installed, installation_id?, resolvable?, reason?, note?}`: is the Fishhawk-specific authorization a run needs in place. On GitHub: is the GitHub App installed on the target repo. On GitLab there is no App: since E45.68 / #3582 `installed` is true ONLY when a gitlab installation is registered for EXACTLY this project path (`fishhawkd installation register` — the authorization `POST /v0/runs` checks) AND the project resolves with the deployment GitLab credential; `note` says so, `installation_id` is never set, and the weaker fact is its own `resolvable` pointer (gitlab only; `false` when not visible, ABSENT when the gitlab forge is unconfigured — never read — and always absent on github). A resolvable-but-unregistered project reports `installed:false` with a `reason` pointing at `gitlab_registration.remediation`; a not-visible project reports `resolvable:false` with a reason naming the credential (`FISHHAWKD_GITLAB_TOKEN`), no longer the register command.
  - `spec` — `{source, valid, error?, note?}`: the committed `.fishhawk/workflows.yaml` fetch + parse + validate state (`source` is `fetched` or `unavailable`; `valid` is only meaningful when fetched). Only checked once the app is installed (on GitLab: once the project RESOLVED — the spec and `gitlab_merge_gate` cascades key on `resolvable`, not on registration).
  - `reviewers[]` — `{provider, model?, reasoning_effort?, available, model_status?, model_hint?, priced?, missing_hint?}`: per spec-declared reviewer availability on **this** deployment. `available` = provider wired AND the resolved model not authoritatively rejected; `model_status` is `verified | rejected | unverifiable` (#3578), computed from the resolved deployment default when the spec omits the model; `model_hint` carries the did-you-mean / verbatim-to-vendor / unpriced sentence; `priced` (`*bool`, absent only when the resolved model id is unavailable) flags an unpriced family; `missing_hint` is the env-var hint when a provider can't be resolved. Empty when the spec is unavailable or invalid.
  ...
  - `gitlab_merge_gate` — `{status, branch?, protected?, matched_rules[]?, allow_force_push?, push_access_levels[]?, merge_access_levels[]?, pipeline_must_succeed?, allow_skipped_pipeline?, discussions_must_be_resolved?, authoritative, reason?, detail?, remediation?, note}` (E45.66 / [#3580](https://github.com/kuhlman-labs/fishhawk/issues/3580)): the GitLab-shaped sibling of `merge_gate`, under its OWN key because it answers a DIFFERENT question — GitLab has no per-context required status check, so it reports whether the project's REAL default branch is protected (every matching exact/wildcard rule, the effective UNION of access levels across them per GitLab's most-permissive semantics) and whether the project requires a successful head pipeline to merge. `status` is `pipeline_gated` / `not_pipeline_gated` / `unknown`, FAIL-CLOSED like `merge_gate`: every degrade (no gitlab forge, project not visible, an adapter without `forge.MergeProtectionReader`, an unresolved default branch, a 401/403 — the `protected_branches` API needs Maintainer — a transport error, a probe timeout) resolves to `unknown` with a naming `reason`, and every unread signal is ABSENT (pointer bools), never `false`. `note` is constant and states that `pipeline_gated` is NOT a claim that `fishhawk_audit_complete` individually gates the merge. Long-form: `backend/internal/server/README.md`.

  - `gitlab_registration` — `{status, project_path?, installation_ref?, resolved_ref?, ref_matches?, reason?, detail?, remediation?, note}` (E45.68 / [#3582](https://github.com/kuhlman-labs/fishhawk/issues/3582), GitLab only): is an `installations` row registered for EXACTLY this project path — the check `POST /v0/runs` performs through the SAME registry seam and exact-path rule and refuses `422 gitlab_project_not_registered` on, so `registered` means a run for this path will pass it (the backend pins the agreement end to end). `status` is `registered` / `not_registered` / `unknown`; `not_registered` is a POSITIVE finding (no row, or an ambiguous double registration), and `unknown` is FAIL-CLOSED — the registry could NOT answer, `reason` is `registry_unwired` (no installation registry / no database on the deployment) or `registry_lookup_failed`, and it is NOT evidence the project is unregistered. `installation_ref` is the registered `gitlab:<project_id>` (the project a run would act on), `resolved_ref` the id the path resolved to with the deployment credential, and `ref_matches` compares them — set ONLY when both are known; `false` means the registration is bound to a DIFFERENT project than the path resolves to (`detail` names both refs) and a run would silently act on the registered one. `remediation` is a copy-pasteable `fishhawkd installation register` command carrying the REAL resolved project id when the path resolved — a human action on the fishhawkd host, never performed by the agent. The rung does NOT check the `account_key` binding the webhook receiver additionally enforces (`note` says so). The mirror type `onboardingGitLabRegistration` is unexported like its merge-gate sibling (the export baseline is untouched) and a POINTER: `TestOnboardingReadinessReport_GitLabRegistrationMirrorsBackendTags` decodes a literal body in which EVERY field is non-empty so a tag typo cannot pass on zero values, `_GitLabRegistrationAbsentKeepsNil` pins the github-body nil, `_ResolvableFalseSurvivesReemit` pins why `app.resolvable` is a pointer (a read `false` must not be dropped as absence), and `TestDoctor_GitLabReport_ReemitsGitLabRegistration` / `TestDoctor_GitHubReport_NoGitLabRegistrationKey` pin the re-emitted wire bytes in both directions. Long-form: `backend/internal/server/README.md` § "GitLab registration rung".
  - `trace_store` — `{configured, kind, note?, remediation?}` (E45.75 / [#3600](https://github.com/kuhlman-labs/fishhawk/issues/3600), BOTH families): is a trace store wired on this deployment, i.e. will `POST /v0/runs/{id}/trace` accept the run's bundle or respond 503 AFTER the agent has run and been billed. `kind` is one of four values: `s3` (durable S3/RustFS), `memory` (the `--dev-fixtures` in-memory store — configured but EPHEMERAL), `none` (`configured:false`; `remediation` names `FISHHAWKD_S3_BUCKET` and `make s3-init`) or `other` (no durability claim). DEPLOYMENT-scoped: it never cascades on `app`/`spec`, so a not-installed repo still carries it. The mirror type `onboardingTraceStore` is unexported and a POINTER — an older fishhawkd's absent key re-emits NO `trace_store` key, and absence is "cannot answer", not `configured:false`. Pinned by `TestOnboardingReadinessReport_TraceStoreMirrorsBackendTags` (every field non-empty), `_TraceStoreAbsentOrNullKeepsNil`, `TestDoctor_TraceStore_ReemitsWireBytes` (both directions) and `TestDoctorToolDescription_DescribesTraceStore`; the served-body cross-boundary decode lives in the server package. Long-form: `backend/internal/server/README.md` § "Trace-store rung".
  - `review_grounding` — `{enabled, adapters[]?, note?, remediation?}` (E45.90 / [#3625](https://github.com/kuhlman-labs/fishhawk/issues/3625), BOTH families): does this deployment ground its plan-/implement-review agents against an exported read-only tree at the reviewed commit, or leave them DIFF-ONLY? `enabled:false` is the SUPPORTED DEFAULT — grounding ships dormant behind `FISHHAWKD_REVIEW_GROUNDING` (#2522) — so it is reported as an opt-in posture, never as a misconfiguration; `remediation` names the flag. `adapters[]` renders in BOTH postures and states each adapter's read bound: `confined` on codex (an OS-level deny-by-default allowlist — an out-of-tree read returns EPERM) and `blocklist` on claude (a tool-layer `--disallowed-tools` deny-rule list: defence-in-depth, NOT confinement). The two are NOT the same strength and are never collapsed into one word; the table is a STATIC restatement of the shipped posture, not a live probe. DEPLOYMENT-scoped: it never cascades on `app`/`spec`. The mirror types `onboardingReviewGrounding` / `onboardingReviewGroundingAdapter` are unexported and the field is a POINTER — an older fishhawkd's absent key re-emits NO `review_grounding` key, and absence is "cannot answer", not `enabled:false`. Pinned by `TestOnboardingReadinessReport_ReviewGroundingMirrorsBackendTags` (every field, nested row included, non-empty and distinct), `_ReviewGroundingAbsentOrNullKeepsNil`, `TestDoctor_ReviewGrounding_ReemitsWireBytes` (both directions) and `TestDoctorToolDescription_DescribesReviewGrounding`; the served-body cross-boundary decode lives in the server package. Long-form: `backend/internal/server/README.md` § "Review-grounding rung".

    **The key is OPTIONAL, and absence is a FOURTH state — with a THIRD reason since #3348.** A pre-#3161 fishhawkd serves no `merge_gate` key at all, and a GitLab-family report omits it BY DESIGN (branch protection and rulesets are GitHub-only surfaces the backend never reads there, so it renders no verdict rather than a `not_required` it never established — deliberate, not a stale backend; `TestDoctor_GitLabReport_ReemitsForgeAndNoteAndNoMergeGate` pins the re-emitted bytes), so `OnboardingReadinessReport.MergeGate` is a `*OnboardingMergeGate` with `json:"merge_gate,omitempty"` — mirroring the CLI's `*mergeGateReadiness` — and the tool re-emits **no key** rather than a zero-valued object. A value field would decode the missing key into `{"status":"","check":"",…}`: a verdict outside the `required|not_required|unknown` enum that no forge read ever established, which is the same class of over-claim #3161 exists to remove, reappearing on the compatibility path. Absence means *this backend cannot answer* and is NOT the same claim as `unknown` (*the question was asked and could not be settled*), so it is rendered as neither. The invariant — the report never carries a `merge_gate` whose `status` is the empty string — is pinned on the re-emitted wire bytes by `TestDoctor_MergeGateAbsent_PreservesAbsence` (omitted key and explicit `null`), the MCP counterpart to the CLI's `TestDoctorOnboarding_MergeGateAbsent_EmitsNoRung`. **The same pointer discipline applies in the other direction since #3580:** `merge_gate` stays omitted on GitLab and `gitlab_merge_gate` is omitted on GitHub (and against a pre-#3580 fishhawkd), so `OnboardingReadinessReport.GitLabMergeGate` is a `*onboardingGitLabMergeGate` with `json:"gitlab_merge_gate,omitempty"`; `TestDoctor_GitLabReport_ReemitsGitLabMergeGate` pins the re-emitted `"gitlab_merge_gate":{` bytes with no `"merge_gate":` key, `TestDoctor_GitHubReport_NoGitLabMergeGateKey` pins the absence on a github body (a value field turns it red), and `TestDoctor_GitLabReport_ReemitsForgeAndNoteAndNoMergeGate`'s omission check is on the QUOTED key `"merge_gate":` so the sibling key cannot trip it.

  - `runner_credentials` — `{status, variable, forge?, scope, note?, remediation?}` (E45.82 / [#3617](https://github.com/kuhlman-labs/fishhawk/issues/3617)): the **first LOCALLY-computed doctor rung**. Every other rung is a byte-mirror of what fishhawkd served; this one is computed in the MCP process about the MCP process. A gitlab run needs TWO credentials — fishhawkd's `FISHHAWKD_GITLAB_TOKEN` (what `app` covers) and the RUNNER's `FISHHAWK_GITLAB_TOKEN` — and before #3617 nothing reported the second, so a registration that omitted it produced a run that planned and implemented correctly and then failed at the push after a paid agent pass. The daemon genuinely cannot answer it (the runner's spawning environment is not its own), but the MCP server IS that environment on the local channel: `run_stage.go`, `dispatch_stage.go`, `drive_run.go` and `run_children.go` each spawn the runner with `append(os.Environ(), …)`, so a local runner inherits this process's variables verbatim — which is why the rung reports a fact it actually holds rather than "cannot determine from here".

    **It sits BESIDE `report`, never inside it.** `DoctorOutput.RunnerCredentials` is a sibling key (`runner_credentials`) because `report`'s json tags are pinned byte-identical against the backend response; a locally-computed key inside it would be a false claim about provenance. Nothing about the REST surface changes — `docs/api/v0.openapi.yaml` is untouched, because this rung never traverses it.

    `status` is `present` | `missing` | `not_applicable` (a github-family run mints from the App installation broker, with the `gh` CLI fallback — no runner-held env credential applies) | `unknown` (the report named no `forge` family, e.g. an older fishhawkd, so applicability could not be decided — NOT a finding that the credential is missing). It reports **PRESENCE only**: it never reads the value, so an expired, revoked or wrong-scope token still reports `present`; a whitespace-only value reports `missing`, matching the runner's own resolver. The fixed `scope` sentence carries the two bounds on every branch — whose environment was read, and that the result describes the MCP server's spawning environment ONLY (local runner) and says **nothing about a `gitlab_ci` runner**, whose credential is a CI/CD variable on the GitLab instance this process cannot see. (The runner's own #3617 preflight DOES cover both channels; only this rung is channel-blind.) On `missing`, `remediation` names where the variable must live — the environment that LAUNCHES `fishhawk-mcp` (`claude mcp add … -e FISHHAWK_GITLAB_TOKEN=…`, then `/mcp`) — which is the concrete step a failing registration omits. The mirror type `runnerCredentialsRung` is unexported like `onboardingTraceStore` (the export baseline is untouched) and a POINTER, but it is rendered on EVERY branch including `not_applicable`, so the operator sees the question was asked. Pinned by `TestRunnerCredentials_BranchPerStatus` (one case per status, plus whitespace-only, an unrecognised forge and a nil `getenv`, each asserting the marshalled rung never echoes the seeded token), `TestRunnerCredentials_VariableIsTheRunnerLiteral` (the cross-module env-var name), `TestDoctor_RunnerCredentials_PopulatedAndReportUnchanged` (end to end, asserting `report` is byte-unchanged and the key did not leak inside it), `_GitHubStillRendered`, `_UnknownForge`, `TestNewServer_WiresNonNilGetenv` (the production constructor wires `os.Getenv`, so the nil fallback never fires) and `TestDoctorToolDescription_DescribesRunnerCredentials`.

  `repo` is `owner/name` on GitHub or a `namespace/project` path (nested groups allowed) on GitLab; it falls back to `GITHUB_REPOSITORY`, then to GitLab CI's `CI_PROJECT_PATH` (its predefined `path_with_namespace`) when omitted (a fast local fail when none is present, before the HTTP hop). The optional `forge` input (`github` | `gitlab`) is passed through as `&forge=`; omitted, it defaults to `gitlab` ONLY when `CI_PROJECT_PATH` supplied the repo, otherwise the backend resolves the family from its account registry and defaults to `github` — so pass `forge: gitlab` for a GitLab project whose namespace is not registered (a nested path on the github family is a 400 naming that remedy). Pinned by `TestDoctor_ForgePassthrough`, `_RepoFromCIProjectPath`, `_GitHubRepositoryOutranksCIProjectPath`, `_ForgeInputOverridesEnvDefault`. The endpoint gates on **authentication only** (401 anonymous) — scope adequacy is itself a reported field, so a scope-gapped token still gets a report naming its gap rather than a 403. Backend 4xx map onto clean tool errors: `authentication_required` (401, with a `FISHHAWK_API_TOKEN` pointer) and `validation_failed` (400, malformed repo or forge — the backend's message, which names both shapes, is wrapped rather than restated).

- **`fishhawk_init`** generates the starter spec **in-process** via `backend/internal/spec.PresetShapeBytes(preset, shape)` — there is **no HTTP generation endpoint** (spec generation is CLI-local `spec.Generate`), and the `fishhawk-mcp` binary is built from the backend module (ADR-021) so it may import `backend/internal/spec` directly (it already does for spec parsing). Returns `{preset, shape, workflow_yaml, target_path}` where `preset` is `low` / `medium` / `high` (default `medium`), `shape` is `app` / `config-only` (default `app`, chosen explicitly and echoed in the output — E45.67; `config-only` seeds a repository with no test entrypoint, omitting `verify`, dropping `tests_added_or_updated`, keeping `ci_green` and raising `max_files_changed`), `workflow_yaml` is the canonical workflow-v2 preset bytes, and `target_path` is `.fishhawk/workflows.yaml`. An unknown preset or shape fails closed with a clean error naming the valid values.

  **Preset-only scoping:** `fishhawk_init` returns the scaffold bytes for the conversational agent to **write** — it writes no file itself. The delta options (`--budget-usd` / `--single-reviewer` / `--human-gates`) and the AGENTS.md/CLAUDE.md bridge (E29.2) the CLI `fishhawk init` performs are a **follow-up**, because the delta-applying `Generate` lives only in `cli/internal/spec` and porting it into the backend module is beyond a thin tool. Since #3579 the output also carries `next_step`: a fixed sentence pointing at `fishhawk_validate` as the pre-commit check and stating that `fishhawk_doctor`'s spec rung reads the DEFAULT BRANCH, so it cannot confirm the uncommitted file (`TestInit_NextStepNamesValidate`).

- **`fishhawk_validate`** (E45.65 / [#3579](https://github.com/kuhlman-labs/fishhawk/issues/3579)) is the **pre-commit spec check** an MCP-driven onboarding was missing: before it, an agent that wrote `fishhawk_init`'s scaffold could only learn the spec was bad after commit → push → merge, because `fishhawk_doctor`'s `spec` rung reads the default branch, and the skill's Step 3 told it to loop on the doctor anyway. It runs `backend/internal/spec.ParseBytes` **in-process** — no HTTP call — which is the SAME validator `fishhawk_start_run`'s local pre-parse (`specValidate`, `spec_discover.go`) and `POST /v0/runs` already run, so there is one validator and no second implementation to drift. The issue named `cli/internal/spec`; that module is not importable from the backend module the `fishhawk-mcp` binary is built from (ADR-021, dependency direction `cli → backend`), and the backend parser is a SUPERSET of the CLI's local sweeps (version-routed schema v0/v1/v2, v2 removed-forms sweep, v2 `defaults`/`extends` reuse resolution, stage-reference resolution, plus the server-side stage-binding rules the CLI defers to run creation).
  - **Input precedence** mirrors `fishhawk_start_run`: `workflow_spec` inline bytes win (`source: inline`); else `discoverSpec(working_dir, spec_file)` — the `.git`-bounded `.fishhawk/workflows.yaml` walk from `working_dir` (default `.`), or the explicit `spec_file` (`source: file`, `path` set). No input at all, a missing `spec_file`, or a `working_dir` holding no spec are **tool errors** — the verb could not read anything to validate (`TestValidateSpec_NoInput_FailsClosed`, `_SpecFile_Missing_FailsClosed`, `_WorkingDir_NoSpec_FailsClosed`; precedence pinned by `_InlineWinsOverWorkingDir` with an INVALID file in the dir).
  - **A validation failure is a RESULT, not an error:** `{valid: false, diagnostics: [{kind, path, workflow, stage_index, stage, message}]}` with a nil tool error. `kind` is `yaml` (`*spec.YAMLError`, no pointer) / `schema` (`*spec.SchemaError`) / `validation` (`*spec.ValidationError`) / `other`. `workflow`, `stage_index` and `stage` are derived from the pointer so the agent gets the failing workflow AND stage without re-parsing prose: `workflow` is the `/workflows/<wf>` segment, `stage_index` the integer after `/stages/`, and `stage` the `id` at `workflows.<wf>.stages[<idx>]` read back from the INPUT bytes with a minimal permissive `yaml.v3` decode (no raw-document plumbing beyond the bytes the verb was given). `stage` is empty when the pointer has no stages segment, when the id cannot be read, or when the workflow declares `extends` (the index then addresses the MERGED stage list the reuse pass produced, not the author's own). One index-less shape is still resolved: the v2 reuse pass rejects a duplicate stage id at `/workflows/<wf>/stages` (no index), so `stage` is recovered from the rejection's own `duplicate stage id "<id>"` message — the prefix is derived from the exported `spec.MsgFmtDuplicateStageID`, so a reworded rule fails `TestValidateSpec_Inline_ValidationError_DuplicateStageID` rather than silently emptying the field. `TestValidateSpec_Inline_ValidationError_BadNeeds` pins the indexed shape (`needs: [nonexistent]` → `stage: implement`).
  - **`ParseBytes` returns the FIRST error only**, so `diagnostics` carries at most one entry per call; the CLI's multi-entry `ValidationError.Errors` has no backend counterpart. Stated on the wire (the field's schema description) and in the skill rather than papered over.
  - **`hint`** carries `annotateStaleSpecError`'s stale-binary text when the failure is the unsupported-`/version` `SchemaError` — the one shape a stale `fishhawk-mcp` uniquely causes (#1422) — so the agent is told to `/mcp` reconnect instead of "fixing" a valid version (`TestValidateSpec_UnsupportedVersion_CarriesStaleHint`).
  - **On success:** `{valid: true, version, charter_required_by[]}` — the sorted workflow keys for which the pure `spec.WorkflowRequiresCharter` is true (`TestValidateSpec_CharterRequiredBy`). The spec bytes are never echoed back, so no bound ladder is needed.
  - **What it does NOT check, stated on every response (`checked` / `not_checked[]`):** (1) the **mandatory-charter rule** for `grooming_report`-producing workflows — it needs `workmgmt`, which the ADR-064 `TestNoBoardReadOnMCPToolSurface` invariant forbids this package from reaching, so the verb reports WHICH workflows require a charter and the rule stays enforced by the CLI `fishhawk validate` and at run creation; (2) **reviewer model ids** (E45.64 / #3578) — a deployment-oracle question, surfaced by `fishhawk_doctor`'s `reviewers[].model_status` post-merge; (3) **deployment wiring** (App, providers, scopes, merge gate) — `fishhawk_doctor`. `TestValidateToolDescription_StatesResiduals` pins the description's statement of all three plus the default-branch limitation.
  - Scope table: `fishhawk_validate` is `mcpScopeAuthenticatedOnly` in `backend/internal/server/mcpscopes.go` — it makes no HTTP call, so authentication is the whole gate; `TestMCPToolScopeTable_CoversRegistry` fails on a registered tool with no entry.

Wiring: the `OnboardingReadinessReport` wire mirror (+ nested `OnboardingApp`/`OnboardingSpec`/`OnboardingReviewer`/`OnboardingScopes`)
lives in `client.go` — all scalar/string/slice fields, so it is #371-safe. All three tools register in `tools.go` (the
tool-count guard went to 39 for doctor/init and 55 for validate); tests live in `onboard_test.go` and
`validate_spec_test.go` (the low `client_test.go` stem-sibling needs no new coverage). All three tool descriptions
point at the `fishhawk://onboarding-skill` resource so a connecting agent discovers the full walk without operator
memory. The conversational-entry SKILL.md seed ships as that embedded resource (E29.6 /
...
The durable `(run_id, stage_id)` handle is the unit of waiting on a stage's execution. `fishhawk_get_run_status` carries `plan_stage_wait_status` + `implement_stage_wait_status` — each a `StageWaitStatus` whose `status` is one of `pending`/`running`/`succeeded`/`failed`/`cancelled`/`superseded`, derived from the stage row (distinct from the `*_review_status` pair, which tracks a stage's **review** rather than its execution).

### Two stage vocabularies, one documented mapping ([#2494](https://github.com/kuhlman-labs/fishhawk/issues/2494))

The six-value `stage_wait_status.status` is a coarser **BUCKET** than the raw stage `state` the REST API reports — the fourteen `run.StageState` values (`GET /v0/runs/{run_id}` `stages[].state`, `docs/api/v0.openapi.yaml`). They are deliberately different — the bucket answers *"should I keep polling?"*, the raw state answers *"where exactly is this stage?"* — and neither is renamed, because renaming either is a breaking wire change for every existing consumer. The mapping is **total** and is written down once, in `stage_wait.go`'s `StageWaitStatus` doc comment:
...
- **The dynamic detail is UNTRUSTED and handled as such.** `source_failure_reason` and `source_failure_category` are lifted out of an audit payload whose text was produced while an agent ran commands against an untrusted repository, and this marker promotes them from an explicitly-requested audit view into ROUTINE `await_stage` / `get_run_status` results — so provenance cannot be established for them and they are handled as untrusted instead. `latestFixupRecovery` is the single choke point: both fields go through `neutralizeUntrustedFailureText` (control characters — including newlines — collapse to spaces; runs of 3+ of ANY structural rune — `<`, `>`, the backtick and `~` — are RUN-SPLIT into chunks of two, so neither a live `<<<`/`>>>` delimiter nor a live triple-backtick / `~~~` fence can be emitted at ANY run length) before they are capped, so the structured JSON fields and the prose built from them are both defanged. Every WORD survives — the text exists to be diagnosed from — but the shipped value is therefore **STRUCTURE-NEUTRALIZED and bounded, NOT a verbatim transcript** of the audit entry it names. Both schema descriptions and the message preamble say so plainly and point at the `stage_fixup_recovered` audit entry, readable with `fishhawk_list_audit`, as the source of the exact unaltered bytes: a field whose name promises the audit's value while its content quietly differs is the same surface-says-one-thing defect class this marker exists to close, so the disclosure is pinned by `TestFixupRecoverySchemaDisclosesNeutralization` rather than left to prose. The message then reproduces the detail inside a labelled `<<<BEGIN UNTRUSTED FIX-UP FAILURE TEXT>>>` / `<<<END …>>>` quarantine envelope preceded by a preamble naming its provenance and the handling rule, mirroring the untrusted-issue-body / issue-comment / acceptance-failure envelopes `backend/internal/prompt` already uses (ADR-029 / [#650](https://github.com/kuhlman-labs/fishhawk/issues/650)). ONE hardening primitive, applied uniformly to all four runes: the run-splitting is deliberately not a pairwise `ReplaceAll`, which is non-overlapping and left-to-right — `ReplaceAll("<<<", "<< <")` leaves `>>>>` as a live `> >>>`, and a pairwise fence-breaker likewise left a run of FIVE backticks still carrying a live triple-backtick fence, because the single backtick it emitted rejoined the two it had not looked at ([#3081](https://github.com/kuhlman-labs/fishhawk/issues/3081) fix-up). Keeping a second, weaker technique in the same function is how that hole survived review, so the fences get the same run-splitter the delimiters do, and `TestNeutralizeUntrustedFailureText_ExhaustiveDelimiterRuns` is the property pin over all four runes at every run length 1..12. `restored_state` and `restored_review_stage_id` are backend-authored enum/UUID values and are passed through unchanged.
- **Delivered-nothing round closure (#3395).** A fix-up trigger writes no `implement_review_started` entry — the round it opens is purely the floor `reviewStatusFor` applies at the trigger's sequence (#894) — so a pass that died having pushed nothing used to leave the prior round's verdicts below that floor and `implement_review_status` `pending` with `reviews:[]` forever, while `fishhawk_reconcile_reviews` correctly found no server-side round to synthesize. The backend's recovery now stamps `delivered_nothing` (no `fixup_pushed` after the trigger — the same veto as the refund), `reopened_concern_ids` and `concern_reopen_error` on the `stage_fixup_recovered` entry, and `latestImplementFixupSeq` derives the floor from the latest LIVE trigger via the pure `liveFixupFloor`: a trigger followed — before the next trigger, same stage — by a recovery carrying `delivered_nothing:true` is VOID and no longer floors, so the prior round's verdicts resurface, the status reads `complete`, and `await_review` returns immediately. The recovery read is BEST-EFFORT (a failure falls back to the un-voided floor — `pending`, the pre-#3395 answer, never a false `complete` — pinned by `TestReviewStatusFor_Implement_RecoveredReadErrorFallsBackToPending`; the trigger read stays fail-closed). A legacy recovery entry WITHOUT the key never voids (pre-#3395 runs keep pre-#3395 behaviour), and `delivered_nothing:false` (the pass pushed before it died) keeps its trigger as the floor because the pushed head's re-review governs. The marker carries the three fields and `fixupRoundClosureSentence` renders them as THREE DISTINCT states: N concerns re-opened / none needed re-opening; re-opening could NOT be completed (naming `concern_reopen_error`, so an empty `reopened_concern_ids` is never read as a clean ledger); the pass pushed before dying — and on that last state the advisory's OPENING and its `git log` confirmation switch with it ("FAILED after pushing a commit" / "the fix-up commit is present"), never the delivered-nothing "pushed no commit" / "the fix-up commit is absent" sentences that contradicted the closure (`TestFixupRecoveryMessage_PushedOutcomeCompleteAdvisory` pins the whole string). `concern_reopen_error` carries store error text and takes the same neutralize-then-cap path as the reason. Pinned by `TestLiveFixupFloor`, `TestReviewStatusFor_Implement_CompleteAfterDeliveredNothingRecovery` (+ the legacy-stays-pending pin), `TestAwaitReview_ResolvesAfterDeliveredNothingRecovery`, `TestLatestFixupRecovery_RoundClosureFields`, and end to end through the real binary in `integration/mcp/fixup_test.go`. `fishhawk_reconcile_reviews` now also reports `landed_before` on every skip (the server counts before its boot gate, so `round_already_settled` wins over `review_dispatched_by_this_process`).
- **The marker and its message are PAIRED.** `decorateSettledWithFixupRecovery` attaches the structured block and the top-level `message` together or not at all — prose without a block would be a warning a caller cannot act on programmatically. Its nil-block arm is unreachable today (`awaitStageSettledOutput` always populates `StageWaitStatus` from `classifyStageWaitStatus`, which never returns nil); it is a pure function so that rule stays falsifiable against a future refactor.
...
- **Budget: the marker STATES the current rule, it does not change it.** The message says a fix-up pass that delivered **nothing** to the PR branch is refunded against the normal budget — whether it died **category-A** (harness, [#3085](https://github.com/kuhlman-labs/fishhawk/issues/3085)) or **category-C** (infrastructure, [#1957](https://github.com/kuhlman-labs/fishhawk/issues/1957)), or produced no commit at all ([#967](https://github.com/kuhlman-labs/fishhawk/issues/967)) — while a **category-B** (policy) failure still **CONSUMES** a pass, as does any pass that **pushed** a commit before it died. Whether a failed pass SHOULD consume budget is a separate tracked question; nothing here alters the accounting.
...
`Run.CompletionBlocked` mirrors the backend `runResponse.completion_blocked` block (`backend/internal/server/runs.go`'s `runCompletionBlockedPayload`; the json tags MUST byte-match or the mirror decodes to nil — the #371-class hand-maintained-wire-mirror trap). The backend emits it on the SINGLE-run read only, so it is populated when the `Run` came from `GetRun` and nil otherwise; an older backend omits it entirely.

It names the lowest-sequence non-terminal stage holding a `running` run open against `Orchestrator.completeRun`'s #968 guard, and — load bearing — **DISCRIMINATES the recovery** rather than always pointing at a verb:

- `recovery: "reconcile-merge"` — `POST /v0/runs/{run_id}/reconcile-merge` will supersede this stage and complete the run. Emitted ONLY when the blocker's `(stage_type, state)` is a row of the default-deny merge-supersede pair table AND the run's PR is observably merged, which are exactly the endpoint's own preconditions.
- `recovery: "record-merge-observation"` — the run's PR merge has NOT been observed yet, so `POST /v0/runs/{run_id}/record-merge-observation` records it off the forge FIRST, after which recovery flips to `reconcile-merge` (E64.40 / [#3151](https://github.com/kuhlman-labs/fishhawk/issues/3151)). Emitted ONLY when the blocker is admissible, the run carries a PR URL that resolves under its forge family, and the chain carries NO merge evidence. It mirrors the observe verb's CHAIN-LOCAL preconditions only — the surface performs no forge read, so the named verb can still refuse with `pr_not_merged` if the PR is not actually merged; that residual is strictly better than `none`, which names no verb for a shape where one applies.
- `recovery: "none"` — no verb applies; `reason` names the state and says what the stage needs instead (`running`, `dispatched`, `awaiting_children`, the deploy park states, `awaiting_scope_decision`, and an admissible park on a run whose PR has NOT merged and carries no resolvable PR URL, or whose merge-evidence read failed).

A consumer must branch on `recovery`. Pointing an operator at a verb guaranteed to refuse is the defect #3083 reports against `merge_run`, so an unconditional "try reconcile-merge" hint would reproduce it.
...
- **Synchronous-with-progress `fishhawk_run_stage` (negotiated fallback).** The synchronous call runs the stage to completion and returns the terminal outcome (also surfacing `stage_wait_status` on the handle — normally already terminal, so the interval is omitted). It is the fallback for clients that prefer to block or for short stages; it is not the primary mechanism. Its run-terminal result also carries the `next_actions` block (#1024) so the operator gets the legal next move directly — see [Server-suggested next actions](#server-suggested-next-actions-next_actions-1024). That block's **acceptance** signals are derived from a **50-entry** best-effort recent-audit window (`runStageAuditWindow`, deliberately `auditLimitMax` — the same ceiling `fishhawk_get_run_status` clamps to), which costs **no extra round-trip**: it is the SAME single audit fetch the `audit_pointer` already made, widened from `limit=1` (E31.15 / [#1562](https://github.com/kuhlman-labs/fishhawk/issues/1562)).
...
- **Terminal wait `fishhawk_await_stage` ([#2491](https://github.com/kuhlman-labs/fishhawk/issues/2491)).** The single terminal wait for the durable handle `fishhawk_dispatch_stage` returns — dispatch previously handed back a `log_path` and nothing to await, forcing a hand-rolled poll loop. It **reuses the [#1252](https://github.com/kuhlman-labs/fishhawk/issues/1252) `?wait` long-poll** (`GET /v0/runs/{run_id}/stages/{stage_id}?wait=<n>` → the `{state, terminal, failure_*}` settledness envelope) as a new api-client read (`apiClient.GetRunStageWait` decoding a purpose-built `RunStageWait` projection — NOT an embed of `Stage`, which would shadow-collide on the `state` json tag and hide the top-level `terminal`), so it adds **no** backend, orchestration, or runner surface. Contract:
  ...
  - **The `timeout` status is discriminated ([#3626](https://github.com/kuhlman-labs/fishhawk/issues/3626)).** `timeout` is the vocabulary of failure, but the only deadline this verb can ever hit is the **CALLER's wait cap**: a stage that blows its OWN per-attempt agent deadline is **killed by the runner** and arrives here as `settled` / `state: failed`, so `timeout` was never that event. The `status` token is deliberately **unchanged** — it is what callers branch on, and minting a new value would break a strict consumer to repair a reporting defect — so the distinction rides on an additive `timeout_kind` field, alongside the raw `state` and the classified `stage_wait_status` block the timeout response previously dropped. Two kinds:
    - `client_wait_cap` — your cap expired. That is **all** it asserts: it is **not** a health claim. The message states the stage is within its deadline **only** when the health probe actually read a positive `deadline_seconds_remaining`, and names the seconds it observed; when the probe failed or `agent_timeout_seconds` is unresolved the message says the stage's **OWN deadline status is UNKNOWN** — a failed read can neither confirm nor rule out that it too expired, so the message carries no "not the stage's own deadline" denial — and routes to `fishhawk_get_run_status` rather than implying a healthy stage it never read.
    - `stage_deadline_exceeded` — the probe read `deadline_seconds_remaining: 0` while the stage is **still unsettled**. Re-arming may just burn a second cap, so the message routes to `fishhawk_get_run_status` and `fishhawk_reap_stage` instead of offering the resumable re-call. It is deliberately **not** proof of a stuck runner — it can fire legitimately in the seconds between the budget reaching 0 and the runner killing the stage.
    - **Probe cost and direction.** `awaitStageTimeoutHealth` reuses the EXISTING `ListRunStages` + `GetRun` api-client reads and the EXISTING `stageWaitStatusFor` classifier (so the #2540 budget derivation and the #3335 per-attempt dispatch clock are inherited verbatim; no new REST surface, no `stage_wait.go` signature change). It is paid **once, at timeout** — two short GETs per armed wait, i.e. once per 600s or 7200s window — and **never per poll tick**. It runs on the **PARENT** ctx, not the expired `pollCtx`, or it would fail by construction on every timeout and silently reduce `timeout_kind` to a constant. It is **best-effort** in the same direction as the ADR-036 backstop and the #2588 amendment probe: any read error, a nil run, or a run carrying no stage of the awaited type yields the unclassified `client_wait_cap` response with no block and **no error** — losing a classification beats killing a wait that may have been held for two hours.
...
**Fail-closed on an unreadable run over HTTP (Condition 1 / #2482):** the sibling pre-spawn guards in these handlers (`guardHostDispatch`, `guardNoPRImplement`, the self-host advisory) fail OPEN on a `GetRun` error; this one must not — and since E45.46 / #3463 the forge-target read (`resolveRunForgeTarget`, "GitLab runs" below) fails closed on EVERY transport for the same reason, a wrong guess there being a cross-forge push. Over the HTTP transport a run-read failure means the binding is UNKNOWN — the property the conflict/inheritance checks depend on — so the resolver **refuses regardless of whether an explicit value was supplied** (an explicit absolute path is self-validating, but its non-conflict is not, and non-conflict is the property the criterion demands). On stdio the same read failure degrades to today's behavior (explicit value used verbatim; omitted value resolves to the process cwd).
...
## GitLab runs ([E45.46 / #3463](https://github.com/kuhlman-labs/fishhawk/issues/3463))

The MCP surface can create AND drive a GitLab-project run with a LOCAL runner (`runner_kind: local`). The DECISION #3463 records: a GitLab project MAY be driven by a local runner; the `gitlab_ci`-locked refusal in `guardHostDispatch` (a host dispatch against a run LOCKED to `runner_kind=gitlab_ci`) is correct and unchanged. Three locks were opened, each on this package's side of the wire (the backend half — the `POST /v0/runs` forge ladder, the `installation_ref` stamp and the `forge` / `forge_base_url` response fields — is `backend/internal/server/README.md` § "Forge resolution on POST /v0/runs").

**`fishhawk_start_run` gains a `forge` input (`github` | `gitlab`), and `repo` accepts the GitLab `path_with_namespace` (nested groups allowed).** The value is validated client-side (anything else is a tool error naming the field, no backend round-trip) and forwarded verbatim as the request's `forge`. The LADDER DIFFERENCE against REST is the no-github-fetch guarantee: REST derives the forge from the installation registry when `forge` is omitted, while this tool PINS `forge: github` on the create request whenever it fetched the issue from github.com via `gh` — i.e. `issue` given, no inline `issue_context`, `forge` omitted. The pin lands on the fetch ATTEMPT, not on `gh` success, so an absent `gh` binary or a transient `gh` error cannot make two otherwise-identical calls mint runs of different forges (the existing fetch warning already covers the degraded prompt). `forge: gitlab` with `issue` SKIPS `gh` entirely (a github.com fetch cannot serve a GitLab issue) and the tool result carries a warning naming `issue_context` as the supported path. `forge` omitted with NO github.com fetch — no `issue`, or `issue_context` supplied INLINE — sends NO `forge` key, and the backend's registry ladder derives it (explicit always wins there, so the pin can never be re-derived to gitlab). Pinned by `TestStartRun_ForgePassthrough`, `TestStartRun_InvalidForge_NoRoundTrip`, `TestStartRun_GitLabWithIssue_SkipsGhFetch`, `TestStartRun_OmittedForgeWithIssue_PinsGitHubAndInvokesGh` (both the `gh`-success and `gh`-error subcases pin) and `TestStartRun_OmittedForgeNoIssue_SendsNoForge` (raw-body assertions — the typed decode cannot tell an absent key from an empty string).

**Every runner-spawn producer derives its forge target through ONE helper, `runResolver.resolveRunForgeTarget(ctx, runUUID)` (`run_stage.go`).** `fishhawk_run_stage`, `fishhawk_dispatch_stage`, `fishhawk_drive_run` (once per invocation, BEFORE the loop — the loop's per-iteration `fetchRunDriveView` is the same route but is deliberately NOT used for this: one derivation path) and `fishhawk_run_children` (for the PARENT; children inherit the parent's `installation_ref` / forge server-side via `run.ChildParamsFrom`, so the parent row is the authority for every child argv) all call it, and nothing else derives a forge. It performs exactly ONE single-run `GET /v0/runs/{id}` — NEVER a list-shaped read — because the backend populates `forge_base_url` on the single-run route ONLY, so a list read would make a correctly configured gitlab run hit the refusal below spuriously. The test fixture models that asymmetry (`newFakeBackend`'s list route strips `forge_base_url`), which is what makes the per-producer `*_ForgeTarget_ReadViaSingleRunGet` pins discriminating; for drive_run the same-route shape cannot discriminate, so its pin is a pre-loop `GET` call-count assertion plus a fake that serves `forge_base_url` on pre-loop reads only.

**The helper's rules are fail-closed** — a gitlab run spawned as github would push to api.github.com, so no guess is safe. (1) A run-read error is a REFUSAL (`could not read run … to resolve its forge; not spawning`), never a github default with a warning — note this is the OPPOSITE posture from the sibling `guardHostDispatch` / `guardNoPRImplement` guards, which fail OPEN on the same read, and is why `TestRunStage_ReviewActionHint_StoreUnavailableWhenRunFetchFails` arms its 500 at spawn time rather than before the call. (2) `forge` absent (a pre-#3463 backend) → github, the one mixed-version degrade. (3) `forge: gitlab` with an EMPTY `forge_base_url` → REFUSAL naming BOTH remedies: set `FISHHAWKD_GITLAB_BASE_URL` on fishhawkd, or re-register the installation with `--forge-base-url`. (4) Any other value → REFUSAL naming it. Each refusal lands BEFORE the host-dispatch marker, the drive record-act and the spawn, so it commits no state (`TestRunStage_GitLabRun_NoBaseURL_RefusesBeforeHostDispatchMarker`, `TestDispatchStage_GitLabRun_NoBaseURL_RefusesBeforeMarker`, `TestRunChildren_GitLabParent_NoBaseURL_RefusesBeforeAnyMarker`, and drive_run's `stopped_reason: forge_unresolved` with zero spawns / markers / acts in `TestDriveRun_GitLabRun_NoBaseURL_StopsWithoutSpawn`).

**Argv:** `composeRunnerArgv` and the inline child argv in `run_children.go` append `--forge gitlab --gitlab-base-url <url>` right after `--github-repo` for a gitlab target (`runForgeTarget.argv()`, one renderer for both producers) and NOTHING for github — the github argv is byte-identical to its pre-#3463 shape (`TestComposeRunnerArgv_GitHubTargetUnchanged`, `TestRunChildren_GitHubParent_ArgvUnchanged`). The runner's `--github-repo` flag is reused as the GitLab project slug (the runner splits it on the first `/` and round-trips a nested group path intact), so `github_repo` on the four verbs now reads "repo slug (owner/name, or the GitLab path_with_namespace)"; on a gitlab run an OMITTED `github_repo` defaults to the run row's repo BEFORE the origin auto-detect, which is github.com-only and would refuse a GitLab origin (`TestRunStage_GitLabRun_DefaultsRepoFromRunRow_SkipsOriginDetect` and its dispatch / drive / children siblings).

**Credentials on the MCP host:** the runner reads `FISHHAWK_GITLAB_TOKEN` from its own environment (`runner/cmd/fishhawk-runner/main.go`, `resolveGitLabPushToken`), and the spawn passes `os.Environ()` through, so the operator must export it on the host running `fishhawk-mcp`. Since E45.82 / [#3617](https://github.com/kuhlman-labs/fishhawk/issues/3617) a missing token is refused by the runner at PROMPT-FETCH time (`runner_failed`, reason `gitlab_push_credential_missing`, zero agent tokens burned) rather than failing the push after a complete paid agent pass; `fishhawk_doctor`'s `runner_credentials` rung reports its presence in THIS process's environment before the run starts. Operator walkthrough: `docs/deploy/gitlab.md` § "Driving a GitLab project from MCP / CLI with a local runner".

### `fishhawk_start_run` over-cap issue-comment warning ([#2946](https://github.com/kuhlman-labs/fishhawk/issues/2946))

`fishhawk_start_run` already carries a best-effort `warnings` channel on its tool result (the `gh CLI not on PATH` / issue-fetch-failed advisories). It is extended (`overCapCommentWarnings`) to scan the **RESOLVED** `issue_context` — whether fetched via `gh` or supplied inline by the caller, so the scan runs on what the run will actually carry, not on the fetch — and warn when any comment body exceeds `prompt.MaxIssueCommentBytes` (**2000 bytes**). The warning states the true total count, then names the first `maxOverCapDetailLines` (**5**) over-cap comments individually (author, `created_at`, byte count, overflow) and summarises the remainder as a count — bounded so a pathological thread of many over-cap comments cannot dominate the tool result. Contract is "name the first five, summarise the rest", not "name each": an operator staring at a truncated result learns nothing from the 200th line, and the summary count still gives the true total. Each named comment carries the two remedies: **split the comment** into several, or **move the durable content into the issue BODY**, which is not per-comment capped. Nothing is added when no comment is over cap, and the new entries are ADDITIVE — the pre-existing `gh`-absent warning still surfaces on the same result. This is the MCP-path visibility surface for the per-comment truncation the renderer marks with an elision marker (`backend/internal/prompt` `writeIssueComments`); run paths with no tool result (webhook, campaign) instead get a greppable `fillIssueContext` server-side WARN. No new tool is added — the tool count and registration are unchanged.
...
- `dangling_state_unreadable` (#2953) — the target's state could not be read: the message says assembly refused rather than assume satisfaction, and to retry. **This clause now NAMES its edges (#2956)** — where it previously named none — via `formatDanglingEdges`, so a cross-repo/unparseable token reaches the operator as `issue:2032->unparsable:<digest>:"other/repo#12"`. The cross-repo explanatory sentence (`an unparsable:<digest>: target is a depends_on token that is not a same-repo issue number …`) is appended ONLY when at least one rendered edge actually carries the `unparsable:` prefix (`danglingEdgesHaveUnparsable`); a transient forge-read failure on a NUMERIC target renders `issue:N` and gets the base retry clause with no misleading cross-repo label.
...
The backend classifies a non-empty `epic_ref` with `workmgmt.ClassifyEpicRef`. `startCampaign` maps the two new `422` codes — but they are refused at DIFFERENT points, because the group-epic remedy has to be capability-aware:

- **`campaign_epic_ref_invalid`** — a cross-repo (`owner/name#N`) or unrecognized `epic_ref`. Refused at the request edge, before any forge round-trip (a same-repo epic ref is a caller-input fault regardless of provider). The epic is resolved in the campaign's OWN repo, so the remedy names the three accepted same-repo forms (`25` / `#25` / `issue:25`). This REPLACES the old `502 epic_children_query_failed` an unparseable `epic_ref` used to draw (a transport code for caller input).
- **`campaign_epic_ref_group_unsupported`** — a GitLab group-epic ref (`group&5`, or a `/-/epics/N` URL). Refused AFTER work-item provider resolution (not at the request edge — conventions load is a forge read), so the remedy can name only sources the resolved provider serves. Group epics are a Premium group-level object v0 does not model. The remedy is **capability-aware**: it recommends `items` mode ONLY when the resolved provider serves it (the backend passes `campaign_sources_supported`), and otherwise points at driving each issue standalone — never at a mode that also `501`s.
...
**What used to happen.** `CreateCampaign` ran on the shared 30s short `http.Client`, and the server had **no deadline on this path at all** (`server.go` sets `ReadHeaderTimeout` only — no `WriteTimeout`, no `TimeoutHandler`). So the **client** was what gave up, at 29999ms, with a bare transport error carrying no counts. #3113's original Summary diagnosed a *server-side* 30s deadline on issue-set resolution; **that diagnosis was wrong**.

**What happens now.** Two changes, and the ORDER between them is the contract:

- The server bounds its own no-epic resolution and refuses `504 issue_set_resolution_timeout` with `resolved`, `items_total`, `budget_seconds` and — only when one could be **proven** to fit — `suggested_grooming_order_limit`. (All four are on the 5xx detail allow-list; a 5xx is default-deny redacted, so without that the numbered refusal would ship empty.)
- `CreateCampaign` routes through its **own** client, `httpIssueSet`, whose `issueSetClientTimeout` (11m) sits **above `server.MaxIssueSetResolutionBudget`** (10m) — so the client can never be what gives up first, and the operator sees the server's numbered refusal instead of a transport error.

**Why the relationship holds by construction, not by default.** The ceiling is enforced where the value is **consumed**: `Server.issueSetResolutionBudget()` clamps to `MaxIssueSetResolutionBudget` however the `Config` was built, so no construction path — a test, an embedded use, a second binary — can raise the effective budget past the number the client sits above. `fishhawkd`'s startup refusal on `--issue-set-resolution-budget` is a *second* line, not the guarantee.

**Why the one-minute margin is enough — an ARGUED margin, not a "same span" guarantee.** The client's timeout measures the ENTIRE request; a budget starting at the resolver call would leave the handler's pre-resolution work outside the measured span and unbounded. `handleCreateCampaign` therefore anchors its deadline at **handler entry** (`requestStart`), which folds that pre-resolution work into the bounded span. It is **not** literally the same span: the server bounds only `[handler entry .. resolver return]`, while the client's 11-minute wall also covers **(a)** network transit both directions, **(b)** the middleware chain *ahead* of the handler (request-id, bearer auth — `requireWriteScope` reads an Identity the auth middleware already resolved, so the token lookup is before `requestStart`), and **(c)** all post-resolution handler work (campaign + item persistence, idempotency record, response encode + write). The one-minute margin absorbs (a)+(b)+(c); it is adequate for (c) because that is local DB writes on an already-open pool plus a small JSON encode, not per-item forge round-trips, so it does not scale with issue count. No server-side deadline can bound client-side transit, so this is bounded by inspection, not by construction — and the handler is deliberately not wrapped in a deadline, since aborting a resolution that already succeeded would discard up to ten minutes of completed forge reads to report a timeout for finished work.

**The rendered refusal** (`issueSetTimeoutRemedy`) names how far the server got and branches on whether a fitting value was proven: with a suggestion it names `grooming_order_limit=N`; **without** one it says so and offers bisection rather than inventing a number — in particular it never advises requesting zero items. Detail values arrive as JSON numbers, which decode into `map[string]any` as `float64` and not `int`, so every read goes through `detailInt` and a missing or wrong-typed value degrades to the generic remedy instead of rendering a bogus zero.
...
### Runner self-host bootstrap advisory ([E64.5 / #3086](https://github.com/kuhlman-labs/fishhawk/issues/3086))

Both host-spawn verbs — `fishhawk_dispatch_stage` and `fishhawk_run_stage` — carry `guardRunnerSelfHost`, a **purely advisory** guard that warns when a run is fixing `fishhawk-runner` **itself**. Every host dispatch spawns `bin/fishhawk-runner` built from **main** — which still carries the very defect the run is fixing — so a stage that triggers that defect fails **category-A repeatedly** and burns fix-up budget with nothing pushed, leaving the operator staring at four identical failures with no signpost. On a non-`plan` dispatch the guard resolves the run's terminal `standard_v1` plan artifact's declared `scope.files` (self, then a bounded `parent_run_id` walk for CI-retry chains and decomposition children, the same shape `fishhawk_get_plan` uses), and when any declared path can change the built runner binary it appends **exactly one** advisory line naming the offending path, the concrete consequence, and the documented escape in [`runner/README.md`](../../../runner/README.md) (rebuild `bin/fishhawk-runner` from the run branch into a scratch worktree, then re-dispatch).

- **Advisory only, structurally non-blocking.** Unlike the three sibling guards it returns `[]string` with **no error return**, so it can never refuse a dispatch — the no-error signature makes that structural, not conventional. Its warning rides the same accumulator the other guards' fail-open warnings use.
- **Deny-list, defaulting to FIRE.** A `runner/` path fires **unless** its basename is `README.md` or it ends in `_test.go` — neither can change the compiled binary (Go excludes `_test.go` from the non-test build; no package under `runner/` embeds a README). It is deliberately a **deny-list**, not a `*.go` allow-list — the same asymmetry `AGENTS.md` records for `scripts/dev`'s `_path_affects_build`. The fail-safe direction is **over-warning**: a spurious advisory is cosmetic, a suppressed one drops the operator back into the four-identical-failures loop. So embedded assets under `runner/` (e.g. `runner/internal/plan/schemas/*.json`, `//go:embed` inputs that genuinely change the binary) keep firing **on purpose**.
- **Fail-open and silent** on every read error (a stage-list, artifact-list, decode, or parent-walk `GetRun` failure) — it never strands a dispatch. A `plan`-stage dispatch returns immediately with **zero** reads (no plan artifact exists yet).
- **Cost** is **per visited run**, not a flat two reads: each plan-resolution attempt does up to two reads (list stages, list stage artifacts) and a miss adds one `GetRun` to find the parent, so the honest worst-case bound is up to `(2 plan-resolution reads + 1 GetRun)` per visited run, capped at `retryPlanChainDepth` (8) levels — i.e. up to `3 × retryPlanChainDepth` backend reads. A plan resolved on the run itself costs the two reads and stops. Host dispatch is a low-frequency operator action, not a hot path.
- **The advisory reaches an operator ONLY through these two MCP verbs.** A direct `bin/fishhawk-runner` invocation, or a stage spawned by the auto-drive loop's own path, bypasses both verbs and sees nothing — which is why `runner/README.md`, `AGENTS.md`, and `docs/ARCHITECTURE.md` carry the written deadlock explanation as well as this runtime line.
- **Two known residuals.** (1) The warning fires on the **DECLARED** plan scope, so a `runner/` path added later by an **approved scope amendment** does **not** trigger it — this is the gap most likely to surprise an operator whose run grew into the runner via amendment. (2) The parent walk is capped at `retryPlanChainDepth`; a chain deeper than that resolves no plan and the guard stays silent. Both are accepted — this is a fail-open advisory, not a correctness gate.
...
When admission returns `short_circuited:false` **and** the approved plan needs LIVE validation (no short-circuit predicate matched) **and** the acceptance stage's spec declares egress `target_hosts`, the response carries `needs_target:true` + `target_hosts` (verbatim spec hosts) + `expected_head_sha` (the resolved merge-candidate head SHA; empty only when the backend could not resolve one, which the gate below refuses on). On such a result each host-dispatch verb runs a **verb-side target-identity gate** BEFORE any spawn evidence: it probes the first declared target host **from the dispatch host** (the same network position the local runner would probe from) — a direct-dial `GET <scheme>://<host>/healthz` (`Proxy:nil`, so an ambient operator proxy can't fake reachability; http-first for loopback/IP-literal hosts, https-first otherwise) whose `git_sha` is classified against `expected_head_sha` with the **runner's semantics** (`unreachable < unverifiable < stale < verified`; a `-dirty` suffix is stale/fail-closed, a `<7`-char sha is unverifiable, a `>=7`-char prefix match is verified). This intentionally **mirrors** the runner's `previewprobe.go` (a separate, non-importable Go module) — the classification table is pinned in `acceptance_target_test.go`.

Outcomes:

- **stale / unreachable** → **refuse to spawn**: `fishhawk_dispatch_stage` / `fishhawk_run_stage` return a structured `needs_target` (`{target_host, expected_head_sha, detail, remediation, preview_action}`, `outcome:"needs_target"` on run_stage) recording **no** spawn evidence and stamping **no** host-dispatch marker; `fishhawk_drive_run` stops with `stopped_reason=acceptance_needs_target` + a `NextActions` pointer, records **no** act and spawns nothing. The stage stays `awaiting_host_dispatch`/`pending`, so re-dispatch is clean once the operator brings up the target at the named head SHA. Since [#3321](https://github.com/kuhlman-labs/fishhawk/issues/3321) the refusal carries a **`preview_action`** — a `run_preview` suggested action whose `params.command` is the **concrete** bring-up to paste, e.g. `scripts/dev preview <expected head sha>`. That command is populated **even with no `FISHHAWK_ACCEPTANCE_PREVIEW_CMD` configured**: the renderer (`acceptancePreviewCommandOrDefault`) owns the built-in default, so the most common configuration never renders an empty string. A configured `FISHHAWK_ACCEPTANCE_PREVIEW_CMD` **overrides** that default everywhere it is rendered. The head SHA is appended only when it passes a strict `^[0-9a-fA-F]{7,64}$` shape check — a malformed backend- or agent-supplied value degrades the **SHA**, never the command. The empty-head refusal below deliberately carries **no** `preview_action`.
...
- **empty `expected_head_sha`** (older backend / ledger resolution failure) → **refuse to spawn** with the same structured `needs_target` shape, `expected_head_sha:""`, and a `detail` naming `acceptance_expected_head_unresolved` ([#3091](https://github.com/kuhlman-labs/fishhawk/issues/3091)). This branch is checked **FIRST**, ahead of the `FISHHAWK_ACCEPTANCE_PREVIEW_CMD` proceed path below: a provision command cannot provision an *unknown* head, so letting it outrank the refusal would reproduce the same unfalsifiable-pass hazard. It replaces the earlier proceed-with-warning, which reported success while unable to fail for the reason it named — the acceptance stage then validated whatever answered at the shared slot and recorded a verdict bound to no tree. The runner gate refuses with the byte-identical reason string (runner parity preserved, in the fail-closed direction).
- **a resolved provision command** (and a resolved expected head) → proceed with an informational note naming the command and its source, **without probing**: the spawned local runner carries it and provisions the target itself ([#1569](https://github.com/kuhlman-labs/fishhawk/issues/1569)). The command comes from `resolveAcceptancePreviewCmd`, which is the **gate decision** and deliberately does **not** default: `FISHHAWK_ACCEPTANCE_PREVIEW_CMD` in the verb env (source `env`, the operator's value always wins) → else an `auto_preview:true` acceptance dispatch (source `auto_preview`, the built-in default) → else empty, meaning *do not take this branch and inject nothing*. Defaulting the gate would make every `needs_target` dispatch proceed while provisioning nothing — which is why the gate decision and the display rendering above are separate functions ([#3321](https://github.com/kuhlman-labs/fishhawk/issues/3321)).
...
#### `auto_preview` on `fishhawk_dispatch_stage` ([E68.43 / #3321](https://github.com/kuhlman-labs/fishhawk/issues/3321))

`fishhawk_dispatch_stage(stage:"acceptance", auto_preview:true)` collapses the provision-then-re-dispatch dance into one call. What it does, precisely:

- It sets `FISHHAWK_ACCEPTANCE_PREVIEW_CMD=scripts/dev preview` **and** `FISHHAWK_ACCEPTANCE_PREVIEW_TEARDOWN_CMD=scripts/dev preview-down` **on the spawned runner's environment**, and the gate takes its existing provision-command proceed branch off the **same** resolved value. The injection and the proceed decision are wired from one `resolveAcceptancePreviewCmd` call, so the gate can never proceed on a command the runner was never handed; the teardown is resolved from that call's result (below), so it can never be injected without the provision it is the counterpart of.
- The **runner** performs the bring-up, through the already-shipped provision → readiness → identity → teardown pipeline (`runner/cmd/fishhawk-runner/previewprobe.go`, `docs/acceptance-preview.md`). The verb does **not** provision inline: `fishhawk_dispatch_stage` returns the durable handle immediately (ADR-037 / #1232), which is what makes in-band mid-stage scope-amendment handling possible, while a preview bring-up on this repo takes minutes.
- An **operator-set** `FISHHAWK_ACCEPTANCE_PREVIEW_CMD` always wins and is passed through unchanged (it is already in `os.Environ()`, so nothing is appended).
- It is **inert on every other stage type** — the `in.Stage == "acceptance"` conjunct — and defaults to `false`, in which case the preview command resolves from the environment exactly as before.
- The teardown hook is resolved by `resolveAcceptancePreviewTeardownCmd(getenv, provisionSource)` ([#3394](https://github.com/kuhlman-labs/fishhawk/issues/3394)), three arms in the provision resolver's shape: an operator-set `FISHHAWK_ACCEPTANCE_PREVIEW_TEARDOWN_CMD` → (that value, `env`), passed through unchanged whatever the provision source; else provision source `auto_preview` → (`scripts/dev preview-down`, `auto_preview`), injected next to the provision default; else empty, inject nothing. **It keys on the provision SOURCE, not on the raw `auto_preview` flag**: an operator-set `FISHHAWK_ACCEPTANCE_PREVIEW_CMD` resolves with source `env` and gets no default teardown even under `auto_preview:true`, because `scripts/dev preview-down` only knows how to take down what `scripts/dev preview` stood up — pairing it with a foreign provision hook would tear down something fishhawk did not provision (`TestDispatchStage_AutoPreviewOperatorProvisionGetsNoDefaultTeardown` pins it; `TestDispatchStage_AutoPreviewInjectsProvisionCmdIntoSpawnEnv` pins that both defaults land together). Before #3394 the verb injected only the provision hook, so every `auto_preview` dispatch drew the runner's `acceptance_preview_teardown_missing` and leaked the preview `fishhawkd` on the target port (observed on #3328).

**Local-only by construction**, recorded rather than assumed. This verb's only spawn is `dispatchSpawnDetached` → `spawnRunnerStageDetached` → `runStageCommand(binary, argv...)` with `cmd.Env = env` — an `os/exec` of the runner binary on the MCP server's **own** host. There is no remote-dispatch branch in the verb (its tool description: *"this tool is local-only by design, ADR-024 Q5"*), and `guardHostDispatch` already refuses, before any spawn, a run locked to a **known** non-host-dispatched kind (`KindHostDispatched` returns `(false, true)` for `github_actions` and `gitlab_ci`). So a `runner_kind` conjunct on the injection would be redundant: *local* is a property of the **spawn path**, not of the run's `runner_kind` field. Residual, stated rather than glossed: `guardHostDispatch` deliberately **allows** an un-resolved run (so #1346's first-dispatch auto-resolve can fire) and an unknown locked kind — but in both cases the spawn is still that local exec, so the injected command still reaches a local runner running from `working_dir`. `TestDispatchStage_AutoPreviewCannotReachNonLocalRunnerKind` is the behavioural alarm if a remote-dispatch branch is ever added here.

`scripts/dev preview` is **fishhawk's own dogfood default** and is meaningless in a non-fishhawk checkout; such a stack must set `FISHHAWK_ACCEPTANCE_PREVIEW_CMD`, which overrides the default everywhere.

#### The `run_preview` next_actions fold ([#3321](https://github.com/kuhlman-labs/fishhawk/issues/3321))

`foldAcceptancePreviewAdvisory` inserts the same `run_preview` action into the `next_actions` block **immediately before** the first suggested acceptance `fishhawk_dispatch_stage` / `fishhawk_run_stage`, because the preview is that dispatch's precondition. It is display-only and additive (the `foldLiveValidationAdvisory` idiom): nil-safe, no round-trips, and it never gates a run. The head SHA is read off the **already-fetched** recent-audit slice via `latestReportedHeadSHA` (the backend's `lineageLedgerCategories` set: `pull_request_opened`, `child_pushed`, `fixup_pushed`); a head that has aged out of the bounded window degrades the SHA, never the command.

It is wired at **both** `nextActionsFor` call sites — `tools.go`'s `getRunStatus` and `run_stage.go`'s post-stage snapshot — so the two snapshot surfaces cannot diverge, and **both** are asserted (`TestGetRunStatus_AcceptanceParkNamesDefaultPreviewCommand` and `TestRunStage_AcceptanceParkNamesDefaultPreviewCommand` share one assertion helper). `next_actions` cannot know whether a given run's acceptance stage will actually park at `needs_target` — that needs the acceptance-admission result, which comes from a **mutating** POST a read-only status call must not issue — so the fold is conditioned on the loop already suggesting an acceptance dispatch and its reason is worded conditionally. Its only no-op arms are a nil run/block and an actions list carrying no acceptance dispatch; an **empty command is not a no-op arm**, because the renderer defaults it.
...
### Grooming apply status on the run snapshot (`grooming_apply_status`, [E54.77 / #3232](https://github.com/kuhlman-labs/fishhawk/issues/3232))

`fishhawk_get_run_status` carries a typed `grooming_apply_status` block reporting where the on-approval grooming apply stands. Since #3232 the server hook runs `workmgmt.ApplyGrooming` DETACHED from the approve request (it returns once the capture window is settled), so `fishhawk_approve_plan` no longer waits for the mutations — this block is how the operator sees the apply progress and knows when it is safe to attest the `confirm` gate. It is DERIVED here (`runResolver.groomingApplyStatusFor`) from three audit categories the server writes; the payload keys cross the server→mcpserver boundary with NO shared type, so the fixtures in `tools_test.go` are LOADED from the goldens slice 0 committed at `backend/internal/audit/testdata/grooming_apply/` (one file per category, bytes produced by marshalling the SERVER's real payload structs and pinned there by `TestGroomingApplyGoldens_MatchServerPayloadStructs`), never hand-authored — a key rename on either side reddens `TestGroomingApplyGoldens_CarryTheDecodedKeys` and the status tests, instead of silently zeroing a count.

- **Read order is load-bearing.** (1) ONE page of `grooming_apply_started`; none → the block is omitted and NEITHER ledger category is read (`TestGetRunStatus_GroomingApplyStatus_OmittedWithoutStartedRow` asserts this on the fake's per-category call log), so an ordinary run pays exactly one small read and stays byte-identical. (2) The NEWEST started row by sequence is the anchor, so a re-approval's fresh apply is never mixed with an older one's rows (`_AnchorsOnNewestStartedRow`). (3) `grooming_mutation_applied` and `grooming_apply_completed` are each read with `since_sequence = anchor` and WALKED by `next_cursor` (`walkGroomingApplyAudit`: pages of 500, the server's `auditMaxLimit`, bounded to 10 pages) because a large apply exceeds one page (`_WalksAuditPages`); exhausting the page cap reports the PARTIAL `recorded` count — fail-open, stated on the DTO — never an error or an unbounded walk (`_PageCapBoundsTheWalk`).
- **Fields.** `state` is `in_flight` until a completed row lands after the anchor, then `completed`. `candidate_count` is the started row's denominator; `recorded` is the number of per-entry rows seen; `remaining = candidate_count - recorded` (floored at 0) while in flight, 0 once completed. `applied` / `failed` / `skipped` / `refused` / `budget_exhausted` are TALLIED from the per-entry rows while in flight and TAKEN FROM the completed summary (the writer's own counts) once it lands. `budget_exhausted` counts skipped rows carrying `skip_reason: apply_budget_exhausted` — the never-evaluated tail of an over-budget apply, which the churn guard maps to no baseline disposition so each entry RESURFACES on the next grooming run (no hand-apply). `degrade_reason` is carried verbatim when the completed row is a server-authored degrade (`_CompletedDegradeCarriesReason`).
- **Best-effort.** A read error (`_ReadErrorOmits`, injected on ONE category via the fake's `reviewFlip` hook so every other read succeeds) or an undecodable started payload (`_UndecodablePayloadOmits`) omits the block, never fails the snapshot. Stated residual: a grooming apply that degraded BEFORE launch (provider unresolvable, window unsettled, …) writes no started row, so the block is omitted for it; that degrade is on `recent_audit` / `fishhawk_list_audit` as a `grooming_apply_completed` row with `degraded: true`, as before.
- **`next_actions` advisory.** `foldGroomingApplyAdvisory` PREPENDS a `fishhawk_get_run_status` re-poll action while the block is `in_flight` (reason: `grooming apply in flight: <recorded>/<candidate_count> recorded — re-poll until grooming_apply_status.state is completed, or fishhawk_await_audit on grooming_apply_completed; do not attest the confirm gate on a partial apply`), displacing `approve_review_gate` to second on a parked backlog_grooming run; a completed or omitted block leaves the actions byte-identical. It is a FOLD off the already-derived block at the `getRunStatus` call site (no extra round-trip, no new `classifyNextActions` parameter). Pinned by `TestFoldGroomingApplyAdvisory_{InFlightPrepends,CompletedNoop,NilNoop}` and end to end by `TestGetRunStatus_GroomingApplyStatus_InFlight`.
- **Byte budget.** Classified `T3` / `stored` in `runStatusPathTable` and dropped in the same tier step as `security_findings` / `acceptance_transcript`. Because the block is derived from THREE categories no single pointer returns it, so — like T4's two implement-review entries — the drop emits THREE entries, one per originating category, each with its own anchored `fishhawk_list_audit(category=…, since_sequence=0)` pointer and `omitted_count = recorded`; the one pointer promise is kept per entry. Those three entries ride inside every skeleton measurement and grew the constant skeleton from 12465 to 13446 bytes, so `bound_test.go`'s skeleton probe band moved from 13 KiB to 15 KiB (window now [~13446, ~17511]; `TestElisions_SkeletonBandEngagesSkeletonTier` fails loudly if it moves again).
...
**`review_reject_without_concern` (display-only, E68.41 / [#3319](https://github.com/kuhlman-labs/fishhawk/issues/3319)).** `PlanReview` decodes an additive `reject_without_concern` from the `implement_reviewed` audit payload — the server's VERDICT-LEVEL marker for a `reject` that named NO concern anywhere (no `concerns[]` entry, no `concern_resolutions` note). `foldRejectWithoutConcernAdvisory` appends one advisory action naming the rejecting reviewer model(s) and pointing the operator at the review's `free_form`, because there is nothing to route through `fishhawk_fixup_stage`. Modelled on `foldLiveValidationAdvisory`: nil-safe, `consumes: none`, appended LAST so it never reorders the primary next move or suppresses the unclassified fallback; a run with no flagged review appends nothing and every existing surface stays byte-identical. It is DELIBERATELY the verdict-level question — NOT the per-resolution `planreview.RejectSubstantiatesResolution` the server-side reopen veto asks, which can refuse a resolution on a verdict this advisory correctly stays silent about (see `backend/internal/server/README.md` § "Per-resolution reopen veto"). Additive + `omitempty` means a payload stored before #3319 decodes `false`.
...
- **The concern arm never recommends a fix-up the endpoint would refuse** ([#3116](https://github.com/kuhlman-labs/fishhawk/issues/3116)). `fixupGateOpen(impl, review)` is the MCP MIRROR of the backend's applicability switch (`run.FixupStage` / `run.findOpenReviewStage`): the gate is open only when the implement stage is at `awaiting_approval` (commit-yourself) or `succeeded` WITH the review stage at `awaiting_approval` (push_and_open_pr). In the shipped `feature_change` order — plan → implement → acceptance → review — a succeeded implement stage sits with its review stage still `pending` until acceptance settles, and `POST /v0/stages/{id}/fixup` refuses that state 422 `fixup_not_applicable`; the classifier used to recommend `fishhawk_fixup_stage` there anyway. With the gate closed it now returns one of two new states instead: **`implement_concerns_open_acceptance_pending`** (a non-terminal acceptance stage is holding the gate) or **`implement_concerns_open_gate_closed`** (no acceptance stage is — poll, then `fishhawk_defer_concern`). The acceptance arm splits its ACTIONS by `acceptanceIsDispatchable` (since E64.63 / [#3222](https://github.com/kuhlman-labs/fishhawk/issues/3222) a thin string-typed adapter over the exported `run.AcceptanceIsDispatchable`, not a duplicated predicate): at `pending` / `awaiting_host_dispatch` it is the standard acceptance dispatch-or-poll set then `fishhawk_defer_concern`; at `dispatched` / `running` a spawn attempt already exists (#1912), so it drops the dispatch entirely and returns poll-then-defer — offering a dispatch there would both double-drive the stage and name a remedy the operator cannot take, which is the same inapplicable-instruction defect in a second place. Neither carries a `fishhawk_fixup_stage` entry; both keep `fishhawk_defer_concern`, which is legal now and spends no fix-up budget. When the gate IS open the existing `implement_concerns_open` return is byte-for-byte unchanged, so every commit-yourself case keeps today's behaviour. `fixupGateOpen` ships with a **KEEP IN SYNC** comment naming `run.findOpenReviewStage`, in the same style as the `maxFixupPasses` / `fixupCeiling` / audit-category mirrors; drift re-introduces exactly this class of disagreement, and `backend/internal/integration/mcp/fixup_test.go` is the machine check that the mirror and the endpoint still agree at one instant.
- **The hint reports the ordering, not just the budget** (#3116). When the fix-up gate is not open, `reviewActionHintFor` APPENDS one sentence to `message` after the three budget/ceiling wordings and the two degraded-source notes (so none of them changes): with a blocking acceptance stage it says the review gate opens after acceptance settles and that the remaining budget is preserved, SPLIT by `acceptanceIsDispatchable` exactly as the endpoint refusal is — a dispatchable acceptance stage (`pending` / `awaiting_host_dispatch`) draws "dispatch acceptance first", one already in flight (`dispatched` / `running`) draws "already in flight (state …), so wait for acceptance to settle rather than dispatching it", because a single sentence would reproduce #3116's own defect inside the message that fixes it; without an acceptance stage it says the review stage has not reached `awaiting_approval` so `fishhawk_fixup_stage` refuses until it does. `concerns`, `source`, `remaining_fixup_budget` and `override_available` are deliberately UNCHANGED — waiting consumes no budget, and the surviving route-back is what the sentence is there to make visible. Both production call sites pass the stage slice they already hold; a nil/empty slice degrades to the un-annotated message, so a partial caller loses the sentence rather than gaining a false one.
- **`review_action_hint.concerns` is the store-derived OPEN implement-stage count (#3043).** `reviewActionHintFor` reads the count from the run row's store-derived concerns block (`run.concerns.open_implement`) — the AUTHORITATIVE set the gate view reads (`fishhawk_get_gate_view(stage_kind=implement).open`), computed at the source over the FULL open set and transported alongside the items, so a bounded/compacted `items` list can never undercount. Zero → no hint: this is the case the issue hit, where every concern was waived/deferred/addressed yet the old audit-derived count still fired `implement_concerns_open` while the gate view read `open:[]`. The audit-derived per-round sum (`latestRoundConcerns`) survives ONLY as the DEGRADED fallback in two distinguishable states; `hint.source` reports which of THREE paths produced the count so a fallback count is never mistaken for an authoritative one, and — because a note that falsely claims the store was unavailable is worse than no note — the two fallbacks carry distinct wording: `store` (authoritative — the block was present WITH `open_implement`); `audit_fallback_store_unavailable` (the block was ABSENT — `run.concerns` unwired or the read errored, so "unavailable" is true); and `audit_fallback_legacy_peer` (the block was PRESENT but a backend peer predating the field omitted `open_implement`, so the read SUCCEEDED and only the authoritative scalar is missing). The MCP client decodes `open_implement` as a POINTER so that legacy-peer state reads as nil (degrade) rather than as an authoritative zero that would silently suppress the hint.
- **A retry's DISCARD is not reported as a settle (E45.83 / [#3618](https://github.com/kuhlman-labs/fishhawk/issues/3618)).** When an implement-stage retry supersedes the prior attempt's open implement-review concerns (#3593), `open_implement` drops to 0 and the hint correctly goes nil — but the settled arm then returned `implement_gate_settled` with the merge ritual, telling the operator the review settled with no open concerns when in fact nothing ANSWERED those concerns. `implementStageNextActions` now reads `run.concerns.superseded_implement` (transported as the POINTER `RunConcerns.SupersededImplement`) AFTER the `hint == nil` branch and BEFORE the acceptance branch: a non-zero count returns the distinct state **`implement_gate_settled_after_supersede`**, whose actions are (1) `fishhawk_get_gate_view(stage_kind=implement)` — the `settled[]` ledger already carries every superseded concern's id, `reviewer_model`, severity, category and note; (2) `fishhawk_fixup_stage` with `concern_ids`, because a SUPERSEDED implement-stage id of the target stage IS routable and re-opens against the new tree with its reviewer/round/severity intact (see `server/README.md`); then (3) whatever the ordinary arm would have produced, APPENDED unchanged — the acceptance arm's actions when an acceptance stage exists (the acceptance gate is never skipped), else the merge ritual with a reason that names the discard instead of claiming the review settled. Nothing is removed: the merge is still offered, just not as the lead recommendation. **Both degraded shapes fall through to today's behaviour**: a nil `SupersededImplement` (a backend peer predating the field) and an absent concerns block (the store was unavailable) are both UNKNOWN, and fabricating a demotion from a number the peer never sent would be worse than the status quo — so only an AUTHORITATIVE non-zero count demotes, and an authoritative `0` keeps `implement_gate_settled` byte-identically. The count is not scoped to the latest attempt, so once a run has any superseded implement concern its settled gate keeps returning the new state for the rest of the run; that is the fail-safe direction. The branch sits after the hint check on purpose — a run with OPEN concerns AND a prior discard still classifies `implement_concerns_open`, because routing the open ones is the live obligation. `next_actions_test.go::TestNextActions_SettledAfterSupersede_*` pins the state string, the ORDERED action list, the appended-acceptance case, both degraded shapes, and that the merge action's reason no longer carries the "settled with no open concerns" claim when a discard is present; `client_test.go::TestRunConcerns_SupersededImplement_WireBoundary` pins the wire tag against the key a REAL server emits and drives the decoded run through this classifier.
- **Compact-default review prose is elided with a VISIBLE marker (#3043).** On the compact-by-default `fishhawk_get_run_status` path, `compact.go::stripReviewProse(reviews, capNotes)` replaces a NON-EMPTY review `free_form` and each non-empty concern `note` with a short stand-in (`elidedReviewProseMarker`) naming the full-note surfaces (`fishhawk_get_gate_view` / `include_review_prose=true`), instead of blanking it to `""`. A genuinely-empty note is left empty — so an operator can tell an elided note apart from one the reviewer never wrote (the two states this issue's half (b) is about). **`free_form` is ALWAYS marker-elided** (unaffected by `capNotes`). **The concern-note projection has two paths (E45.92 / #3627):** with `capNotes=false` — every call site except the deduped `implement_review_status.reviews` one — a non-empty note is today's content-free marker byte-for-byte; with `capNotes=true` — reached ONLY on a deduped response whose freed bytes funded it — a note within `concernNoteCapBytes` (240, ENCODED) is carried verbatim with NO marker, and a longer note is capped (via `bound.go::capJSONString`, escape-exact) to a bounded prefix plus `cappedNoteMarker`. See the dedup subsection below.
- **The implement verdicts are serialized once, not twice (E45.92 / [#3627](https://github.com/kuhlman-labs/fishhawk/issues/3627)).** `get_run_status` used to carry the per-reviewer implement verdicts on BOTH the flat `implement_reviews[]` AND `implement_review_status.reviews[]`. `compact.go::dedupImplementReviews` drops the flat listing when it is provably CONTAINED in the status field — the exact condition under which it carries nothing new — replacing it with a small `implement_reviews_elided` note so the omission is legible rather than silent (the #3043 lesson).
  - **Why CONTAINMENT, not equality.** The two fields are NOT unconditionally identical. `implement_reviews` is the UNFLOORED listing (`loadImplementReviews`, `sinceSeq=0` — every round across every fix-up, reviewed+skipped only), while `implement_review_status.reviews` is the CURRENT-round union floored at the latest `stage_fixup_triggered` and additionally carrying synthesized failed rows (`reviewRound.LandedRows`: reviewed+failed+skipped). So on a no-fix-up run they are equal (dedup fires); when a reviewer failed the flat field is a strict SUBSET (dedup fires); and AFTER a fix-up the flat field carries stale pre-fix-up rows the status field floors away (dedup does NOT fire — the flat listing is then the only surface carrying a pre-fix-up verdict). `implementReviewsContained` is MULTISET `reflect.DeepEqual` containment (distinct partner per duplicate row, order-insensitive).
  - **The guard branches** (each with a per-branch test in `compact_test.go`): nil `ImplementReviewStatus` (nil-deref); `implementReviewsContained(flat, rich)` false for an empty `rich` (a none/pending status has no rows, so nothing is duplicated — the flat listing is the only verdict surface) and for a flat row absent from `rich`; and the **size guard** — the fire is refused when the bytes freed by dropping the flat listing (`saved`, measured on a marker-elided copy) do not cover even the elision note, which BY CONSTRUCTION also refuses an empty flat listing (freed ~22 bytes) so it never gains a spurious elision.
  - **Runs REGARDLESS of `include_review_prose`.** The duplication is a wire-shape defect, not a verbosity lever, so `include_review_prose=true` does not resurrect the redundant copy — the full verdicts are then read from `implement_review_status.reviews`, and `implement_reviews_elided` names where they went.
  - **The note cap is FUNDED BY and CONDITIONAL ON the dedup, so no response can grow (ENFORCED, not argued).** When the dedup fires, `dedupImplementReviews` compares the freed bytes (`saved`) against the added bytes (the elision note `costElided` + the extra bytes capping the status-field notes would add over the content-free markers, `costCap`). The status-field concern notes carry capped prefixes (`capNotes=true`) ONLY when `saved >= costElided + costCap`; otherwise the content-free markers are kept (dedup only). Either way the deduped response is byte-for-byte **≤** today's duplicated shape, which keeps the ~7KB compact-default backstop and the ADR-077 32KiB bound honest. The cap applies at the `implement_review_status.reviews` call site ONLY — `implement_reviews` (when it survives) and `plan_review_status.reviews` keep today's content-free marker unconditionally, so the plan surface is untouched.
  - **Residual.** A response where the dedup does NOT fire (a post-fix-up run, a none/pending status, or a listing too small to fund the elision note) is byte-identical to today and still carries the content-free note marker, with `fishhawk_get_gate_view` the one-call surface for full prose.
...
- **An acceptance stage gates the merge** (E31.9 / [ADR-049](https://github.com/kuhlman-labs/fishhawk/issues/1519)). When the workflow declares an acceptance stage, the settled-implement path branches to it **before** the merge ritual, reusing the existing verbs — no new tool (the registry stays at 39). Arms: a non-terminal acceptance stage classifies `acceptance_pending` (offering `fishhawk_dispatch_stage` first — non-blocking, since acceptance runs long against the customer-provisioned preview — with `fishhawk_run_stage` as the blocking opt-in; `github_actions` runs get a poll) or `acceptance_running` (poll); a succeeded stage with verdict `passed` classifies `acceptance_passed` and returns the merge ritual (ADR-049 decision #6: the merge is gated on the `acceptance_passed` evidence condition); a succeeded stage with the SERVER-DERIVED verdict `not_validated` ([#2347](https://github.com/kuhlman-labs/fishhawk/issues/2347), [#3397](https://github.com/kuhlman-labs/fishhawk/issues/3397)) classifies **`acceptance_not_validated`** and returns the SAME merge ritual — the stage verified ZERO criteria, reached by TWO origins: the orchestrator short-circuited it pre-spawn (no runner, no preview; bases `empty-criteria` / `all-skip-with-basis`), OR the validator RAN and shipped a verdict whose every non-retired row was skipped, itemized none, or had every itemized row retired, which the ingest precedence ladder records as `not_validated` (bases `all-skip-observed` / `no-rows-observed` / `all-retired-observed`, #3397). Either way the run is merge-eligible (a change with no observable must not be stranded) but is emphatically not a validated pass; the arm's reason tells the operator zero criteria were verified and asks them to acknowledge that in their merge verdict, which is a **prompt, not an enforcement** (text-matching operator prose to gate a merge would strand runs over wording, a worse failure than the dishonest word being removed) — the terminal-run twin arm is `succeeded_acceptance_not_validated`, and both the reason's load-bearing claims are pinned by `next_actions_test.go` so a refactor cannot silently drop the acknowledgement ask; a failed verdict whose deterministic-triage disposition is a **paged** variant (`paged` / `rerun_budget_exhausted` / `fixup_unavailable_paged` / `retry_unavailable_paged` / `unsettled_paged`) classifies `acceptance_triage_paged` (read the evidence, then arbitrate: manual `fishhawk_fixup_stage`, `merge_and_file_follow_up`, or `fishhawk_cancel_run`); an **auto-routed** disposition (`fixup_dispatched` / `retry_dispatched`) re-opens the implement/acceptance stage server-side, so the next snapshot's existing stage-state arm serves it (`acceptance_triage_rerouting` is the transient poll in between). A recorded failed verdict carrying **no** disposition at all classifies **`acceptance_triage_no_disposition`** ([#2512](https://github.com/kuhlman-labs/fishhawk/issues/2512)) — a state the severity-monotone ladder made reachable BY DESIGN, since a body the agent ships as `passed` with a `failed` criterion row records `failed` yet routes no triage (`handleShipAcceptance` keys triage on the agent's own failed claim, so there is no `failure_mode` for the classifier to read). It returns the paged arbitration menu — `fishhawk_arbitrate_acceptance` is the only verb that discharges `acceptanceGateTriage` for this outcome — with the leading audit-read action's precondition rewritten to name the no-disposition case and a trailing poll for the other reading of an empty disposition (one decided but not yet landed in, or aged out of, the recent-audit window). Routing it to the rerouting poll instead would tell the operator to wait for a re-opened stage that never appears, leaving the merge gate undischargeable; `next_actions_test.go` pins the arm, its action order, and the precondition rewrite. A settled stage whose verdict is not in the recent-audit window classifies the **defensive** `acceptance_settled_outcome_unknown` (point at `fishhawk_list_audit`; **never** the merge ritual — fail toward read, not toward merge). That arm also offers the **`fishhawk_retry_stage` settled-outcome-unknown recovery** (E31.16 / [#1567](https://github.com/kuhlman-labs/fishhawk/issues/1567)): once `fishhawk_list_audit` confirms no `acceptance_outcome_recorded` verdict exists for the stage (the agent shipped a non-schema field and the verdict failed closed), retrying the acceptance stage re-opens it `succeeded → pending` for a re-run (operator token only; the server 422s if a verdict IS recorded) — the reopen lands the stage in pending so the `acceptance_pending` arm's `fishhawk_dispatch_stage` then serves the actual re-run. **Exception — the out-of-scope skip disposition** (E38.3 / [#1877](https://github.com/kuhlman-labs/fishhawk/issues/1877)): a succeeded verdict-less stage whose recent-audit window carries the `acceptance_skipped_out_of_scope` marker classifies the **pre-succeeded** `acceptance_skipped_out_of_scope` state and returns the merge ritual — the orchestrator auto-terminated a degenerate stage (`verification.out_of_scope`, zero `acceptance_criteria`), a legitimate terminal disposition equivalent to a recorded outcome, so the run is merge-eligible and no verdict was recorded **by design**. This is checked **before** the `acceptance_settled_outcome_unknown` arm, so the futile `fishhawk_retry_stage` reopen arm is **not** offered for the skip (the server also `422 retry_not_applicable`s a direct retry against a skip-marked stage — a reopen would re-fire the same skip). When the marker ages out of the recent-audit window the flag is false and the arm degrades gracefully to the read-first `acceptance_settled_outcome_unknown` arm (fail toward read, never toward merge). **Unshipped verdict** (E72.11 / [#3447](https://github.com/kuhlman-labs/fishhawk/issues/3447)): a succeeded stage whose newest acceptance signal in the recent window is an `acceptance_verdict_unshipped` marker (the reap-failure handler's record that the runner's verdict ship 413'd after the trace upload settled the stage) classifies the NAMED **`acceptance_verdict_unshipped`** state — `fishhawk_list_audit` on that category, then `fishhawk_retry_stage` keyed to the acceptance stage, then a poll — and **never** the merge ritual (the server gate refuses the merge 409 on the same marker). The marker rides the existing verdict signal: `latestAcceptanceVerdict` walks newest-first and returns the classifier-local sentinel `acceptanceVerdictUnshipped` (`"verdict_unshipped"`, never a wire verdict) when the first hit among {`acceptance_outcome_recorded`, LIVE `acceptance_verdict_unshipped`} is the marker, so `nextActionsFor`'s signature is unchanged; an outcome newer than the marker wins as before, and a marker aged out of the window degrades to the `acceptance_settled_outcome_unknown` arm. **The marker is ANCHOR-AWARE in-window** (E72.12 / [#3458](https://github.com/kuhlman-labs/fishhawk/issues/3458)), mirroring the backend gate's `acceptanceVerdictUnshippedLive` `M > A` rule: every `acceptance_dispatched` / `acceptance_reopened` entry seen ABOVE the marker in the walk is an episode anchor, and the marker is RETIRED when ANY of them is unscoped or scoped to the marker's stage (the anchors' stage ids are collected into a set as the walk proceeds — a newer anchor scoped to another stage never shadows an older same-stage one). A retired marker is walked past: an older outcome in the window then decides, or the walk ends at `""` and the settled-outcome-unknown read arm — so `dispatch(11)/reopen(10)/marker(9)` classifies `acceptance_settled_outcome_unknown`, exactly as the backend gate does after a `fishhawk_retry_stage` re-open. Outcomes deliberately stay UN-anchored (the backend's `latestAcceptanceOutcome` is un-anchored too); an anchor OLDER than the marker retires nothing (the run-8b911565 live shape). The state string byte-matches `drive.RuleAcceptanceVerdictUnshipped` and the audit category (pinned by `TestAcceptanceStateStringsMatchDrive`); `TestLatestAcceptanceVerdict_UnshippedMarker`, `TestLatestAcceptanceVerdict_MarkerRetiredByAnchor` (the anchor table, including the condition-1 `dispatch(B,12)/reopen(A,11)/marker(A,9) → ""` row and its `dispatch(B,12)/marker(A,9) → unshipped` inverse), `TestNextActions_AcceptanceVerdictUnshipped` and `TestNextActions_AcceptanceVerdictUnshipped_RetiredByAnchor` pin the fold and the arm (delete the `verdict == acceptanceVerdictUnshipped` check → RED; revert the marker case to an unconditional return → the retired rows go RED). **The terminal-run twin is `succeeded_acceptance_verdict_unshipped`** (beside the not_validated / undecidable twins, checked BEFORE the `succeeded_pr_open` fallthrough): a run that reported succeeded with its PR open but whose newest acceptance signal is a live marker gets the SAME read → retry → poll block and NEVER the merge ritual — the server gate refuses the merge 409 on that marker, so offering `approve_pr` / `fishhawk_merge_run` there (the pre-#3458 `succeeded_pr_open` fallthrough) pointed the operator at a verb that could not fire; `TestNextActions_SucceededAcceptanceVerdictUnshipped_NoMerge` pins the twin plus the control that an outcome newer than the marker still classifies `succeeded_pr_open` (delete the twin branch → RED). A succeeded stage with the SERVER-DERIVED verdict `undecidable` ([#2512](https://github.com/kuhlman-labs/fishhawk/issues/2512)) classifies **`acceptance_undecidable`** and returns the SAME merge ritual. This is a POST-RUN twin of `acceptance_not_validated` and the two stay mutually exclusive **by construction**, not by convention: one severity ladder decides between them (`passed < not_validated < undecidable < failed`), so `undecidable` means the stage RAN, drove the preview, and reported rows of which at least one carries `result: "undecidable"` with an `undecidable_reason` and **no row failed**, while the #3397 post-run `not_validated` means every non-retired row was skipped or none was itemized. The backend derives both with that total precedence ladder over the rows (any failed → `failed`; else any undecidable → `undecidable`, including an ALL-undecidable set; else all-skipped/empty → `not_validated`, #3397; else `passed`), so a producer cannot forge either — the ship endpoint still admits only `passed`/`failed` at top level. It is deliberately **not** the failed arm: an undecidable row is not a defect, so no `acceptance_triage_decided` disposition is written and there is **nothing to arbitrate** — which is precisely the [#2474](https://github.com/kuhlman-labs/fishhawk/issues/2474) wedge-surface reduction the ticket claims, since before it a validator that could not decide a criterion had to ship `failed` and page a human. The arm's reason asks the operator to name the undecided criteria in their merge verdict and states plainly that this is not a triage; both, plus the absence of an arbitration action, are pinned by `next_actions_test.go`. The terminal-run twin arm is `succeeded_acceptance_undecidable`. The verdict/disposition vocabulary is **mirrored, not imported** from `backend/internal/server/acceptance.go` (the #875 compile trap), pinned by a literal-table test. A FAILED verdict leaves the STAGE `succeeded`, so the classifier reads the `acceptance_outcome_recorded` / `acceptance_triage_decided` audit payloads, never the stage state. **Every arm above is reachable from BOTH surfaces** (E31.15 / [#1562](https://github.com/kuhlman-labs/fishhawk/issues/1562)): `fishhawk_get_run_status` and the blocking `fishhawk_run_stage` derive the four acceptance signals — verdict, correlated triage disposition, the E38.3 skip marker and the E66.37 arbitration discharge — from their own recent-audit windows with the SAME helpers, so a blocking acceptance `run_stage` no longer always classifies the defensive `acceptance_settled_outcome_unknown` arm while `get_run_status` reported a verdict for the same run. `run_stage`'s window is best-effort: an audit fetch error or a window that has aged the acceptance entries out leaves all four signals at their zero values, which degrades to that same defensive `acceptance_settled_outcome_unknown` arm — never to a merge offer.
...
- **The run lifecycle owns its post-merge tail** ([#1370](https://github.com/kuhlman-labs/fishhawk/issues/1370)). A succeeded run with an open PR URL classifies `succeeded_pr_open` (the full `approve_pr` → `merge_pr` → `post_merge` merge ritual) **until** the backend observes the PR merge resolve: `resolveReviewStageOnMerge` emits a `post_merge_observed` audit row alongside the `pr_merged` / `run_merged` board move (from both the `pull_request.closed` webhook and the merge-reconciler poll, which share that path). `get_run_status` reads that entry off the recent-audit slice it already fetches and reclassifies the run `succeeded_merged` — dropping the now-completed `approve_pr` / `merge_pr` steps and surfacing **only** the operator `post_merge` dev-host step (the `scripts/dev post-merge` rebuild/reload stays an operator/deploy concern, [ADR-038](https://github.com/kuhlman-labs/fishhawk/issues/925)). So a merged run's tail state is owned and observable in `get_run_status` rather than implicit in whether the operator ran the script. (`fishhawk_run_stage`'s run-terminal `next_actions` never observes a post-merge — its PR is not open at stage exit — so it always passes `mergeObserved=false`. That literal stays a **choice**, not a gap: as of E31.15 / [#1562](https://github.com/kuhlman-labs/fishhawk/issues/1562) `run_stage` DOES hold a recent-audit window and derives the four acceptance signals from it with the same helpers `get_run_status` uses, so a blocking acceptance `run_stage` classifies the same arm the authoritative surface would. The `release` signals stay the zero value for the same kind of reason — `run_stage` never executes the delegating release workflow's operator loop, and `releaseSignalsFor` costs the cost-gated extra fetches `get_run_status` owns.)

- **A non-terminal acceptance stage is named on the merge-ritual arms** (E64.63 / [#3222](https://github.com/kuhlman-labs/fishhawk/issues/3222)). `foldAcceptanceRedispatchAdvisory` is a DISPLAY-ONLY, additive fold in the `foldLiveValidationAdvisory` idiom, wired at BOTH `nextActionsFor` call sites (`tools.go` `getRunStatus` and `run_stage.go`'s post-stage snapshot) off the SAME stages + recent-audit slices already fetched — zero extra round-trips, and it can never fail a snapshot. The gap it closes: a run whose acceptance stage a fix-up push re-opened ([#1682](https://github.com/kuhlman-labs/fishhawk/issues/1682)) sits succeeded with its PR open, so the classifier reported `succeeded_pr_open` and offered the merge — while the merge could not fire, because `fishhawk_audit_complete` was pending on that very stage. Nothing on this surface named the cause, and the operator reached for an admin bypass twice.
  - It keys on a **NON-TERMINAL acceptance stage**, not on the audit entry: a merge blocked by an acceptance stage that was never dispatched is blocked exactly as hard, and the advisory is exactly as true. A stage-scoped `acceptance_reopened` entry only SHARPENS the wording from generic to "was re-opened by a fix-up push"; the correlation is on the entry's stage id, never the category alone, so a stale entry from an earlier acceptance stage cannot draw the stronger claim (#3222 binding condition 1).
  - Two new states, keyed on that audit evidence: **`succeeded_acceptance_reopened`** and **`succeeded_acceptance_pending`**. The ACTION shape is keyed on the STATE instead: dispatchable (`pending` / `awaiting_host_dispatch`) PREPENDS `fishhawk_dispatch_stage` with `stage=acceptance`; already in flight APPENDS a wait-shaped `fishhawk_await_stage` and **never** a dispatch (#3222 binding condition 2 — naming a remedy the operator cannot take is the defect #3116 and #3224 each fixed on another surface). The existing merge actions are RETAINED in both branches; the server stays the authority on whether the merge fires.
  - The acceptance stage is selected by NON-TERMINALITY (`blockingAcceptanceStage`, the same selector `acceptanceBlockerAdvisoryFor` uses and a mirror of `run.blockingAcceptanceStage`), NOT by first-match-on-type: a run can carry more than one acceptance stage row in its history, and a first-match selector lands on an earlier superseded/succeeded one, takes the terminal no-op guard and stays silent on a merge the later stage genuinely blocks. `TestFoldAcceptanceRedispatchAdvisory_MultipleAcceptanceStages` seeds both rows into the stages slice.
  - No-op — deep-equal to the unfolded block — when the run carries no NON-TERMINAL acceptance stage (none at all, or every row already terminal), or the block no longer offers `fishhawk_merge_run` (`naOffersMerge`, which keeps the fold off `succeeded_merged` and every non-merge arm). Those guards are what keep a HEALTHY merge byte-identical.
  - The sentence itself comes from `run.AcceptanceBlockerAdvisory`, the single owner shared with the fix-up refusal, the audit-complete check and the merge checkpoints. Contract: `backend/internal/run/README.md`.

- **A human-executor review gate on a run with no implement stage classifies `human_review_gate_parked`** (E54.52 / [#3014](https://github.com/kuhlman-labs/fishhawk/issues/3014)). A `backlog_grooming` run past its plan gate has no implement stage, so it fell through every arm and read `unclassified` — while `drive_status` pointed at an implement stage the workflow never declares. `humanReviewGateNextActions` sits AFTER the implement arm and BEFORE the release/deploy arms, and returns two actions: the named ritual step `approve_review_gate` (params `run_id` / `stage_id` / `attest`, `consumes: approval_slot`) whose reason names the CLI invocation `fishhawk approve-review-gate <run-id> --attest "..."` (E54.55 / [#3051](https://github.com/kuhlman-labs/fishhawk/issues/3051)) and whose precondition states plainly that NO MCP verb exists for this gate and that a delegated operator-agent token is refused `403 operator_agent_forbidden` BY DESIGN (the gate declares `executor: human` with `not: [agent]`); then a `fishhawk_list_audit` read so the operator can check what the server-side apply actually recorded before attesting to the forge. Its three guards are each load-bearing and each has its own counterfactual test:
  - **No implement stage row.** `feature_change` ALSO declares a human-executor review stage, which `implementStageNextActions` already owns. The ordering makes the implement arm win for a run that reaches it, but that function can return nil and fall through — the guard is what stops this arm re-labelling a feature_change run. Its counterfactual vehicle is the DIRECT unit `TestHumanReviewGateNextActions_ImplementRow_ReturnsNil`, not the classifier-level test (through `nextActionsFor` the implement arm owns that fixture, so the deletion is unreachable there); the classifier-level `..._FeatureChangeUnaffected` is kept as the ORDERING guard, and asserts the POSITIVE pre-#3014 output (`implement_gate_settled` plus the exact `[approve_pr, fishhawk_merge_run]` action list) rather than only that the state is not `human_review_gate_parked` — a negative-only assertion stays GREEN if that fixture's classification drifts to any OTHER arm, so it would guard the hijack defect but not the preservation claim.
  - **No `pull_request_url`.** A PR-input human review gate (`human_led_change`, `routine_change`) is [ADR-018](https://github.com/kuhlman-labs/fishhawk/issues/313) PR-merge-managed, where the verb is refused `409 review_stage_managed_by_github` / `pull_request_managed`. This is a PROXY for that server-side admission leg, which this layer cannot evaluate — the classifier is a pure function over the wire DTO and holds no workflow spec. Residual, stated plainly: a PR-input gate on a run whose row carries no `pull_request_url` WOULD be classified here and then refused at the CLI; the action's precondition names that outcome, so the failure is a refused call rather than a wrong write.
  - **`executor.kind == "human"` on a review stage at `awaiting_approval`.** An agent-executor review stage is not a human gate, and a review stage not yet parked is not approvable.

  The drive fold contributes nothing on this arm by design: `drive.RulePlanApprovedHumanGate` stamps NO `next_action` (`backend/internal/drive/README.md`), so there is no duplicate entry and no de-duplication logic in the fold. The classifier tests decode the SHARED wire fixture `backend/internal/server/testdata/human_review_gate_wire.json` — the backend's own serialized `GET /v0/runs/{id}` + `/stages` bodies, pinned by `server.TestGetRun_HumanReviewGate_MatchesWireFixture` — through the real `Run` / `Stage` / `StageExecutor` types, so a json-tag drift on either side reddens the pair.
...
**The deployment-capability gate (E45.93 / [#3628](https://github.com/kuhlman-labs/fishhawk/issues/3628)).** A suggestion pointing at a verb the deployment would refuse is worse than no suggestion: `fishhawk_report_product_issue` collects the run's bundle, fingerprints it, and only THEN resolves the feedback provider — so on a deployment that registered none, accepting the suggestion spends the assembly and returns `501 provider_unimplemented`. Both RUN-level emission sites (`foldProductIssueSuggestion` and the `unclassifiedNextActions` fallback) therefore route through ONE chokepoint, `productIssueActionFor`:

- **The capability comes off the run row, at no cost.** `GET /v0/runs/{run_id}` carries a `capabilities.product_feedback_providers` block (single-run read ONLY — the list endpoint omits it, the `forge_base_url` precedent), mirrored here as `Run.Capabilities` (whose type is deliberately unexported, the `runCompletionBlocked` precedent). No extra round-trip, no new classifier parameter.
- **Substitute, never omit.** When the deployment positively has no provider, the chokepoint returns a `file_product_issue_manually` ritual step (no params, `consumes: none`) whose reason names the gap and the configuration that would close it. Omitting instead would hand a terminal `failed`/`cancelled` run an EMPTY actions list, since the filing suggestion is that arm's ONLY action — a regression of the surface's own structural invariant, which guards non-terminal runs only.
- **Fail OPEN on an absent block.** `productFeedbackPositivelyUnavailable` returns false for a nil run AND a nil `Capabilities`. A nil block is UNDECIDABLE — an older backend omits it, and so does a Run decoded off the LIST route — so reading it as "unavailable" would silently strip the suggestion from every mixed-version deployment. Only a PRESENT block with an EMPTY list is the positive fact; that three-state contract is why the backend's inner key carries no `omitempty` and normalizes nil to `[]`.
- **Declared residual: the CAMPAIGN surface is NOT gated.** `foldCampaignProductIssueSuggestion` and `campaignUnclassifiedNextActions` still emit the tool action unconditionally. They hold `CampaignItem` rows carrying only a run id and never fetch a run row, so the capability is unreachable there without a per-item round trip — and `next_actions`' stated contract is that it costs no extra round-trip and never fails the snapshot. That half needs its own issue rather than a round trip smuggled in here.

`TestNextActions_ProductFeedbackCapabilityGate` and `TestNextActions_UnclassifiedFallbackCapabilityGate` pin all three states on both sites; `client_test.go::TestRunMirror_DecodesCapabilities` pins the wire tags; and `backend/internal/integration/mcp/ci_failed_test.go` drives the whole seam (real server → HTTP → mirror → classifier) in BOTH registry states, which is what catches a tag drift every per-layer unit misses.
...
### Approve-reason lint (E48.78 / [#2512](https://github.com/kuhlman-labs/fishhawk/issues/2512))

`fishhawk_approve_plan`'s `reason` is delivered to the **implement agent** as binding approval conditions ([#558](https://github.com/kuhlman-labs/fishhawk/issues/558)). It is **not** delivered to the plan artifact, and the implement agent has no authority over that artifact — it cannot retire, restate, or reweight an acceptance criterion. So `"approved, but criterion AC-3 is wrong, ignore it"` is silent prose addressed to the wrong actor: acceptance still evaluates AC-3 unchanged and the run fails a criterion the operator already judged bad.

`lintApprovalReason` (`reason_lint.go`, **unexported** — `export_surface_test.go` is a generated exhaustive export baseline that fails in either direction, and a test-only export would trip it) scans the reason for plan-artifact vocabulary on **word boundaries** (`acceptance criterion`/`criteria`, bare `criterion`/`criteria`, `blocking`, `non-blocking`, `retire`, `demote`) and returns a warning appended to the tool result through `approvalSubmitResult`'s warning channel, which became variadic so the lint can surface alongside the `gh`-login warning.

It **warns and never refuses**: the approval submits with the reason **byte-unmodified**, because an operator may legitimately discuss criteria while conditioning something else, and a gate that strands approvals over wording is a worse failure than the prose it would block. The warning names `amend_acceptance_criteria` **first** — [#2581](https://github.com/kuhlman-labs/fishhawk/issues/2581)'s same-gate retire/restate-by-id channel — and `fishhawk_revise_plan` second as the full-replan fallback. That ordering is the substance: #2512's original text predates #2581 and names only the replan, which is a disproportionate answer to one bad criterion. Word boundaries are load-bearing (`retire` must not fire on `retirement`); the never-refuses half is pinned at the call site by `TestApprovePlan_ReasonLint_WarnsOnTheResultAndStillApproves`, which asserts the submitted comment matches the operator's reason byte-for-byte.
...
| Argument | Meaning |
|---|---|
| `concern_ids` | PRIMARY addressing: stable concern UUIDs from `fishhawk_get_run_status`'s `run.concerns.items[].id`, plus the SUPERSEDED implement-stage ids from `fishhawk_get_gate_view`'s `settled[]` (E45.83 / [#3618](https://github.com/kuhlman-labs/fishhawk/issues/3618)). |
...
**Gate-view disputes ([#2551](https://github.com/kuhlman-labs/fishhawk/issues/2551)).** `GateViewConcern` mirrors the backend's `disputed` (bool) and `disputes[]` (`gateViewDispute`: `{sequence, round, veto_reason, resolution, confirming_reviewer_model, raising_reviewer_model, note}`, `veto_reason` one of `raiser_rejected_same_round` | `operator_evidence_routed` | `fixup_pass_no_changes` | `evidence_lookup_failed` | `reopen_without_named_concern`). `disputed` means a `confirmed` resolution was recorded on a concern that is STILL OPEN — the same-round raiser-reject / peer-confirm split. **Since #3319 `disputes[]` ALSO carries refused `reopened` resolutions (`reopen_without_named_concern`), which do NOT set `disputed`** — a refused reopen leaves the concern `addressed_pending`, so it is not a confirmation that failed to settle it; read `resolution` to tell the two apart. The backend derives it from the durable concern row, so `disputed` can be true with an EMPTY `disputes[]` when the best-effort veto record did not land; treat `disputes[]` as detail, never as the presence test. Same json-tag byte-match rule as above — the wire test drives raw server-shaped JSON. `gateViewDispute` is deliberately unexported (the exported surface is pinned by `exportBaseline`); the MCP schema reflection walks it through the exported `Disputes` field.

**Gate-view diff truncation ([#2875](https://github.com/kuhlman-labs/fishhawk/issues/2875)).** `GateView` mirrors the backend's `review_diff_truncated` (`gateViewReviewDiffTruncated`: `{reason, changed_file_count, omitted_file_count, omitted_files, omitted_files_residual, delta_re_review, best_effort}`), present when an implement review ran on a TRUNCATED diff — the runner cut the patch at 256 KiB (`reason: runner_patch_cap`) or the forge capped a compare (`best_effort: true`, its file inventory itself capped). `omitted_files` is the COMPLETE list (the reviewer prompt is capped; `omitted_files_residual` counts what the prompt dropped). Omitted (nil) when no review ran on a truncated diff or against an older backend — the mixed-version degrade. Same json-tag byte-match rule as above (a mistyped tag silently decodes empty, never a decode error), so the wire test drives raw server-shaped JSON. `gateViewReviewDiffTruncated` is deliberately unexported (the exported surface is pinned by `exportBaseline`); the MCP schema reflection walks it through the exported `ReviewDiffTruncated` field. A HIGH "control missing" finding against a truncated review is the false-reject class this surfaces for operator scrutiny.
...
| `concern_ids` | one of `concern_ids`/`concerns` | **Primary addressing ([#964](https://github.com/kuhlman-labs/fishhawk/issues/964))** — stable concern UUIDs to route back (at least one). Read them from `fishhawk_get_run_status`'s `run.concerns.items[].id`. This stage's **open** implement-stage concerns resolve, and since E45.83 / [#3618](https://github.com/kuhlman-labs/fishhawk/issues/3618) so does a **`superseded`** one — a concern a RETRY discarded, whose id lives in `fishhawk_get_gate_view`'s `settled[]` rather than `items[]` (which lists open concerns only) and which re-opens against the new tree with its reviewer/round/severity intact. An unknown, foreign, plan-stage, or `addressed`/`waived`/`deferred`/`addressed_by_condition` ID is still `validation_failed`, with a message naming the admitted set. Routed concerns are marked `addressed_pending` (with `reason` as `state_reason`) in the durable concern store. |
...
- **Delivered-nothing refund ([#967](https://github.com/kuhlman-labs/fishhawk/issues/967) + [#1957](https://github.com/kuhlman-labs/fishhawk/issues/1957) + [#3085](https://github.com/kuhlman-labs/fishhawk/issues/3085)):** a fix-up pass that landed **nothing** on the PR branch is refunded against the **normal** budget, so the next trigger is admitted without `force_additional_pass`. `server.fixupRefundedPasses` is the single chokepoint: it computes the stage's `stage_fixup_triggered` windows once and asks **one question per window** — does it hold at least one delivered-nothing signal and **no** `fixup_pushed` entry? A window refunds **at most once** however many signals of however many kinds land inside it. Four signal shapes are unioned: `fixup_no_changes` (the re-dispatch produced no commit — **not** a crash), `dispatch_reaper_failed` with `failure_category` `C` (the #1747 pre-agent spawn-phase death), `stage_fixup_recovered` with `source_failure_category` `C` (the #788 post-agent-work death on the push/report), and `stage_fixup_recovered` with `source_failure_category` **`A`** (the #3085 harness death that pushed nothing). Category **B** (policy) still consumes a pass — it is a verdict on delivered work, not a delivery failure. The **push veto applies uniformly** across all four: a pass that pushed a commit delivered something, so it consumes budget however it later died. Since [#3335](https://github.com/kuhlman-labs/fishhawk/issues/3335) a delivered-nothing refund **credits the hard ceiling** too, bounded by `maxCeilingRefundCredits = 3` so the **absolute** cap is `defaultFixupCeiling + 3 = 6` triggered passes — two harness loop-detector kills that pushed nothing no longer exhaust a run's whole remediation budget (`refunded_passes`, `ceiling_refunded_passes` and `remaining_ceiling_slots` on the `stage_fixup_triggered` audit entry record the credit; both the `fixup_budget_exhausted` and `fixup_ceiling_reached` 422 details carry `remaining_ceiling_slots` and `ceiling_refunded_passes`, plus `crashed_without_push`, the last counting the refunded passes that **died** rather than merely producing no commit). The `review_action_hint` on `fishhawk_get_run_status` mirrors the **same single unioned per-window evaluation** AND the same `min(refunds, maxCeilingRefundCredits)` ceiling credit (`mcpserver.maxCeilingRefundCredits`, a KEEP-IN-SYNC mirror of the backend constant) so its `remaining_fixup_budget` / `override_available` agree with the backend's admit decision — a mirror that double-counts, or that credits the ceiling where the backend refuses, is worse than no mirror.
...
**Revision-base elision — visible at revise time ([#3442](https://github.com/kuhlman-labs/fishhawk/issues/3442)).** The prior plan is rendered into the re-dispatched prompt whole under the 60000-byte revision-base cap and as a step-complete digest above it ([#3087](https://github.com/kuhlman-labs/fishhawk/issues/3087)); before this the digest was visible only to the planner, and the operator learned of it from the revised plan's `risks_and_assumptions`. Now:

- The output carries **`revision_base`** (decoded straight into `prompt.RevisionBaseAssessment` — same module, one definition): `original_bytes`, `cap_bytes`, `elided`, `mode` (`whole` | `digest` | `digest_shrunk` | `cut`), `rendered_bytes`, `elided_bytes`, the named `elisions[{field, bytes_shown, bytes_dropped}]` (bounded at 50 + `elisions_omitted`) and `unrendered_keys`. It is the server's measurement via the identical serialization the prompt renders, and the same object is stamped on the `plan_revised` audit entry. Absent on an older backend or when no prior plan artifact was loadable (`TestRevisePlan_LegacyBackend_NoRevisionBase`).
- An **elided** base draws one **`warnings`** line AND a `CallToolResult` text `WARNING` equal to it (the `start_run` / `dispatch_stage` advisory pattern — never only in structured output): the mode-selected summary from `prompt.RevisionBaseElisionSummary`, up to 5 named elisions (`summary: 2000 of 5073 bytes shown` …) plus `N more`, the unrendered keys, and the operator's levers — the planner is instructed to record the gap in `risks_and_assumptions` and to fetch the full artifact via `fishhawk_get_plan`; if a section must survive verbatim, restate it in the constraint (`force_additional_pass` grants one more pass) or reject to a fresh-run replan. A whole base yields no warning and a nil result — the pre-#3442 shape.
- **Wording agrees with mode**: "step-complete digest" is claimed only for `digest` / `digest_shrunk`; a `cut` base (the artifact did not decode as a plan carrying approach steps) says "byte-cut prefix … NOT a digest" (`TestRevisePlan_CutModeBase_WarningSaysCutNotDigest`). The tool description and the `revision_base` output-schema description carry the same split (`TestRevisePlanDescription_DocumentsRevisionBaseElision`).
- The cap is not changed and no refusal is added: the operator cannot shrink a prior plan, so the assessment makes the elision visible before review rather than gating on it.
...
- **The check re-post is BOUND to the run's own live PR head (E64.26 / [#3129](https://github.com/kuhlman-labs/fishhawk/issues/3129)) — only the RECORD is verbatim.** Read this next to the bullet above: the two properties differ deliberately. Before republishing, the backend resolves the run's live pull-request head through the same determinability ladder `fishhawk_rebase_run_branch` uses (installation → repo → PR number from the run's tracked pull request → live head sha) and stamps the `fishhawk_audit_complete` check **only** when the vouched sha equals it. A **mismatch** records the vouch and SKIPS the publish, with `audit_check_republish_warning` naming BOTH shas; an **unresolvable** head fails closed the same way, naming the resolution failure. The check is never posted at an unverified sha — without the bound an operator carrying `write:stages` could have this run's recomputed check state stamped onto any commit in the repository, including the head of an unrelated PR. The compare is a case-insensitive FULL-string compare, so an **abbreviated** sha takes the mismatch arm by design; vouch the full sha. Nothing else moved: the `operator_commit_vouched` append and the 200 are identical for a matching and a non-matching sha, and no response field, error code or status code was added.
...
- **Re-posts the `fishhawk_audit_complete` check at the vouched head (E64.14 / [#3109](https://github.com/kuhlman-labs/fishhawk/issues/3109)), subject to the publish bound above.** After the declaration is durable, and once the vouched sha is confirmed to BE the run's own live PR head, the endpoint republishes the required Check Run AT that commit — an operator-pushed head is in no head-report audit category, so the publisher's normal head resolution would re-post at a STALE sha and leave the check absent from the live merge head (the base-advance wedge). The re-post is best-effort for the vouch's success but its outcome is **reported** on the response, not swallowed: `audit_check_republished:false` with a non-empty `audit_check_republish_warning` means the required check may be missing from the merge head. The merge reconciler's heal cannot recover it (it uses the same operator-vouched-blind head resolution), so **re-invoking the vouch is the sanctioned idempotent retry** — the publisher dedups on success, so a re-vouch re-posts exactly the dropped check and no-ops once it is live. This keeps vouch the operator's route out of a **conflicting** base advance: resolve the conflict on the run branch, then vouch the resulting commit. It is no longer the route for a base advance that merely fell BEHIND — [`fishhawk_rebase_run_branch`](#run-branch-base-advance-fishhawk_rebase_run_branch) (E64.23 / [#3125](https://github.com/kuhlman-labs/fishhawk/issues/3125)) now has the RUNNER advance its own branch, and on a CONFLICTING merge authorizes a bounded agent-driven conflict-resolution pass (202, ceiling 1 — E64.62 / [#3202](https://github.com/kuhlman-labs/fishhawk/issues/3202)) rather than refusing; it falls back to `rebase_conflict`, and so to this route, only once that budget is spent or no pass was startable.
...
- **On a conflict: the budgeted 202 trigger arm (E64.62 / [#3202](https://github.com/kuhlman-labs/fishhawk/issues/3202)).** A CONFLICTING base merge is no longer refused outright. **Nothing** is written to the branch either way — no merge commit, no audit entry, no check re-post — but instead of a refusal the backend AUTHORIZES a bounded, agent-driven conflict-resolution pass: it records a `stage_conflict_resolution_triggered` entry, re-opens the run's implement stage, and answers **202**. The runner then performs the base merge LOCALLY on the run branch, resolves only inside the hunks git itself marked, and pushes the merge commit through the App installation. Await the stage, then re-invoke this verb — the behind-probe short-circuits the merge and the check re-posts at the resulting head. Every uncertain anchor is still refused (`rebase_not_determinable`, 422) rather than merged on a guess.
- **The 202 is LEGIBLE only because `RebaseBranchResult` carries the trigger fields.** `apiClient.do`'s only status branch is `>= 400`, so a 202 decodes EXACTLY like a 200 — and without `conflict_resolution_triggered` / `conflict_resolution_stage_id` / `conflict_resolution_pass` / `conflict_resolution_note` a triggered pass would surface as a success-shaped result with an empty `new_head_sha` and no signal, reading as "advance succeeded, head unchanged". That is strictly worse than the loud refusal it replaces, which is why the fields are load-bearing rather than cosmetic. `new_head_sha` and `merge_commit_sha` are EMPTY on a 202 by construction.
- **The pass has its OWN ceiling-of-one budget.** It is counted on the `stage_conflict_resolution_triggered` audit category, which the fix-up counter never reads and which never reads `stage_fixup_triggered` — so a pass can never spend a fix-up slot and a fix-up can never spend the pass. A REFUSED pass writes `stage_conflict_resolution_failed`, restores the run to its pre-pass review gate (a pass is an assist, never an escalation) and CONSUMES its trigger; a SUCCESSFUL pass writes `conflict_resolution_pushed`, which consumes the trigger just as surely — either terminal outcome settles the pass, so no later ordinary fix-up on that stage is served the completed instruction.
- **Fail-closed once the budget is spent**, and whenever no pass can be started at all (no implement stage, an unresolvable branch/base/head anchor, an unreadable budget count, a failed trigger append, a failed stage re-open): the pre-#3202 `rebase_conflict` 422, now naming the FAILED pass, its refusal reason and the fallback route (resolve on the run branch, [`fishhawk_vouch_commit`](#operator-commit-vouch-fishhawk_vouch_commit) the resulting commit, re-merge). Because the conflict arm otherwise answers 202, that refusal is normally observable on the **next** invocation. Backend contract: `backend/internal/server/README.md` § "Bounded conflict-resolution pass".
- **`next_actions` names the pass on BOTH snapshot surfaces.** `foldConflictResolutionAdvisory` is wired at both `nextActionsFor` call sites — `getRunStatus` and `run_stage` — so the status surface and the post-stage surface cannot disagree about whether a pass is pending or its budget is spent. A re-opened implement stage looks like any other pending implement stage and a spent budget looks like nothing at all, so neither is visible from stage state alone. The fold is DISPLAY-ONLY and ADDITIVE: a live trigger appends a wait-shaped `fishhawk_await_stage` and relabels the state `conflict_resolution_pass_pending`; a failed pass appends a `fishhawk_rebase_run_branch` action whose reason names the refusal and the resolve-push-vouch fallback and relabels the state `conflict_resolution_pass_failed`; a SUCCEEDED pass (`conflict_resolution_pushed`) appends a `fishhawk_rebase_run_branch` action naming the pushed merge commit and the re-parked review gate and relabels the state `conflict_resolution_pass_succeeded`. It selects by SEQUENCE across ALL THREE categories together — a SETTLEMENT (failure or push) NEWER than its trigger means the pass ran and is over — the same consumption rule the server's `resolveConflictResolutionTrigger` applies, so the two cannot disagree. Reading only the failure category left a completed pass described as permanently pending, sending the operator to wait on a stage that had already gone terminal. Fail-open on every uncertainty (an un-fetched or aged-out recent-audit slice, a non-object payload), because a false advisory naming a pass that is not there would send the operator to wait on a stage nothing will dispatch.
...
- The tool **always re-POSTs on resume with NO client-side skip.** The endpoint's idempotence has **TWO halves, and saying which is which is the point** (E45.87 / [#3622](https://github.com/kuhlman-labs/fishhawk/issues/3622) narrows what used to read as a bare "the endpoint is idempotent"):
  - **the VERDICT ROW half** — a repeated POST that finds an existing `merge_verdict_recorded` row appends no duplicate and responds `already_recorded:true`. That, and only that, is what a re-POST never duplicates.
  - **the DISPATCH half** — the re-dispatch is now GUARDED rather than blind. Re-queuing a merge for an already-merged PR/MR errors on both forges, so the bare claim above made the documented resumable re-invoke return `502 merge_dispatch_failed` in exactly the case where the merge had SUCCEEDED during the wait (run `f199dcf1`, GitLab). The endpoint OBSERVES the pull request before dispatching and answers `already_merged:true` / `merge_queued:false` instead.

**The already-merged arm (E45.87 / [#3622](https://github.com/kuhlman-labs/fishhawk/issues/3622)).** When the POST comes back with `already_merged:true` the tool **SKIPS `awaitMergeTerminal` entirely** — there is nothing left to wait for — and returns at once with `Status:"merged"`, `RunState` taken from the endpoint's own response, and the reused `postMergeStep` `next_action`. Two shape decisions:

- **`merged` is REUSED rather than a sixth status value.** The PR is merged, which is what a consumer switching on the status string needs; a new value would break every such consumer for a distinction that rides the new `already_merged` field and the message.
- **The message PREFERS the endpoint's own `message` verbatim** (`alreadyMergedMessage`). The backend is the only side that knows whether the `merge_observation_recorded` row actually persisted, and on the append-failure path (server binding condition 1) its message is what names `POST /v0/runs/{run_id}/record-merge-observation` as the recovery. A backend sending no message — an older one, or the additive-compatibility shape — degrades to a locally-composed equivalent rather than an empty string: it names `record-merge-observation` when `merge_observation_recorded` is false, and `reconcile-merge` when `run_state` is non-terminal.

`MergeRunResult`'s four new fields (`already_merged`, `merge_observation_recorded`, `run_state`, `message`) are ADDITIVE: a backend omitting them decodes to `false`/`false`/`""`/`""`, pinned by `client_test.go`'s legacy-body control, so an older backend can never make the tool claim the PR was already merged. `TestMergeRun_AlreadyMerged_SkipsAwait` proves the await was SKIPPED rather than merely fast by asserting the fake backend's audit endpoint recorded **zero** requests while a resolvable `pr_merged` entry sat there unread.

**The `timeout` and `checks_pending` messages name a blocking acceptance stage** (E64.63 / [#3222](https://github.com/kuhlman-labs/fishhawk/issues/3222)). Both statuses are reached because a bounded wait ran out, and the commonest reason the merge never fires is that a NON-TERMINAL acceptance stage keeps `fishhawk_audit_complete` pending — which this tool used to say nothing about. `acceptanceBlockerAdvisoryFor` APPENDS one sentence from `run.AcceptanceBlockerAdvisory` (the same renderer the fix-up refusal, the audit-complete check and `next_actions` use) after the existing sentence, leaving that sentence unmodified. It costs two extra reads and only on those two arms — paths already at least the clamped timeout deep by construction — so the happy merge path pays nothing, and the reads are DETACHED from the caller's cancellation (the bounded wait that just expired is often the shared deadline itself, so inheriting it would silence the explanation on exactly the paths that exist to explain it).

The failure contract, four degrades each with its own outcome:

| # | Condition | Outcome |
|---|---|---|
| D1 | the `GET /stages` read FAILS | `""` — the message ships BYTE-IDENTICAL. We know nothing, so we say nothing. |
| D2 | no NON-TERMINAL acceptance stage | `""` — byte-identical. No new noise on a healthy merge. The stage is selected by non-terminality (`blockingAcceptanceStage`, mirroring `run.blockingAcceptanceStage`), NOT by first-match-on-type, so on a run carrying more than one acceptance stage row an earlier terminal one never masks a later blocking one. |
| D3 | the `acceptance_reopened` audit read FAILS | the GENERIC wording. A non-terminal acceptance stage DOES block the merge; we just cannot claim a fix-up re-opened it. |
| D4 | the audit read succeeds but matches no entry SCOPED TO THAT STAGE | the same GENERIC wording. |

So a failed STAGES read says nothing while a failed or empty AUDIT read still says the true generic thing; the two never both apply. Best-effort by construction — every path returns a string, never an error, so this can never turn a resolved merge status into a tool error. One test per branch and per degrade in `merge_run_test.go`, with D1/D2 asserted BYTE-IDENTICAL against a control captured from an un-advised run rather than by substring absence. `TestMergeRun_Timeout_MultipleAcceptanceStages` is the selector's load-bearing case: it seeds TWO acceptance stage rows (an earlier terminal one, then the live non-terminal one) and fails against a first-match selector, which would take D2 and stay silent.

Statuses: `merged` (a `pr_merged` / `post_merge_observed` entry landed past the anchor, OR the endpoint reported `already_merged:true` and the await was skipped; `next_action` carries the operator post-merge dev-host step, **surfaced not invoked** — ADR-038), `timeout` (resumable — re-invoke; the re-POST never duplicates the verdict ROW, and re-dispatching is safe because the endpoint observes the pull request first), `run_terminal` (the run reached failed/cancelled while waiting — the merge will most likely never settle; check `fishhawk_get_run_status`), `checks_pending` (E67.56 / #2717 — the PR's required checks have NOT all passed, so GitHub reports it in unstable status and will not queue the merge within the wall-clock budget), `conflicting` (E64.14 / #3109 — the PR has a merge conflict against its base, so GitHub can never queue the merge; **not** resumable by waiting, so the tool returns it IMMEDIATELY with no verdict row recorded and names the resolve-then-vouch-then-re-merge path).
...
- The pending-after-timeout message no longer asserts liveness AT ALL (#3395 narrowed #2712's wording) — its outer clause reads "no terminal verdict has landed (…)", not the earlier "the review is still running (…)", which asserted exactly what the parenthetical disclaims: on a
  not-stranded verdict THAT ACTUALLY COMPARED THE BOUNDARY it says it "verified only that the round's dispatch postdates
  the serving fishhawkd's boot … a timestamp ordering, not observed reviewer liveness" and points at the
  `fixup_recovery` marker for the fix-up-opened-round case; the earlier "so its reviewer(s) are still running" clause was
  INFERRED from that ordering and was false for a round a delivered-nothing fix-up had orphaned. The not-stranded
  `reviewStrand.Reason` carries the same bounded wording. Both are pinned on the exact emitted string
  (`TestAwaitPendingTimeoutOutput_VerifiedWordingNamesTimestampOrdering`,
  `TestReviewRoundStrandFrom_NotStrandedReasonIsTimestampOrdering`) because acceptance cannot drive a timing-out review
  round. On `Undecidable` it says so and names the recovery. The claim is gated on a non-zero `DaemonProcessStart` — set only after `boundary.OK`, so it
...
## Retry park derivation (`fishhawk_retry_stage`, [E45.89 / #3624](https://github.com/kuhlman-labs/fishhawk/issues/3624))

`fishhawk_retry_stage` **never spawns a runner.** It wraps `POST /v0/stages/{stage_id}/retry`, which re-opens the stage to `pending`, hands the run to `Orchestrator.Advance`, and then **RE-FETCHES** the stage — so the `stage.state` on the response is the OUTCOME of that handoff, not the intermediate `pending`. Before #3624 the tool description asserted the CI outcome unconditionally (`failed → pending → dispatched (orchestrator fires fresh workflow_dispatch)`), which on `runner_kind local` sent a caller off to poll a stage nothing was driving — the E45.73 / [#3598](https://github.com/kuhlman-labs/fishhawk/issues/3598) shape, reached from the docs.

**Per-runner-kind retry outcome.**

| `runner_kind` | post-retry `stage.state` | `parked` | `next_step` |
|---|---|---|---|
| `github_actions` / `gitlab_ci` | `dispatched` | `false` | absent — the orchestrator fired a fresh `workflow_dispatch` |
| `local` | `awaiting_host_dispatch` (the #1912 explicit park; ADR-024 — the runner is host-spawned, the backend has no channel to start it) | `true` | `fishhawk_dispatch_stage`, or `fishhawk_run_children` on the PARENT run id for a decomposed child |
| any | `pending` | `true` | same dispatch pointer — the DEGRADED shape: no `Orchestrator` was wired, or its `Advance` errored (`retryStageAs` logs and deliberately does not fail the request, "the audit row recorded the retry intent and the stage is in pending") |
| any | `awaiting_approval` | `false` | absent — the category-D SLA-timeout re-open is a GATE, not a dispatch |
| any | `awaiting_children` | `false` | absent — the [#1891](https://github.com/kuhlman-labs/fishhawk/issues/1891) decomposed-parent fan-in restore re-engages the sweeper and `/consolidate` |
| any | anything unrecognised | `false` | absent — fail QUIET rather than emit a pointer derived from a state the code does not understand |

A local retry therefore does **not** ALWAYS park at `awaiting_host_dispatch`: dispatch only on `awaiting_host_dispatch` or `pending`, and for every other state read `stage.state` / `next_step`. The tool description states the same enumeration, and `TestRetryStageDescription_IsRunnerKindAware` reads it off the **registered** tool listing (not the source literal) so a comment-only touch of `tools.go` cannot satisfy it.

**State-driven derivation, and why the state is authoritative.** `retryStagePark` (`retry_park.go`) is PURE — no context, no I/O — and switches on the returned state alone, because that state IS the record of what the retry reached. The verb on a park comes from the run row: a non-nil `decomposed_from` selects `fishhawk_run_children` keyed on the PARENT run id (`dispatch_stage` checks out `main` and so cannot see a `depends_on` slice's dependency, and `run_children` admits an `awaiting_host_dispatch` child — see the run_children compensation note above), otherwise `fishhawk_dispatch_stage` keyed on `(run_id, stage)`.

**Cost posture.** The extra `GetRun` is paid ONLY on a parked state — the handler tests the cheap `retryStageStateParks` predicate first — mirroring `guardNoPRImplement`'s one-additional-read-on-the-rare-path stance. `TestRetryStage_GitHubActionsDispatched_NoExtraRunRead` asserts the read count is zero on the CI path, so a regression that makes it unconditional is caught rather than merely slower.

**Fail OPEN on the run read.** A `GetRun` failure (or an unparseable `run_id`) must never turn a successful retry into a tool error — the stage is already re-opened. The park is already PROVEN by the stage state, so the `fishhawk_dispatch_stage` pointer still ships plus exactly ONE `warnings` entry saying the decomposition-child check was skipped and to prefer `fishhawk_run_children` for a decomposed child. Swallowing the pointer would put the caller back in the poll-a-stage-nothing-is-driving shape.

**Why the tool does NOT auto-dispatch on local** (issue option 2, declined). `fishhawk_retry_stage` is a thin HTTP wrapper over `POST /v0/stages/{stage_id}/retry` with no `working_dir`, no host-dispatch mark and no runner-spawn machinery. Making it spawn would duplicate `fishhawk_dispatch_stage`'s whole surface — spawn compensation, acceptance target preflight, `guardNoPRImplement`, lineage locking — inside a verb whose documented contract is "re-open the stage". The honest fix is to say what actually happens and hand the caller the pre-filled next call.
...
### Intake signals on the response (`intake`, E54.7 / [#2239](https://github.com/kuhlman-labs/fishhawk/issues/2239))

The response also carries an **optional `intake` object**: ADVISORY signals the backend derived at filing time from a bounded, newest-first window of the target repo's existing items.

| Field | Notes |
|---|---|
| `duplicates[]` | Possible duplicates, best first, capped at 3. Each carries `number` / `url` / `title`, a `score`, a `confidence` band (`high` / `medium` / `low`), the shared-token `basis`, and `closed`. Matching is **LEXICAL** token overlap, not semantic — false positives and negatives are expected, and the band plus the basis are what make a wrong candidate cheap to dismiss. A **closed** candidate is surfaced deliberately: a closed duplicate is often the resolution. |
| `epic_suggestion` | The single best parent-epic candidate, present only when the filing declared no `relations.parent_epic` and something scored above threshold. A suggestion — **no relation was created**. |
| `score` | The provisional charter-anchored **structural** score. `value` is advisory only (it sorts a list; it does not justify a ranking) — the `citations[]` (`rubric_id`, `quote`, `note`) are what a reviewer reads. Only rubric ids that actually exist in the repo's declared charter are cited; a rule whose id is absent drops its citation, and a filing that fires none is reported `unscored` with a `charter_gap` rather than given a fabricated one. |
| `degraded` / `degrade_reason` | Whether grooming completed, and why not. Closed set: `reader_unavailable`, `reader_error`, `charter_undeclared`, `charter_unresolved`, `charter_rubric_unparsed`, `budget_exceeded`, `hook_panic`, `seam_unwired`. |
| `scanned_items` / `window_truncated` / `duration_ms` | How many items the filing was compared against, whether the scan window was **cut at its cap** rather than exhausted (so an older duplicate could be missed), and the measured cost. |

**Nothing was acted on.** No item was closed, merged, relabelled or transitioned on the strength of a duplicate candidate — the dedup decision stays a human/workflow action. Read the signals; do not treat them as decisions. The same signals are also appended to the created item's body as an advisory section plus a hidden `<!-- fishhawk-intake:v1 … -->` marker, so a later grooming sweep reads the analysis back rather than redoing it.

**A degraded hook is NORMAL, not an error.** The asymmetry with the rest of E54 is deliberate: elsewhere a missing charter fails closed, but work-item filing is a load-bearing write path (this tool, `fishhawk_report_product_issue`, deferred concerns, and refinement / split filing all share one server-side core), and making it depend on grooming health would convert an advisory enhancement into a new failure mode. The hook runs under its own deadline and scan cap, swallows every failure into a typed reason, and recovers a panic — **a filing never fails, and never slows beyond its budget, because grooming was unavailable.** The latency bound is stated conditionally: it holds for a cancellation-respecting reader, which the production forge read is; it is not preemptive against one that ignores cancellation.

**Why these are local decode-only types.** `IntakeSignals` and its satellites in `client.go` are LOCAL mirrors, deliberately not the backend's `workmgmt` / `intakegroom` types. [ADR-064](https://github.com/kuhlman-labs/fishhawk/issues/2230) forbids any board-READ path from reaching this agent tool surface, and `board_read_guard_test.go` enforces it by banning both a transitive `workmgmt` import and a set of board-read symbol names in this package's non-test code. The reads that produce these signals happen **server-side**, where the capability already lives; this package only decodes JSON the 201 already carried.
...
`fishhawk_report_product_issue` files an upstream **Fishhawk product** bug or feature request — when you hit friction with Fishhawk itself, not the repo you're working in — carrying an auto-collected diagnostic bundle. It wraps `POST /v0/runs/{run_id}/product-reports`. The destination is the **fixed** upstream product repo; it is not caller-controlled. The backend collects the run's product-facts bundle and fingerprints it by report **shape** (#3233). On a **failing** run the fingerprint keys on `(failure category, failing surface, failure detail class, version family)` — unchanged, so open deduped failure reports keep matching (the detail class is a closed-enum normalization of the failure reason (`auth-401` | `bad-object-ref` | `target-unreachable`) that splits distinct root causes sharing a surface (#1962), included only when classified). On a **healthy** run (no failing stage) it keys on the workflow plus a digest of the consented description, or — with no free text — on the run id, in which case the dedup search is **skipped** and the report is filed fresh (this is what stops two unrelated friction reports on running/awaiting_approval runs from colliding onto one occurrence comment). When a search runs it queries the product repo for an open report already carrying that fingerprint marker, and either appends an occurrence comment (dedup hit — nothing new is created) or files a new fingerprint-marked report (dedup miss). A source-side `product_report_filed` audit entry records what left the boundary, including the `fingerprint_basis` naming what was hashed.
...
Returns the egress outcome (`report.action` `created`\|`occurrence`, `fingerprint`, `report.fingerprint_basis` `{kind` (`failure` | `healthy_description` | `healthy_unique`)`, components, dedup_searched}`, upstream `number`/`url`, `destination`), a transparency preview of the product facts that were attached (`diagnostics`), and `free_text_included`.
...
| T3 | `security_findings` + `acceptance_transcript` + `grooming_apply_status` | stored (grooming_apply_status: **three** entries, one per originating category) | `fishhawk_list_audit(category=implement_security_findings)` / `(acceptance_outcome_recorded)` / `(grooming_apply_started)` + `(grooming_mutation_applied)` + `(grooming_apply_completed)` |
...
**Capping is on MARSHALLED cost, not raw bytes.** `jsonEncodedLen` returns the exact encoded length (two-byte short escapes; six-byte `\u00XX` for other control bytes; six-byte forms for `<` `>` `&` and U+2028/U+2029; and, for each **invalid UTF-8** byte, a cost that is **encoder-dependent**: `\ufffd`, six bytes, on the classic `encoding/json` implementation (Go 1.25 default), or the raw three-byte U+FFFD on jsonv2 (the Go 1.27 default) — measured once from the running encoder at package init rather than hard-coded, so the bound stays **exact** under either (#3237) — an inflation factor reaching 3x–6x), verified **differentially against the real encoder**. `capJSONString(s, budget)` guarantees `len(json.Marshal(result)) <= max(budget, 2)`; the `max(budget, 2)` floor makes a sub-minimum or negative budget **defined** (no JSON string encodes smaller than `""`). It stops at an invalid byte, so the result is always valid UTF-8 and always a rune-boundary prefix. `compact.go`'s raw-byte `truncateOversizedString` / `auditPayloadStringCap` are **deliberately untouched** (#1749) — a raw-byte cap is not a size bound under escaping.

The differential test (`TestJSONEncodedLen_MatchesEncoder_Differential`) is the oracle for this fix and passes under both the classic and jsonv2 encoders. Done-means is satisfied for THIS differential test only: verification ran `go test` on this package under the pinned Go 1.25.6 toolchain and under `GOEXPERIMENT=jsonv2` (the offline reproduction of the Go 1.27 default encoder); a full `scripts/test verify` run under Go 1.27.1 itself was not performed, so a second, unrelated 1.27 divergence elsewhere in the tree — if one exists — is a separate issue (#3237).
...
records that the stage verified ZERO criteria — short-circuited pre-spawn, or, since #3397, run-then-all-skip/no-rows/all-retired —
...
`latestAcceptanceTriageDisposition` over the recent-audit slice (`latestAcceptanceVerdict` also folds the E72.11
`acceptance_verdict_unshipped` marker, retired in-window by a newer same-stage or unscoped
`acceptance_dispatched` / `acceptance_reopened` anchor per E72.12 / #3458 — outcomes stay un-anchored, matching the
backend gate); the verdict/disposition vocabulary is **mirrored, not
