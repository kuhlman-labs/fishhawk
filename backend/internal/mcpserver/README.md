...
- **`fishhawk_doctor`** (read-only) wraps `GET /v0/onboarding/readiness` (E29.4 / [#1511](https://github.com/kuhlman-labs/fishhawk/issues/1511)) and returns the `report` — the server-side-only readiness checks a repo's first `feature_change` run needs (five on each family — E45.43 / [#3348](https://github.com/kuhlman-labs/fishhawk/issues/3348) and E45.66 / [#3580](https://github.com/kuhlman-labs/fishhawk/issues/3580)), which the agent cannot introspect locally:
  - `forge` — `github` | `gitlab`: the family that answered (absent against a pre-#3348 fishhawkd).
  - `app` — `{installed, installation_id?, reason?, note?}`: is the GitHub App installed on the target repo. On GitLab there is no App: `installed` means the project is resolvable with the deployment GitLab credential, `note` says so, `installation_id` is never set, and a not-visible project carries a `reason` naming `fishhawkd installation register --provider gitlab --project-path <path>`.
