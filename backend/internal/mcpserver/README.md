...
- **`fishhawk_doctor`** (read-only) wraps `GET /v0/onboarding/readiness` (E29.4 / [#1511](https://github.com/kuhlman-labs/fishhawk/issues/1511)) and returns the `report` — the server-side-only readiness checks a repo's first `feature_change` run needs (five on GitHub, four on GitLab — E45.43 / [#3348](https://github.com/kuhlman-labs/fishhawk/issues/3348)), which the agent cannot introspect locally:
  ...
  - `reviewers[]` — `{provider, model?, reasoning_effort?, available, model_status?, model_hint?, priced?, missing_hint?}`: per spec-declared reviewer availability on **this** deployment. `available` = provider wired AND the resolved model not authoritatively rejected; `model_status` is `verified | rejected | unverifiable` (#3578); `model_hint` carries the did-you-mean / verbatim-to-vendor / unpriced sentence; `priced` (`*bool`, absent when the spec omits the model) flags an unpriced family; `missing_hint` is the env-var hint when a provider can't be resolved. Empty when the spec is unavailable or invalid.
