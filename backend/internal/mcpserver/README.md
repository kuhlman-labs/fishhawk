...
- It sets `FISHHAWK_ACCEPTANCE_PREVIEW_CMD=scripts/dev preview` **on the spawned runner's environment**, and the gate takes its existing provision-command proceed branch off the **same** resolved value. The injection and the proceed decision are wired from one `resolveAcceptancePreviewCmd` call, so the gate can never proceed on a command the runner was never handed.
...
- It does **not** set the teardown hook; that stays `FISHHAWK_ACCEPTANCE_PREVIEW_TEARDOWN_CMD` under the runner's existing contract.
...
`error_code` reuses the single verb's vocabulary (`concern_waive_conflict`, `audit_append_failed`, `internal_error`). Error surfaces propagated as tool errors: a local pre-flight refusal (bad `run_id` UUID, empty `concern_ids`, blank `reason` — all caught before the HTTP hop), `validation_failed` (400 — over the cap, a duplicate, a non-UUID, or an id from another run carrying `details.rule` `concern_run_mismatch`), `cross_run_waive` (403), `concern_not_found` (404), `concern_waive_conflict` (422 — an id is not open; the WHOLE batch is refused), `concern_store_unconfigured` (503).
