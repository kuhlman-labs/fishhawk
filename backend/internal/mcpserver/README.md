...
- **A non-terminal acceptance stage is named on the merge-ritual arms** (E64.63 / [#3222](https://github.com/kuhlman-labs/fishhawk/issues/3222)). `foldAcceptanceRedispatchAdvisory` is a DISPLAY-ONLY, additive fold in the `foldLiveValidationAdvisory` idiom, wired at BOTH `nextActionsFor` call sites (`tools.go` `getRunStatus` and `run_stage.go`'s post-stage snapshot) off the SAME stages + recent-audit slices already fetched — zero extra round-trips, and it can never fail a snapshot. The gap it closes: a run whose acceptance stage a fix-up push re-opened ([#1682](https://github.com/kuhlman-labs/fishhawk/issues/1682)) sits succeeded with its PR open, so the classifier reported `succeeded_pr_open` and offered the merge — while the merge could not fire, because `fishhawk_audit_complete` was pending on that very stage. Nothing on this surface named the cause, and the operator reached for an admin bypass twice.
  ...
  - No-op — deep-equal to the unfolded block — when the run declares no acceptance stage, its acceptance stage is TERMINAL, or the block no longer offers `fishhawk_merge_run` (`naOffersMerge`, which keeps the fold off `succeeded_merged` and every non-merge arm). Those guards are what keep a HEALTHY merge byte-identical.
...
| D2 | no acceptance stage, or it is TERMINAL | `""` — byte-identical. No new noise on a healthy merge. |
