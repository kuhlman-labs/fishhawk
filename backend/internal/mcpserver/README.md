...
- **`FISHHAWK_ACCEPTANCE_PREVIEW_CMD` set in the verb env** (and a resolved expected head) → proceed with an informational note **without probing**: the spawned local runner inherits `os.Environ()` and provisions the target itself ([#1569](https://github.com/kuhlman-labs/fishhawk/issues/1569)).
