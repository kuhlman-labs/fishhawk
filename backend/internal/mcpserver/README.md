...
3. **Empty input AND empty binding** — falls through to `resolveWorkingDir("")`, which refuses over HTTP (#2479) and resolves the process cwd on stdio.
...
**Weak-assertion warnings ([#2501](https://github.com/kuhlman-labs/fishhawk/issues/2501) proposal 3).** A binding assertion is operator-authored prose compiled into a machine gate, so a path typo or a literal paired with the wrong file is invisible at the approval gate and only surfaces post-implement — after a full implement pass has been spent (run 85f52d88). So an approve carrying `binding_assertions` now also runs three cheap, **named** heuristics against the approved plan artifact and returns whichever fired as `binding_assertion_warnings` on `fishhawk_approve_plan`'s output, plus an `ADVISORY binding-assertion warnings` block in the printed result: **`path-not-in-scope`** (the asserted path is not in the plan's `scope.files`, so a compliant implement pass will never touch it — evaluated against the union of the flat plan's scope and every decomposition slice's scope, so a slice-owned path is not flagged), **`literal-absent-from-plan`** (the literal appears nowhere in the plan's summary / approach descriptions / `verification.test_strategy`, so nothing in the approved plan promises to produce it), and **`literal-paired-with-another-path`** (the literal DOES appear in the plan but every plan sentence mentioning it names a DIFFERENT scope file, which the message names — the #2501 typo shape verbatim: `checks_unresolved` sat in an approach step naming `drive_test.go` while the assertion named `policy_reeval_test.go`). They are **ADVISORY ONLY and never a refusal** — an assertion about content that does not exist yet is the whole point of the feature, so a warning is regularly correct-but-intentional; the approval on the same response was recorded and the stage advanced. Every indeterminate state is silent (**fail-open**): an absent or unreadable plan artifact, a plan with no `scope.files` (suppresses `path-not-in-scope` only), a reject, and a declaration-free approve all emit nothing, and the response key is omitted so a clean declaration decodes byte-identically to before. The warnings are also recorded on the `approval_submitted` audit payload as `binding_assertion_warnings`, next to the `binding_assertions` declaration they describe, so "we warned at the gate" is durable in the chain and readable later via `fishhawk_list_audit`.
