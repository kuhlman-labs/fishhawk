...
type AcceptanceSlot struct {
	...
	ReadFailures     []string              `json:"read_failures,omitempty" jsonschema:"issue refs of candidate items whose stage read failed, so the block is partial — those items could not be classified (best-effort degrade); present only alongside a classified participant"`
