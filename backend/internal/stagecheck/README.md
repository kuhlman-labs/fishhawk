...
This is a property of the SQL, **not a guard in the writer**. The ingester's `latest.GitLabPipelineID > incoming → skip` is write-avoidance only: with it deleted, a redundant older row is appended and the readers never surface it. The same latest read has ONE correctness-bearing use — the same-id restart floor: a retried job keeps its pipeline id, so the id key ties and `ts DESC` decides, and the ingester stamps a same-id row `latest.ts + 1µs` when the payload timestamp would not out-sort the latest row (`backend/internal/server/README.md` § GitLab pipeline ingest, `TestGitLabPipelinePG_SameIDRestart`). `TestLatestForStageAndName_GitLabPipelineIDPrecedence` pins both readers against the Go const with the adversarial seed (id 100 carries the later `ts` and is appended last; the reader must return 101) plus a NULL-id check name that must stay `ts`-ordered.
