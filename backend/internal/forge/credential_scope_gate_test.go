...
var sanctioned = map[string]string{
	...
	// (#1861 landed: runnerbackend.TriggerParams.InstallationID flipped to a
	// forge.CredentialScope when the gitlab_ci backend became its second
	// consumer, so the previously-deferred sanction is removed — the gate now
	// holds that field to a scope.)
}
