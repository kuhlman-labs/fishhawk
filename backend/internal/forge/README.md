...
### Capability interfaces (`FileFetcher`, `IssueOperations`)

Two capabilities live as STANDALONE interfaces beside `Forge` rather than as methods on it, for one reason: widening `Forge` churns every implementation and test fake for a capability with ONE consumer, while a standalone interface lets both registered adapters carry it off-interface behind a compile-time assertion (`var _ forge.FileFetcher = (*Forge)(nil)` / `var _ forge.IssueOperations = (*Forge)(nil)` in `github/` and `gitlab/`). A consumer names the capability directly and resolves a registered `forge.Forge` to it with a type assertion; a forge that does not implement it is a fail-closed nil, never a fabricated no-op.

- **`FileFetcher`** (`forge.go`, #2022): `FetchFile(ctx, scope, repo, path, ref)` — the per-repo conventions loader's single-file read. Named `FetchFile` rather than `GetFile` so it does not shadow the method the github adapter promotes from its embedded `*githubclient.Client`.
- **`IssueOperations`** (`issues.go`, E50.17 / #2900): exactly the four issue-thread operations the split-parent auto-close watcher needs — `FetchIssue`, `FetchIssueComments`, `PostIssueComment`, `SetIssueState` — with the forge-neutral vocabulary `Issue` / `IssueComment` / `IssueStateUpdate`. Since E45.42 / #3347 the prompt handler's issue fetch (`server/prompt.go::fillIssueContext`) is its second consumer, and `Issue.HTMLURL` carries the forge-reported browse URL (GitHub `html_url`, GitLab `web_url`) — EMPTY when the payload has none, never fabricated from repo + number. Same non-shadowing naming rule against the promoted `GetIssue` / `ListIssueComments` / `CreateIssueComment` / `UpdateIssue`. `Issue.State` is normalized by EVERY adapter onto `"open"|"closed"`, because the watcher's already-closed short-circuit compares against `"closed"` and a passed-through native word would make a match silently unreachable (the failure mode is a duplicate comment on every redelivery, not an error). `FetchIssueComments` pages to exhaustion so an idempotency marker that scrolled onto page 2 is still seen. `SetIssueState` REQUIRES `State` (an update setting none is `ErrValidation` before any HTTP call) and treats `StateReason` as BEST-EFFORT: a forge with a state_reason concept transmits it, a forge without one records the state change and drops the reason rather than failing.

**GitHub adapter:** four thin wrappers over the embedded client (`GetIssue` / `ListIssueComments` — already Link-paged — / `CreateIssueComment` / `UpdateIssue` with BOTH `state` and `state_reason` on the PATCH), errors propagated unmodified.

**GitLab adapter, and its two named gaps** (`gitlab/gitlab.go`, over the `gitlabclient` issue methods `GetIssue` / `ListIssueNotes` / `CreateIssueNote` / `UpdateIssue` in `gitlabclient/issue_ops.go`):

1. **No `state_reason`.** GitLab's issue object carries no such field, so `FetchIssue` leaves `Issue.StateReason` EMPTY (never a fabricated `"completed"`), and `SetIssueState` IGNORES `IssueStateUpdate.StateReason`. A consumer gating on the reason sees the same `""` GitHub sends for a plain close.
2. **State vocabulary is normalized, and writes go through `state_event`.** `FetchIssue` maps GitLab's native `"opened"` → `"open"` (`"closed"` unchanged, anything else passed through). `SetIssueState` translates `"closed"` → `state_event: "close"` and `"open"` → `state_event: "reopen"`, because the v4 edit-issue endpoint changes state only through `state_event` and refuses a direct `state` write; an unknown target state is `ErrValidation` before any HTTP call.

`gitlabclient.ListIssueNotes` walks the `rel="next"` Link header to exhaustion and follows a next link ONLY when it targets the client's own configured scheme+host — the request carries `PRIVATE-TOKEN`, and a forge-supplied absolute URL must never carry that credential off-instance. GitLab `system` notes are surfaced, not filtered: they never carry a Fishhawk marker, so a filter would be a branch with no effect. The tests are in `github/github_test.go`, `gitlab/gitlab_test.go` and `gitlabclient/issue_ops_test.go`; the watcher rewiring that consumes the capability is the sibling slice of #2900.
