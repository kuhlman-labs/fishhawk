...
- **Action-bearing** — when the non-terminal stage is an ACCEPTANCE stage carrying a stage-scoped `acceptance_reopened` entry (a fix-up push invalidated its verdict, #1682), the detail instead names the re-open as the cause and states that the check clears on its own once acceptance settles. That is the exact strand #3190 reports: `reopenAcceptanceOnFixupPush` flips a settled acceptance stage back to `pending`, so every later recompute takes this branch forever.
- **The action-bearing shape is SPLIT on the stage's CURRENT state**, because the `acceptance_reopened` entry is HISTORY and never goes away. A stage still parked pre-dispatch (`pending` — what `run.ReopenAcceptanceStage` writes — or `awaiting_host_dispatch`) is told to re-dispatch and named `fishhawk_dispatch_stage`. A stage whose re-run is already in flight (`dispatched`, `running`, any other non-terminal state) is told its re-run is **already in flight**, named its live state, and told to wait. Keying on the entry alone kept recommending a re-dispatch the operator had already performed — a check read or republish taken after the operator follows the instruction would contradict the state it reports. Each half carries its own test — `TestCompute_ReopenedAcceptanceDetailNamesRedispatch` / `TestCompute_ReopenedAcceptanceAwaitingHostDispatch_NamesRedispatch` for the dispatchable pair, `TestCompute_ReopenedAcceptanceInFlight_DoesNotNameRedispatch` for the in-flight pair.
- **BOTH the split and both sentences are SINGLE-SOURCED from `run.AcceptanceBlockerAdvisory`** (E64.63 / [#3222](https://github.com/kuhlman-labs/fishhawk/issues/3222)). The former local `acceptanceAwaitsRedispatch` helper is GONE, and with it the paragraph that used to declare its duplication of `run.acceptanceIsDispatchable` deliberate: both surfaces now call the ONE exported `run.AcceptanceIsDispatchable`, and the sentences come from the ONE renderer that also feeds the #3116 fix-up refusal, the `fishhawk_merge_run` checkpoints and the `next_actions` merge arms. The rendered detail is BYTE-IDENTICAL to what #3190/#3224 shipped — the three tests named above pass UNMODIFIED, which is that proof — and `TestStageNotTerminalDetail_SingleSourcedFromRunRenderer` pins the stronger EQUALITY (`detail == run.AcceptanceBlockerAdvisory(shortID, state, true, checkTail)`) so a future edit to either side cannot silently reintroduce the copy. The `categoryAcceptanceReopened` literal stays a literal, for the reason given below.
- **The entry is correlated with the stage's OWN id, never matched by category alone** (#3222 binding condition 1). A run can carry more than one acceptance stage in its history; a stale entry from an earlier one must not draw the stronger "a fix-up re-opened this" claim about a stage nothing re-opened. `TestCompute_ReopenedAcceptanceScopedToStage` drives a NON-MATCHING scoped entry and asserts the generic wording — an absence-only test passes just as happily against an unscoped implementation.
