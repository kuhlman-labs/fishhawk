...
- **The action-bearing shape is SPLIT on the stage's CURRENT state** (`acceptanceAwaitsRedispatch`), because the `acceptance_reopened` entry is HISTORY and never goes away. A stage still parked pre-dispatch (`pending` — what `run.ReopenAcceptanceStage` writes — or `awaiting_host_dispatch`) is told to re-dispatch and named `fishhawk_dispatch_stage`. A stage whose re-run is already in flight (`dispatched`, `running`, any other non-terminal state) is told its re-run is **already in flight**, named its live state, and told to wait. Keying on the entry alone kept recommending a re-dispatch the operator had already performed — a check read or republish taken after the operator follows the instruction would contradict the state it reports. The helper mirrors `run.acceptanceIsDispatchable`, which draws the same split on the #3116 fix-up refusal; the duplication is deliberate (that symbol is unexported and `auditcomplete` does not import `run`'s refusal path), and each half carries its own test — `TestCompute_ReopenedAcceptanceDetailNamesRedispatch` / `TestCompute_ReopenedAcceptanceAwaitingHostDispatch_NamesRedispatch` for the dispatchable pair, `TestCompute_ReopenedAcceptanceInFlight_DoesNotNameRedispatch` for the in-flight pair.
