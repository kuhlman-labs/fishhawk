...
// single-hop assignment in the same function body. Detected details form: a
// `.Error()` call, an fmt embed, a `+` concat, a bare err-identifier, OR a
// single-hop alias, assigned to an ALLOW-LISTED key (the smuggle surface the
// key-based allow-list cannot see on its own). A NON-allow-listed key is
// deliberately not flagged: the runtime allow-list drops it anyway, so a
// finding there would prove nothing. Still outside the tripwire on BOTH
// halves: multi-hop aliases, cross-function flow, helper-returned strings,
// and computed (non-literal) detail keys.
			...
			case statusFivexx:
				...
				if lit, ok := call.Args[5].(*ast.CompositeLit); ok {
					for _, elt := range lit.Elts {
						...
						} else if id, ok := kv.Value.(*ast.Ident); ok && identAliasesCause(fn.Body, id.Name) {
							// Single-hop alias whose NAME does not read as an error
							// (`cause := err.Error()` then `{"reason": cause}`), which
							// argCarriesCause cannot see. Gives the details half the same
							// reach the message half already has (E67.29 / #2631).
							res.Findings = append(res.Findings, rawCauseFinding{Func: fnName, Line: line, Kind: "details:" + key})
						}
					}
				}
...
// TestRawCauseGuard_DetectsAliasedCauseUnderAllowlistedKey pins the gap E67.29
// / #2631 closes: a SINGLE-HOP ALIAS of a cause under an allow-listed details
// key, whose alias name does not read as an error. Before the fix the details
// half tested only argCarriesCause, which sees `.Error()`, an fmt call, a `+`
// concat and a bare err-identifier — but not `cause := err.Error()` followed by
// `map[string]any{"reason": cause}`, leaving the runtime allow-list to admit the
// key and disclose the cause.
//
// Counterfactual: delete ONLY the new `identAliasesCause` clause in the details
// half and this goes RED while the message-half tests stay green — proving the
// new clause, not a shared code path, is what this test pins.
func TestRawCauseGuard_DetectsAliasedCauseUnderAllowlistedKey(t *testing.T) {
	fset, f := parseFixture(t, "violations.go.txt")
	got := kindsByFunc(scanRawCause(fset, f).Findings)

	kinds := got["detailsAliasSmuggle"]
	if len(kinds) != 1 || kinds[0] != "details:reason" {
		t.Errorf("detailsAliasSmuggle: got kinds %v, want exactly [details:reason]", kinds)
	}
}

// TestRawCauseGuard_CleanFixtureHasNoFindings pins the negative control: a
// static message + allow-listed literal details yields zero findings. Since
// E67.29 it ALSO covers cleanAlias — a single-hop alias of a NON-cause
// expression under the same allow-listed key the positive alias fixture uses —
// so the new alias clause cannot be satisfied by flagging every identifier.
