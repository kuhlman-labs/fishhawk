...
type campaignItemResponse struct {
	...
	// `autonomy:<tier>` label ("low"/"medium"/"high"). Omitted (omitempty) when
	// the issue carried no autonomy label — the unchanged-behavior default (#1551).
...
type campaignRollupPayload struct {
	...
	// HumanLed holds would-be-eligible items whose autonomy tier is human-led
	// (autonomy:low) — kept out of Eligible so the auto-driver never dispatches
	// them; a human leads/authors the work (#1551). Always an array (never null).
...
//     ranks ABOVE attend_human_led so start_run wins whenever ANY autonomously-
//     drivable item is eligible — a human-led item never stalls autonomous,
//     DAG-independent work.
//  4. else any human-led item -> "attend_human_led" on the first human-led ref.
//     Reached only when the sole remaining would-be-eligible work is human-led
//     (autonomy:low): the auto-driver will not dispatch it, so a human must
//     lead/author the change (#1551).
...
func computeCampaignNextAction(e campaign.Eligibility) campaignNextActionPayload {
	...
	case len(e.HumanLed) > 0:
		return campaignNextActionPayload{
			...
			Detail:   "this item is autonomy:low (human-led); the auto-driver will not dispatch it — a human must lead and author the change (page the approver)",
		}
