...
func (s *Server) handleShipPullRequest(w http.ResponseWriter, r *http.Request) {
	...
	// refresh that rebuilds the anchor, rendering the drop — see the handler doc).
...
func (s *Server) closePRAfterGatingReject(r *http.Request, runID uuid.UUID,
	...
	comment := fmt.Sprintf(
		...
		reason)
	if _, cerr := s.cfg.GitHub.CreateIssueComment(ctx, forge.FromGitHubInstallationID(*runRow.InstallationID), repo, pr.PRNumber, comment); cerr != nil {
		s.cfg.Logger.LogAttrs(ctx, slog.LevelWarn,
	...
	if cerr := s.cfg.GitHub.ClosePullRequest(ctx, forge.FromGitHubInstallationID(*runRow.InstallationID), repo, pr.PRNumber); cerr != nil {
		s.cfg.Logger.LogAttrs(ctx, slog.LevelWarn,
...
// comment refresh, and responds 200 with no transition. The refresh rebuilds
// the anchor, which renders the drop through issuecomment's activityCategories
// + renderAcceptanceRetirementDroppedLine (#3392), naming every dropped
// scenario id and reason so the operator sees the anchor/status comment as a
// live surface for this category, not only the durable audit chain (see
