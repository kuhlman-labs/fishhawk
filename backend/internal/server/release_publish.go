...
type releasePublisher interface {
	...
	GetReleaseByTag(ctx context.Context, installationID int64, repo githubclient.RepoRef, tag string) (*githubclient.Release, error)
	UpdateReleaseBody(ctx context.Context, installationID int64, repo githubclient.RepoRef, releaseID int64, body string) error
	DeleteReleaseAsset(ctx context.Context, installationID int64, repo githubclient.RepoRef, assetID int64) error
	UploadReleaseAsset(ctx context.Context, installationID int64, repo githubclient.RepoRef, releaseID int64, name, contentType string, data []byte) error
}
...
func (s *Server) handleReleasePublish(w http.ResponseWriter, r *http.Request) {
	...
	rel, err := pub.GetReleaseByTag(r.Context(), instID, repoRef, tag)
	if err != nil {
	...
	if err := pub.UpdateReleaseBody(r.Context(), instID, repoRef, rel.ID, markdown); err != nil {
	...
	for _, a := range rel.Assets {
		...
		if err := pub.DeleteReleaseAsset(r.Context(), instID, repoRef, a.ID); err != nil {
			s.writeError(w, r, http.StatusBadGateway, "release_publish_failed",
	...
	if err := pub.UploadReleaseAsset(r.Context(), instID, repoRef, rel.ID,
