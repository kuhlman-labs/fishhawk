...
func (s *Server) scopeProvenanceForReview(ctx context.Context, runID, stageID uuid.UUID, approvedPlan *plan.Plan, trig prompt.Trigger, diff policy.Diff, ev *prompt.GateEvidence) *prompt.GateScopeProvenance {
	...
	// committedPathSet folds rename SOURCES into the touched set so a
	// declared-deleted path realized as the source side of an R row reads as
	// TOUCHED, not a false untouched signal (#2398). renames carries the
	// (old,new) pairs; renameIndeterminate flags a diff whose R rows lack a
	// source path (legacy/consolidated), making untouched labels non-determinable.
	committed, renames, renameIndeterminate := committedPathSet(diff)
	...
	// scopeRenames are the (old,new) pairs whose OLD side is a declared
	// plan/fold path — bounded to the paths the reviewer is reconciling, not
	...
	// plain-plan-scope case (no folds, every plan file touched, no residual, no
	// declared-path rename, and a determinable diff mode).
	if len(folds) == 0 && len(planUntouched) == 0 && unexplained == 0 && !fixupPass &&
		len(scopeRenames) == 0 && !renameIndeterminate {
		return nil
	}
	...
	return &prompt.GateScopeProvenance{
		...
		RenameProvenanceIndeterminate: renameIndeterminate,
	}
}
...
//
// An R/C row carrying a rename/copy SOURCE (OldPath) renders as
// `- R <old> -> <new>` so the changed-file list the reviewer reads no
// longer hides the source path (#2398). Rows without an OldPath — every
// ordinary change and any legacy/consolidated R row with no source —
// render byte-identically to before this field existed, so a diff with no
// renames produces the exact same text (prompt-hash replay stability).
...
// renamePair is a rename's (old, new) path pair collected from the
// committed diff's R rows by committedPathSet (#2398).
type renamePair struct {
	OldPath string
	NewPath string
}

// committedPathSet builds the rename-aware set of committed paths that
// both operatorScopeUndelivered and scopeProvenanceForReview use to
// decide whether a declared path was touched (#2398). It inserts every
// row's destination Path AND, for Status == StatusRenamed rows carrying
// a non-empty OldPath, the rename SOURCE — because porcelain rename
// detection collapses a declared delete+create pair into a single R row
// that records only the destination, so the source path never appears as
// a row of its own and would otherwise read as untouched. It also
// collects the (old, new) rename pairs so callers can render the source
// as a positively-touched declared path.
//
// A COPY (StatusCopied) source is deliberately NOT inserted: a copy
// leaves its source byte-unchanged, so treating it as touched would
// replace a false negative (source reported untouched) with a false
// positive (source reported touched when it genuinely was not).
//
// indeterminate is true when the diff carries any RENAME row with an
// EMPTY OldPath — a pre-field (legacy) bundle or the forge-compare
// consolidated-review diff, both of which report R statuses with no
// source path, so an absent declared path cannot be distinguished from a
// rename source and the untouched label is not determinable. COPY rows
// with an empty source do NOT set it (binding condition 2): a copy source
// is never counted as touched, so a missing copy source cannot turn an
// absent declared path into a secretly-touched one — the untouched label
// stays provably accurate there.
func committedPathSet(diff policy.Diff) (set map[string]struct{}, renames []renamePair, indeterminate bool) {
	set = make(map[string]struct{}, len(diff.ChangedFiles))
	for _, f := range diff.ChangedFiles {
		set[f.Path] = struct{}{}
		if f.Status == policy.StatusRenamed {
			if f.OldPath != "" {
				set[f.OldPath] = struct{}{}
				renames = append(renames, renamePair{OldPath: f.OldPath, NewPath: f.Path})
			} else {
				indeterminate = true
			}
		}
	}
	return set, renames, indeterminate
}
...
// deploySpecStageForStage resolves the SPEC deploy stage the record side keys
// on for a SPECIFIC stage row (E23.19 / #2642). It loads the run and the stage
// row and delegates to the shared resolveDeploySpecStage, so the record keys on
// the SAME stage the gate that admitted the approval did — and, on a
// multi-deploy-stage workflow, on the stage actually being deployed (by its
// deploy ordinal) rather than the first deploy stage. (Distinct from
// deployStageForRun in deploy_rollback.go, which returns the persisted run.Stage
// ROW; this returns the parsed spec.Stage carrying the pre-flight constraints.)
// Best-effort: every failure (run/stage lookup failure, nil/absent spec, parse
// error, workflow not in spec, or a deploy ordinal with no spec counterpart)
// returns ok=false rather than erroring, since the environment label is
// best-effort and the authoritative outcome fields are external_run_url +
// outcome.
func (s *Server) deploySpecStageForStage(ctx context.Context, runID, stageID uuid.UUID) (spec.Stage, bool) {
	runRow, err := s.cfg.RunRepo.GetRun(ctx, runID)
	if err != nil {
	...
	st, reason, _ := s.resolveDeploySpecStage(ctx, runRow, stage)
	return st, reason == deployStageResolveOK
}

// deployEnvironmentForStage resolves the environment label for a deploy record
// (E23.18 / #2324, stage-scoped E23.19 / #2642): the environment the operator
// ACTUALLY approved (deployApprovedEnvironment), falling back to the spec
// derivation (deployEnvironmentFromSpecStage) only when no explicit
// --environment= approval is recorded. Both deploy-record sites
// (ResolveDeploymentFromPollState and ResolveDeploymentRollbackFromPollState)
// call this so the deployment artifact body and the deployment_outcome_recorded
// audit carry the approved label, not a schema-order-derived one — and, on a
// multi-deploy-stage workflow, the label of the stage actually being deployed.
func (s *Server) deployEnvironmentForStage(ctx context.Context, runID, stageID uuid.UUID) string {
	...
	return s.deployEnvironmentFromSpecStage(ctx, runID, stageID)
}
...
func (s *Server) deployApprovedEnvironment(ctx context.Context, runID, stageID uuid.UUID) string {
	...
	st, ok := s.deploySpecStageForStage(ctx, runID, stageID)
	if !ok {
		return ""
	}
...
// deployEnvironmentFromSpecStage derives the deployed environment label from the
// run's cached workflow spec — the deploy stage's FIRST allowed_environments
// entry, resolved for THIS stage row (E23.19 / #2642). This is the FALLBACK used
// by deployEnvironmentForStage only when no explicit `--environment=` approval is
// recorded (the genuinely single-environment case); it is NOT the primary
// answer, and it does NOT reflect the operator's choice on a multi-environment
// stage. First-wins here is retained deliberately as the #2218-characterized
// fallback fold (a duplicate-kind document reports its first entry). Best-effort:
// an unparseable/absent spec yields "" rather than failing the resolve, since the
// authoritative outcome fields are external_run_url + outcome, not the
// environment label.
func (s *Server) deployEnvironmentFromSpecStage(ctx context.Context, runID, stageID uuid.UUID) string {
	st, ok := s.deploySpecStageForStage(ctx, runID, stageID)
	if !ok {
		return ""
	}
