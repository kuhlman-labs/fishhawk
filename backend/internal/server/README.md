...
| `trigger` | run admission | the run's `trigger_source` → `spec.TriggerForm` | `applies_to.go::checkAppliesTo` |
...
- **One renderer, two points.** Both rejections build `appliesToRejection` and call `renderAppliesToRejection` (`applies_to.go`), so both carry the same shape: the workflow, the criterion that failed, the value observed, and **which workflows would accept this change**. An operator refused at the plan gate is further into a run than one refused at admission and needs more help, not less.
...
**The override's source of truth is the run-scoped audit entry, and its residual failure mode is stated rather than hidden.** The audited exception is one concept with one name: `applies_to_override` + a **required** reason on `POST /v0/runs`, recorded as the run-scoped `run_admitted_applies_to_override` entry. The plan gate suppresses its own refusal by **looking that entry up** (`runHasAppliesToOverride`), never by re-reading request state — audit-entry lookup, no run column, no migration. The consequence, which is fail-closed by construction: **if the audit append at run-create fails, the run is admitted but the plan gate re-rejects it.** That is recoverable (re-start the run with the override, or widen the declaration) and it is the price of surfacing the override in the audit log with no schema change. `TestAppliesToPlanGate_OverrideAbsent_Rejects` pins it — a run whose creation carried an override but whose entry is absent has no override here.
...
**Trust boundary (documented, not implied).** Labels are fetched by the *caller's* `gh` and shipped inline on the create request, so a caller determined to route a change through the wrong workflow can attest whatever labels it likes. `applies_to` prevents **misrouting**, not a determined authorized caller; that is an accepted boundary for a routing control whose sanctioned exception is an audited override, and server-side label fetching is the named hardening follow-up. Overlap is benign by construction: `applies_to` *filters* an operator-named `workflow_id`, it never *selects* one, so two matching workflows is not a coin flip.
