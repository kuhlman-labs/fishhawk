...
- **The phase split is exhaustive over the predicate grammar, and a TRIPWIRE — not prose — is what keeps it that way.** `appliesToPhasePredicate` is a hand-maintained field-by-field copy of `spec.Predicate` whose `constrains` check enumerates today's four criteria by name, so a *fifth* criterion added by a sibling slice (#2227 `escalations`, #2211 review conventions) would be picked up by the `applies_to` `$ref` automatically and copied into **neither** half — declared and silently never enforced, the failure mode the function's own comment names for `change_kind`. `TestAppliesTo_PhaseSplit_IsExhaustive` therefore asserts against the struct via `reflect`: a field-count check (with the actionable message: route it into a phase half **and** `constrains`, extend `firstFailingCriterion`) plus a per-field check that every criterion survives into at least one half. A new criterion fails the test rather than shipping unenforced.
