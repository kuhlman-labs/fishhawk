...
- **Run-status concerns block presence + open-set equality (#3043).** `handleGetRun` emits the `concerns` block on any SUCCESSFUL `ConcernRepo.ListOpenByRun` read, INCLUDING a zero-open run (`open:0`, `by_state:{}`, `open_implement:0`, `items:[]`) — `buildRunConcernsPayload` no longer returns nil on an empty set. The field is left ABSENT only on the UNAVAILABLE paths (`ConcernRepo` unwired, or the read errored — both warn-logged), so PRESENCE is authoritative: present == a store read, absent == unavailable, never "zero". The block's open set is BY CONSTRUCTION the same rows the gate view's `open[]` shows for the same run: both read `ListOpenByRun` / `concern.State.IsOpen()` — the run-status/gate-view open-set equality invariant (pinned by `runs_get_test.go::TestGetRun_ConcernsMatchGateViewOpenSet`). `buildRunConcernsPayload` also computes `open_implement` — the AUTHORITATIVE count of open `implement`-stage concerns over the FULL open set — which the MCP `review_action_hint` transports and reads rather than re-deriving a count from the bounded `items` list.
