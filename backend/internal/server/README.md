...
- **Budget**: `maxPlanScopeRetries = 1`, counted from the run's `plan_scope_retry` audit entries (`countScopeRetries`, the exact read shape `countSchemaRetries` uses).
...
- **Return contract, per leg** (each leaves a *different* observable state, and each has its own behavioural test):
  ...
  |---|---|---|
  | nil `Orchestrator` / nil `AuditRepo` / budget exhausted / `AppendChained` error | `false` | Nothing mutated; caller's fall-through parks the stage at `awaiting_approval` with the evidence |
