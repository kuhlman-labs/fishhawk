...
2. **File a split with a contract child.** Drive a run on a project issue whose plan is a `split_proposal` with a contract phase and approve it; `fileSplitProposalChildren` files the children and writes `split_children_filed`. Confirm the linkage the watcher will read: `GET /v0/audit?category=split_children_filed` (bearer token) must show an entry whose `parent_repo` equals the project's `path_with_namespace` and whose `contract_child_number` is the child's iid — note them as `$PARENT` and `$CHILD`. `parent_repo` is the SAME string GitLab puts in `project.path_with_namespace` on the webhook; if the two ever differ the watcher fails QUIET (no linkage match).
