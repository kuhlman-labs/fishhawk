...
- **Pinned by** `approve_advance_cas_test.go`: the concurrent two-approver end-to-end (per-filing-path exactly-once — 3 split children, 1 walk — plus the loser's typed error), the stale-observation refusal (error identity AND committed state), the HTTP 409 rendering, the non-CAS degradation, the `#986` duplicate submission, the deploy leg's un-fired dispatch seam, the untouched reject leg, and the three slash-reply branches.
