...
- Renders a distinct high-priority block (`prompt.GateEvidence.FixupUnattemptedConcerns` / `FixupUnattemptedFiles`) framing untouched files as evidence the concern was NOT ATTEMPTED and explicitly NOT proof it was not ADDRESSED, with the undeterminable count as a coverage caveat. Only the concern's severity/category and file PATHS are rendered — never the note text, so this block adds no new untrusted-text channel. The fix-up prompt states the same obligation up front (`writeTrustedFixupConcerns`).
- Appends ONE advisory `fixup_concern_unattempted` audit entry (payload `{routed_count, unattempted_count, undeterminable_count, concerns:[{id, position, severity, category, implicated_files}], unattributed_untouched_files}`) whenever the pass routed concerns AND there is a finding, an untouched named file, or a NON-ZERO undeterminable count — the last so a total coverage gap cannot read as silence. A pure coverage-gap record is operator-facing only and leaves the reviewer prompt byte-identical.
...
- Best-effort: nil `AuditRepo`, a list error, a malformed trigger payload, a rename-indeterminate diff, or an empty candidate union each contribute nothing; an append failure WARNs and the review proceeds.
