...
3. per-reviewer availability via the same `ReviewerSet.For(provider, model, reasoningEffort)` probe `unavailableSpecReviewers` performs (`runs.go`), surfacing the adapter's missing-env hint (`probeReviewers`). Since #3578 each rung also carries the model-id honesty fields: `available` means "the provider is wired AND the resolved model was not authoritatively rejected" (it does NOT assert the model is served when `model_status` is unverifiable); `model_status` is the tri-state `verified | rejected | unverifiable` from `modeloracle.Verify` (a `For()` rejection wraps a `modeloracle.RejectedError` so the resolved DEFAULT model — the spec-omitted case — is judged too); `model_hint` is the did-you-mean / passed-to-vendor-verbatim / unpriced sentence; and `priced` (`*bool`, nil when the spec omitted the model) flags an id whose family the pricing table does not know;
