...
- **Timestamps (`gitLabPipelineTimestamp`).** `finished_at`, else `created_at`, parsed as RFC 3339 or GitLab's documented `2006-01-02 15:04:05 MST` (`"2016-08-12 15:23:28 UTC"`), else `now()` with a WARN (`TestGitLabPipelineTimestamp`). Since precedence is by pipeline id, a mis-parse can only mis-stamp `ts`, never flip which row is latest.
