...
- **Two arms, one filing — parent under the TRUE epic (#2179).** `resolveWalkParentEpic` resolves the triggering child's sub-issue PARENT via `githubclient.Client.IssueParent` (the parent-direction query added for this, see `../githubclient/README.md`). When the parent's title is the **bracket-closed** `[E<n>]` epic form — matched by a NEW `walkEpicTitleRE` (`^\s*\[E(\d+)\]`), deliberately DISTINCT from `workitems.go`'s `epicTitleRE` (`^\s*\[E(\d+)`), which would ACCEPT a child title like `[E48.35]` and re-introduce the exact mis-parenting this fixes — and the provider can discover a child number, the walk is filed with `Relations.ParentEpic = "#<epic>"` and `TitleVars{epic}` from the epic title, with `{n}` left UNSET so `deriveChildNumberTitleVar` discovers the next child number under the real epic (and takes the per-epic lock). It renders `[E<epic>.<n>]` as a genuine child of the epic.
- **Seven companion fallback modes.** On ANY resolution failure the walk falls back to the pre-#2179 companion filing byte-for-byte (`{epic}=<issue>`/`{n}=1` → `[E<issue>.1]`, `CompanionTo` the triggering issue), which neither mis-parents nor collides with the real epic's child numbering. The modes: (1) no GitHub client; (2) a zero credential scope; (3) `IssueParent` errors; (4) `IssueParent` returns no parent; (5) the parent title is not the bracket-closed `[E<n>]` epic form (e.g. another child `[E48.35]`); (6) the provider does not implement `EpicChildrenQuerier`, so `{n}` could never be discovered; (7) **the epic is resolvable but FULL** — it already holds the per-parent sub-issue cap (`githubSubIssueParentCap = 100`) or its child count could not be read, so a new child cannot attach. GitHub REJECTS the 101st `addSubIssue` with a GraphQL error (surfaced through `doGraphQL` as `ErrValidation`); the mode-7 pre-count degrades to companion BEFORE filing rather than catching that error after the issue already exists (which would leave an unparented `[E<epic>.<n>]` walk). E22 #389, E48 #1940 and E67 #2561 are each at the cap today, so this is a live condition, not a hypothetical (binding condition 4).
...
- **RESIDUAL — epic-arm `{n}` discovery can still fail at file time.** On the epic arm `deriveChildNumberTitleVar` can fail closed with a 422 (a query error, or children present but none in the numbered `[E<epic>.<n>]` form, #2101). That routes to the `filing_failed` linked marker (the operator files by hand), NOT to a companion retry — a second attempt within one approval would reopen the double-file window. The mode-7 pre-check removes only the provider-capability and full-epic cases, not an at-file query failure.
