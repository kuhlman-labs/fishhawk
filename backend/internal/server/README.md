...
- **RESIDUAL — epic-arm `File` can still fail.** On the epic arm the `File` itself can fail (a provider 502), which routes to the `filing_failed` linked marker (the operator files by hand), NOT to a companion retry — a second attempt within one approval would reopen the double-file window. The cap decision no longer fails at file time: `{n}` is allocated under the held lock BEFORE `File`, so the resolvable-but-unattachable cases (full, unreadable, non-numbered children) are resolved to companion in `resolveWalkParentEpic` rather than surfacing as an at-file 422. The in-process lock covers a single daemon; a multi-instance deployment sharing one tracker would need a Postgres advisory lock (mirrors `childNumberLocks`).
