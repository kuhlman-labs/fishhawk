...
- **Budget**: `maxPlanScopeRetries = 1`, counted from the run's `plan_scope_retry` audit entries (`countScopeRetries`, the exact read shape `countSchemaRetries` uses). The count **fails closed**: a read error returns the error and `tryScopeRetry` declines to refuse, because an unreadable budget is an *unbounded* one — a persistent audit-list failure would otherwise grant a refusal on every corrective ship and the one-shot would never exhaust. Losing one legitimate refusal to a transient read error is the cheaper failure (the plan still parks with the evidence and the refunded revise pass). The count→append window is not atomic, so racing ships can both read 0; audit-first ordering bounds that in the safe direction — the budget is over-consumed, never under-consumed, and the `>=` comparison makes the next ship park. A strictly one-shot budget under concurrency would need a unique constraint on `(run_id, category)`; out of scope.
...
- **Ordering is audit-first**, mirroring `trySchemaRetry` and `handleRetryStage` so the retry intent is durable even if a later step fails: append `plan_scope_retry` → `FailStage(FailureA)` → `RetryStage(pending)` → `Orchestrator.Advance`. The transient category A never leaks into the run's terminal state.
- **Return contract, per leg** (each leaves a *different* observable state, and each has its own behavioural test):
  ...
  |---|---|---|
  | nil `Orchestrator` / nil `AuditRepo` / budget read error / budget exhausted / `AppendChained` error | `false` | Nothing mutated; caller's fall-through parks the stage at `awaiting_approval` with the evidence |
...
## PR-open checkpoint resume gate (#2169)

`prompt.go::resolvePushCheckpointResume` is the structural sibling of the exempt gate above. It decides whether an implement-stage prompt response carries the SAME three held-commit fields plus `held_commit_resume_kind: "pr_open"` — the discriminator that makes the runner re-attempt only the idempotent `OpenPR` instead of re-running the whole implement agent after a sustained PR-open outage.

- **Where the checkpoint comes from.** `pullrequest.go::failPullRequestStage` records a `push_checkpoint` object (`{branch, head_sha, base_sha, verified_tree_sha}`) into the `pull_request_failed` audit payload when the runner's failure report carries BOTH `branch` and `head_sha` — which the runner arms only once `CommitAndPush` has succeeded on the standalone open-PR path. There is no new column and no migration: the checkpoint rides the existing audit chain.
- **Both-or-nothing, and never a 400.** A report carrying only one of the two records NOTHING. `validate()`'s `failed` arm is deliberately left permissive: rejecting a half-populated report would turn a runner bug into a stage stranded in `running` until the SLA watchdog reaps it.
- **Newest-entry-wins, over a WIDER category set.** The walk spans `pull_request_failed`, `pull_request_opened`, the three `scope_completeness_*` kinds, `fixup_pushed`, and `child_pushed`, and emits only when the newest entry for the stage is a checkpoint-bearing `pull_request_failed`. That is what makes the gate SELF-INVALIDATING: once the resume succeeds a `pull_request_opened` entry becomes newest, so a later retry takes the ordinary agent path instead of re-opening a PR that already exists.
- **Fail-closed.** A nil `AuditRepo`, any `ListForRunByCategory` error, a non-implement or nil stage, a fix-up dispatch (which exists to re-invoke the agent on routed concerns — resuming would skip the requested fix), no entry for the stage, an undecodable payload, a checkpoint-less `pull_request_failed`, and a half-populated checkpoint all return `ok=false`. A false negative costs the pre-#2169 behavior (a full agent re-run); a false positive would open a PR from a commit this gate cannot prove is current.
- **Exempt wins.** Wired into both `handleGetStagePrompt` and `handleGetStagePromptRender` AFTER `resolveHeldCommitExemption` and taken only when that returned false, so #1231 keeps precedence and the two resolutions can never both set the held-commit fields.
- **The runner adds a second, independent control.** On the `pr_open` kind it re-reads the run branch's remote tip (`gitops.RemoteBranchTip`) and refuses to open anything unless it still equals `held_commit_sha` — the checkpoint is backed by the audit chain rather than a persisted park row, so an operator `fishhawk_reset_run_branch` between dispatches must not produce an artifact whose `head_sha` lies.
