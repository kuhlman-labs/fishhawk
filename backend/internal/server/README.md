...
| 4 | `no_implement_stage` | `findImplementStage` nil | `evaluated_run_id` |
...
- **The check READ stays on the IMPLEMENT stage.** `findImplementStage` is unchanged; on GitHub, `ingestCheckRun` records rows against the REVIEW stage (`FindRunStagesForCheckRun`, #254), so a GitHub run with a snapshot reports `contexts_pending` today even when its checks are green — the new message makes that visible, and the reader-stage move is #3489, not this change.
