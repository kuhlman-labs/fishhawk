...
| `cfg.GitLabInstallations == nil` (no database) | `unknown` / `registry_unwired` | false, reason naming the code | `_GitLab_Registration_RegistryUnwired` |
| `ResolveGitLabProject` errored (WARN log) | `unknown` / `registry_lookup_failed` | false, reason naming the code | `_GitLab_Registration_LookupFailed` |
...
| Project did not resolve (not visible / forge unconfigured) | whatever the registry says; `resolved_ref` + `ref_matches` ABSENT (never compared) | false, the visibility/forge reason wins | `_GitLab_ProjectNotVisible`, `_GitLab_ForgeUnconfigured`, `_ForgeResolverTypedNil` |
...
**Fail-closed, like the merge-gate rungs.** `unknown` is reachable ONLY when the registry could not answer (unwired / lookup fault), always with a naming `reason`; a registry that answered found=false is the POSITIVE `not_registered`. `ref_matches` is absent whenever either ref is unknown, so an unresolved project never renders a false match and a row with an empty ref is never compared.
