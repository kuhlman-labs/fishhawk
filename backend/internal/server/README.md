...
Verdict tests: `_EndToEnd` (HTTP → handler → the REAL `forgegitlab.Forge` → the real `gitlabclient` → the v4 stub → the RAW body: `pipeline_gated`, every signal mapped, the note, NO `merge_gate` key), `_OverlappingRules_Union` (exact `main` + wildcard `m*` → `matched_rules [main, m*]`, merge levels ascending `[30, 40]`, `allow_force_push` OR'd true), `_NotProtected_NotPipelineGated`, `_PipelineNotRequired_NotPipelineGated`.
