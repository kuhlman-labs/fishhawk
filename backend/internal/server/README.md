...
**Ordering on the terminal half is load-bearing**, and it was settled empirically rather than argued. The call must sit AFTER `advanceRunAfterReviewResolve`: `auditcomplete.ComputeResult` reports `pending` while any non-review stage is non-terminal, and the Advance is what walks the remaining stages to terminal (a `pending` acceptance stage short-circuits to `succeeded` inside Advance; the merge-supersede sweep that precedes it terminalizes a fix-up-re-parked one). Hoisting the call above the Advance turns `TestResolveReviewStageOnMerge_RepublishesAuditCheckOnTermination` RED — and the OBSERVED failure is sharper than "it republishes pending": the recompute yields the state already published at that head, the publisher's `(forge, repo, head_sha, state)` dedup cache suppresses it, and NOTHING is posted at all, so the stale `in_progress` simply rides. No deletion counterfactual can detect an ordering constraint, which is why this one is pinned by a mutation.

Both calls are placed BEFORE the `notifyStatusUpdate` / `notifyBoardTransition` / `writePostMergeObservedAudit` / `stampEconomicsIntoPRBody` tail so a failure in one of those cannot skip the republish. Both are **best-effort**: the helpers are nil-safe (no `RunRepo`/`ArtifactRepo`/`AuditRepo`, or no publisher in the dev posture, is a clean no-op), a compute or publish failure logs at WARN inside `recomputeAndPublishAuditComplete`, and neither ever unwinds the merge, its durable verdict row, or the run completion.
