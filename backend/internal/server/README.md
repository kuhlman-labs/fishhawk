...
- **The parent side.** A parked child is settled but NOT terminal, so the decomposition parent stays in `awaiting_children`. That wait is correct but must not be silent: a `parent_awaiting_child_scope_decision` entry is emitted on the PARENT, from BOTH `parkScopeCompletenessStage` at park time and — de-duplicated per child stage — `orchestrator.maybeAdvanceDecomposedParent` when a sibling settles. See `backend/internal/orchestrator/README.md` for why the emission has to be dual.
