...
- **Guard order, all fail-closed and BEFORE any write** (so a refused arbitration leaves ZERO rows): `400 validation_failed` (non-UUID `run_id`, malformed body, empty/whitespace `reason`) → `503 acceptance_arbitration_unconfigured` → `404 run_not_found` → read stages + the latest outcome → **idempotence short-circuit** → `409 acceptance_arbitration_not_applicable` unless the gate reads `acceptance_triage` (a gate read error is `500`, never a write) → `409 acceptance_arbitration_not_applicable` unless the CORRELATED triage disposition pages → `409 acceptance_arbitration_requires_acknowledgement` when `criteria_failed > 0` without `acknowledge_failed_criteria:true` → **atomic append** (the write-side revalidation and the authoritative dedupe now run INSIDE that append's transaction, under the run-row lock — see below).
