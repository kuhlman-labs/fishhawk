...
- **One fail-OPEN precondition.** When the plan-artifact subsystem is unconfigured (`ArtifactRepo`/`RunRepo` nil) no decomposition can exist, so a non-empty `add_scope_files` proceeds — mirroring the sibling gates' `ArtifactRepo==nil` posture. Production always wires both repos, so this branch never fires there and the universal fail-closed guarantee holds where it matters.
