...
- **Degradation is visible, never silent.** `AuditRepo` nil, or any per-category `ListForRunByCategory` error, returns 200 with the concerns intact, `history_incomplete=true`, and `history_gaps` naming each failed category; a single malformed payload entry is skipped warn-only while its siblings still join. `ConcernRepo` unconfigured → 503 `gate_view_unconfigured` (mirrors `fixup_unconfigured`); unknown run → 404; bad `stage_kind` → 400. A run-bound `mcp:run:<uuid>` token may read only its own run (403 `cross_run_gate_view`, mirroring the fix-up handler); other subjects get the open-read posture of `handleListRunAudit`.
