...
- The agent's `declined` reason is AGENT-authored and crosses into the reviewer's prompt without appearing in the committed diff, so it is treated as UNTRUSTED at both ends: the runner flattens its structure at upload (`sanitizeFixupObligationText`) and `prompt.writeFixupObligationDeclineReasons` renders it only inside a `<<<BEGIN/END UNTRUSTED AGENT DECLINE REASON>>>` quarantine envelope. The audit payload carries only the OPERATOR's `text_excerpt`, never the agent's text.
