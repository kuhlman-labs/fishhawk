...
- **Bounding, and what the bound may drop.** At most `prompt.MaxSliceVerifyEntries` (20) rows and `prompt.MaxSliceVerifyRunsPerSlice` (6) non-superseded runs per row (keeping the LAST N, so the terminal run survives), with the output tail blanked on any `passed` run. The entry bound is applied AFTER a NON-PASSING-FIRST ordering (`sliceVerifyNonPassing`: an unresolved slice, a `failed` summary, or a non-`passed` terminal run), so it can **only ever drop PASSING rows** — under a plain index ordering a failed slice at position 21 would be invisible, and that is precisely the row whose absence endangers a merge. The rendered truncation line states that the omitted slices are all passing.
- **`VerifiedHeadSHA`, and the residual it makes visible.** Each row names the commit its gate actually ran against, read from the first non-empty `head_sha` on the RAW run list (before superseded filtering, so a slice whose only head-bearing run was absorbed still names its commit). That is the child's PUSHED head at gate time, which is normally but not necessarily the commit the fan-in integrated; nothing here cross-checks the two against the `integration_commit_recorded` ledger. The residual is rendered in the block in the reviewer's own terms rather than left in a plan artifact, and carrying the SHA plumbs the value a future cross-check would need.
