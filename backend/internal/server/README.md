...
- **The carry-forward** (`recordAppliesToOverride`) appends the **run-scoped** entry post-insert. The plan gate suppresses its own refusal by **looking that entry up** (`runHasAppliesToOverride`), never by re-reading request state — audit-entry lookup, no run column, no migration. Its residual failure mode is stated rather than hidden: if *this* append fails the run is admitted (the bypass is already audited globally) but the plan gate re-rejects a deferred `paths` violation. That is fail-closed by construction and recoverable. `TestAppliesToPlanGate_OverrideAbsent_Rejects` and `TestAppliesTo_M11b_OverrideLookup_IsAuditEntryNotRequest` pin it — a run whose creation carried an override but whose entry is absent has no override here.
