...
- **Local-runner issue context (#415)**: for runs minted outside the webhook flow with `trigger_source=github_issue`, the CLI's `--issue <number-or-URL>` flag shells to `gh issue view --json title,body,url,number` and ships the payload inline as `issue_context`; the backend persists it to migration 0025's new `runs.issue_context` JSONB column.
  ...
  **Honest residual.** A LOCAL GitLab run minted via `POST /v0/runs` / `fishhawk_start_run` carries neither an `installation_ref` nor a `gitlab_ci` runner kind (`runs.go` resolves only a GitHub App installation), so it derives as github-family and lands on the `no_credential` audit row rather than a GitLab fetch — visible now, where it was silent before, but not fetched. The operator's inline `issue_context` (with `url`) is the fully-fixed path for that shape: the cached title/body/URL render verbatim with no github.com fabrication.
