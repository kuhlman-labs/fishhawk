...
**WHERE THE RULE IS ENFORCED, AND WHERE IT IS NOT.** Enforcement is at **run admission only** — `POST /v0/runs`. It is deliberately **not** enforced by static validation (`fishhawk validate`, the CLI spec check), because the CLI parses no work-management conventions anywhere today: teaching static validation about the charter means giving the validator a conventions loader it has no other reason to have. Read that precisely rather than charitably — a grooming workflow with no charter **validates**, and then **fails to start**. Run admission is the load-bearing half: it is what makes *starting* an unanchored grooming run impossible, and it fails closed on a loader error too. `WorkflowRequiresCharter` is kept **pure and exported** (no receiver, no I/O, no request context) precisely so wiring it into static validation later is a call rather than a rewrite; the static half is tracked as its own follow-up.
