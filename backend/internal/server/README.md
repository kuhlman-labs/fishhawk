...
- **Two-sided fold.** The DESTINATION side (`gained`) is unioned into `resolveApprovalAddScopeFiles` — the documented SOLE fold source — so the enforced implement scope, shown set, reviewer drift baseline, and trace provenance all inherit it with no new call site. The SOURCE side (`lost`) is the genuinely new machinery: `requireDecomposedScope` resolves it once and subtracts it from BOTH halves it returns — `resolveDecomposedScopeFiles` (before the coupled-`_test.go`-sibling fold, so a moved-away `foo.go` cannot drag `foo_test.go` back) and `resolveDecomposedScopeConstraint` (the agent-facing constraint). `resolveApprovalSliceMoves` returns `lost` as every path under every non-own key without re-reading the plan — subtracting a path a slice never owned is a harmless no-op.
