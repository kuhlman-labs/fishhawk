...
**Append arms & UpdateIssue-error routing.** `appendRollingWalkSection` (a) does a FRESH `GetIssue` (the `EpicChildren` snapshot is stale by construction); (b) if the fresh state is not open, or the GetIssue fails, degrades to filing a NEW rolling walk (never a lost walk — at most one extra issue); (c) if the body already carries this run's section key, writes NOTHING (idempotent re-entry); (d) otherwise `UpdateIssue`s the appended body. An `UpdateIssue` FAILURE on an existing walk routes to the SAME `filing_failed` linked marker as a `File` error and NEVER re-files — a second walk would reopen the #2045 double-file window.
