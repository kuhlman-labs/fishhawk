...
### One-verb operator merge (`merge_run.go`, E48.7 / #1954)

`POST /v0/runs/{run_id}/merge` (`handleMergeRun`, MCP verb `fishhawk_merge_run`) takes a gate-approved run from verdict to merged. It records the operator's merge verdict as a chained `merge_verdict_recorded` audit entry (modeled on `handleVouchCommit`; `ActorUser`, payload `{run_id, verdict, pr_url, delegated:false}`) and queues the squash merge through `s.cfg.GateMerger` — the SAME seam the delegated `may_merge` arm uses, extracted into `runMergeDispatch` (`autodrive.go`) so `drive_run`'s merge act and this operator verb converge by construction. The PR-approval review stays a separate `gh pr review --approve` step under the operator's own GitHub identity (App-identity approval deferred to E39).

- **Idempotent by construction** (binding condition): on a POST that finds an existing `merge_verdict_recorded` row it does NOT append a duplicate (responds `already_recorded:true`) but ALWAYS re-dispatches the merge seam — so a verdict-recorded-but-`502`-queue state is retried by a plain re-POST (the MCP tool re-POSTs on resume with no client-side skip).
- **Fail-closed guards, all before any write:** `401` anonymous, `403` `run_token_forbidden` (a run-bound `mcp:run:<uuid>` token may not merge), `403` `insufficient_scope` (missing `write:approvals`), `400` empty verdict, `404` unknown run, `409` `no_pull_request`, `409` `run_not_mergeable` (failed/cancelled), `409` `acceptance_gate_not_passed`/`acceptance_gate_unverified` (ADR-049 decision #6 — read error or any non-passed/not-declared/skipped-out-of-scope state), `503` `merge_unconfigured` (nil `GateMerger`, BEFORE any write). A merger error after the durable verdict returns `502` `merge_queue_failed` (retryable).
- It deliberately does NOT reuse `mergeGateReady`: in `feature_change` the implement-review stage stays parked at `awaiting_approval` until the merge itself settles it (`resolveReviewStageOnMerge`), so blocking on that gate would deadlock the human merge path. It does NOT wait for the terminal run state — the await is client-side in the MCP tool (the `await_audit` idiom).
- **REST-merge fallback** (`serve.go::githubAutoMerger`): `enablePullRequestAutoMerge` errors on a PR that is already merge-ready (GitHub's "Pull request is in clean status" class — the common flow after a `gh` approval with checks green), so on that error class the merger falls back to a synchronous REST squash merge (`githubclient.MergePullRequest`). Any unrelated enable error surfaces unchanged.
- `merge_verdict_recorded` is registered in `audit.KnownCategories` and is an internal, non-comment audit kind (living-anchor projected; no `docs/issue-comment-surfaces.md` row).
...
- `GET /v0/campaigns/{id}/status`: campaign + items + `campaign.NextEligible` readiness rollup + the server-computed `next_action`, FAILED-wins precedence: attention > resume > start_run > wait > complete.
  **Reconcile-on-read** as of E26.2 — `reconcileCampaignItemsOnRead` settles any running item whose linked run reached terminal and re-derives the campaign, best-effort + idempotent.
  A second `settleIssueClosedItems` arm (#1558) settles a run-less, deps-satisfied item whose GitHub issue is closed-as-completed `succeeded` with `settled_via=issue_closed` + no `run_id`, unblocking descendants — same fail-closed posture.
