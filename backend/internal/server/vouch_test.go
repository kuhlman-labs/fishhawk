...
func TestVouchCommit_HappyPath(t *testing.T) {
	...
	w := postVouchCommit(t, s, runID,
		vouchCommitRequest{SHA: vouchedSHA, Reason: "sync-schemas remediation commit"}, withAuth)
...
func TestVouchCommit_EmptySHA(t *testing.T) {
	...
	w := postVouchCommit(t, s, runID, vouchCommitRequest{SHA: "   ", Reason: "x"}, withAuth)
...
func TestVouchCommit_EmptyReason(t *testing.T) {
	...
	w := postVouchCommit(t, s, runID, vouchCommitRequest{SHA: vouchedSHA, Reason: "  "}, withAuth)
	if w.Code != http.StatusBadRequest {
		t.Fatalf("status = %d, want 400:\n%s", w.Code, w.Body.String())
	}
	if !bytes.Contains(w.Body.Bytes(), []byte("validation_failed")) {
...
func TestVouchCommit_RunNotFound(t *testing.T) {
	...
	unknown := uuid.New()
	w := postVouchCommit(t, s, unknown, vouchCommitRequest{SHA: vouchedSHA, Reason: "x"}, withAuth)
