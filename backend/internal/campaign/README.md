...
- **The no-epic and epic paths now classify every out-of-set target by reading its state (#2953 — SUPERSEDES the earlier #2120 no-epic difference).** The recorded #2120 decision was that the no-epic path fails-dangling for EVERY out-of-set target, applying no completion refinement. That is RETIRED: both source paths (`ResolveDependencies` and `EpicChildren`) now share one classifier (`workmgmt/github`'s `classifyOutOfSetTarget`) that reads each out-of-set target's issue state once (memoized per resolution). A **closed-AND-completed** target is a SATISFIED dependency (`SatisfiedEdges`, elided from the DAG); an **open** target keeps `DropNotChild`; a **closed-but-not-completed** (not_planned/duplicate) target is `DropTargetClosedIncomplete`; an **unreadable** target (GetIssue error, nil issue, or a non-positive/unresolvable ref) is `DropTargetStateUnreadable`. This closes the unactionable-refusal defect where a batch whose prerequisite was already done refused with advice (raise/drop `grooming_order_limit`) that could never include a closed issue.
