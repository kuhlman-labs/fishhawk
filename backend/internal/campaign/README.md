...
- `items` are issue refs (bare number or `issue:N`); every ref MUST parse AND resolve to a child in the result. The FIRST miss fails closed naming the ref, and the two causes are DISTINCT sentinels (#2176): a ref that does not PARSE yields a wrapped `workmgmt.ErrInvalidItemRef` → `422 campaign_item_ref_invalid`; a ref that parses but is not among the children yields `ErrItemNotChild` → `422 campaign_item_not_child`. `ErrInvalidItemRef` is the SHARED sentinel the no-epic `IssueSetDependencyResolver` also wraps, so both assembly paths answer one code for one cause — and a ref that names no issue no longer claims to be "not a child of the epic".
