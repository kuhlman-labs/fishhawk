...
// sourced from the epic child's `autonomy:<tier>` label (#1551).
type AssembledItem struct {
	...
	// Autonomy is the child's autonomy tier ("low"/"medium"/"high", or "" when
	// unlabelled), carried through Persist onto campaign_items.autonomy so the
	// engine can route a human-led (low) item away from autonomous dispatch.
	Autonomy string
}
