...
## Undecidable-before-deadline control (#2540)

A request filed near the agent wall clock is **undecidable by construction**: the agent's ~15-minute wait-poll window can outlive the stage's remaining agent runtime, so the stage is SIGKILLed while the agent is still polling and a correct, correctly-approved amendment is lost with the whole pass (run 138281e7, amendment filed at t+3555s of a 3600s clock). The request handler now derives the executing implement stage's **remaining agent wall clock** — the spec-resolved `agent_timeout_seconds` (`resolveAgentTimeout`) minus the stage's elapsed time — and, when that remainder is **provably** shorter than `AmendmentPollWindowSeconds` (900, the server-side mirror of the poll window), marks the created row's REQUEST response + `scope_amendment_requested` audit with `undecidable_before_deadline:true` plus `stage_deadline_seconds_remaining` + `amendment_poll_window_seconds`. The row is still **created** and still `pending` (a late decision plus `fishhawk_retry_stage` still folds the paths, and `detectUndecidedScopeAmendments` still names them on the failure path) — only the AGENT is told up front, via the prompt's step-1 undecidable branch, not to enter a wait it cannot finish.

**The control FAILS OPEN.** It refuses ONLY when the remainder is provably insufficient; whenever the estimate is uncertain for ANY reason it attaches nothing and behaves exactly as before, because a false positive would refuse a WINNABLE amendment and kill a stage that would have succeeded — strictly worse than the bug it fixes (which merely loses an already-unwinnable amendment). `amendmentDeadlineRemaining` fails open in four named modes:
