...
**`stages.updated_at` side effect — consumer audit (#2541 approval condition 4).** The `stages_set_updated_at` BEFORE-UPDATE trigger (migration 0001) fires on any `stages` UPDATE, so each heartbeat bumps `stages.updated_at` every ~15s for the life of a running stage. Audited every `updated_at` reader: the `ORDER BY updated_at` queries (`ListStagesAwaitingApproval`, `ListReviewStagesAwaitingApproval`, `ListDeployStagesAwaitingDeployment`, `ListDeployStagesRollbackPending`, `ListStagesDispatched`, `ListStagesAwaitingChildren`) and every `Stage.UpdatedAt` Go read. All but one filter to states in which no agent runs (`awaiting_*`, `deploy_*`), so a heartbeat — which fires only during agent execution — never touches their rows. **The one genuine consumer is the dispatch watchdog** (`backend/internal/dispatchwatchdog`): it computes `now - UpdatedAt` for `state='dispatched'` stages and fails infra-stuck ones (category C). A stage stays `dispatched` DURING agent execution (`server/trace.go` transitions `dispatched → running → terminal` only at trace-upload time), so heartbeats DO bump a dispatched stage's `updated_at`. This is a real semantic change surfaced (not absorbed) for the reviewer: the watchdog now measures "time since last heartbeat" rather than "time since dispatch". Its primary purpose is preserved — a truly infra-stuck stage has no runner, emits no heartbeat, so `updated_at` stays at dispatch time and the watchdog still fires — but a legitimately long-running agent that keeps heartbeating no longer trips it (arguably a fix: the dispatch watchdog is an infra-stuck detector, not an agent-runtime cap; the agent's own `agent_timeout_seconds` is that cap). **Operator sign-off (condition 4).** The operator SIGNED OFF on this tradeoff; it is no longer an open merge gate. Reasoning: the dispatch watchdog is off by default (`--enable-dispatch-watchdog`), and even when enabled its default timeout is 1h — equal to the 3600s agent wall clock the runner enforces — so the residual wedged-but-heartbeating case is already bounded by the wall clock (the runner SIGKILLs the stage at 3600s regardless of `updated_at`). A follow-up is filed to give the watchdog a dedicated heartbeat timestamp instead of the generic `updated_at` trigger before the watchdog is enabled by default. Decided by the operator at the gate.
