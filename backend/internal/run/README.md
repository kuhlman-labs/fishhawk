...
**Clock domains — the attempt-relative comparison spans TWO of them (#3048, production hazard #3084).** `dispatched_at` is stamped by migration 0072's trigger with Postgres `now()` — the DATABASE's clock, at MICROSECOND resolution — while `StageProgress.ReportedAt` is stamped SERVER-SIDE from the backend's ingest clock. So `ListDispatchedStageLiveness`'s `hb.Before(*l.DispatchedAt)` compares a backend-clock instant against a database-clock instant. That is a PRODUCTION hazard, not merely a test one: under backend-ahead-of-DB lag a genuine fresh heartbeat reads as absent and the watchdog reports `never_checked_in` for a stage that is really `wedged_after_checkin` — the two modes #2744 exists to separate, swapped. **#3084** is filed for the production half; nothing in this package addresses it yet.

The TEST half is closed by `dispatchliveness_test.go`: every COMPARATIVE seed is anchored to the row's own DB-stamped `dispatched_at` via `dbAnchoredHeartbeat` and offset with `timescale.D`, so both sides of the comparison come from one clock and the ordering holds by construction at any host/container skew (skew is unbounded, so a scaled margin alone would not have sufficed). A non-comparative fixture timestamp — one never compared against `dispatched_at` — is deliberately left on the host clock and justified in place. Two measured precision facts make the EQUAL-INSTANT boundary expressible, both re-confirmed inside `pgtest`'s own `postgres:16-alpine`: `now()` stamps at microsecond resolution, and the `stages.progress` JSONB round-trip is LOSSLESS — a `time.Time` marshals to an RFC3339Nano string that `jsonb` stores verbatim, so no `timestamptz` coercion exists in that path to truncate or round. A zero-offset anchor therefore names the dispatch instant exactly, which is what `TestListDispatchedStageLiveness_HeartbeatEqualToDispatchIsCurrent` pins.
...
|---|---|---|
| `reopened` | a stage-scoped `acceptance_reopened` entry exists (#1682), or not | The caller correlates the entry with THIS stage's id, never by category alone: a run can carry more than one acceptance stage in its history, and a stale entry from an earlier one would draw the stronger "a fix-up re-opened this" claim with nothing supporting it (#3222 binding condition 1). Unestablished → the GENERIC shape, still true, just less sharp. |
