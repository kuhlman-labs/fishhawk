...
**The terminal refusal IS the UPDATE's own `WHERE state NOT IN ('succeeded','failed','cancelled')` predicate** (`RecordStageProgress`, `:execrows`): a heartbeat that arrives after the stage settled matches zero rows, so `RecordStageProgress` returns `applied=false` (nil error) and the handler answers 409 `stage_terminal` — there is no prior read and so no check-then-write window (#2536). The three queries touch ONLY the `progress` column, so no existing `stages` SELECT list expands and the sqlc `db.Stage` model is unchanged (the three functions were hand-added to `db/queries.sql.go`, matching the standing practice of hand-editing in-scope `db/*.go` rather than running `sqlc generate`). Reads are fail-open: an undecodable stored payload degrades to nil rather than erroring the read.
