...
## `installation_ref` — the forge-neutral credential reference (E45.22 / #2043)

`runs.installation_ref` (migration `0076_runs_installation_ref`, `TEXT` NULLABLE) is the persisted form of `forge.CredentialScope` (ADR-057 / ADR-058). `runs.installation_id` is a `bigint`, so it can only ever name a GitHub App installation; a `gitlab_ci` run had nowhere to record WHICH GitLab project to authenticate as, which is why HIGH 1 of #2043 left every GitLab stage warn-skipping on the zero scope. The ref carries the forge: the BARE base-10 decimal of the installation id for GitHub (exactly what `forge.FromGitHubInstallationID` produces — NOT a `github:`-prefixed string, which no GitHub credential consumer parses), `gitlab:<project_id>` for GitLab.

**Three states, deliberately distinguishable.** `nil` = no ref recorded (a pre-0076 row, or a mint that stamped none) → consumers fall back to `installation_id`. A pointer to `""` = recorded-as-empty → ALSO falls back, but it is a different provenance and a caller must never store `""` to mean "absent". A value = authoritative, wrapped verbatim by `forge.FromRef`. `rowToRun` deliberately does NOT normalize `""` to `nil`; `TestCreateRun_PersistsInstallationRef` asserts all three round-trip distinctly across `CreateRun` / `GetRun` / `ListRuns`.

**Backfill.** 0076's `UPDATE` derives the ref for existing GitHub rows from `installation_id::text`, EXCLUDING `installation_id = 0`. Zero is the codebase's unresolved-installation sentinel (`forge.FromGitHubInstallationID(0)` returns the ZERO scope, and the webhook path is known to persist a pointer-to-zero — see the retry-site nil-normalization decision below), so backfilling `'0'` would silently convert a no-credentials run into one claiming installation 0. Pinned by `TestMigration0076_BackfillsInstallationRef`.

**Consumers.** `orchestrator.runCredentialScope` is the ladder: a non-nil non-empty ref wins via `forge.FromRef`, otherwise `installation_id`, otherwise the zero scope (which `runnerbackend`'s `Scope.IsZero()` guard warn-skips, unchanged). `server.runForge` reads the ref's SCHEME as one of the two signals its fail-closed approval cross-check refuses on. Inherited verbatim by every run minted from another run (`childParamsInheritance`) — a retry of a `gitlab_ci` run that dropped the ref would fall back to a GitHub installation id it does not have.

**sqlc.** Unlike `working_dir` (#2482), the `db` package was REGENERATED rather than hand-edited. **Recorded deviation (#2880):** that #2043 / C7 regeneration ran `sqlc generate` from `backend/` against a config NARROWED to the `internal/run` entry, not the plan's stated full-config procedure, and the ratified R4 `git status --porcelain` check (only `internal/run/db/` changed) was never evidenced in that PR. The narrowing was forced by two pre-existing parse errors the FULL config aborted on — `internal/account/queries.sql` used the two-argument `unnest(keys, granularities)` (sqlc's catalog has no two-argument `unnest`) and `internal/audit/queries.sql`'s `ListAuditEntriesForRunChain` anchored its recursive CTE on a bare `id` that sqlc's analyzer resolves as ambiguous — both fixed by behavior-preserving rewrites in #2880 (two `unnest(...) WITH ORDINALITY` sources joined on their ordinal; `runs.id` qualification), so the full `backend/sqlc.yaml` now parses. The regeneration also restored column-list expansions a prior hand-edit had let drift — `account_id` on the non-`GetRun` Run-returning queries, `progress` / `dispatched_at` on `ParkScopeCompleteness` — so those columns are populated on every read path (pinned by `TestPostgres_AccountID_PopulatedOnEveryRunReturningPath` and the `DispatchedAt` assertion in `TestPostgres_ParkScopeCompletenessAndAppend_RoundTrip`). **Procedure going forward:** run `sqlc generate` on the FULL config in a THROWAWAY copy of `backend/` (never in the worktree), copy back ONLY the target package's `db/`, then `git status --porcelain` must list only the intended files — the other 13 packages' `db/*.go` are hand-maintained and a full run still churns every one of them; that is drift to leave behind, not a config abort.
...
**The caller contract the index CANNOT enforce.** It only dedups when both racing inserts compute the SAME `retry_attempt`. Both deliveries MUST derive it as `parent.RetryAttempt + 1`, read from the PARENT row. A value derived from the latest existing CHILD makes the two inserts disagree (`0+1` and `1+1`), the index never fires, two children land — and a concurrency test still passes, because there is exactly one row per attempt value. Any new retry mint site derives from the parent.

**Key shape and partiality are both load-bearing.** The key is `(parent_run_id, retry_attempt)`, not `parent_run_id` alone: one parent legitimately carries a CHAIN of retries (attempt 1, then 2). The predicate excludes `retry_attempt = 0` because `parent_run_id` is also the #216 follow-up/lineage column, whose ordinary children are legitimately many-per-parent. `TestMigration0076_RetryChildOnceIndexDedups` asserts all three: the duplicate is refused, the next chain link is accepted, and two attempt-0 siblings are accepted.

**Recognizing the benign loser.** `run.IsRetryChildDuplicate(err)` mirrors `audit.IsDuplicateOnConstraint`'s idiom — it matches a SQLSTATE 23505 whose `ConstraintName` is `run.RetryChildOnceIndex`, or the `run.ErrRetryChildDuplicate` sentinel a fake `Repository` can return so a retry path's benign branch is exercisable without Postgres. It deliberately does NOT match a 23505 on any other constraint the same INSERT can trip (notably `runs_idempotency_key_repo_idx`); swallowing those would treat an unrelated integrity failure as a won race and silently drop a retry that never happened.

## Stage progress heartbeat store (`StageProgressStore`, E48.96 / #2541)

`stages.progress` (migration `0070_stages_progress`, nullable `JSONB`) carries the runner's most recent mid-execution `stage_progress` heartbeat — `{last_event, turns_this_attempt, tokens_this_attempt, reported_at}` — so an operator poll during a running stage returns real activity instead of a single `running` bit. `StageProgress` (`progress.go`) is the domain type; the counters are cumulative WITHIN THE CURRENT AGENT ATTEMPT and reset on an in-driver re-spawn (the per-attempt meaning is in the field names). Elapsed is deliberately NOT stored — the operator-facing `elapsed_seconds` is derived from `stages.started_at`, which makes it cumulative and monotonic across re-spawns.

`StageProgressStore` is an **optional concrete-repo capability** (three methods: `RecordStageProgress` / `StageProgressByID` / `StageProgressForRun`), deliberately NOT part of `Repository` — exactly the `runnerKindResolver` / `AddRunCost` / `ParkScopeCompletenessAndAppend` precedent. Adding it to `Repository` would fan the three progress-only methods out across the 23 hand-rolled implementations for a capability only the postgres repo and the server's ingest/read handlers use; instead the consumer type-asserts for it and degrades (503 `progress_unsupported`) when it is absent.

**The terminal refusal IS the UPDATE's own `WHERE state NOT IN ('succeeded','failed','cancelled','superseded')` predicate** (`RecordStageProgress`, `:execrows`): a heartbeat that arrives after the stage settled matches zero rows, so `RecordStageProgress` returns `applied=false` (nil error) and the handler answers 409 `stage_terminal` — there is no prior read and so no check-then-write window (#2536). The predicate tracks `StageState.IsTerminal()`, so `superseded` (#3083) is in it: a merge-superseded stage has settled and a late heartbeat must be refused exactly like one against a succeeded stage. `queries.sql` is the AUTHORITY for the literal; `db/queries.sql.go` carries a hand-mirrored copy under a preserve-on-regeneration comment and only that copy EXECUTES, so the two are edited in lockstep and the drift is caught behaviorally by `TestPostgres_RecordStageProgress_RefusedOnSupersededStage`, not by inspection. The three queries touch ONLY the `progress` column, so no existing `stages` SELECT list expands and the sqlc `db.Stage` model is unchanged (the three functions were hand-added to `db/queries.sql.go`, matching the standing practice of hand-editing in-scope `db/*.go` rather than running `sqlc generate`). Reads are fail-open: an undecodable stored payload degrades to nil rather than erroring the read.

**`stages.updated_at` side effect — RESOLVED (#2744).** The `stages_set_updated_at` BEFORE-UPDATE trigger (migration 0001) fires on any `stages` UPDATE, so each heartbeat bumps `stages.updated_at` every ~15s for the life of a running stage. That bump is BY DESIGN and remains. The `ORDER BY updated_at` queries (`ListStagesAwaitingApproval`, `ListReviewStagesAwaitingApproval`, `ListDeployStagesAwaitingDeployment`, `ListDeployStagesRollbackPending`, `ListStagesDispatched`, `ListStagesAwaitingChildren`) all filter to states in which no agent runs (`awaiting_*`, `deploy_*`), so a heartbeat — which fires only during agent execution — never touches their rows. The one former exception was the dispatch watchdog: a stage stays `dispatched` DURING agent execution (`server/trace.go` transitions `dispatched → running → terminal` only at trace-upload time), so heartbeats DO bump a dispatched stage's `updated_at`, and while the watchdog read `updated_at` a heartbeating-but-wedged stage could forge its clock. **That is now closed:** the dispatch watchdog NO LONGER reads `updated_at`. Migration `0072_stages_dispatched_at` adds a dedicated `stages.dispatched_at` column stamped only on the transition INTO `dispatched` (a transition-keyed trigger a progress-only UPDATE cannot advance), and the watchdog measures its deadline from that column via the `DispatchLivenessLister` capability below. What #2541 recorded as an accepted tradeoff — the watchdog measuring "time since last heartbeat" — is resolved, not merely bounded by the runner's 3600s wall clock.

**Dispatch-liveness capability (`DispatchLivenessLister`, #2744).** `ListDispatchedStageLiveness` returns, per `dispatched` stage, its dedicated `dispatched_at`, its `updated_at`, and its last heartbeat time decoded ATTEMPT-RELATIVE from `stages.progress` (a heartbeat whose `reported_at` predates the current `dispatched_at` — a leftover from a previous dispatch attempt on the `dispatched → running → … → dispatched` retry path — is treated as absent, so a fresh un-checked-in attempt reads `never_checked_in`, not `wedged_after_checkin`). Like `StageProgressStore` it is an **optional concrete-repo capability**, deliberately NOT part of `Repository`, so the ~23 hand-rolled doubles stay untouched; the dispatch watchdog FAILS CLOSED (refuses to start) when the wired repo lacks it rather than degrading back to the heartbeat-defeatable `Stage.UpdatedAt`. Read via a dedicated narrow query (`dispatchliveness.go`), so no existing `stages` SELECT list expands and the sqlc `db.Stage` model is unchanged.

**Clock domains — ENUMERATED, and the attempt-relative comparison is now SINGLE-domain (#3084, test half #3048).** Every timestamp on this path comes from one of three clocks, and exactly one of them survives on the comparison path. (1) **The DATABASE clock** — `stages.dispatched_at`, stamped by migration 0072's trigger with Postgres `now()` at MICROSECOND resolution, and, since #3084, `stages.progress.reported_at` too: `RecordStageProgress`'s statement is `SET progress = jsonb_set($2::jsonb, '{reported_at}', to_jsonb(now()))`, so it OVERWRITES whatever the caller marshalled. Both operands of `ListDispatchedStageLiveness`'s `hb.Before(*l.DispatchedAt)` are therefore stamped by Postgres and the comparison is correct BY CONSTRUCTION. **NO tolerance band was chosen, deliberately**: a band would have to pick an arbitrary bound, would still misclassify silently past it, and would trade a sharp failure for a quiet one. (2) **The BACKEND PROCESS clock** — no longer on this path at all (the ingest handler contributes no timestamp), but still the source of the dispatch watchdog's `now` in `handleStage`'s deadline arithmetic; see `dispatchwatchdog/README.md` for that residual. (3) **The RUNNER HOST clock** — VERIFIED not to reach the store: `runner/internal/upload/upload.go`'s `reportStageProgressBody` sends only `last_event` / `turns_this_attempt` / `tokens_this_attempt`, and `POST /v0/runs/{id}/stages/{id}/progress` accepts no `reported_at`, so there is no third clock and stamping in the DB was not ruled out. Before #3084 `reported_at` came from clock (2), so backend-vs-DB lag could report `never_checked_in` for a stage that was really `wedged_after_checkin` — the two modes #2744 exists to separate, swapped. **One legacy residual:** a heartbeat row persisted BEFORE #3084 still carries a backend-clock `reported_at` and stays skew-exposed until the next ~15s heartbeat overwrites it; no backfill is warranted for a value with a 15-second half-life.

The TEST half (#3048) now reads as HISTORY on the write side: comparative seeds no longer need anchoring THROUGH the domain write, because that write is DB-stamped and discards any caller instant. `dbAnchoredHeartbeat` survives in a narrowed role — it feeds DB-anchored instants to the RAW seeder `seedProgressAt` (which writes `reported_at` straight to the column, bypassing `RecordStageProgress`) for the boundary and prior-attempt fixtures, and supplies a deliberately SKEWED caller value to the real store in the skew tests, where the whole point is that it is discarded. The prior-attempt fixture `TestDispatchedAt_RedispatchWithStaleHeartbeatReportsNeverCheckedIn` records NATURALLY now: the DB stamp plus the forward re-stamp produce the ordering by construction. Two measured precision facts still make the EQUAL-INSTANT boundary expressible, both re-confirmed inside `pgtest`'s own `postgres:16-alpine`: `now()` stamps at microsecond resolution, and the `stages.progress` JSONB round-trip is LOSSLESS. A zero-offset anchor therefore names the dispatch instant exactly, which is what `TestListDispatchedStageLiveness_HeartbeatEqualToDispatchIsCurrent` — re-seeded via `seedProgressAt` at exactly the row's own `dispatched_at` — pins. Separately, the `to_jsonb(now())` SPELLING is load-bearing: `to_jsonb(now() AT TIME ZONE 'UTC')` renders WITHOUT an offset, Go's RFC3339 unmarshal rejects it, and every heartbeat would become an undecodable payload the fail-open read degrades to nil; `TestRecordStageProgress_ReportedAtIsDatabaseStamped` is that guard.
...
## Merge-supersede terminal state (`superseded`, E64.2 / #3083)

`superseded` records the fact that a stage was made UNREACHABLE by the merge of the change it was gating. It exists because every pre-#3083 escape hatch recorded something untrue: `reap_stage` writes `failed` (says the work was ATTEMPTED and did not pass), `cancel_run` writes `cancelled` (says an OPERATOR halted the run), and re-dispatching a re-parked acceptance stage runs the agent against a preview bound to a commit that is no longer the change. Meanwhile `Orchestrator.completeRun`'s #968 guard correctly refuses to stamp a run `succeeded` around a non-terminal stage, so the run strands in `running` with a merged PR and a closed issue.

**Classification.** `IsTerminal()` true — which is what stamps `ended_at` in `transitionStage` and what makes the #968 guard accept it — and therefore `IsSettled()` true. It is NOT folded into `completeRun`'s failed/cancelled target arms, so a run whose acceptance stage a merge superseded still completes `succeeded`.

**Entry is a DEFAULT-DENY (stage_type, state) PAIR TABLE, not a state allow-list** (`stageMergeSupersedeTransitions`, `transition.go`). Exactly two rows:

| stage_type | state | why the merge made it unreachable |
|---|---|---|
| `acceptance` | `awaiting_host_dispatch` | a fix-up pass re-parked it; after the merge nothing re-dispatches it, and dispatching it would run the acceptance agent against a preview bound to a commit that is no longer the change |
| `review` | `awaiting_approval` | the human review gate on a change that has ALREADY merged — the judgment it exists to collect can no longer alter the outcome |

The type half is load-bearing, not decoration. Because `superseded` is terminal, the #968 guard passes it, so this table is the ONLY thing standing between a merge and a fabricated success: a state-only allow-list would admit a `pending` plan or implement stage and complete a run `succeeded` around work never done — defeating exactly the invariant #968 protects. Adding a row here is a decision about run integrity, not a convenience.

**Enforced at the REPOSITORY boundary, not only in the sweep.** `transitionStage` (`postgres.go`) unions `ValidStageMergeSupersedeTransition(StageType(current.StageType), from, to)` as a fifth arm, consulted with the ROW-LOCKED stage's own `stage_type`. That is what makes the table binding rather than advisory guidance a caller reaching the repository directly could bypass. Two counterfactuals pin the two halves separately, because neither mutation reddens the other's test: deleting the union arm reddens `TestPostgres_TransitionStage_AdmitsSupersedeForAdmittedPair` (the POSITIVE test — the ordinary transition table contains no route into `superseded`, so the arm is the only admitter), and deleting the `p.StageType == stageType` clause inside `MergeSupersedable` degrades the predicate to state-only and reddens `TestTransitionStage_RefusesSupersedeForUnadmittedPair` (the NEGATIVE test, which re-reads the COMMITTED row rather than trusting error identity).

`MergeSupersedable(stageType, from)` is the classification half the merge sweep consults to decide WHICH parked stages a merge may terminalize; `ValidStageMergeSupersedeTransition` is the transition-admissibility half.

**Persistence.** Migration `0079_stages_superseded` widens `stages_state_check` to admit the value (the 0035/0038/0053 CHECK-widening precedent) with NO backfill and NO column change — nothing wrote the state before, so there is no legacy row to migrate. The down is NORMALIZE-THEN-NARROW (the 0040/0053 precedent): it maps every `superseded` row to `cancelled` BEFORE re-adding the narrower 0053 CHECK, because the narrower CHECK would raise SQLSTATE 23514 against a surviving row. That direction is LOSSY — after a rollback a merge-superseded stage is indistinguishable from an operator-cancelled one — and the durable record survives in the `stage_superseded_by_merge` audit rows, which the rollback does not touch.

**Internal consumers taught the state:** the `stage_not_runnable` guards in `server/prompt.go` (BOTH switches — the runner-facing endpoint and the SPA-facing render endpoint carry separate copies), the `auditcomplete` trace sweep's skip (a superseded stage was parked and never dispatched, so demanding a trace from it would red-line every merge-superseded run's audit-complete check), `issuecomment.stageStateIcon` (its own glyph, deliberately NOT cancelled's — reusing it would reintroduce the dishonesty the state removes), and the SPA `StageState` union + `stageStateStyles` badge map.

This slice is purely additive — NOTHING WRITES the state. The merge sweep, the `POST /v0/runs/{run_id}/reconcile-merge` recovery endpoint, the `completion_blocked` projection, the `stage_superseded_by_merge` audit category, the MCP mirror (including `mcpserver/stage_wait.go`'s own string-literal terminal set) and every API/doc surface land in sibling slices, so the tree stays green standalone.
...
**#860 bounded operator override**: the request's `force_additional_pass: bool` (threaded into `FixupOptions.ForceAdditionalPass`, with `HardCeiling: defaultFixupCeiling == 3` supplied by the handler) grants ONE pass beyond the normal budget — audited via a `forced` flag on the `stage_fixup_triggered` entry. At the ceiling `FixupStage` returns the DISTINCT `ErrFixupCeilingReached` → 422 `fixup_ceiling_reached` (the handler arm is ordered before `fixup_budget_exhausted` so it is not masked).

**#3335 ceiling refund credit**: a delivered-nothing fix-up pass is credited against the HARD CEILING too, not just the normal budget. The handler passes `FixupOptions.CeilingRefundedPasses` (bounded by `server.maxCeilingRefundCredits == 3`); `FixupStage` clamps it into `[0, PriorPassCount]` and compares `PriorPassCount - credit >= HardCeiling`, so the **absolute** cap is `HardCeiling + maxCeilingRefundCredits == 6` triggered passes. The two clamp bounds are SEPARATE controls: the LOWER bound (a negative value → 0) stops a caller manufacturing a refusal by driving the effective count UP; the UPPER bound (a value above `PriorPassCount` → `PriorPassCount`) cannot change the admit/refuse verdict but bounds the reported `FixupDecision.RemainingCeilingSlots` (`HardCeiling - (PriorPassCount - credit + 1)`, floored at 0). Every admission reports `RemainingCeilingSlots`; the handler surfaces it on both 422s and the audit receipt.
...
- It restores the gate, writes a `stage_fixup_recovered` audit entry (system actor; restored states + source failure category/reason; since #3395 additionally `delivered_nothing` / `reopened_concern_ids` / `concern_reopen_error`, and on a delivered-nothing pass — no `fixup_pushed` after the trigger — it re-opens the trigger's routed concerns `addressed_pending → reopened` so the round the fix-up opened is closed rather than left pending forever; long-form in `server/README.md` "Fix-up delivered-nothing round closure"), and returns true so the two implement-failure chokepoints — `trace.go::advanceAfterFailure` (cat-A agent-fail, cat-B implement-review reject, failStageCategoryB/C) and `pullrequest.go::failPullRequestStage` (the #742 PR-open / #794 fix-up push failure) — SKIP the run-failing `Orchestrator.Advance`, leaving the run `running` at its gate.
