...
### Effective acceptance verification (#3181)

The three predicates do NOT read `approvedPlan.Verification` verbatim. `Orchestrator.EffectiveAcceptance` (`EffectiveAcceptanceResolver`, one method `EffectiveAcceptanceVerification(ctx, runID, *plan.Plan) (plan.Verification, error)`) returns the plan's verification with `acceptance_criteria` replaced by the run's EFFECTIVE live set — operator retirements removed, restatements applied, operator-added criteria (`fishhawk_approve_plan` `amend_acceptance_criteria` action `add`) appended. It is resolved ONCE per evaluation and every predicate, count and log field reads that one value. Without it an all-skip-with-basis plan carrying an operator-added drivable criterion still short-circuits to `not_validated` and the added criterion is never driven — the motivating run 0aad7486 shape.

It follows the `RunCancelled` back-reference pattern: `server.New` assigns the `*server.Server` (`cfg.Orchestrator.EffectiveAcceptance = s`, pinned by `server`'s `TestServerNew_WiresEffectiveAcceptanceHook`), whose implementation (`server/acceptance_effective_verification.go`) is a thin adapter over `resolveEffectiveAcceptanceCriteria`, the single seam that computes the effective set; an un-amended run returns the plan verbatim. A nil hook (CLI/dev posture) or a resolver error (WARN-logged naming the run) evaluates the approved plan verbatim — byte-identical to pre-#3181 behaviour. The predicate SET is unchanged, so `server`'s `TestAcceptanceRetirementRefusal_PredicateSetEqualsShortCircuitSet` scan still holds. Residual: a plan-stageless recovery child (#2028) resolves its ancestor's plan but the hook reads the CHILD run's approval chain, which carries no amendments, so it evaluates the ancestor plan verbatim. Pinned by `TestTryShortCircuitAcceptance_EffectiveVerification` (add → no short-circuit, `liveValidationRequired` true, stage untouched; no add / nil hook / resolver error → short-circuit as before).
