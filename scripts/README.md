...
| r14 | the GitLab family (E45.32 / #2922): GitHub-only ConfigMap/Secret carry no `FISHHAWKD_GITLAB_*` key, a complete GitLab config renders all five ConfigMap keys + three Secret keys, and one case per named `fishhawk.validateGitLabOAuthTrio` branch + derived-requiredness + the explicit-null edge. The GitHub-only claim (r14a) is asserted over the WHOLE render — every manifest template including `externalsecret.yaml` via a plain `render` with no `--show-only`, plus NOTES.txt separately via the `notes()` helper (`helm template` never emits NOTES.txt and `--show-only` cannot select it, so a manifest-only render is structurally blind to it) — so a GitLab key leaking into any non-Secret template or into NOTES.txt cannot decay silently. r14m additionally pins the `existing`-mode UNKNOWN posture explicitly: no derived-requiredness message naming `FISHHAWKD_GITLAB_OAUTH_CLIENT_SECRET` on stdout or stderr, since the chart cannot inspect a pre-created Secret |
