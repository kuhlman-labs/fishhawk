...
// runTestMain (#3503). autoOpenPRTestRepo and its callers push into
// t.TempDir() bare origins, so without this pin a `git push` here can spawn
// a detached `git maintenance run --auto --detach` child that races a
// fixture's t.TempDir() cleanup ("directory not empty", #3503).
...
func runTestMain(m *testing.M) int {
	...
	// rather than widened. No test in this package t.Setenvs
	// GIT_CONFIG_GLOBAL, so there is no per-test override to repoint.
	if gitConfigDir, err := os.MkdirTemp("", "fishhawk-cli-gitconfig-*"); err == nil {
...
// child.
//
// Vacuity guard: a trace2 file that never recorded the push or the
// receive-pack child could satisfy the negative "no maintenance child"
// assertion for the wrong reason — an empty or unexercised trace, not a
// working pin. So this test first requires BOTH the push "start" record
// and a receive-pack child_start record to be present before trusting the
// absence of a maintenance one.
func TestHarnessDisablesAutoMaintenanceOnLocalPush(t *testing.T) {
	...
	mustGitCLI(t, repo, "remote", "add", "origin", origin)
