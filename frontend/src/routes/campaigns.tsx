...
import type { CampaignState } from '@/api/types';
...
export function Campaigns() {
  const { state, hasNext, hasPrev, next, prev, pageIndex } = usePaginated(
    (cursor) => api.listCampaigns({ limit: CAMPAIGNS_PAGE_SIZE, cursor: cursor ?? undefined }),
    [],
  );
    ...
    <section className="space-y-4">
      <header>
        ...
        <p className="text-sm text-neutral-600 dark:text-neutral-400">
          Epic-driven, multi-issue runs across your repositories.
        </p>
      </header>
      ...
      {state.status === 'error' && (
        ...
        >
          <div className="font-medium">Couldn&apos;t load campaigns.</div>
      ...
      {state.status === 'ok' && state.data.items.length === 0 && (
        <div className="rounded-md border border-dashed border-neutral-300 p-8 text-sm text-neutral-500 dark:border-neutral-700">
          No campaigns yet. The first one will appear here when an epic is decomposed.
        </div>
      )}
            ...
            <table className="w-full text-sm">
              <thead className="bg-neutral-100 text-left text-xs tracking-wide text-neutral-600 uppercase dark:bg-neutral-900 dark:text-neutral-400">
                <tr>
                  ...
                  <th className="px-4 py-2 font-medium">Pause policy</th>
